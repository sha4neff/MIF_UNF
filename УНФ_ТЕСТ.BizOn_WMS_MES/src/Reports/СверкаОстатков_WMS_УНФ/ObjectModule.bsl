...
Функция ПолучитьДвиженияWMS(ПараметрыОтчета)
	...
	JSONПакет.Вставить("Номенклатура",?(ЗначениеЗаполнено(ПараметрыОтчета.Номенклатура),Строка(ПараметрыОтчета.Номенклатура.УникальныйИдентификатор()),""));
	JSONПакет.Вставить("ХарактеристикаНоменклатуры",?(ЗначениеЗаполнено(ПараметрыОтчета.Характеристика),Строка(ПараметрыОтчета.Характеристика.УникальныйИдентификатор()),""));
	...
	Для Каждого JSДвижение Из ТБДвижения Цикл		
		Попытка
			JSНоменклатура=Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(JSДвижение.Номенклатура));			
			Если ЗначениеЗаполнено(JSДвижение.ХарактеристикаНоменклатуры) Тогда
				JSХарактеристикаНоменклатуры=Справочники.ХарактеристикиНоменклатуры.ПолучитьСсылку(Новый УникальныйИдентификатор(JSДвижение.ХарактеристикаНоменклатуры));
			Иначе
		...
		Для Каждого JSДокумент Из JSДвижение.Документы Цикл
			...
			Попытка   
				Если JSДокумент.Тип = "ОЗ" Тогда
					JSКИСДокумент = Документы.ОтгрузочнаяВедомость.ПолучитьСсылку(Новый УникальныйИдентификатор(JSДокумент.КИСGUID));
				КонецЕсли;	
			Исключение
...
Функция ПолучитьДвиженияУНФ(ПараметрыОтчета)
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Номенклатура) Тогда
		Если ПараметрыОтчета.Номенклатура.ЭтоГруппа Тогда
			Фильтр="Номенклатура.БЗ_WMS_Дата <> ДАТАВРЕМЯ(1,1,1) И Номенклатура В ИЕРАРХИИ(&Номенклатура)";
			Запрос.УстановитьПараметр("Номенклатура", ПараметрыОтчета.Номенклатура);
		Иначе
			Запрос.УстановитьПараметр("Номенклатура", ПараметрыОтчета.Номенклатура);
			Фильтр="Номенклатура.БЗ_WMS_Дата <> ДАТАВРЕМЯ(1,1,1) И Номенклатура=&Номенклатура";
		...
		КонецЕсли;		
	Иначе
		Фильтр="Номенклатура.БЗ_WMS_Дата <> ДАТАВРЕМЯ(1,1,1)";
	КонецЕсли;	
	ЗапросТекст = 
"ВЫБРАТЬ
...
|			СтруктурнаяЕдиница В (&Склады)
|				И &Фильтр) КАК ТоварыНаСкладахОстаткиИОборотыТовары
|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.ОстаткиИОбороты(
|				&НачДата,
|				&КонДата,
|				Регистратор,
|				Движения,
...
Функция СформироватьТаблицуОтчета(ПараметрыОтчета) Экспорт
	...
	//ВнешниеОбработки.Создать("C:\Users\Администратор\AppData\Local\Temp\zapr.epf",Ложь).Дамп(Запрос);
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		Если выборкаНоменклатура.номенклатура.БЗ_WMS_Дата>ПараметрыОтчета.датаНачала Тогда 
			Продолжить;
		КонецЕсли;
