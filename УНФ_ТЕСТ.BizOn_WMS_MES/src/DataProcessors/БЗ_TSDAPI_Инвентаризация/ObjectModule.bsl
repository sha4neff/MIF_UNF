...
Процедура Инициализация()
	БЗ_TSDRU.Сессия().ПараметрыСессии.Вставить("Инвентаризация",Новый Структура());
	АктивироватьРежим("ВыборДокумента");	
КонецПроцедуры

Процедура ДобавитьВведенныеДанные(ДНоменклатура,ДКоличество=1,ШК="")
	АктивныйДокумент=БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.АктивныйДокумент;
	СтартТранзакции();
...
ПараметрыСеанса.БЗ_TSDAPI_ДанныеСеанса=Новый ХранилищеЗначения(Новый Структура("Сессия,ВременныеПеременные",БЗ_TSDRU.Сессия(),БЗ_TSDRU.ВременныеПеременные()));	
	...
	|	ПеремещениеПоЯчейкамТСД_ВводДанных.Характеристика КАК Характеристика,
...
Процедура СтартТранзакции()
	...
	ЭлементБлокировки = Блокировка.Добавить("Документ.ИнвентаризацияЗапасов");
...
Процедура ВыборДокумента_Активация()
	...
	ТПоле.Вставить(ТП.tx,"Отсканируйте ящик для инвентаризации");
...
Процедура ВыборДокумента_ОбработчикСобытий()
	...
	ИначеЕсли   СТЗапрос.action="scan" Тогда
		...
		БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("АктивныйДокумент",док.Ссылка);
...
функция ВыборДокумента_найтиДокЯчейки(Ячейка)
	...
	|ГДЕ
	|	ИнвентаризацияЗапасов.Ячейка = &ячейка
	|	И НЕ ИнвентаризацияЗапасов.Завершен";
...
Процедура ОсновноеМеню_Активация()
	...
	ТПоле=Новый Структура();
	ТПоле.Вставить(ТП.tx,"ДОКУМЕНТ "+АктивныйДокумент.номер+" от " + формат(активныйДокумент.дата, "ДФ=dd.MM.yyyy" ));
	ТПоле.Вставить(ТП.al,"cc");
...
Процедура ОсновноеМеню_ЗавершитьДокумент()
	...
	|ГДЕ
	|	ЗапасыНаСкладахОстатки.Ячейка.БЗ_Документ = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТОстатки.Характеристика.Наименование,
	|	ВТОстатки.Ячейка,
	...
	ДокументОбъект.ТСД_ВводДанных.Загрузить(тб);
	ТБТовары=тб;
	ТБТовары.Свернуть("Номенклатура,Характеристика,Ячейка","Количество, План");
	...
	Для каждого стр из ТБТовары цикл
		...
		новстр.количествоУчет=стр.План;
	КонецЦикла; 
	
	ДокументОбъект.Записать(); 
	НаборЗаписей=РегистрыСведений.ТСД_Документы.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Документ.Установить(БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.АктивныйДокумент);
...
Процедура ОсновноеМеню_ОбработчикСобытий()
	...
	Если СТЗапрос.action="select" Тогда
		...
		Если Выбрано.Операция="ВводДанных" Тогда
			БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Удалить("ВыбранныйТовар");
			АктивироватьРежим("ВводДанных");
...
Процедура ВводДанных_Активация()
	...
	ТПоле=Новый Структура();	
	ТПоле.Вставить(ТП.tx,"Документ инвентаризации");
	ТПоле.Вставить(ТП.al,"cc");
	ТПоле.Вставить(ТП.fs,"16");		
	ТПоле.Вставить(ТП.pd,"2 2 2 2");
	...
	Меню=Новый Массив();
	Меню.Добавить(Новый Структура("tx,ac,dt,gr","Добавить по коду","menu","new",1));
	БЗ_TSDRU.Ответ().Вставить("menu",Меню);
...
Функция ВводДанных_ПолучитьСписок(Обновление=Ложь)
	...
	|ПОМЕСТИТЬ ВТОстаткти
	|ИЗ
	|	РегистрНакопления.ЗапасыНаСкладах.Остатки КАК ЗапасыНаСкладахОстатки
	|ГДЕ
	|	ЗапасыНаСкладахОстатки.Ячейка = &Ячейка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ВТОстаткти.Номенклатура,
	|	ВТОстаткти.Номенклатура.Наименование,
	|	ВТОстаткти.Характеристика,
	|	ВТОстаткти.Характеристика.Наименование,
	|	ВТОстаткти.Ячейка,
	|	ВТОстаткти.Количество,
	|	0
	|ИЗ
	|	ВТОстаткти КАК ВТОстаткти
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТОбщая.Ячейка,
	|	ВТОбщая.Характеристика";	         
	Док=БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.АктивныйДокумент;
	Запрос.УстановитьПараметр("Ссылка",Док);  
	Запрос.УстановитьПараметр("Ячейка", док.ячейка);
//	Запрос.УстановитьПараметр("ДокументОснование", док.ДокументОснование);
	...
	Если НЕ Обновление Тогда
		Если БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Свойство("ВыбранныйТовар") Тогда
			НСтрока=ТБТовары.Найти(БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.ВыбранныйТовар.номенклатура,"Номенклатура");
...
Процедура ВводДанных_НайтиТоварПоШК(Штрихкод)   
	...
	Запрос.УстановитьПараметр("Ссылка",БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.АктивныйДокумент);
	...
	Если Выборка.Следующий() Тогда               
		...
		БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("ВыбранныйТовар",структураТовара);
		БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("СканированиеШК",Штрихкод);
...
Процедура ВводДанных_ОбработчикСобытий()
	...
	Если СТЗапрос.action="select" Тогда
		...
		БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("ВыбранныйТовар",структураТовара);
		БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("СканированиеШК","");
		АктивироватьРежим("ВводКоличества");
	ИначеЕсли СТЗапрос.action="scan" Тогда
	//	ВводДанных_НайтиТоварПоШК(СТЗапрос.data); 
	БЗ_TSDAPI.ПустойОтветНаЗапрос();

	ИначеЕсли СТЗапрос.data="new" и СТЗапрос.action="menu" Тогда
		АктивироватьРежим("НайтиПоКоду");	
...
Процедура ВводКоличества_Активация()
	...
	ТПоле=Новый Структура();
	ТПоле=Новый Структура();	
	ТПоле.Вставить(ТП.tx,БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.ВыбранныйТовар.Наименование);
	ТПоле.Вставить(ТП.al,"cc");
	...
	ТПолеВвода.Вставить(ПВ.tp,"float_number");
	ТПолеВвода.Вставить(ПВ.ff,"roboto_medium");
	ТПолеВвода.Вставить(ПВ.fs,"40");      
	ТПолеВвода.Вставить(ПВ.tx,БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.ВыбранныйТовар.количество);	
	...
	Если СТЗапрос.action="key" Тогда
		...
		ИначеЕсли Данные=БЗ_TSDRU.ПеречислениеКнопок().КН_ENT Тогда // Ввод данных
			...
			Исключение
				...
				Возврат;
			КонецПопытки;
			Параметры=БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация;
...
#Область ВводПоКоду
Процедура НайтиПоКоду_Активация()
	Перем Шапка,Данные,Подвал,РядШапка,РядПодвал,Колонки,ТП;
	БЗ_TSDAPI.ИнициализацияПакетаОтвета(БЗ_TSDRU.ТипыФреймов().edit);
	БЗ_TSDAPI.ИнициализацияПараметров(Шапка,Данные,Подвал,РядШапка,РядПодвал,Колонки,ТП);
	БЗ_TSDAPI.ПерехватСобытий(БЗ_TSDRU.ПеречислениеКнопок().КН_ESC + БЗ_TSDRU.ПеречислениеКнопок().КН_ENT); // Кнопка назад + Ввод значения
	...
	ТПоле=Новый Структура();
	ТПоле=Новый Структура();	
	ТПоле.Вставить(ТП.tx,"ВВЕДИТЕ КОД ТОВАРА");
	ТПоле.Вставить(ТП.al,"cc");
	ТПоле.Вставить(ТП.fs,"22");
	ТПоле.Вставить(ТП.tf,"b");
	ТПоле.Вставить(ТП.pd,"2 2 4 4");
	...
	ТПолеВвода=Новый Структура();		
	ТПолеВвода.Вставить(ПВ.al,"сс");
	ТПолеВвода.Вставить(ПВ.tp,"text");
	ТПолеВвода.Вставить(ПВ.ff,"roboto_medium");
	ТПолеВвода.Вставить(ПВ.fs,"40");      
//	ТПолеВвода.Вставить(ПВ.tx,БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.ВыбранныйТовар.количество);	
	ТПолеВвода.Вставить(ПВ.mx,"15");
	ТПолеВвода.Вставить(ПВ.pd,"5 5 8 8");
	РядПоляВвода=Новый Структура();
	РядПоляВвода.Вставить(ПоляВвода.edt,ТПолеВвода);
	РядПоляВвода.Вставить(ТП.pd,"35 35 0 0");
...
Процедура НайтиПоКоду_ОбработчикСобытий()
	Перем Шапка,Данные,Подвал,РядШапка,РядПодвал,Колонки,ТП;
	...
	Если СТЗапрос.action="key" Тогда
		...
		ИначеЕсли Данные=БЗ_TSDRU.ПеречислениеКнопок().КН_ENT Тогда // Ввод данных
			...
			Если Введено="" Тогда
				БЗ_TSDAPI.ПустойОтветНаЗапрос();
			КонецЕсли;     
			ссылка=ВводКоличества_НайтиТоварПоКоду(введено);    
			Если ссылка=Неопределено Тогда 	
				БЗ_TSDAPI.ПоказатьОшибку("ТОВАР СО ШТРИХКОДОМ"+Символы.ПС+введено+Символы.ПС+"НЕ НАЙДЕН !");
			Иначе
				
				структураТовара= НОвый Структура();       
				Если ТипЗНЧ(ссылка)=Тип("СправочникСсылка.ХарактеристикиНоменклатуры") Тогда
					структураТовара.Вставить("Номенклатура",Ссылка.владелец);
					структураТовара.Вставить("Характеристика", Ссылка);  
				иначе
					структураТовара.Вставить("Номенклатура",Ссылка);
					структураТовара.Вставить("Характеристика", Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());  
				КонецЕсли;
				
				структураТовара.Вставить("Ячейка", Справочники.Ячейки.ПустаяСсылка());  
				структураТовара.Вставить("Количество", 0);
				структураТовара.Вставить("Наименование",структураТовара.Номенклатура.наименование+ " " +структураТовара.характеристика.наименование);		
				БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("ВыбранныйТовар",структураТовара);      
				БЗ_TSDRU.Сессия().ПараметрыСессии.Инвентаризация.Вставить("СканированиеШК","Ручной");
                АктивироватьРежим("ВводКоличества");
...
Функция ВводКоличества_НайтиТоварПоКоду(Код)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ХарактеристикиНоменклатуры.Код = &Код
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
