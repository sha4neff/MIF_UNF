...
Процедура Инициализация()
	БЗ_TSDRU.Сессия().ПараметрыСессии.Вставить("Перемещение",Новый Структура());
	АктивироватьРежим("ВыборДокумента");	
КонецПроцедуры

Процедура ДобавитьВведенныеДанные(ДНоменклатура,ДКоличество=1,ШК="")
	АктивныйДокумент=БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.АктивныйДокумент;
	СтартТранзакции();
...
Процедура ВыборДокумента_ОбработчикСобытий()
	...
	ИначеЕсли   СТЗапрос.action="scan" Тогда
		...
		Если не  ЗначениеЗаполнено(Выбрано) Тогда
			БЗ_TSDAPI.ПустойОтветНаЗапрос();
			Возврат
		КонецЕсли;                
		док=ВыборДокумента_найтиДокЯчейки(выбрано);		
		БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Вставить("АктивныйДокумент",док.Ссылка);
...
Функция ВыборДокумента_НайтиЯчейкуПоШК(ШК)    
	...
	Запрос.УстановитьПараметр("БЗ_ШК", ШК);
...
функция ВыборДокумента_найтиДокЯчейки(Ячейка)
	...
	док.ВидОперации=Перечисления.ВидыОперацийПеремещениеПоЯчейкам.ИзОднойВНесколько;   
...
Процедура ОсновноеМеню_ЗавершитьДокумент()
	...
	НаборЗаписей=РегистрыСведений.ТСД_Документы.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Документ.Установить(БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.АктивныйДокумент);
...
Процедура ОсновноеМеню_ОбработчикСобытий()
	...
	Если СТЗапрос.action="select" Тогда
		...
		Если Выбрано.Операция="ВводДанных" Тогда
			БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Удалить("ВыбранныйТовар");
			АктивироватьРежим("ВводДанных");
...
Процедура ВводДанных_Активация()
	...
	ТПоле=Новый Структура();	
	ТПоле.Вставить(ТП.tx,"Документ перемещения");
	ТПоле.Вставить(ТП.al,"cc");
	ТПоле.Вставить(ТП.fs,"16");		
	ТПоле.Вставить(ТП.pd,"2 2 2 2");
...
Функция ВводДанных_ПолучитьСписок(Обновление=Ложь)
	...
	|	ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка) КАК Поле1
	|ПОМЕСТИТЬ ВтОст
	|ИЗ
	|	РегистрНакопления.ЗапасыНаСкладах.Остатки(, Ячейка = &ячейка) КАК ЗапасыНаСкладахОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	ВтОст.Номенклатура,
	|	ВтОст.Номенклатура.Наименование,
	|	ВтОст.Характеристика,
	|	ВтОст.Характеристика.Наименование,
	|	ЗНАЧЕНИЕ(справочник.ячейки.пустаяСсылка),
	|	ВтОст.КоличествоОстаток,
	|	0
	|ИЗ
	|	ВтОст КАК ВтОст
	|;
	...
	|	ВТОбщая.Ячейка,
	|	ВТОбщая.Характеристика";	         
	Док=БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.АктивныйДокумент;
...
//	Запрос.УстановитьПараметр("ДокументОснование", док.ДокументОснование);
	...
	Если НЕ Обновление Тогда
		Если БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Свойство("ВыбранныйТовар") Тогда
			НСтрока=ТБТовары.Найти(БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.ВыбранныйТовар.номенклатура,"Номенклатура");
...
Процедура ВводДанных_НайтиТоварПоШК(Штрихкод)   
	...
	Запрос.УстановитьПараметр("Ссылка",БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.АктивныйДокумент);
	...
	Если Выборка.Следующий() Тогда               
		...
		БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Вставить("ВыбранныйТовар",структураТовара);
		БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Вставить("СканированиеШК",Штрихкод);
...
Процедура ВводДанных_ОбработчикСобытий()
	...
	Если СТЗапрос.action="select" Тогда
		...
		БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Вставить("ВыбранныйТовар",структураТовара);
		БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.Вставить("СканированиеШК","");
		АктивироватьРежим("ВводКоличества");
	ИначеЕсли СТЗапрос.action="scan" Тогда   
		БЗ_TSDAPI.ПустойОтветНаЗапрос();
...
Процедура ВводКоличества_Активация()
	...
	ТПоле=Новый Структура();
	ТПоле=Новый Структура();	
	ТПоле.Вставить(ТП.tx,БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.ВыбранныйТовар.Наименование);
	ТПоле.Вставить(ТП.al,"cc");
	...
	ТПолеВвода.Вставить(ПВ.tx,БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение.ВыбранныйТовар.количество);	
	...
	Если СТЗапрос.action="key" Тогда
		...
		ИначеЕсли Данные=БЗ_TSDRU.ПеречислениеКнопок().КН_ENT Тогда // Ввод данных
			...
			Исключение
				...
				Возврат;
			КонецПопытки;
			Параметры=БЗ_TSDRU.Сессия().ПараметрыСессии.Перемещение;
