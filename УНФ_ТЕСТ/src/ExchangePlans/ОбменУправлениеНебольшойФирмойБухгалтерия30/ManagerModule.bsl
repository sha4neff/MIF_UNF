...
Функция ОпределитьМассивПолучателей(Выгрузка, Объект, Получатели) Экспорт
	...
	Для Каждого Узел Из Получатели Цикл
		...
		ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказПокупателя")
			...
			И Не Узел.ПереноситьЗаказыКакСчетаНаОплату
			И Не (ТипЗнч(Узел) = Тип("ПланОбменаСсылка.СинхронизацияДанныхЧерезУниверсальныйФормат") И (Узел.ВариантНастройки = "ВебВитрина" ИЛИ Узел.ВариантНастройки = "КабинетКлиента")) Тогда
