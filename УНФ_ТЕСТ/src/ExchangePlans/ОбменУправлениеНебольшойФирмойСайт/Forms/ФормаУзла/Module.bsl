...
Процедура ЗаполнитьСписокВыбораСоответствиеПолейНоменклатуры()
	...
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Наименование", "Наименование");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("НаименованиеПолное", "Наименование для печати");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Комментарий", "Описание");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Штрихкод", "Штрихкод");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Артикул", "Артикул");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Код", "Код");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("СтранаПроисхождения", "Страна происхождения");
	СписокВыбораСоответствиеПолейНоменклатуры.Добавить("Вес", "Вес");
...
Процедура ЗаполнитьГиперссылкиЦенСервер()
	...
	Если ЗначениеЗаполнено(Объект.ВидЦенУслуг) Тогда
		Элементы.ВыбратьВидЦенУслуг.Заголовок = "Вид цен: " + Объект.ВидЦенУслуг;
	КонецЕсли;
...
Процедура ВыбратьВидыЦенУслугЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено И ТипЗнч(РезультатВыбора) = Тип("СправочникСсылка.ВидыЦен") Тогда
		...
		Элементы.ВыбратьВидЦенУслуг.Заголовок = "Вид цен: "+РезультатВыбора;
    КонецЕсли;
...
Функция НастройкиВыгрузкиРеквизитовПредставление(НастройкиВыгрузкиРеквизитов)
	...
	Иначе
		СтруктураНастроки = ОбменССайтомСлужебныйВызовСервера.ЧтениеJSONВСтруктуру(НастройкиВыгрузкиРеквизитов, Ложь);
...
Функция НастройкиВыгрузкиДопРеквизитовЗаказовПредставление(НастройкиВыгрузкиДопРеквизитовЗаказов)
	...
	Если НастройкиВыгрузкиДопРеквизитовЗаказов = "" Тогда
		СтрокаПредставление = "<Реквизиты не выбраны>";
	Иначе
		СтруктураНастроки = ОбменССайтомСлужебныйВызовСервера.ЧтениеJSONВСтруктуру(НастройкиВыгрузкиДопРеквизитовЗаказов, Ложь);
		Для каждого СтрокаНастроек Из СтруктураНастроки Цикл
			СтрокаПредставление = СтрокаПредставление + СтрокаНастроек.Ключ + ", ";
		КонецЦикла;
		СтрокаПредставление = Лев(СтрокаПредставление, СтрДлина(СтрокаПредставление)-2);
...
Процедура ВыбратьРеквизитыЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Объект.НастройкиВыгрузкиРеквизитов = ОбменССайтомСлужебныйВызовСервера.ЗаписьJSONВСтруктуру(ВыбранныйЭлемент);
	КонецЕсли;
...
Процедура ВыбратьДопРеквизитыЗаказовЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Объект.НастройкиВыгрузкиДопРеквизитовЗаказов = ОбменССайтомСлужебныйВызовСервера.ЗаписьJSONВСтруктуру(ВыбранныйЭлемент);
	КонецЕсли;
