...
Процедура ОбработкаСобытия(НовостьСсылка, Форма, СписокПараметров) Экспорт
	...
	НайденноеДействие = ИнтернетПоддержкаПользователейКлиент.НайтиЭлементСпискаЗначений(СписокПараметров, ПараметрыПоиска);
	...
	Если НайденноеДействие.Значение = "ПодключитьЭДО" Тогда
		...
		Если УправлениеНебольшойФирмойВызовСервера.ПолучитьФункциональнуюОпциюСервер("ИспользоватьОбменЭД") Тогда
			...
			Если УправлениеНебольшойФирмойУправлениеДоступомПовтИсп.ЕстьПравоНастройкиЭДО() Тогда
...
// Параметры:
//  Форма                            - ФормаКлиентскогоПриложения - форма, в которой необходимо обработать событие ПриОткрытии;
//  ИдентификаторыСобытийПриОткрытии - Строка, Массив из Строка - идентификаторы событий, по которым необходимо отбирать
//                                                                контекстные новости;
//  СтандартнаяОбработка             - Булево - если установить в Ложь, то стандартная обработка, а также
...
// Как правило, используется для интерактивной обработки представления новости в случае срабатывания действий новости
// типа "Оповещение".
//
// Параметры:
...
Процедура РазрешенаРаботаСНовостямиТекущемуПользователю(Результат) Экспорт
	...
	ТекущийПользовательЗаписан = ЗначениеЗаполнено(ПользователиКлиент.АвторизованныйПользователь());
	Результат = ТекущийПользовательЗаписан;
