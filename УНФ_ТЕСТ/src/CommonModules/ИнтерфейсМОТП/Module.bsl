...
//	  ** МРЦ - Число - МРЦ.
//
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ВариантПолученияМРЦ = "ВычислениеИЗапрос", Организация = Неопределено) Экспорт
	...
	ДеревоУпаковок.Колонки.Добавить("ДатаПроизводства",          Новый ОписаниеТипов("Дата"));
	...
	ДеревоУпаковок.Колонки.Добавить("ПредставлениеНоменклатуры", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	ДеревоУпаковок.Колонки.Добавить("ВСеройЗоне",                Новый ОписаниеТипов("Булево"));
	ДеревоУпаковок.Колонки.Добавить("СодержитСерыеКоды",         Новый ОписаниеТипов("Булево"));
	...
	ПакетыВложенныхКодовМаркировки  = Новый Массив;
	ПакетВложенныхКодовМаркировки   = Неопределено;
	КоличествоВложенныхКодовВПакете = 1000;
	
	ПакетыКодовМаркировкиБезВидаУпаковки = Новый Массив;
	ПакетКодовМаркировкиБезВидаУпаковки  = Неопределено;
	КоличествоВПакетеБезВидаУпаковки     = 1000;
	КодыМаркировкиБезВидаУпаковки        = Новый Соответствие;
	...
	Для Каждого КлючИЗначение Из СтатусыКодовМаркировки Цикл
		...
		Для Каждого СтрокаВложенногоКодаМаркировки Из ВложенныеУпаковки Цикл
			...
			ВидУпаковки = СтрокаВложенногоКодаМаркировки.СтрокаКодаМаркировки.ВидУпаковки;
			Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				Или ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				
				Если ПакетВложенныхКодовМаркировки = Неопределено
					Или ПакетВложенныхКодовМаркировки.Количество() >= КоличествоВложенныхКодовВПакете Тогда
					ПакетВложенныхКодовМаркировки = Новый Массив;
					ПакетыВложенныхКодовМаркировки.Добавить(ПакетВложенныхКодовМаркировки);
				КонецЕсли;
				
				ПакетВложенныхКодовМаркировки.Добавить(СтрокаВложенногоКодаМаркировки);
				
			ИначеЕсли ШтрихкодированиеИСКлиентСервер.ВозможнаГрупповаяУпаковка(
				СтрокаВложенногоКодаМаркировки.СтрокаКодаМаркировки.ВидУпаковки,
				СтрокаВложенногоКодаМаркировки.СтрокаКодаМаркировки.ДанныеРазбора) Тогда
				
				Если ПакетКодовМаркировкиБезВидаУпаковки = Неопределено
					Или ПакетКодовМаркировкиБезВидаУпаковки.Количество() >= КоличествоВПакетеБезВидаУпаковки Тогда
					ПакетКодовМаркировкиБезВидаУпаковки = Новый Массив;
					ПакетыКодовМаркировкиБезВидаУпаковки.Добавить(ПакетКодовМаркировкиБезВидаУпаковки);
				КонецЕсли;
				
				ПакетКодовМаркировкиБезВидаУпаковки.Добавить(СтрокаВложенногоКодаМаркировки.СтрокаКодаМаркировки);
				
				КодыМаркировкиБезВидаУпаковки.Вставить(СтрокаВложенногоКодаМаркировки.СтрокаКодаМаркировки, СтрокаВложенногоКодаМаркировки.КодМаркировки);
	...
	Для Каждого ПакетКодовМаркировкиБезВидаУпаковки Из ПакетыКодовМаркировкиБезВидаУпаковки Цикл
		
		РезультатЗапросаСтатусовКодовМаркировок = ЗапроситьСтатусыКодовМаркировкиПакетно(ПакетКодовМаркировкиБезВидаУпаковки, СтатусыКодовМаркировки);
		...
		Для Каждого СтрокаКодаМаркировки Из ПакетКодовМаркировкиБезВидаУпаковки Цикл
			...
			Если ПараметрыКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				...
				КодМаркировки = КодыМаркировкиБезВидаУпаковки[СтрокаКодаМаркировки];
				
				Если ДетализироватьДоПачек Тогда
					ДанныеОбАгрегации.Вставить(КодМаркировки, ВложенныеУпаковки);
				Иначе
					ДанныеОбАгрегации.Вставить(КодМаркировки, ВложенныеУпаковки.Количество());
				КонецЕсли;
	...
	Пока ПакетыВложенныхКодовМаркировки.Количество() > 0 Цикл
		
		СледующиеПакетыВложенныхКодовМаркировки = Новый Массив;
		ПакетВложенныхКодовМаркировки           = Неопределено;
		
		Для Каждого ПакетКодовМаркировки Из ПакетыВложенныхКодовМаркировки Цикл
			
			Результат = ЗапроситьДанныеОбАгрегацииКодовМаркировкиПакетно(ПакетКодовМаркировки, Организация);
			Если Результат.ДанныеОбАгрегации <> Неопределено
				...
				Для Каждого ЭлементПакета Из ПакетКодовМаркировки Цикл
					...
					СоответствиеВложенныхКодов = Результат.ДанныеОбАгрегации[ЭлементПакета.КодМаркировки];
					Если СоответствиеВложенныхКодов = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					
					ВидУпаковки  = ЭлементПакета.СтрокаКодаМаркировки.ВидУпаковки;
					ВидПродукции = ЭлементПакета.СтрокаКодаМаркировки.ВидПродукции;
					Если ДетализироватьДоПачек Или ВидУпаковки <> Перечисления.ВидыУпаковокИС.Групповая Тогда
						...
						Для Каждого КлючИЗначение Из СоответствиеВложенныхКодов Цикл
							...
							ЭлементДанныхВложенногоКодаМаркировки = Новый Структура;
							ЭлементДанныхВложенногоКодаМаркировки.Вставить("КодМаркировки",        ВложенныйКодМаркировки);
							ЭлементДанныхВложенногоКодаМаркировки.Вставить("СтрокаКодаМаркировки", СтрокаКодаМаркировки);
							
							ВложенныеУпаковки.Добавить(ЭлементДанныхВложенногоКодаМаркировки);
							
							Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
								Или СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
								
								Если ПакетВложенныхКодовМаркировки = Неопределено
									Или ПакетВложенныхКодовМаркировки.Количество() >= КоличествоВложенныхКодовВПакете Тогда
									ПакетВложенныхКодовМаркировки = Новый Массив;
									СледующиеПакетыВложенныхКодовМаркировки.Добавить(ПакетВложенныхКодовМаркировки);
								КонецЕсли;
								
								ПакетВложенныхКодовМаркировки.Добавить(ЭлементДанныхВложенногоКодаМаркировки);
								
							КонецЕсли;
							
						КонецЦикла;
						
						ДанныеОбАгрегации.Вставить(ЭлементПакета.КодМаркировки, ВложенныеУпаковки);
						
					Иначе
						
						ДанныеОбАгрегации.Вставить(ЭлементПакета.КодМаркировки, СоответствиеВложенныхКодов.Количество());
		...
		ПакетыВложенныхКодовМаркировки = СледующиеПакетыВложенныхКодовМаркировки;
	...
	Для Каждого ПакетКодовМаркировки Из ПакетыКодовМаркировки Цикл
		...
		Для Каждого СтрокаКодаМаркировки Из ПакетКодовМаркировки Цикл
			...
			Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
				...
				СтрокаДерева.ДатаПроизводства          = ДанныеСтатуса.ДатаПроизводства;
				СтрокаДерева.ПредставлениеНоменклатуры = ДанныеСтатуса.Наименование;
			...
			Иначе
				...
				Если ДанныеСтатуса.Статус <> Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен Тогда
					...
					Результат.Вставить("ДанныеОбАгрегации", ДанныеСтатуса.ВложенныеУпаковки);
					...
					Если Результат.ДанныеОбАгрегации <> Неопределено
						...
						ДанныеСтроки.Вставить("ДатаПроизводства",          ДанныеСтатуса.ДатаПроизводства);
						ДанныеСтроки.Вставить("ПредставлениеНоменклатуры", ДанныеСтатуса.Наименование);
						ДанныеСтроки.Вставить("СоставКодаМаркировки",      СтрокаКодаМаркировки.СоставКодаМаркировки);
						ДанныеСтроки.Вставить("ВидУпаковки",               СтрокаКодаМаркировки.ВидУпаковки);
						...
						Если ДобавлятьУпаковкуНаВерхнийУровень Тогда
							...
							СтрокаДерева.ДатаПроизводства          = ДанныеСтроки.ДатаПроизводства;
							СтрокаДерева.ПредставлениеНоменклатуры = ДанныеСтроки.ПредставлениеНоменклатуры;
	...
	Для Каждого СтрокаТаблицы Из МассивИсходныхСтрок Цикл
		...
		Для Каждого ВидПродукции Из ВидыПродукции Цикл
			...
			Если СтрокиПоКодуМаркировки = Неопределено Тогда
				СтрокиПоКодуМаркировки = Новый Массив();
				СоответствиеКодовМаркировки[НормализованныйКодМаркировки] = СтрокиПоКодуМаркировки;
			КонецЕсли;
			
			СтрокиПоКодуМаркировки.Добавить(СтрокаТаблицы);
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Для Каждого ЭлементМассива Из ДанныеОбработки Цикл
						...
						Если Значение <> Неопределено Тогда
							ВидУпаковки = ИнтерфейсИСМПСлужебный.ВидУпаковки(Значение);
						Иначе
...
Функция ЗапроситьДанныеОбАгрегацииКодовМаркировкиПакетно(ПакетКодовМаркировки, Организация = Неопределено) Экспорт
	...
	ТелоЗапроса = Новый Массив;
	Для Каждого ЭлементПакета Из ПакетКодовМаркировки Цикл
		ТелоЗапроса.Добавить(ЭлементПакета.КодМаркировки);
	КонецЦикла;
			...
			Иначе
				...
				Для Каждого СтрокаДанных Из ДанныеОбработки Цикл
					...
					Если СтрокаДанных.Свойство("packageType") И ЗначениеЗаполнено(СтрокаДанных.packageType) Тогда
						СтрокаПродукции.ТипУпаковки = ИнтерфейсИСМПСлужебный.ВидУпаковки(СтрокаДанных.packageType);
					КонецЕсли;
...
Процедура ЗапросМРЦПоСтрокамДерева(ЗапрашиваемыеСтроки) Экспорт
	...
	Если ЗапрашиваемыеСтроки.Количество() = 0
		Или Не ИнтеграцияИСМПКлиентСерверПовтИсп.УчитыватьМРЦ() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоУпаковокРекурсивно(ПараметрыЗаполнения)
	...
	Если ТипЗнч(КодыМаркировкиУпаковок) <> Тип("Число") Тогда
		...
		Для Каждого ЗначениеВложенногоКода Из КодыМаркировкиУпаковок Цикл
			...
			КодМаркировки        = ЗначениеВложенногоКода.КодМаркировки;
			СтрокаКодаМаркировки = ЗначениеВложенногоКода.СтрокаКодаМаркировки;
			...
			Иначе
				ВложенныеКодыМаркировки = ДанныеОбАгрегации[КодМаркировки];
			КонецЕсли;
			...
			Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				...
				GTIN = "";
				МРЦ  = 0;
				СтрокаКодаМаркировки.СоставКодаМаркировки.Свойство("GTIN", GTIN);
				...
				Если ДанныеВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
					ДанныеВерхнегоУровня.СоставКодаМаркировки.Свойство("GTIN", GTINКоробаВерхнегоУровня);
				КонецЕсли;
				...
				Если Не ПропуститьУпаковку Тогда
					...
					СтрокаДерева.ДатаПроизводства = ДанныеВерхнегоУровня.ДатаПроизводства;
					...
					ПараметрыЗаполненияВложенные.ДанныеОбАгрегации      = ДанныеОбАгрегации;
				...
				Иначе
					...
					ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("GTIN",                 GTIN);
					ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("МРЦ",                  МРЦ);
					...
					ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("ДатаПроизводства",     ДанныеВерхнегоУровня.ДатаПроизводства);
					...
					ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("СоставКодаМаркировки", СтрокаКодаМаркировки.СоставКодаМаркировки);
					ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("ВидУпаковки",          СтрокаКодаМаркировки.ВидУпаковки);
		...
		Если ПустаяСтрока(ДанныеВерхнегоУровня.ИННВладельца) И ДанныеСтроки.ИННВладельца <> NULL Тогда
	...
	Иначе
		...
		КоличествоПачек  = КодыМаркировкиУпаковок;
		КоличествоБлоков = 0;
