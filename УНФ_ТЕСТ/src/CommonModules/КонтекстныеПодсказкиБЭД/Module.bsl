...
Процедура ЗадатьЗначениеСчетчикаКоличестваНепрочитанныхСтатей(Форма)
	...
	Если КоличествоНовостейДляСчетчика > 0 Тогда
		...
		ДанныеСклонения = ПолучитьСклоненияСтрокиПоЧислу("совет", КоличествоНовостейДляСчетчика, "", "ЧС=Количественное", "ПД=Именительный ");
		
		Если ЗначениеЗаполнено(ДанныеСклонения) Тогда
			Форма.Элементы.ПанельКонтекстныхНовостей_СчетчикНепрочитанных.Заголовок = "еще " + ДанныеСклонения[0];
		КонецЕсли;
