...
Процедура ЗаполнитьПараметрыНалогооблаженияДокумента(ФормаВладелец, ПараметрыПодбора)
	...
	ИначеЕсли ФормаВладелец.ИмяФормы = "Документ.ПеремещениеЗапасов.Форма.ФормаДокумента"
		...
		ИЛИ ФормаВладелец.ИмяФормы = "Документ.СборкаЗапасов.Форма.ФормаДокумента" Тогда
...
Процедура ЗаполнитьВидСкидкиНаценки(ФормаВладелец, ПолноеИмяФормыПодбора, ПараметрыПодбора)
	...
	Если Найти(ПолноеИмяФормыПодбора, "Обработка.ПодборРеализация.Форма.КорзинаЦенаОстатокРезервХарактеристика") > 0 Тогда
...
// Процедуры адаптированные для новой формы подбора

Процедура ЗаполнитьЗначенияПараметровНачальнаяНастройка(ФормаВладелец, ИмяТабличнойЧасти, ПараметрыПодбора)
	
	Если СтрНайти(ФормаВладелец.ИмяФормы, "ПеремещениеЗапасов") > 0 Тогда
	...
	Если (ФормаВладелец.ИмяФормы = "Документ.ПеремещениеПоЯчейкам.Форма.ФормаДокумента"
		...
		Или ФормаВладелец.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаКалькуляции") Или ЭтоМаркировка Тогда
