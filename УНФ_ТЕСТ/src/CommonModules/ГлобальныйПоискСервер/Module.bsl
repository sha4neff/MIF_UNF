...
Функция ЗаказыПокупателейПоКонтакту(Контакт)
	...
	ЗаказыПокупателей.Вставить("ПредстояшиеЗаказы", Новый Массив);
	
	Результат = Запрос.ВыполнитьПакет();
	ПоследниеЗаказы = Результат[0].Выбрать();
	ПредстояшиеЗаказы = Результат[1].Выбрать();
	...
	КонецЦикла;
	Пока ПредстояшиеЗаказы.Следующий() Цикл
		ДанныеЗаказа = Новый Структура("Ссылка,СостояниеЗаказа,ВариантЗавершения");
		ЗаполнитьЗначенияСвойств(ДанныеЗаказа, ПредстояшиеЗаказы);
		ЗаказыПокупателей.ПредстояшиеЗаказы.Добавить(ДанныеЗаказа);
	КонецЦикла;
...
Функция ЗаказыПоставщикуПоКонтакту(Контакт)
	...
	ЗаказыПоставщику.Вставить("ПредстояшиеЗаказы", Новый Массив);
	...
	Пока ПредстояшиеЗаказы.Следующий() Цикл
		ДанныеЗаказа = Новый Структура("Ссылка,СостояниеЗаказа,ВариантЗавершения");
		ЗаполнитьЗначенияСвойств(ДанныеЗаказа, ПредстояшиеЗаказы);
		ЗаказыПоставщику.ПредстояшиеЗаказы.Добавить(ДанныеЗаказа);
	КонецЦикла;
...
Функция НайтиДокументыПоКонтакту(Контакт)
	...
	Для каждого ЗаказПокупателя Из Заказы.ПредстояшиеЗаказы Цикл
		НоваяСтрока = РезультатПоиска.Добавить();
		НоваяСтрока.Значение = ЗаказПокупателя.Ссылка;
		НоваяСтрока.Метаданные = ЗаказПокупателя.Ссылка.Метаданные();
...
Процедура ВыполнитьПоискКонтрагентовПоДаннымЕГРН(СтрокаПоиска, ИНН, ЭтоЮрЛицо, РезультатПоиска)
	...
	Если ЭтоЮрЛицо Тогда
		Если ДанныеКонтрагента.ЮридическийАдрес <> Неопределено Тогда
			СтруктураОписание.Вставить(НСтр("ru='Юридический адрес'"), ДанныеКонтрагента.ЮридическийАдрес.Представление);
		КонецЕсли;
		Если ДанныеКонтрагента.Руководитель <> Неопределено Тогда
			СтруктураОписание.Вставить(НСтр("ru='Руководитель'"), ДанныеКонтрагента.Руководитель.Представление);
		КонецЕсли;
	КонецЕсли;
...
Функция КартинкаКонтакта(Контакт)
	...
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если ВидКонтрагента = Перечисления.ВидыКонтрагентов.ИндивидуальныйПредприниматель
			...
			Возврат БиблиотекаКартинок.КолекцияКонтрагентыФизическоеЛицо;
		Иначе
			Возврат БиблиотекаКартинок.КолекцияКонтрагентыКомпания;
		КонецЕсли;
