...
// Дата действия новой формы 6-НДФЛ
// за 2017 год  (Приказ N ММВ-7-11/18@)
Функция ДатаДействияПриказа6НДФЛЗа2017год() Экспорт
	Возврат '20180325';
КонецФункции

Процедура ПриИнициализацииФормыРегламентированногоОтчета(Форма) Экспорт
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		ТаблДокумент = Форма.Элементы.Найти("ТабличныйДокумент");
		Если ТаблДокумент <> Неопределено Тогда
			Форма.Элементы.ТабличныйДокумент.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
