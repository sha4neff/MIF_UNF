...
Функция ОткрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
		...
		Если ПараметрыРегистрации.Свойство("КодыСистемыНалогообложения") И ПараметрыРегистрации.КодыСистемыНалогообложения = "3" Тогда
				ТекстСообщения = НСтр("ru='В соответствии с Федеральным законом от 29.06.2012 № 97-ФЗ система налогообложения в виде единого налога на вмененный доход (ЕНВД) с 1 января 2021 года не применяется.'") + Символы.ПС +
					НСтр("ru='Для текущего ККТ указана единственная система налогообложения – ЕНВД.'") + Символы.ПС + 
					НСтр("ru='Для продолжения работы необходимо дополнить список применяемых СНО с помощью операции ""Изменение параметров регистрации фискального накопителя.""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
