...
Функция ПараметрыКодаМаркировки(ЭлементДанных = Неопределено, ВидПродукции = Неопределено) Экспорт
	...
	ПараметрыКодаМаркировки.Вставить("Наименование");
	...
	ПараметрыКодаМаркировки.Статус = ИнтерфейсМОТПСлужебный.СтатусКодаМаркировки(ИсточникДанных["status"]);
	
	Значение = ИсточникДанных["packageType"];
	Если Значение <> Неопределено Тогда
		ПараметрыКодаМаркировки.ВидУпаковки = ИнтерфейсИСМПСлужебный.ВидУпаковки(Значение);
	КонецЕсли;
	
	Значение = ИсточникДанных["productGroup"];
	Если Значение <> Неопределено Тогда
		ПараметрыКодаМаркировки.ВидПродукции = ИнтерфейсИСМПСлужебный.ТоварнаяГруппа(Значение);
	КонецЕсли;
	...
	Если Значение <> Неопределено Тогда
		ПараметрыКодаМаркировки.Наименование = Значение;
	КонецЕсли;
	...
	Если ИсточникДанных["child"] <> Неопределено
		...
		ПараметрыКодаМаркировки.ВложенныеУпаковки = Новый Массив;
		Для Каждого ВложенныйКодМаркировки Из ИсточникДанных["child"] Цикл
			
			ВидПродукцииДляРазбораКода = ВидПродукции;
			Если Не ЗначениеЗаполнено(ВидПродукцииДляРазбораКода)
			...
			СтрокаКодаМаркировки = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
				ВложенныйКодМаркировки, ВидПродукцииДляРазбораКода, Ложь);
			...
			ПараметрыКодаМаркировки.ВложенныеУпаковки.Добавить(ЭлементДанныхВложенногоКодаМаркировки);
