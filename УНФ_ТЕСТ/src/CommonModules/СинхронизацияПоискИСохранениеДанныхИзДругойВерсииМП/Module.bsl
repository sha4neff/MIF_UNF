...
Функция НайтиДанныеИзДругойВерсии(ОбъектXDTOВыгрузки, ОбъектXDTOКонфигурации, ВерсияПакета, СтруктураПредопределенныхДанных, Ссылка, КодУзла) Экспорт
	...
	СтруктраМассивов = ПоискНовыхРеквизитовВТЧ(ОбъектXDTOКонфигурации, ОбъектXDTOВыгрузки);
	
	Возврат Новый Структура("МассивСНовымиРеквизитами, СтруктраМассивов", МассивСНовымиРеквизитами, СтруктраМассивов)
	...
	Если Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) Тогда
		...
		Если РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
			...
			Для каждого FilterItemВыгрузки Из ОбъектXDTOВыгрузки.Filter.FilterItem Цикл
				МетаданныеОбъекта = Метаданные.НайтиПоТипу(ТипЗнч(FilterItemВыгрузки.Value));
				Если МетаданныеОбъекта <> Неопределено Тогда 
					ХешИзмерениий = ХешИзмерениий + "_" + FilterItemВыгрузки.Value.УникальныйИдентификатор();
					Продолжить;
				КонецЕсли;
				ХешИзмерениий = ХешИзмерениий + "_" + FilterItemВыгрузки.Value;
			КонецЦикла;
			
			Хеш = Хеш + ХешИзмерениий;
			Возврат Хеш
		КонецЕсли;
	КонецЕсли;
...
Функция ПоискНовыхРеквизитовВТЧ(ОбъектXDTOКонфигурации, ОбъектXDTOВыгрузки)
	...
	СтруктраМассивов = Новый Структура("МассивСНовымиРеквизитамиТЧ, МассивНаименованийТЧ, МассивСНаименованиямиНовыхТЧ", МассивСНовымиРеквизитамиТЧ, МассивНаименованийТЧ, МассивСНаименованиямиНовыхТЧ);
	
	Возврат СтруктраМассивов
...
Процедура СохранитьНовыеДанные(СтруктураСНовымиДанными, ОбъектXDTOВыгрузки, ВерсияПакета, СтруктураПредопределенныхДанных, КодУзла) Экспорт
	...
	Если СтруктураСНовымиДанными.МассивСНовымиРеквизитами.Количество() > 0 ИЛИ СтруктураСНовымиДанными.СтруктраМассивов.МассивСНовымиРеквизитамиТЧ.Количество() > 0 ИЛИ СтруктураСНовымиДанными.СтруктраМассивов.МассивСНаименованиямиНовыхТЧ.Количество() > 0 Тогда
