...
Процедура ПолучитьШаблон(НастройкаИнтеграции, ДанныеШаблона, Ошибки) Экспорт
	...
	ДанныеШаблона.СсылкаНаПредпросмотр = ДанныеОтвета.ПараметрыТела.result.fullsize_screenshot_url;
...
Процедура ЗаполнитьВнешниеURLРассылки(НастройкаИнтеграции, ДанныеРассылки) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеРассылки.Идентификатор) Тогда
		ШаблонURL = "https://cp.unisender.com/ru/v5/bursts/view/%1";
	...
	Если ЗначениеЗаполнено(ДанныеРассылки.ИдентификаторПисьмаОснования) Тогда
		ШаблонURL = "https://cp.unisender.com/ru/v5/letter/send/%1";
		ДанныеРассылки.ВнешниеСсылки.СтраницаПисьма = СтрШаблон(ШаблонURL, ДанныеРассылки.ИдентификаторПисьмаОснования);
	КонецЕсли;
