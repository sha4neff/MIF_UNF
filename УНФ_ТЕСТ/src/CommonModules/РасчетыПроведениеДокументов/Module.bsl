...
// * НомерСертификата - СправочникСсылка.СерийныеНомера -
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Документ) = Тип("ДокументСсылка.Взаимозачет") Тогда
				СтруктураЗначения = УправлениеНебольшойФирмойСервер.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Документ, "ВидОперации, Дата, Курс, Кратность");
				...
				КонецЕсли;
			Иначе
				СтруктураЗначения = УправлениеНебольшойФирмойСервер.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Документ, "ВалютаДокумента, Курс, Кратность");
			КонецЕсли;
...
Процедура СформироватьТаблицаОплатаДокументовНакладные(ДокументСсылка, СтруктураДополнительныеСвойства, МассивДоступныхОпераций) Экспорт
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			...
			СтруктураЗначения = УправлениеНебольшойФирмойСервер.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Договор,ВалютаДокумента,Курс,Кратность");
...
Процедура СформироватьДвиженияПоВзаиморасчетамОплатаАвтоматически(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.УчетВалютныхОпераций Тогда
		...
		Если СтруктураДополнительныеСвойства.ВалютаДокумента = СтруктураДополнительныеСвойства.ВалютаУчета Тогда
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаВВалютеДокумента, Движения, "Сумма", "СуммаВал");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаВВалютеДокумента, Движения, "СуммаПлатежа", "СуммаВал");
		КонецЕсли;
		
		Если СтруктураДополнительныеСвойства.ВалютаДокумента = СтруктураДополнительныеСвойства.НациональнаяВалюта Тогда
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаВВалютеДокумента, Движения, "СуммаРег", "СуммаВал");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаВВалютеДокумента, Движения, "СуммаПлатежа", "СуммаВал");
		КонецЕсли;
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляДокументовОтгрузки(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	Если ТребуетсяЗачетАвансов Тогда
		...
		// 1. Определяем авто или вручную, в завимости от этого выбираем остатки авансов, либо подставляем ТЧ Предоплата
		...
		// Зачтём автоаванс под долги, которые не оплачены вручную.
...
Процедура ВзаимозачетДобавитьДвиженияВТаблицуРасчетов(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Организация", УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(ДокументСсылка.Организация));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
...
Процедура ПодготовитьТаблицуВзаиморасчетовСУчетомАвансов(ДокументСсылка, СтруктураДополнительныеСвойства, ЭтоРасчетыСПоставщиком) Экспорт
	...
	УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(АвансСумма, ВременныеТаблицы.ВременнаяТаблицаПредоплата, "Сумма", "СуммаВал");
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ДополнительныеРасходы") Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаРасходы, "Сумма", "Сумма");
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаЗапасы, "СуммаРасходов", "СуммаРасходовВал");
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаЗапасы, "Себестоимость", "СуммаРасчетовПринятыеПереданные");
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаСВознаграждением, ВременныеТаблицы.ВременнаяТаблицаЗапасы, "Сумма", "СуммаРасчетовПринятыеПереданные");
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		...
		СуммаСВознаграждением = ДолгСумма + ВременныеТаблицы.ВременнаяТаблицаЗапасы.Итог("СуммаВознаграждения");
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаСВознаграждением, ВременныеТаблицы.ВременнаяТаблицаЗапасы, "Сумма", "СуммаВал");
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетПереработчика") Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаЗапасыДляРасчетов, "Сумма", "СуммаВал");
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетОПереработке") ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетПереработчика") Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаПродукция, "Сумма", "СуммаВал");
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПередачаВА") Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаВнеоборотныеАктивы, "Сумма", "СуммаВал");
		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриемИПередачаВРемонт") Тогда
		
		ВременныеТаблицы.Вставить("ВременнаяТаблицаЗапасы", ПолучитьВременнуюТаблицуПоИмени("ВременнаяТаблицаЗапасы", СтруктураДополнительныеСвойства));
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ДолгСумма, ВременныеТаблицы.ВременнаяТаблицаЗапасы, "Сумма", "СуммаВал");
...
Функция ПолучитьДанныеДляЗаполненияТЧРасшифровкаПлатежа(пДокумент, пЭтоРасчетыСПоставщиком, пДоговор)
	...
	// 2. Дозаполним данные движений значениями СтатьиДДС и ДокументаПланирования из строки расшифровки.
	// 3. Удалим строки расшифровки с автораспределением.
...
Процедура УдалитьДанныеОЗачетеТекущегоДокумента(ДополнительныеСвойства, ДокументОбъект)
	...
	СпиоскСтрокДляУдаления = Новый СписокЗначений;
	Для Каждого ТекущаяСтрока Из МассивСтрокДляУдаления Цикл
		СпиоскСтрокДляУдаления.Добавить(ДокументОбъект.Предоплата.Индекс(ТекущаяСтрока));
	КонецЦикла;
	СпиоскСтрокДляУдаления.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
	Для Каждого ТекущаяСтрока Из СпиоскСтрокДляУдаления Цикл
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДокументовДляИзменения Цикл
		...
		Если ТекущаяСтрока.ВариантИзмененияДанных = ПредопределенноеЗначение("Перечисление.ВариантыИзмененияДанныхВДокументахИзПомощника.Добавление")
			...
			ИначеЕсли ДокументОбъект.ВалютаДокумента = Константы.НациональнаяВалюта.Получить() Тогда
				НоваяСтрокаПредоплата.СуммаПлатежа = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(
...
Процедура ОбработатьТаблицуДокументовДляИзмененияПриОтгрузке(ДополнительныеСвойства, Отказ) Экспорт
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДокументовДляИзменения Цикл
		...
		// На данный момент в денежном документе должны остатья только строки с вариантом Вручную
		...
		Для Каждого ТекущаяСтрокаПредоплата Из ДокументОбъект.РасшифровкаПлатежа Цикл
			...
			// Для вариантов Изменение и Удаление в этом цикле выплняется очистка информации о распределении, т.е.
		...
		Если ТекущаяСтрока.ВариантИзмененияДанных <> Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Удаление
			И РаспределитьСуммуРасчетов > 0 Тогда
			Отказ = Истина;
			ТекстОшибки = НСтр("ru = 'Не получилось изменить следюущий документ:'") + Символы.ПС
...
Процедура СформироватьВременнаяТаблицаПредоплатаДляКассовогоМетода(СтруктураДополнительныеСвойства, ИмяРегистра = "РасчетыСПоставщиками") Экспорт
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляПередачиТоваровМеждуОрганизациями(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ПараметрыДокумента.ЭтоРасчетыСПоставщиком Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТ_Предоплата.Контрагент", "ВТ_Предоплата.КонтрагентВладелец");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТ_Предоплата.Организация", "ВТ_Предоплата.ОрганизацияПолучатель");
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТ_Предоплата.Контрагент", "ВТ_Предоплата.КонтрагентПолучатель");
	КонецЕсли;
...
Процедура ДобавитьКолонкиВТаблицуРасчетов(ТаблицаДвиженийПоВзаиморасчетам)
	...
	ТаблицаДвиженийПоВзаиморасчетам.Колонки.Добавить("НомерСертификата", Новый ОписаниеТипов("СправочникСсылка.СерийныеНомера"));
...
Процедура СформироватьДвижениеЗачетаАванса(ДокументОбъект, СтрокаКЗачету, СтрокаАванса, СтруктураДополнительныеСвойства)
	...
	Если ДокументОбъект.ВалютаДокумента = ДокументОбъект.ДоговорВалютаРасчетов Тогда //документ в валюте расчетов
		...
		// При ручном зачете авнас может быть выбран по ошибке
	...
	// Необходимо удалить движение долга для корректировки, в которой изменялись только зачтненные авансы
...
Процедура ДобавитьСтрокуЗадолженности(ДокументОбъект, СтрокаКЗачету, ДополнительныеСвойства)
	...
	Если ПараметрыДокумента.ЭтоОтгрузка Тогда
		Если НЕ ДокументОбъект.ВестиРасчетыПоДокументам Тогда
			Движение.Документ = Неопределено;
		ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			Движение.Документ = ДокументОбъект.Ссылка.ИсправляемыйДокументРеализации;
		Иначе
...
Процедура ПодготовитьТаблицуДокументаРасчетыВВалюте(ДокументСсылка, ТаблицаСуммДокумента, РаспределяемаяТаблица, СтруктураДополнительныеСвойства) Экспорт
	...
	Если НЕ СтруктураДополнительныеСвойства.Свойство("БезУчетаАвансов") Тогда  // По взаиморасчетам
		...
		Если СтруктураДополнительныеСвойства.Свойство("РаспределятьВсюСумму") Тогда
			...
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("Сумма"), 	  ТаблицаСуммДокумента, "Сумма", 	"ВсегоВалютаДокумента");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("СуммаВал"), ТаблицаСуммДокумента, "СуммаВал", "ВсегоВалютаДокумента");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("СуммаРег"), ТаблицаСуммДокумента, "СуммаРег", "ВсегоВалютаДокумента");
		...
		Иначе //От данных по взаиморасчетам зависят суммы в нескольких табличных частях
			...
			Если ДокументСсылка.ВалютаДокумента <> СтруктураДополнительныеСвойства.НациональнаяВалюта Тогда // Документ оформлен в валюте расчетов
				...
				УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(Сумма,    ТаблицаСуммДокумента, "Сумма",    "ВсегоВалютаДокумента");
				УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаРег, ТаблицаСуммДокумента, "СуммаРег", "ВсегоВалютаДокумента");
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТаблицаСуммДокумента.Колонки, "СуммаРасходов") Тогда
					...
					УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаРасходов, ТаблицаСуммДокумента, "СуммаРасходов", "СуммаРасходовВал");
			...
			Иначе // Договор в у.е., документ оформлен в национальной валюте
				...
				УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(Сумма,    ТаблицаСуммДокумента, "Сумма",    "ВсегоВалютаДокумента");
				УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СуммаВал, ТаблицаСуммДокумента, "СуммаВал", "ВсегоВалютаДокумента");
	...
	Иначе 
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.Сумма,    ТаблицаСуммДокумента, "Сумма", 	"ВсегоВалютаДокумента");
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.СуммаВал, ТаблицаСуммДокумента, "СуммаВал", "ВсегоВалютаДокумента");
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.СуммаРег, ТаблицаСуммДокумента, "СуммаРег", "ВсегоВалютаДокумента");
	...
	Если ТаблицаСуммДокумента.Колонки.Найти("СуммаНДСВал") <> Неопределено Тогда
		...
		Если ВВалютеРегламентированногоУчета Тогда // Договор в у.е., документ оформлен в национальной валюте
			...
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.Сумма,    ТаблицаСуммДокумента, "СуммаНДС",    "ВсегоВалютаДокумента");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.СуммаВал, ТаблицаСуммДокумента, "СуммаНДСВал", "ВсегоВалютаДокумента");
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(СтруктураСумм.СуммаРег, ТаблицаСуммДокумента, "СуммаНДСРег", "ВсегоВалютаДокумента");
...
Процедура ОбновитьРасшифровкуПлатежаПоДаннымВзаиморасчетов(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("Сумма"), 	  РасшифровкаПлатежа, "СуммаУчета", 	  "СуммаПлатежа");
	УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("СуммаВал"), РасшифровкаПлатежа, "СуммаРасчетов", 	  "СуммаПлатежа");
	
	// Пересчитаем сумму комисси, при необходимости
	Если РасшифровкаПлатежа.Колонки.Найти("СуммаУчетаКомиссииПоДоговору") <> Неопределено Тогда
		...
		УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(ИтоговаяСуммаКомиссии, РасшифровкаПлатежа, "СуммаУчетаКомиссииПоДоговору", "СуммаКомиссииПоДоговору");
	КонецЕсли; 
...
Функция ПодготовитьТаблицуСуммДокументаСФВыданный(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства, ВременнаяТаблицаЗапасы)
	...
	Если Реквизиты.РасчетыВВалюте Тогда // Документ оформлен в валюте расчетов
		...
		Пока ВыборкаДокОснования.Следующий() Цикл
			...
			УправлениеНебольшойФирмойСервер.РаспределитьСуммуПоКолонкеТаблицы(
...
Процедура РаспределитьАвансыПоТаблицеРеализации(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства, ТаблицаВзаиморасчеты, ИмяТЧ = "Запасы", СмотретьВДопСвойствах = Ложь) Экспорт
	...
	Для каждого СтрокаДоговора Из ТаблицаДоговоров Цикл
		...
		Если ВалютаРасчетов = НациональнаяВалюта 
			...
			УправлениеНебольшойФирмойСервер.ЗагрузитьВТаблицуЗначений(ТаблицаРезультатПоДоговору, ТаблицаРезультат);
