...
//
Функция СтруктурнаяЕдиницаДляШапки(ТабличнаяЧасть) Экспорт
	...
	Результат = ТабличнаяЧасть[0].СтруктурнаяЕдиница;
	
	Если ТабличнаяЧасть.Количество() = 1 Тогда
		Возврат Результат;
	КонецЕсли;
	
	ТаблицаСтруктурныеЕдиницы = ТабличнаяЧасть.Выгрузить(, "СтруктурнаяЕдиница");
	...
	Результат = ТаблицаСтруктурныеЕдиницы[0].СтруктурнаяЕдиница;
...
Функция ПолучитьСпециальныйНалоговыйРежим(ПоложениеСклада, Организация, СтруктурнаяЕдиница, Дата) Экспорт
	...
	Иначе
		СпециальныйНалоговыйРежим = УправлениеНебольшойФирмойСервер.СпециальныйНалоговыйРежим(Организация, , Дата);
	КонецЕсли;
...
Процедура РазыменоватьПоляДисконтнойКарты(РазыменованныеПоля)
	...
	РазыменованныеПоля.Вставить("ПроцентСкидкиПоДисконтнойКарте",
		УправлениеНебольшойФирмойСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(ТекущаяДатаСеанса(),
