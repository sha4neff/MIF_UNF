...
Функция ОбработатьВводШтрихкода(Форма, ВходящиеДанные, КэшированныеЗначения, ПараметрыСканирования = Неопределено) Экспорт
	...
	ЗаполнитьВидыПродукцииВДанныхПоШтрихкодам(ДанныеПоШтрихкодам);
...
Функция ВложенныеШтрихкодыУпаковок(Упаковки, ПараметрыСканирования = Неопределено, ЗаполнитьСведенияПоШтрихкодамEAN = Ложь) Экспорт
...
// * ВидПродукции - ПеречислениеСсылка.ВидыПродукцииИС - Вид продукции.
// * Штрихкод - Строка - Значение штрихкода.
Функция НоваяСтруктураОбработкиШтрихкода(КодМаркировки = Неопределено, ВидПродукции = Неопределено, ПроверятьАлфавитЭлементов = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(КодМаркировки) Тогда
		...
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			КодМаркировки, ВидПродукции, ПримечаниеКРезультатуРазбора, Неопределено, ПроверятьАлфавитЭлементов);
...
Функция ВыполнитьПроверкуНаОшибкиДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если ДанныеПоШтрихкодам.ЕстьОшибки И Не ПараметрыСканирования.ПропускатьСтрокиСОшибкамиПриЗагрузкеИзТСД Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСведенияПоШтрихкодамEAN(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ПрисутствуетПродукцияИСМП(СтрокаДанных.ВидыПродукцииКодаМаркировки)
	...
	Если ДопустимаТабачнаяПродукция(ПараметрыСканирования) Тогда
		...
		КонецЕсли;
		МодульШтрихкодированиеМОТП = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеМОТП");
		МодульШтрихкодированиеМОТП.ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ИсточникТаблицаКодовМаркировки);
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли Не ЕстьОшибки И СтрокаДанных.ЭтоКодВводаОстатков Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			ДанныеУточнения = Новый Структура("Номенклатура, Характеристика, Серия");
...
Процедура ЗаполнитьКоэффициентыВложенностиУпаковокПоШтрихкодамEAN(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	ДоступныеПоляЗаполнения = Новый ФиксированнаяСтруктура("Коэффициент");
	ПоляЗаполнения = Новый Структура;
	Для Каждого Колонка Из ИсточникТаблицаКодовМаркировки.Колонки Цикл
		Если ДоступныеПоляЗаполнения.Свойство(Колонка.Имя) Тогда
			ПоляЗаполнения.Вставить(Колонка.Имя);
		КонецЕсли;
	КонецЦикла;
	ДоступныеПоляЗаполнения = Новый ФиксированнаяСтруктура(ПоляЗаполнения);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			ПоляЗаполнения = Новый Структура;
			Для Каждого КлючИЗначение Из ДоступныеПоляЗаполнения Цикл
				Если ЗначениеЗаполнено(СтрокаИнформации[КлючИЗначение.Ключ]) Тогда
					Если Не ЗначениеЗаполнено(СтрокаДанных[КлючИЗначение.Ключ]) Тогда
						ПоляЗаполнения.Вставить(КлючИЗначение.Ключ, СтрокаИнформации[КлючИЗначение.Ключ]);
					ИначеЕсли СтрокаИнформации[КлючИЗначение.Ключ] <> СтрокаДанных[КлючИЗначение.Ключ] Тогда
...
Процедура НормализоватьИРазобратьНаСоставляющиеКодыМаркировки(ДанныеКодовМаркировки, ПараметрыСканирования, КешДанныхРазбора = Неопределено)
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ДанныеРазбораИПримечание = Неопределено Тогда
			ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
				СтрокаДанных.Штрихкод, ВидыПродукции, СтрокаДанных.ПримечаниеКРазборуШтрихкода, НастройкиРазбораКодаМаркировки);
			СтрокаДанных.ДанныеРазбора = ДанныеРазбора;
		КонецЕсли;
...
Процедура ПроверитьНаСоответствиеДокументуОснованию(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	Иначе
		...
		Для Каждого СтрокаМаркированныеТовары Из МаркированныеТовары Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаМаркированныеТовары.Номенклатура) Тогда
				Продолжить;
			ИначеЕсли ЗначениеЗаполнено(СтрокаМаркированныеТовары.ТекстОшибки) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьДанныеИзвестныхУпаковок(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Если НайденныеУпаковки.Количество() > 0 Тогда
		...
		ВложенныеШтрихкодыИзвестныхУпаковок = ВложенныеШтрихкодыУпаковок(
...
Функция ОбработатьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки, ФормаУникальныйИдентификатор, РезультатыПроверок) Экспорт
	...
	Если ПараметрыСканирования.Свойство("ДопустимыПроверкиСеройЗоныМОТП")
		...
		ПроверятьКоэффициентыУпаковок = Истина;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли СтрокаТаблицы.ТребуетсяСопоставление
			И ПараметрыСканирования.СопоставлятьНоменклатуру Тогда
				Если ЗначениеЗаполнено(СтрокаТаблицы.EAN) Тогда
					ШтрихкодыКСопоставлению.Добавить(СтрокаТаблицы.EAN);
				Иначе
					ШтрихкодыКСопоставлению.Добавить(СтрокаТаблицы.Штрихкод);
				КонецЕсли;
			ТребуетсяСопоставлениеНоменклатуры = Истина;
		КонецЕсли;
		
		Если ПроверятьКоэффициентыУпаковок
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		// Обработка ошибок и уточнений во всех формах
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.ТекстОшибки) Тогда
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.ШтрихкодУпаковки)
			И (ИнтеграцияИСКлиентСервер.ЭтоУпаковка(СтрокаДанных.ТипУпаковки)
				Или (СтрокаДанных.МаркируемаяПродукция И Не СтрокаДанных.ЭтоШтрихкодНоменклатуры)) Тогда
...
Функция ОбработатьУточнениеНоменклатуры(ФормаПараметрыСканирования, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	ВидПродукции = ИнтеграцияИС.ВидПродукцииПоНоменклатуре(ДанныеШтрихкода.Номенклатура);
...
Функция ОбработатьДанныеШтрихкодов(ДанныеПоШтрихкодам, УникальныйИдентификатор, ПараметрыСканирования, РезультатыПроверок)
	...
	Для Каждого КлючИЗначение Из РезультатыОбработки Цикл
		...
		Если РезультатОбработки.ОбщаяОшибка
			...
			Или РезультатОбработки.ЕстьОшибкиВДеревеУпаковок Тогда
...
Процедура ЗаполнитьВидыПродукцииВДанныхПоШтрихкодам(ДанныеПоШтрихкодам)
