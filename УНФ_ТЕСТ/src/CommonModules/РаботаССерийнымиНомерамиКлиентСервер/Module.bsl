...
Процедура ДобавитьСерийныйНомерВСтроку(НоваяСтрока, СерийныйНомер, ДокОбъект, ИмяТЧСерийныеНомера = "СерийныеНомера", ИмяТЧЗапасы = "Запасы", ИмяПоляКлючСвязи = "КлючСвязи") Экспорт
	
	ОтборСерийныйНомер = Новый Структура("СерийныйНомер", СерийныйНомер);
	
	Если НЕ ДокОбъект.Свойство(ИмяТЧСерийныеНомера) Тогда 
		Возврат
	КонецЕсли;
	
	Если ДокОбъект[ИмяТЧСерийныеНомера].НайтиСтроки(ОтборСерийныйНомер).Количество()>0 Тогда
	...
	СтрСерийныеНомера = ДокОбъект[ИмяТЧСерийныеНомера].Добавить();
	...
	ИначеЕсли НЕ НоваяСтрока = Неопределено И НоваяСтрока.Свойство("СерийныеНомера") Тогда
...
Процедура ОбновитьСерийныеНомераКоличество(Объект, СтрокаТабличнойЧасти, ИмяТабличнойЧастиСерийныеНомера="СерийныеНомера", ИмяПоляКлючСвязи = "КлючСвязи") Экспорт
	...
	Если ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения)=Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
		Коэффициент = РаботаССерийнымиНомерамиВызовСервера.КоэффициентЕдиницы(СтрокаТабличнойЧасти.ЕдиницаИзмерения);
	Иначе
		Коэффициент = 1;
	КонецЕсли;
...
Процедура УдалитьСерийныеНомераПоКлючуСвязи(ТабличнаяЧастьСН, СтрокаТабличнойЧасти, ИспользоватьСерийныеНомераОстатки,
	ИмяПоляКлючСвязи = "КлючСвязи", ИмяТабличнойЧастиСерийныеНомера = "СерийныеНомера") Экспорт
