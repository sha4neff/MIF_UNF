...
Функция КонтрагентыПоТегамИСегментам(Теги, Сегменты, ТипКонтактов = Неопределено) Экспорт
	...
	Если Теги.Количество() > 0 И Сегменты.Количество() > 0 Тогда
		Контрагенты = УправлениеНебольшойФирмойКлиентСервер.ПолучитьСовпадающиеЭлементыМассивов(КонтрагентыПоТегам(Теги, ТипКонтактов), КонтрагентыСегментов(Сегменты));
