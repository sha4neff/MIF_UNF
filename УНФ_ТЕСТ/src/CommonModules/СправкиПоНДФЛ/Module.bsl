		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Если ПерваяСтавка Тогда
				...
				Если ОбластьМакетаУведомлениеНаСоцВычет = Неопределено Тогда
					...
					КонецЕсли;
				ИначеЕсли ВычетыПоВидам.Количество() = 0 Или Ставка <> Перечисления.НДФЛСтавки.Ставка13 Тогда
...
Функция ПреобразоватьСтрокуВСтруктуру(СтрокаПолей)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоСтрокаXML(СтрокаПолей) Тогда
		Результат = УчетСтраховыхВзносов.СтруктураПолейАдресаИзСпискаПолейКонтактнойИнформации(СтрокаПолей, УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтрокаПолей));
	Иначе
