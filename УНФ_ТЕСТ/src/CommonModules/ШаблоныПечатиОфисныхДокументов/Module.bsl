...
Процедура ДобавитьКомандыПечати(НазначениеШаблона, КомандыПечати, СписокФорм, МестоРазмещения, Порядок) Экспорт
	...
	Если НЕ (РольДоступна("ПолныеПрава") ИЛИ РольДоступна("ДобавлениеИзменениеШаблоновПечатиОфисныхДокументов")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОпределитьВидимостьКомандШаблоновПечати(КомандаОткрытьСписок) Экспорт
	...
	КомандаОткрытьСписок.Видимость = УправлениеНебольшойФирмойУправлениеДоступомПовтИсп.ЕстьПравоДоступа(
		"Просмотр", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Справочники.ШаблоныПечатиОфисныхДокументов));
...
Процедура ЗаполнитьЗначенияКонтактнойИнформации(ОписаниеПараметров, КлючВладелец, ЗначениеВладелец) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ЗначениеВладелец) Тогда
		Возврат;
	КонецЕсли;
	
	КонтактнаяИнформацияОбъекта = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(ЗначениеВладелец,, ТекущаяДатаСеанса(), Ложь);
