...
// Параметры:
//  Объект - ДокументОбъект - Документ дял проверки
...
Процедура ПроверитьСрокДействия(Объект, ИмяТабличнойЧасти = "", Отказ) Экспорт
	...
	Если ИмяТабличнойЧасти = "Запасы" Тогда
		...
		|	ЕСТЬNULL(СерийныеНомера.НомерСертификата, ЗНАЧЕНИЕ(Справочник.СерийныеНомера.ПустаяСсылка)) КАК НомерСертификата
		|ПОМЕСТИТЬ ПодарочныеСертификаты
		|ИЗ
		|	Запасы КАК Запасы
		|		ЛЕВОЕ СОЕДИНЕНИЕ СерийныеНомера КАК СерийныеНомера
		|		ПО Запасы.КлючСвязи = СерийныеНомера.КлючСвязи
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ Запасы
	...
	ИначеЕсли ИмяТабличнойЧасти = "" Тогда
		...
		ТаблицаСертификата.Колонки.Добавить("НомерСертификата", Новый ОписаниеТипов("СправочникСсылка.СерийныеНомера"));
	...
	|		ПО СертификатыСПериодичностью.НомерСертификата = СерийныеНомераОбороты.СерийныйНомер
	|			И (СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.РасходнаяНакладная
...
Функция ВыполнитьПоискСертификатаПоНомеру(Номер) Экспорт
	...
	|	СерийныеНомера.Ссылка КАК НомерСертификата
	|ПОМЕСТИТЬ Сертификаты
	|ИЗ
	|	Справочник.СерийныеНомера КАК СерийныеНомера
	|ГДЕ
	|	СерийныеНомера.Продан
	|	И СерийныеНомера.Владелец.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
	|	И СерийныеНомера.Наименование = &Номер
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И НЕ ШтрихкодыНоменклатуры.Номенклатура.ИспользоватьСерийныеНомера
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерийныеНомераОбороты.СерийныйНомер КАК СерийныйНомер,
	|	МАКСИМУМ(СерийныеНомераОбороты.Период) КАК Период
	...
	|					Сертификаты.НомерСертификата КАК НомерСертификата
	|				ИЗ
	|					Сертификаты КАК Сертификаты)) КАК СерийныеНомераОбороты
	|ГДЕ
	...
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, ДЕНЬ, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Неделя)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, НЕДЕЛЯ, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Декада)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, ДЕКАДА, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Месяц)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, МЕСЯЦ, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Квартал)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, КВАРТАЛ, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Полугодие)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, ПОЛУГОДИЕ, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					КОГДА СертификатыСРеквизитами.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Год)
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ПродажиСерийныхНомеров.Период, ГОД, СертификатыСРеквизитами.КоличествоПериодовДействия), ДЕНЬ)
	|					ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|				КОНЕЦ
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ КАК ДатаОкончания,
	|	ВЫБОР
	|		КОГДА СертификатыСРеквизитами.ПодарочныйСертификат.ИспользоватьСерийныеНомера
	...
	|		ПО СертификатыСРеквизитами.НомерСертификата = ПродажиСерийныхНомеров.СерийныйНомер
...
//  НомерСертификата - Справочник.СерийныеНомера - Номер искомого сертификата
//
Функция ПолучитьСтруктуруДанныхСертификата(ПодарочныйСертификат, НомерСертификата, Документ = Неопределено) Экспорт
	...
	|		ПО (СерийныеНомера.Ссылка = &НомерСертификата)
	|ГДЕ
	...
	|ПОМЕСТИТЬ ПродажиСерийныхНомеров
	|ИЗ
	|	РегистрНакопления.СерийныеНомера.Обороты(, , Регистратор, СерийныйНомер = &НомерСертификата) КАК СерийныеНомераОбороты
	|ГДЕ
	|	(СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.РасходнаяНакладная
	|			ИЛИ СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.ЧекККМ
	|			ИЛИ СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.ЗаказПокупателя)
	...
	|			И Сертификаты.СерийныйНомер = ОборотПоДокументу.НомерСертификата");
...
Функция ПолучитьСтруктуруДанныхПоТаблицеСертификатов(ТаблицаСертификатов) Экспорт
	...
	|					ТаблицаСертификатов.НомерСертификата КАК НомерСертификата
	|				ИЗ
	|					ТаблицаСертификатов КАК ТаблицаСертификатов)) КАК СерийныеНомераОбороты
	|ГДЕ
	|	(СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.РасходнаяНакладная
	|			ИЛИ СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.ЧекККМ
	|			ИЛИ СерийныеНомераОбороты.Регистратор ССЫЛКА Документ.ЗаказПокупателя)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПродажиСерийныхНомеров КАК ПродажиСерийныхНомеров
	|		ПО Сертификаты.НомерСертификата = ПродажиСерийныхНомеров.СерийныйНомер");
	...
	Для Каждого СтрокаТаблицы Из Объект[ИмяТаблицы] Цикл
		Если (СтрокаТаблицы.Номенклатура.ТипНоменклатуры = ТипСертификат) Тогда
			...
			Если СкладВШапке Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = "Нельзя перемещать, либо приходовать подарочные сертификаты на ордерные склады!";
				Сообщение.Сообщить();
...
Функция СформироватьСписокНомеровСертификатов(Текст, Владелец) Экспорт
	...
	|	И СерийныеНомера.Наименование ПОДОБНО &Текст
	|	И СерийныеНомера.Продан");
