...
Функция ПолучитьПризнакПредметаРасчета(Номенклатура) Экспорт
	...
	Если Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда
		Если Номенклатура.АлкогольнаяПродукция И ЗначениеЗаполнено(Номенклатура.ВидМаркировки) Тогда
			Если Номенклатура.ВидМаркировки = Перечисления.ВидыМаркировки.НеМаркируется Тогда
...
Процедура ДополнитьТоварамиПоДокументу(ДокументОбъект, ОбщиеПараметры, ПризнакСпособаРасчета, ИмяТабличнойЧасти = "Запасы", НомерСтрокиТовара = 0) Экспорт
	...
	ЭтоРН = ДокументОбъект.Метаданные().Имя = "РасходнаяНакладная";
	...
	Иначе
		Для каждого ТабличнаяЧасть Из МассивТабличныхЧастей Цикл
			...
			Если ЭтоРН И ТабличнаяЧасть = "Запасы" Тогда
				ДанныеДляИСМП = УправлениеНебольшойФирмойСервер.СформироватьДанныеДляИСМП_РН(ДокументОбъект);
			Иначе
			...
			Для каждого СтрокаТЧ Из ТаблицаДанных Цикл
				...
				Для Каждого СтрокаДляДобавленияВЧек Из МассивСтрокДляДобавленияВЧек Цикл
					...
					ИначеЕсли ТабличнаяЧасть = "Расходы" Тогда
						РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДляДобавленияВЧек.Номенклатура, "НаименованиеПолное,ТипНоменклатуры,ЭтоНабор,ВидМаркировки,АлкогольнаяПродукция");
					...
					Иначе
						...
						Иначе
							СтрокаПозицииЧека.Количество = СтрокаДляДобавленияВЧек.Количество;
						КонецЕсли;
						ЦенаСтроки = СтрокаТЧ.Цена + ?(ДокументОбъект.СуммаВключаетНДС, 0, СтрокаДляДобавленияВЧек.СуммаНДС / ?(СтрокаПозицииЧека.Количество = 0, 1, СтрокаПозицииЧека.Количество));
						...
						Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.ВидПродукцииИС)
							...
							Если ДанныеШтрихКода <> Неопределено Тогда
								...
								Если СтрокаТЧ.Номенклатура.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.Обувь Тогда
									СтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.ТипМаркировки = Перечисления.ТипыМаркировкиККТ.ОбувныеТовары;
								ИначеЕсли СтрокаТЧ.Номенклатура.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.Табак Тогда
