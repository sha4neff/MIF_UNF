...
Функция ПолучитьДанныеДляФормированияДокументов(ОбщиеДанные)
	...
	|	ВЫБОР
	|		КОГДА ТабличнаяЧастьИсторияСтатусов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВетеринарныхДокументовВЕТИС.Аннулирован)
	|	ТОГДА -1 ИНАЧЕ 1 КОНЕЦ * ВетеринарноСопроводительныйДокументВЕТИС.КоличествоВЕТИС КАК КоличествоВЕТИС,
	...
	|		 И ВЫБОР
	|				КОГДА ВтТаблицаИдентификаторов.Статус = 202 И ВтТаблицаИдентификаторов.КоличествоВЕТИС > 0
	...
	|
	|ВЫБРАТЬ
	|	ВЫБОР КОГДА ВтТаблицаИдентификаторов.КоличествоВЕТИС >= 0 ТОГДА 1 ИНАЧЕ -1 КОНЕЦ * 
	|	втВСД.КоличествоВЕТИС,
	|	втВСД.Ссылка КАК ВетеринарноСопроводительныйДокументВЕТИС,
	|	МАКСИМУМ(ЗаписиСкладскогоЖурналаВЕТИС.Ссылка) КАК ЗаписьСкладскогоЖурналаВЕТИС
	|ИЗ
	...
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР КОГДА ВтТаблицаИдентификаторов.КоличествоВЕТИС >= 0 ТОГДА 1 ИНАЧЕ -1 КОНЕЦ * втВСД.КоличествоВЕТИС,
	|	втВСД.Ссылка
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаписьСкладскогоЖурналаВЕТИС
...
Функция ДокументДляВосстановления(ДатаЭлемента, Отбор, ПараметрыОбмена, ТипДокумента, БезОграниченияДатыНазад = Ложь) Экспорт
	...
	Для Каждого ЭлементДанных Из НайденныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(ЭлементДанных.ДатаСоздания) Тогда
			ЭлементДанных.ДатаСоздания = МестноеВремя(ИнтеграцияВЕТИС.ДатаФормированияРезультатаПоСсылке(ЭлементДанных.Документ, ПараметрыОбмена));
		КонецЕсли;
