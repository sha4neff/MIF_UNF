...
Процедура ВыполнитьОбмен(УзелОбмена, РежимЗапускаОбмена, ВыгружатьТолькоИзменения = Истина, ПараметрыСоставОбмена=Неопределено) Экспорт
	...
	Если Параметры.ВыгружатьНаСайт Тогда
		...
		ОбработатьКаталогиВыгрузкиПередОбменом(КаталогВыгрузки, ПодкаталогБезопасностиКаталогаВыгрузки, Параметры);
	...
	Исключение
		...
		ЗаписатьВТаблицуИнформации(ОписаниеИсключительнойОшибки(), Параметры.ТаблицаИнформации, Истина);
		ВыполнитьДействияПриЗавершенииОбмена(Параметры, Истина);
...
// Обрабатывает старые каталоги выгрузки, удаляя временные файлы.
//
// Параметры:
//	КаталогВыгрузки - абсолютный путь до каталога выгрузки,
//	ПодкаталогБезопасностиКаталогаВыгрузки - относительный путь до подкаталога выгрузки
//	Параметры - параметры выгрузки
//
Процедура ОбработатьКаталогиВыгрузкиПередОбменом(КаталогВыгрузки, ПодкаталогБезопасностиКаталогаВыгрузки, Параметры)
	
	ТекущаяУниверсальнаяДатаВМиллисекундах = ТекущаяУниверсальнаяДатаВМиллисекундах();
	СуткиВМиллисекундах = 86400000;
	...
	Для Каждого СтрФайл Из МассивНайденныхФайлов Цикл
		...
		Если СтрФайл.ЭтоКаталог() Тогда
			...
			Попытка
				...
				Если УниверсальнаяДатаВМиллисекундах <= (ТекущаяУниверсальнаяДатаВМиллисекундах - СуткиВМиллисекундах) Тогда
	...
	ПодкаталогБезопасностиКаталогаВыгрузки = ПодкаталогБезопасностиКаталогаВыгрузки + "_" + XMLСтрока(ТекущаяУниверсальнаяДатаВМиллисекундах);
...
Функция ЗагрузитьЗаписьНаУслуги(ДанныеЗаказов, СтруктураСтатистики, Параметры) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка записей на услуги с сайта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Функция ВыгрузитьФайл(ДанныеФайла, Параметры, ОписаниеОшибки, ВладелецКартинок=Неопределено) Экспорт
	...
	Если ФайлВХранилище Тогда
		...
		ИначеЕсли ДанныеФайла.ХранимыйФайл = "СлишкомБольшойФайл" Тогда
			ДвоичныеДанныеФайла = УправлениеНебольшойФирмойСервер.ПолучитьДвоичныеДанныеФайла(ДанныеФайла.Файл);
		Иначе
	...
	Если Параметры.ИспользоватьХарактеристики
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если Не Параметры.ОбменЧерезВебСервис Тогда  
				...
				ЗаполнитьЗначенияСвойств(Новстрока, НайденнаяСтрока.Строки[0]);
				ЗаполнитьСтрокуУпаковками(новстрока);
				//--
...
//++ ДобавлениеУпаковок
Процедура ЗаполнитьСтрокуУпаковками(строка)
	Свойство=ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "УпаковкиДляСайта");
	строка.наименование=свойство.наименование;
	строка.Свойство=Свойство; 
	строка.значение="";
	Если Строка.Номенклатура.Изготовитель=Справочники.СтруктурныеЕдиницы.НайтиПоКоду("НФ-000014") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	СпецификацииПоУмолчанию.Характеристика = &Характеристика
		...
		|		ПО ВтПродукция.Спецификация = СпецификацииСостав.Ссылка
		|ГДЕ
		|	СпецификацииСостав.Номенклатура В ИЕРАРХИИ(&Продукция)";
		
		Запрос.УстановитьПараметр("Продукция", Справочники.Номенклатура.НайтиПоКоду("НФ-00000008"));
		...
		Если ВыборкаДетальныеЗаписи.Количество()>0 тогда 
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Если ЗНачениеЗаполнено(ВыборкаДетальныеЗаписи.характеристика) Тогда
					код=выборкаДетальныеЗаписи.характеристика.Код;	
				Иначе
					код=выборкаДетальныеЗаписи.Номенклатура.Код;
				КонецЕсли;
				Количество=ВыборкаДетальныеЗаписи.количество;
				структураУпаковки=Новый Структура();
				структураУпаковки.Вставить("Код", код);
				структураУпаковки.Вставить("Количество", Количество);
				МассивУпаковок.Добавить(структураУпаковки);
			КонецЦикла;  
			ЗаписатьJSON(запись,массивУпаковок);
			строка.значение=Запись.Закрыть();	 
		КонецЕсли;
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, СтруктураСтатистики, Параметры, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка заказов с сайта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Параметры.УзелОбмена.Метаданные(),
			Параметры.УзелОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Иначе
			...
			Исключение
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ru = 'Не удалось записать событие №'") + ДокументОбъект.Номер));
...
Функция ЗаполнитьЗаказНарядДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры)
	...
	ДокументОбъект.ВидЭлектронногоДокумента = Перечисления.ВидыЭД.ТОРГ12Покупатель;
...
Функция НайтиНоменклатуруПоАртикулуНаименованию(Артикул, Наименование, СоответствиеПолейНоменклатуры)
	...
	Если ЗначениеЗаполнено(Артикул) Тогда
		...
		ИначеЕсли ИмяПоляАртикул = "Штрихкод" Тогда
			...
			|	И НЕ Номенклатура.ЭтоГруппа");
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Справочники.Номенклатура.ПустаяСсылка(), ИмяПоляАртикул) Тогда
			Запрос = Новый Запрос(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			|	Номенклатура.Ссылка
			|ИЗ
...
Функция ПолучитьМассивУзловДляРегистрации(ВыбиратьУзлыОбменаТоварами = Ложь, ВыбиратьУзлыОбменаЗаказами = Ложь, ВыбиратьУзлыОбменаУслугами = Ложь)
