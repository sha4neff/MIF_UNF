...
Процедура НастроитьШаблоныФормыУточненияДанных(Форма, ВидПродукции) Экспорт
	...
	Если Форма.ЭтоКодМаркировкиСоСрокомГодности Тогда
		Если Форма.Скоропортящаяся Тогда
			Форма.ШаблонКодаМаркировки = Перечисления.ШаблоныКодовМаркировкиСУЗ.СкоропортящаясяМолочнаяПродукция;
		Иначе
			Форма.ШаблонКодаМаркировки = Перечисления.ШаблоныКодовМаркировкиСУЗ.МолочнаяПродукция;
		КонецЕсли;
	КонецЕсли;
...
Функция МассивЗначенийGTINДляВыбора(ТекущиеДанные, Объект, ВключатьGTINНаОстатки = Ложь) Экспорт
	...
	Если ВключатьGTINНаОстатки Тогда
		...
		КодТНВЭД = ИнтеграцияИСМПСлужебный.КодТНВЭДДляПередачиВИСМП(ТекущиеДанные.КодТНВЭД, Объект.ВидПродукции);
