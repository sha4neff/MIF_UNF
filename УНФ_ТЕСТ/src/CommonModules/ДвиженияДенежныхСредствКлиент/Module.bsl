...
// "Денежные средства" 
// 
// Общие клиентские процедуры и функции документов Расход со счета, Расход из кассы, Поступление на счет, Поступление в кассу
//
////////////////////////////////////////////////////////////////////////////////
...
// Обработчик вызова команд создания документов из форм списка денежных документов
// 
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - Форма списка, из которой вызывается команда создания документа
//   Команда - Команда - КомандаФормы, команда создания документа, которую вызвал пользователь
//
Процедура ВыполнитьКомандуСозданияДокумента(Форма, Команда) Экспорт
	
	МассивПараметров = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Команда.Имя, "_");
	
	Если МассивПараметров.Количество() = 2 Тогда
		МассивПараметров.Добавить("");
	КонецЕсли; 
	
	ВидДокумента = МассивПараметров[0];
	ИмяОперации  = МассивПараметров[1];
	ИмяСписка 	 = МассивПараметров[2];
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ДанныеМеток") Тогда
		ДанныеМеток = Форма.ДанныеМеток;
	Иначе
		ДанныеМеток = Неопределено;
	КонецЕсли;
	
	СоздатьДокумент(ВидДокумента, ИмяОперации, ДанныеМеток, ИмяСписка);
