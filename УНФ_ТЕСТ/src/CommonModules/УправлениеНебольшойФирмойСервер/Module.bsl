...
Процедура ПередДобавлениемПодсистемы(Описание) Экспорт
	...
	Если ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойБазовая") Тогда
		...
		Описание.ИдентификаторИнтернетПоддержки = "SmallBusinessBase";
...
//
Функция ПолучитьПоУмолчаниюСпецификацию(Номенклатура, Характеристика = Неопределено, ВключаяШаблоны = Ложь) Экспорт
...
Функция ПолучитьТекстСодержания(Номенклатура, Характеристика = Неопределено) Экспорт
	...
	ШаблонСодержание = ПолучитьПредставлениеНоменклатурыДляПечати(
...
// Возвращает признак большого экрана устройства.
//
...
//
// Параметры: 
// ИмяКонстанты - строковое имя константы, 
//
// Возвращаемое значение:
// значение константы
...
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	СтруктураДополнительныеСвойства.ДляПроведения.Организация = ПолучитьОрганизацию(СтруктураДополнительныеСвойства.ДляПроведения.Организация);
...
Функция ЕстьПроцедураСоздатьПустуюВременнуюТаблицуИзменение(ПолноеИмяРегистра)
	...
	РегистрыСПроцедурой.Добавить(Метаданные.РегистрыНакопления.ЗапасыПереданныеВРазрезеГТД.ПолноеИмя());
	РегистрыСПроцедурой.Добавить(Метаданные.РегистрыНакопления.ЗапасыПринятыеВРазрезеГТД.ПолноеИмя());
...
Процедура ОтразитьДвиженияДенежныхСредств(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	Если Отказ
	 ИЛИ ТаблицаДвиженияДенежныхСредств.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтразитьДенежныеСредстваПрогноз(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	ДвиженияБюджетДенежныеСредстваПрогноз = Движения.УдалитьДенежныеСредстваПрогноз;
...
Процедура ОтразитьЗапасыПереданныеВРазрезеГТД(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаЗапасыПереданныеВРазрезеГТД = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПереданныеВРазрезеГТД;
	
	Если Отказ
	 ИЛИ ТаблицаЗапасыПереданныеВРазрезеГТД.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗапасыПреданныеВРазрезеГТД = Движения.ЗапасыПереданныеВРазрезеГТД;
	ДвиженияЗапасыПреданныеВРазрезеГТД.Записывать = Истина;
	ДвиженияЗапасыПреданныеВРазрезеГТД.Загрузить(ТаблицаЗапасыПереданныеВРазрезеГТД);
	
КонецПроцедуры

Процедура ОтразитьЗапасыПринятыеВРазрезеГТД(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаЗапасыПринятыеВРазрезеГТД = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПринятыеВРазрезеГТД;
	
	Если Отказ
	 ИЛИ ТаблицаЗапасыПринятыеВРазрезеГТД.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗапасыПринятыеВРазрезеГТД = Движения.ЗапасыПринятыеВРазрезеГТД;
	ДвиженияЗапасыПринятыеВРазрезеГТД.Записывать = Истина;
	ДвиженияЗапасыПринятыеВРазрезеГТД.Загрузить(ТаблицаЗапасыПринятыеВРазрезеГТД);
...
Процедура ОтразитьРезервыТоваровОрганизаций(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	ТаблицаРезервыТоваровОрганизаций= ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРезервыТоваровОрганизаций;
	
	Если Отказ
	 ИЛИ ТаблицаРезервыТоваровОрганизаций.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияРезервыТоваровОрганизаций = Движения.РезервыТоваровОрганизаций;
	ДвиженияРезервыТоваровОрганизаций.Записывать = Истина;
	ДвиженияРезервыТоваровОрганизаций.Загрузить(ТаблицаРезервыТоваровОрганизаций);
...
Процедура ОтразитьСерийныеНомераКПоступлению(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаСерийныеНомераКПоступлению = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСерийныеНомераКПоступлению;
	
	Если Отказ ИЛИ ТаблицаСерийныеНомераКПоступлению.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Движения.СерийныеНомераКПоступлению.Записывать = Истина;
	Движения.СерийныеНомераКПоступлению.Загрузить(ТаблицаСерийныеНомераКПоступлению);
	
КонецПроцедуры

Процедура ОтразитьСерийныеНомераКРасходу(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаСерийныеНомераКРасходу = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСерийныеНомераКРасходу;
	
	Если Отказ ИЛИ ТаблицаСерийныеНомераКРасходу.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Движения.СерийныеНомераКРасходу.Записывать = Истина;
	Движения.СерийныеНомераКРасходу.Загрузить(ТаблицаСерийныеНомераКРасходу);
...
// Устарела. Следует использовать ЗаполнениеОбъектовУНФ.РассчитатьСуммыВСтрокеТабличнойЧасти
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданные(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", ДокОбъект.Метаданные())
		...
		ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Не хватает запасов по которым отчитался контрагент %КонтрагентПредставление%'");
	Иначе
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыводитьВЗаголовокРасшифровку Тогда
			...
			Если ЗначениеЗаполнено(СокрЛП(ВыборкаЗаписей.ЗаказПредставление)) Тогда
				ТекстЗаголовкаСообщения = ТекстЗаголовкаСообщения + НСтр("ru = '
				|по: '") + СокрЛП(ВыборкаЗаписей.ЗаказПредставление);
			КонецЕсли;
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданныеВРазрезеГТД(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятыеВРазрезеГТД(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено) Экспорт
	
	ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Не хватает запасов по номерам ГТД, поступивших на реализацию.'");
	ШаблонСообщения = НСтр("ru = 'Номенклатура: %НоменклатураХарактеристикаПартия%,'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыПоставщикам(ДокОбъект, ВыборкаЗаписей, Пояснение, Отказ) Экспорт
...
// Процедура сообщает об ошибках проведения по регистру Серийные номера к расходу.
//
// Параметры:
//  ДокОбъект		 - ДокументОбъект - проверяемый документ,
//  ВыборкаЗаписей	 - Выборка - выборка с полями:
//                      * ТипСтруктурнойЕдиницы,
//                      * СтруктурнаяЕдиницаПредставление,
//                      * ЯчейкаПредставление,
//                      * ХарактеристикаПредставление,
//                      * ПартияПредставление,
//                      * СерийныйНомерПредставление.
//  Отказ			 - Булево - признак наличия ошибок,
//  МассивСообщений	 - Массив - массив сообщений об ошибках.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомераКРасходу(ДокОбъект, ВыборкаЗаписей, Отказ,
	МассивСообщений = Неопределено) Экспорт
	
	ШаблонЗаголовкаСообщения = НСтр("ru = 'Ошибка:
	|Не хватает серийных номеров к расходу %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыводитьВЗаголовокРасшифровку Тогда
			...
			Иначе
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("ЯчейкаПредставление", ВыборкаЗаписей) Тогда
					ПредставлениеСтруктурнойЕдиницы = ПредставлениеСтруктурнойЕдиницы(ВыборкаЗаписей.СтруктурнаяЕдиницаПредставление,
						ВыборкаЗаписей.ЯчейкаПредставление);
				Иначе
					ПредставлениеСтруктурнойЕдиницы = ПредставлениеСтруктурнойЕдиницы(ВыборкаЗаписей.СтруктурнаяЕдиницаПредставление);
				КонецЕсли; 
			...
			ТекстЗаголовкаСообщения = СтрЗаменить(ТекстЗаголовкаСообщения, "%ТипСтруктурнойЕдиницы%",
				ПолучитьПредставлениеТипаСтруктурныхЕдиниц(ВыборкаЗаписей.ТипСтруктурнойЕдиницы));
			СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ, МассивСообщений);
			ВыводитьВЗаголовокРасшифровку = Ложь;
		КонецЕсли;
		
		ПредставлениеНоменклатуры = ПредставлениеНоменклатуры(ВыборкаЗаписей.НоменклатураПредставление,
...
КонецПроцедуры // СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомераКРасходу()
...
// Устарела. См. ПечатьДокументовУНФ.ПредставлениеНоменклатуры
//
Функция ПолучитьПредставлениеНоменклатурыДляПечати(Номенклатура, Характеристика = Неопределено, Артикул = "", 
	СерийныеНомера = "")  Экспорт
	...
	ПараметрыНоменклатуры.Вставить("ПредставлениеСерийногоНомера", СерийныеНомера);
	Возврат ПечатьДокументовУНФ.ПредставлениеНоменклатуры(ПараметрыНоменклатуры);
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	Если НЕ (ЮрФизЛицо.Метаданные().Реквизиты.Найти("ЮридическоеФизическоеЛицо") = Неопределено) Тогда
		...
		ИначеЕсли ИмяСправочника = "Организации" Тогда
			...
			Если НЕ ПустаяСтрока(Данные.СвидетельствоСерияНомер)
				ИЛИ ЗначениеЗаполнено(Данные.СвидетельствоДатаВыдачи) Тогда
				
				Сведения.Свидетельство = СтрШаблон(СтрокаШаблона, СокрЛП(Данные.СвидетельствоСерияНомер), Формат(Данные.СвидетельствоДатаВыдачи, "ДЛФ=D"));
...
Процедура ПриЗаписиДокументаОснованияСчетаФактуры(ДокументОснованиеСсылка, ДополнительныеСвойства, Полученный) Экспорт
	...
	Если ТипЗнч(ОписаниеСчетФактуры) = Тип("Структура") Тогда
		...
		Если ОписаниеСчетФактуры.Свойство("Версия", ДополнительныеСвойства.ВерсияПодчиненногоСчетФактуры)
			...
			ИзменитьПодчиненнуюСчетФактуру(ОписаниеСчетФактуры.Ссылка, Полученный);
...
Функция ПолучитьПодчиненныйСчетФактуру(ДокументОснование, Полученный = Ложь) Экспорт
	...
	Если Полученный Тогда
		...
		|ИЗ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураДокументыОснования
...
Процедура ДобавитьПоляДляПередачиТоваров(ТаблицаДвижений, СписокПолей) Экспорт
	...
	Для Каждого КолонкаТаблицы Из МассивПолей Цикл
		ТаблицаДвижений.Колонки.Добавить(КолонкаТаблицы+"Инт", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	КонецЦикла;
	
	Для Каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
		Для Каждого КолонкаТаблицы Из МассивПолей Цикл
			СтрокаТаблицы[КолонкаТаблицы+"Инт"] = СтрокаТаблицы[КолонкаТаблицы];
		КонецЦикла;
	КонецЦикла;
...
Функция ОчиститьДанныеВБазе() Экспорт
	...
	Попытка
		...
		Для каждого МетаданныеКонстанты Из Метаданные.Константы Цикл
			Если НЕ РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(МетаданныеКонстанты, РаботаВМоделиСервиса.РазделительОсновныхДанных()) Тогда
				Продолжить;
			КонецЕсли;
		...
		Для каждого ВидОбъекта Из ВидыОбъектов Цикл
			...
			Для каждого МДОбъекта Из МетаданныеКоллекция Цикл
				Если НЕ РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(МДОбъекта, РаботаВМоделиСервиса.РазделительОсновныхДанных()) Тогда
					Продолжить;
				КонецЕсли;
		...
		Для каждого ВидТаблицы Из ВидыТаблиц Цикл
			...
			Для каждого МДРегистра Из МетаданныеКоллекция Цикл
				...
				Если НЕ РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(МДРегистра, РаботаВМоделиСервиса.РазделительОсновныхДанных()) Тогда
					Продолжить;
				КонецЕсли;
		...
		Для каждого МДПланаОбмена Из Метаданные.ПланыОбмена Цикл
			...
			Если НЕ РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(МДПланаОбмена, РаботаВМоделиСервиса.РазделительОсновныхДанных()) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьРасшифровкуПлатежаРасход(ТекущийОбъект, Компания = Неопределено, СтавкаНДСПоУмолчанию = Неопределено, Курс = Неопределено, Кратность = Неопределено, Договор = Неопределено) Экспорт
	...
	Если Компания = Неопределено Тогда
		Компания = ПолучитьОрганизацию(ТекущийОбъект.Организация);
	КонецЕсли;
...
// Проверка на возможность снятия опции ИспользоватьНеуникальныеШтрихКоды.
//
Функция ОтказСнятьКонстантуИспользоватьНеуникальныеШтрихКоды() Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА НЕ ОбщееЧислоШК.ВсегоШК = РазлиныхШК.ВсегоРазличныхШК
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Отказ
	|ИЗ
	|	ОбщееЧислоШК КАК ОбщееЧислоШК,
	|	РазлиныхШК КАК РазлиныхШК";
	...
	Если Отказ Тогда
		ТекстОшибки = НСтр("ru = 'В регистре Штрихкоды номенклатуры есть не уникальные значения! Снятие флага запрещено!'");
	КонецЕсли;
...
// Проверка на возможность снятия опции ИспользоватьРучныеСкидкиНаценкиЗакупки.
//
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРучныеСкидкиНаценкиЗакупки() Экспорт
	...
	Для Каждого ЭлементМассива Из ВидыДокументов Цикл
		...
		Запрос.Текст = Запрос.Текст
			+ СтрЗаменить(ШаблонЗапроса, "&ТабличнаяЧастьДокумента", ЭлементМассива);
	...
	Возврат НСтр("ru = 'В документах закупки используются скидки и наценки. Снятие опции запрещено.'");
...
Функция ОтказСнятьУчетПоКомпании() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ТекстОшибки = НСтр("ru='В базе есть проведенные документы по организации, отличной от компании. Для выключения опции необходимо изменить организацию в таких документах.'");
	ПорцияДляПроверки = 3;
	Запрос = Новый Запрос;
...
// Функция - Проверка на возможность снятия опции ВыполнениеЭтаповРазнымиПодразделениями
// 
// Возвращаемое значение:
//  Строка - Описание ошибки, если отключение опции невозможно, иначе пустая строка
//
Функция ОтказСнятиеФункциональнаяОпцияВыполнениеЭтаповРазнымиПодразделениями() Экспорт
	...
	Возврат НСтр("ru = 'В базе используется выполнение этапов разными подразделениями, снятие опции запрещено.'");
...
// Проверка на возможность снятия опции ИспользоватьПараметрическиеСпецификации.
//
Функция ОтказСнятиеФункциональнаяОпцияИспользоватьПараметрическиеСпецификации() Экспорт
	...
	Возврат НСтр("ru = 'В базе используются параметрические спецификации, снятие опции запрещено.'");
...
Функция ОтказСнятиеФункциональнаяОпцияКонтролироватьОстаткиПриПробитииЧековККМ() Экспорт
	...
	Возврат НСтр("ru = 'В базе есть открытые кассовые смены, снятие опции запрещено.'");
...
Процедура ЗаполнитьТекстовоеПредставлениеЧека(СтрокаСостава, ЧекККМ, ИдентификаторУстройства = Неопределено) Экспорт
	...
	СведенияОбОрганизации = СведенияОЮрФизЛице(ЧекККМ.Организация, ЧекККМ.Дата);
	АдресМагазина = ПолучитьКонтактнуюИнформацию(ЧекККМ.СтруктурнаяЕдиница, ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы"));
...
Функция СформироватьДанныеДляИСМП_РН(ДокументОбъект) Экспорт
	...
	МассивУпаковок = ДокументОбъект.ШтрихкодыУпаковок.ВыгрузитьКолонку("ШтрихкодУпаковки");
	...
	|	Запасы.Количество КАК Количество,
	|	Запасы.Цена КАК Цена,
	|	Запасы.СуммаАвтоматическойСкидки + Запасы.СуммаСкидкиНаценки КАК СуммаСкидки,
	|	Запасы.Сумма КАК Сумма,
	...
	|	ВТ_Запасы.Характеристика КАК Характеристика,
	|	ВТ_Запасы.Партия КАК Партия,
	|	ВТ_Запасы.Содержание КАК Содержание,
	|	ВТ_Запасы.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВТ_Запасы.НомерГТД КАК НомерГТД,
	|	ВТ_Запасы.Количество КАК Количество,
	|	ВТ_Запасы.Цена КАК Цена,
	|	ВТ_Запасы.СуммаСкидки КАК СуммаСкидки,
	|	ВТ_Запасы.Сумма КАК Сумма,
	...
	|	ВТ_Запасы.СтавкаНДС КАК СтавкаНДС,
	|	ВТ_Запасы.СуммаНДС КАК СуммаНДС,
	|	ВЫРАЗИТЬ("""" КАК СТРОКА(200)) КАК Штрихкод
	|ИЗ
	|	ВТ_Запасы КАК ВТ_Запасы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ВТ_Запасы.Номенклатура = СправочникНоменклатура.Ссылка
	...
	Для Каждого СтрокаЗапасы Из Запасы Цикл
		...
		Пока КоличествоСтрокМассиваМаркированныхТоваров >= ТекущаяСтрока Цикл
			...
			СтрокаЗапасыРазобранные.СуммаНДС	= Окр(СтрокаЗапасы.СуммаНДС / СтрокаЗапасы.Количество, 2);
			СтрокаЗапасыРазобранные.Сумма		= Окр(СтрокаЗапасы.Сумма / СтрокаЗапасы.Количество, 2);
			СтрокаЗапасыРазобранные.Всего		= Окр(СтрокаЗапасы.Всего / СтрокаЗапасы.Количество, 2);
			СтрокаЗапасыРазобранные.Цена		= СтрокаЗапасыРазобранные.Сумма - СтрокаЗапасыРазобранные.СуммаСкидки;
			...
			СтрокаЗапасы.СуммаНДС		= СтрокаЗапасы.СуммаНДС - СтрокаЗапасыРазобранные.СуммаНДС;
			СтрокаЗапасы.Сумма			= СтрокаЗапасы.Сумма - СтрокаЗапасыРазобранные.Сумма;
			СтрокаЗапасы.Всего			= СтрокаЗапасы.Всего - СтрокаЗапасыРазобранные.Всего;
...
//   * Ответственный - СправочникСсылка.Сотрудникик
