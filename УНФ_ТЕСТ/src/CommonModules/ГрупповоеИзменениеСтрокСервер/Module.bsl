...
Процедура ОбработатьТаблицу(ЭтаФорма, Таблица, Действие, ОбъектДействия, Значение, ИмяЭлементаНоменклатура, ПараметрыОтбора = Неопределено) Экспорт
	...
	ИначеЕсли Действие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроцентСкидкиНаценки") Тогда
		...
		Для каждого Строка Из Таблица Цикл
			...
			Если ПараметрыОтбора <> Неопределено Тогда
				ЭтоПодходящаяСтрока = УправлениеНебольшойФирмойКлиентСервер.ОбъектСоответствуетПараметрамОтбора(Строка, ПараметрыОтбора);
	...
	ИначеЕсли Действие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента") Тогда
		...
		ЗначенияНовойСтроки = Новый Структура("Номенклатура,Количество,Характеристика,Партия,Цена,СтавкаНДС,СерийныеНомера,СтранаПроисхождения,НомерГТД");
...
Процедура РаспределитьСуммуНаСкидки(Таблица, ИмяКолонки, СуммаРаспределения, ПараметрыОтбора = Неопределено)
	...
	Для каждого СтрокаТабличнойЧасти Из Таблица Цикл
		...
		Если ПараметрыОтбора <> Неопределено Тогда
			Если НЕ УправлениеНебольшойФирмойКлиентСервер.ОбъектСоответствуетПараметрамОтбора(СтрокаТабличнойЧасти, ПараметрыОтбора) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		Если ТипЗнч(Таблица) = Тип("ДокументТабличнаяЧасть.ЗаказПокупателя.Запасы")
			ИЛИ ТипЗнч(Таблица) = Тип("ДокументТабличнаяЧасть.РасходнаяНакладная.Запасы") Тогда
	...
	Если ПоследняяСтрока <> Неопределено И НепогашеннаяСумма <> 0 Тогда
		Если ТипЗнч(Таблица) = Тип("ДокументТабличнаяЧасть.ЗаказПокупателя.Запасы")
			...
			ПоследняяСтрока.ПроцентСкидкиНаценки = (ПоследняяСтрока.Цена * ПоследняяСтрока.Количество 
