...
Функция ПараметрыПодбораСерийныхНомеров(ДокОбъект, УИДФормы, ИДСтроки, РежимПодбора = Неопределено, ИмяТЧ="Запасы", ИмяТЧСерийныхНомеров="СерийныеНомера", ИмяПоляКлючСвязи = "КлючСвязи",
	...
	ПараметрыСерийныхНомеров = ПодготовитьПараметрыСерийныхНомеров(ДокОбъект, ТекСтрокаДанные, УИДФормы, ИмяТЧ, ИмяТЧСерийныхНомеров, ИмяПоляКлючСвязи, ЭтоОприходование);
	...
	Если ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдер") Тогда
		ПараметрыСерийныхНомеров.Вставить("ЭтоПриходныйОрдер", Истина);
	КонецЕсли;
	
	Если ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.РасходныйОрдер") Тогда
		ПараметрыСерийныхНомеров.Вставить("ЭтоРасходныйОрдер", Истина);
	КонецЕсли;
	...
	Если ПараметрыСерийныхНомеров.ЭтоМаркировкаГИСМ Тогда
		...
		ПараметрыСерийныхНомеров.Вставить("ЭтоМаркировкаОстатков", ДокОбъект.ОперацияМаркировки = Перечисления.ОперацииМаркировкиГИСМ.МаркировкаОстатковНа12082016);
