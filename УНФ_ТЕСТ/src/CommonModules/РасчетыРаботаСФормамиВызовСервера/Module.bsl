...
//
&НаСервере
Функция ПолучитьЗначенияРеквизитовПоУмолнчаниюДляНовогоКонтрагента() Экспорт
...
КонецФункции // ПолучитьЗначенияРеквизитовПоУмолнчаниюДляНовогоКонтрагента()
...
Процедура СформироватьЗаголовокГруппыДеньги(Форма) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ЗаголовокОкончание) Тогда
		ЗаголовокОкончание = " (" + ЗаголовокОкончание + ")";
	Иначе
		ЗаголовокОкончание = " (" + НСтр("ru = 'без аналитики'") + ")";
	КонецЕсли;
	
	Форма.Элементы.Взаиморасчеты.Заголовок = Заголовок + ЗаголовокОкончание;
...
Функция ПолучитьДанныеОбОплатеПоДокументуОтгрузки(СсылкаНаДокумент, ФормироватьПоДвижениям) Экспорт
	...
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РасчетыСПокупателями", "РасчетыСПоставщиками");
	КонецЕсли;
...
Функция ПолучитьОстаткиОплаты(ДокументОбъект, ЭтоЗаказ, ПараметрыЗаполненияЗаголовка) Экспорт
	...
	Запрос.УстановитьПараметр("Организация", УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(ДокументОбъект.Организация));
...
Функция ПолучитьОписаниеДляСтрокиТЧЗапасыНаСервере(пСтрока, пВалютаОбъектаПредставление) Экспорт
	...
	НоменклатураХарактеристикаИПартия = СокрЛП(пСтрока.НоменклатураНаименование)
		+?(ПустаяСтрока(пСтрока.ХарактеристикаНаименование), "", ". "+пСтрока.ХарактеристикаНаименование)
		+?(ПустаяСтрока(пСтрока.ПартияНаименование), "", ". "+пСтрока.ПартияНаименование)
	;
...
Функция УстановитьВидимостьРеквизитовРасчетовФрагмент(пФорма) Экспорт
	...
	СтруктураРеквизитыРасчетов = УправлениеНебольшойФирмойСервер.ЗначенияРеквизитовОбъекта(пФорма.Объект.Контрагент, РеквизитыКонтрагента);
...
Процедура ПередЗаписьюНаСервереДеньги(пФорма, ТекущийОбъект) Экспорт
	...
	Если пФорма.ВариантЗаполненияРасшифровки = пФорма.ВариантЗаполненияРасшифровкиАвтоматически Тогда
		...
		пФорма.НастроитьЭлементыРаспределенияДолговНаСервере();
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПросмотраНаСервере(пФорма, пПоДвижениям = Ложь) Экспорт
	...
	ПоДвижениям = РасчетыРаботаСФормамиВызовСервера.ФормироватьТаблицуПросмотраПоДвижениямВопрос(
		...
		пФорма.Объект.СпособЗачетаПредоплаты = ПредопределенноеЗначение("Перечисление.СпособыЗачетаИРаспределенияПлатежей.Авто")
	);
	
	пФорма.ТаблицаДляПросмотра = РасчетыПросмотрИнформацииОбОплате.ПолучитьТабличныйДокументОплатаПоДокументуОтгрукзи(
		пФорма.Объект,
		ПоДвижениям, пФорма.Модифицированность, пФорма.ТаблицаДокументовДляИзменения, Ложь
	);
...
&НаСервере
Функция СпособЗачетаПредоплатыПриИзмененииНаСервере(пФорма) Экспорт
	
	ЗаполнилиТаблицуПросмотра = Ложь;
	
	Если пФорма.Объект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто Тогда
		пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Просмотр");
		пФорма.Элементы.ВариантЗаполненияРасшифровки.Видимость = Ложь;
		пФорма.Элементы.ОткрытьПомощникЗачетаПредоплаты.Видимость = Ложь;
		пФорма.Элементы.ГруппаКомандыТаблицыДляПросмотра.Видимость = Истина;
		ЗаполнитьТаблицуПросмотраНаСервере(пФорма);
		ЗаполнилиТаблицуПросмотра = Истина;
		пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаПросмотр;
	Иначе
		пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Вручную");
		пФорма.Элементы.ВариантЗаполненияРасшифровки.Видимость = Истина;
		пФорма.Элементы.ОткрытьПомощникЗачетаПредоплаты.Видимость = Истина;
		пФорма.Элементы.ГруппаКомандыТаблицыДляПросмотра.Видимость = (пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Просмотр"));
		Если пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Просмотр") Тогда
			ЗаполнитьТаблицуПросмотраНаСервере(пФорма);
			ЗаполнилиТаблицуПросмотра = Истина;
			пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаПросмотр;
		Иначе
			пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаРедактирование;
		КонецЕсли;
		ВариантЗаполненияРасшифровкиПриИзмененииНаСервере(пФорма);
	КонецЕсли;
	
	Если Ложь Тогда
		Если пФорма.Объект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто
			И пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаРедактирование Тогда
			пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаПросмотр;
			пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Просмотр");
			ВариантЗаполненияРасшифровкиПриИзмененииНаСервере(пФорма);
		ИначеЕсли пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаПросмотр Тогда
			ЗаполнитьТаблицуПросмотраНаСервере(пФорма);
			ЗаполнилиТаблицуПросмотра = Истина;
		КонецЕсли;
	КонецЕсли;
	
	пФорма.Элементы.Предоплата.Доступность = (пФорма.Объект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Вручную);
	Если пФорма.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаЗаказНаряда" Тогда
		пФорма.Элементы.ЗНГруппаПредоплата.Заголовок = РаботаСФормойДокументаКлиентСервер.ПолучитьЗаголовокОплаты(пФорма.Объект.СпособЗачетаПредоплаты, пФорма.Объект.Предоплата);
	Иначе
		пФорма.Элементы.ГруппаПредоплата.Заголовок = РаботаСФормойДокументаКлиентСервер.ПолучитьЗаголовокОплаты(пФорма.Объект.СпособЗачетаПредоплаты, пФорма.Объект.Предоплата);
	КонецЕсли;
...
&НаСервере
Процедура ВариантЗаполненияРасшифровкиПриИзмененииНаСервере(пФорма, ПриСозданииНаСервере = Ложь) Экспорт
	
	Если пФорма.ВариантЗаполненияРасшифровки = ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияРасшифровкиПлатежа.Просмотр") Тогда
		пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = пФорма.Элементы.СтраницаПросмотр;
		Если НЕ ПриСозданииНаСервере Тогда
			ФормироватьПоДвижениям = пФорма.Объект.Проведен И НЕ пФорма.Модифицированность;
			пФорма.ТаблицаДляПросмотра = РасчетыПросмотрИнформацииОбОплате.ПолучитьТабличныйДокументОплатаПоДокументуОтгрукзи(пФорма.Объект,
				ФормироватьПоДвижениям, пФорма.Модифицированность, пФорма.ТаблицаДокументовДляИзменения, Ложь
			);
		КонецЕсли;
		пФорма.Элементы.ГруппаКомандыТаблицыДляПросмотра.Видимость = Истина;
	Иначе
...
&НаСервере
Процедура РаспределитьСуммуПлатежаПослеРаспределенияЗавершение(пФорма) Экспорт
	
	пФорма.Объект.СпособЗачетаПредоплаты = ПредопределенноеЗначение("Перечисление.СпособыЗачетаИРаспределенияПлатежей.Вручную");
	ЗаполнилиТаблицуПросмотра = СпособЗачетаПредоплатыПриИзмененииНаСервере(пФорма);
	
	Если пФорма.Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница <> пФорма.Элементы.СтраницаПросмотр Тогда
		пФорма.ВариантЗаполненияРасшифровки = пФорма.ВариантЗаполненияРасшифровкиПросмотр;
		РасчетыРаботаСФормамиВызовСервера.ВариантЗаполненияРасшифровкиПриИзмененииНаСервере(пФорма, Ложь);
	КонецЕсли;
...
Функция УчестьОплатуДругимиДокументами(МассивРезультатов) Экспорт
	...
	// Учтём оплату по атку, выполненную другими документами
	ОплаченоВалДок = 0;
...
Функция СтавкаНДСПоОрганизациияНалогообложениеНДСИСпособуЗаполнения(Организация, Дата, СпособЗаполненияСтавки) Экспорт
...
КонецФункции // СтавкаНДСПоОрганизациияНалогообложениеНДСИСпособуЗаполнения()
