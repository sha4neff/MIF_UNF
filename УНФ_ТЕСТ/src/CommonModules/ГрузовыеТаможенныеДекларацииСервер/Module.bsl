...
Процедура ПодобратьНомераГТДПоПредыдущимПоступлениям(ПараметрыПодбора) Экспорт
	...
	|ИЗ
	|	ТаблицаЗапасы КАК ТаблицаЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыПринятыеВРазрезеГТД.Обороты(, , Запись, Организация = &Организация) КАК ЗапасыПринятыеВРазрезеГТДОбороты
...
Процедура АвтоматическийПодборВРеализацииНомераГТД(СтруктураДанные) Экспорт
	...
	ИндекТаблицы = ?(Константы.КонтролироватьОстаткиПоНомерамГТД.Получить() = Истина, 1, 0);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация",	УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(СтруктураДанные.Организация));
	...
	ТаблицаНомеровГТД = РезультатЗапроса[ИндекТаблицы].Выгрузить();
