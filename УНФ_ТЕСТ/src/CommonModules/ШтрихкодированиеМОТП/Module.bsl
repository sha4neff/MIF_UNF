...
Процедура ЗаполнитьДанныеНеизвестнойМаркируемойПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	ОбработкаДанныхПоШтрихкодамУчетМРЦ(ДанныеПоШтрихкодам);
			...
			Или ((Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.ГрупповыеУпаковкиСПотребительскими
				...
				Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
					...
					Для Каждого ВложеннаяСтрокаДерева Из СтрокаДерева.Строки Цикл
						УдалитьСтрокиПоСсылкамНаСрокиДереваРекурсивно(ВложеннаяСтрокаДерева, МаркированныеТовары);
					КонецЦикла;
				...
				Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
					...
					GTIN = РазборКодаМаркировкиИССлужебныйКлиентСервер.GTINИзКодаМаркировки(
						СтрокаДерева.Штрихкод, СтрокаДерева.ВидУпаковки);
	...
	Если РезультатПроверки.СостояниеСеройЗоны <> Неопределено Тогда
		...
		Для Каждого СтрокаДерева Из КандидатыВСеруюЗону Цикл
			...
			Если СостояниеСеройЗоны.СодержитСерыеКоды Тогда
				...
				Если СостояниеСеройЗоны.КоличествоБлоков > 0 Тогда
					...
					ИначеЕсли СостояниеСеройЗоны.КоличествоВложенныхЕдиниц > 0 Тогда
						ПлановоеКоличествоВложенныхПачек = СтандартнаяВложенностьУпаковок[СтрокаДерева.GTIN];
				...
				СтрокаДерева.ВСеройЗоне        = СостояниеСеройЗоны.ВСеройЗоне И ПараметрыСканирования.ПроверятьПотребительскиеУпаковкиНаВхождениеВСеруюЗонуМОТП;
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаДерева.ДатаПроизводства)
				И СтрокаДерева.ДатаПроизводства < ПараметрыСканирования.ДатаПроизводстваНачалаКонтроляСтатусовКодовМаркировкиМОТП Тогда
				
				// До первого июля не действует контроль...
				СтрокаДерева.ВСеройЗоне        = Истина;
				СтрокаДерева.СодержитСерыеКоды = Истина;
...
Процедура УдалитьСтрокиПоСсылкамНаСрокиДереваРекурсивно(СтрокаДерева, ДанныеКодовМаркировки)
	...
	Для Каждого ВложеннаяСтрокаДерева Из СтрокаДерева.Строки Цикл
		УдалитьСтрокиПоСсылкамНаСрокиДереваРекурсивно(ВложеннаяСтрокаДерева, ДанныеКодовМаркировки);
	КонецЦикла;
	...
	Если СтрокаДерева.Строки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаДереваВложенная Из СтрокаДерева.Строки Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДереваВложенная.GTIN) Тогда
				...
				КоэффициентТекущейУпаковки = СтандартнаяВложенностьУпаковок[СтрокаДереваВложенная.GTIN];
...
Функция ВложенныеШтрихкодыПоДаннымМОТП(ДанныеШтрихкодовСписок, ПараметрыСканирования)
	...
	Если ПараметрыСканирования.КонтролироватьСтандартнуюВложенность Тогда
...
Функция ПравилаПроверкиНаОшибки(ПараметрыСканирования, ДанныеПоШтрихкодам, ПроверенныеШтрихкоды = Неопределено)
	...
	Если Не ПараметрыСканирования.ЗапрашиватьСтатусыИСМП Тогда
...
Функция УстановитьШтрихкодДляПечатиККТ(Коллекция) Экспорт
	...
	Для Каждого КлючИЗначение Из Коллекция Цикл
		ШтрихкодированиеИСМП.ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(
			КлючИЗначение.Ключ,
			КодыМаркировки);
		ОшибочныеКоды.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Ключ);
	КонецЦикла;
...
Процедура УдалитьСтрокиДереваУпаковокПоДетализации(ДеревоУпаковок, МассивУдаляемыхСтрок, МаркированныеТоварыВложенныхУпаковок, МаркированныеТовары)
	...
	Для Каждого Строка Из МаркированныеТоварыВложенныхУпаковок Цикл
	...
	Если МаркированныеТовары <> Неопределено Тогда
		Для Каждого Строка Из МаркированныеТовары Цикл
			СсылкаНаСтрокуДерева = СоответствиеСтрокИсточникаПриемника.Получить(Строка.СтрокаДерева);
			Строка.СтрокаДерева = СсылкаНаСтрокуДерева;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
			...
			Если СтрокаДерева.Родитель.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
				...
				GTINКоробаВерхнегоУровня = РазборКодаМаркировкиИССлужебныйКлиентСервер.GTINИзКодаМаркировки(
					СтрокаДерева.Родитель.Штрихкод, СтрокаДерева.Родитель.ВидУпаковки);
...
Процедура ОбработатьДеревоПолученноеИзМОТП(Результат, ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	ИменаКолонокДляКопированияСвойств.Добавить("КоличествоПачек");
	ИменаКолонокДляКопированияСвойств.Добавить("КоличествоБлоков");
	ИменаКолонокДляКопированияСвойств.Добавить("МРЦ");
	ИменаКолонокДляКопированияСвойств.Добавить("Статус");
	ИменаКолонокДляКопированияСвойств.Добавить("ИННВладельца");
	ИменаКолонокДляКопированияСвойств.Добавить("ДатаПроизводства");
	ИменаКолонокДляКопированияСвойств.Добавить("ПредставлениеНоменклатуры");
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект)
		...
		Если ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
			...
			Если СтрокиДляСопоставленияПоУПД.Количество() > 0 Тогда
				ШтрихкодированиеИСМПСлужебный.СопоставитьКодыМаркировкиНоменклатуреПоДаннымУПД(
	...
	Если ТребуетсяСопоставление Тогда
		ЗаполнитьПредставлениеНоменклатурыПоДаннымМОТП(СтрокиДанныхДляСопоставления, МассивНеизвестныхEAN);
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеНоменклатурыПоДаннымМОТП(ДанныеКодовМаркировки, СписокEAN)
	
	СведенияМОТППоШтрихкодамEAN = ИнформацияНеизвестныхEANПоДаннымМОТП(СписокEAN);
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольПолногоКодаМаркировки") Тогда
			...
			Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
				И (СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак
					Или (СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
					И СтрокаДанных.ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix)) Тогда
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольПолногоКодаМаркировки") Тогда
			...
			Если (СтрокаДерева.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак
				Или СтрокаДерева.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
					И СтрокаДерева.ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix)
				И СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
				И (СтрокаДерева.Родитель = Неопределено
				Или СтрокаДерева.Родитель.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Групповая) Тогда
...
Процедура НормализоватьДанныеДереваУпаковок(ДеревоУпаковок, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено)
	...
	ЗапросИРасчетМРЦПоСтрокамДерева(ПараметрыЗаполнения.Кэш.ЗапросИУстановкаМРЦ, ПараметрыЗаполнения.Кэш.ВычислениеМРЦ);
...
Процедура ЗапросИРасчетМРЦПоСтрокамДерева(ЗапрашиваемыеСтроки, ВычисляемыеСтроки) Экспорт
	
	СтрокиЗапроса    = Новый Массив();
	СтрокиВычисления = Новый Массив();
	КэшРезультат     = Новый Соответствие();
	
	Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.УчитыватьМРЦ() Тогда
		Возврат;
	КонецЕсли;
	...
	ИнтерфейсМОТП.ЗапросМРЦПоСтрокамДерева(СтрокиЗапроса);
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиСпецификой(
	...
	ИменаКолонокДляКопированияСвойств.Добавить("ИННВладельца");
	ИменаКолонокДляКопированияСвойств.Добавить("ДатаПроизводства");
	ИменаКолонокДляКопированияСвойств.Добавить("ВСеройЗоне");
	ИменаКолонокДляКопированияСвойств.Добавить("СодержитСерыеКоды");
...
Процедура ОбработкаДанныхПоШтрихкодамУчетМРЦ(ДанныеПоШтрихкодам)
