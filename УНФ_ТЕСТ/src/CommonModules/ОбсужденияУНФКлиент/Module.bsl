...
// Вызывает процедуру записи обсуждения в регистр сведений Обсуждения для Ссылки, а также для упомянутых (по
// навигационным ссылкам) в тексте комментария объектов, формирует напоминания пользователям, упомянутым в тексте комментария
...
Процедура ДобавитьСообщение(Комментарий, Ссылка, Форма) Экспорт
	...
	Если (Ссылка.Пустая() Или Форма.Модифицированность) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьСообщениеПодтверждениеЗаписи", ОбсужденияУНФКлиент, ДополнительныеПараметры);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
		Возврат;
			
	КонецЕсли;
	
	ОбсужденияУНФВызовСервера.ДобавитьСообщение(Комментарий, Ссылка);
	Комментарий = "";
		...
		Если Параметр.Свойство("ВладелецФайла") И Параметр.ВладелецФайла = Ссылка Тогда
			...
			Если ТипЗнч(Источник)=Тип("Массив") Тогда
				...
				Для каждого стр Из Источник Цикл
					ТекстИсточник = ТекстИсточник + Строка(стр)+" ";
				КонецЦикла;
			Иначе
				ТекстИсточник = Строка(Источник);
			КонецЕсли; 
			Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый=Истина Тогда
				ОбсужденияУНФВызовСервера.ДобавитьСообщение(НСтр("ru = 'Присоединен файл '")+ТекстИсточник, Ссылка, Истина);
			Иначе
				ОбсужденияУНФВызовСервера.ДобавитьСообщение(НСтр("ru = 'Записан файл '")+ТекстИсточник, Ссылка,Истина);
			КонецЕсли;
...
Процедура СообщениеАвтоПодбор(Форма, Ссылка, Текст, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	Если ПозицияПользователь>0 И ПозицияПользователь>ПозицияПробел Тогда
		...
		СоответствиеЧастейИменПользователей = ОбсужденияУНФКлиентПовтИсп.СловарьОписанийПользователей();
...
Процедура СообщениеОбработкаВыбора(Форма, Ссылка, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	...
	ПозицияСобака = СтрНайти(ТекстПоля, "@", НаправлениеПоиска.СКонца);
	ПозицияПробел = СтрНайти(ТекстПоля, " ", НаправлениеПоиска.СКонца);
...
Процедура ОбработатьНажатие(Форма, Элемент, ДанныеСобытия, СтандартнаяОбработка) Экспорт
	...
	Если ДанныеСобытия.Свойство("Href") И ЗначениеЗаполнено(ДанныеСобытия.Href) Тогда
		...
		ИначеЕсли СтрНайти(ДанныеСобытия.Href, "Справочник")>0 И СтрНайти(ДанныеСобытия.Href, "ПрисоединенныеФайлы")>0 Тогда
			...
			ДанныеФайла = ОбсужденияУНФВызовСервера.ПолучитьДанныеФайлаСправочникаПоНавигационнойСсылке(ДанныеСобытия.Href);
			РаботаСФайламиСлужебныйКлиент.ОткрытьФайлСОповещением(Неопределено, ДанныеФайла, Форма.УникальныйИдентификатор); 
			
		ИначеЕсли Прав(ДанныеСобытия.Href,12) = "/ПоказатьВсе" Тогда
			Форма.Обсуждение = ОбсужденияУНФВызовСервера.ОбсужденияПоОбъекту(Форма.Объект.Ссылка);
		Иначе
