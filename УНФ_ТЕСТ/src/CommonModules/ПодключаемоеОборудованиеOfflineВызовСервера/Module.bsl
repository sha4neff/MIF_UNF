...
Процедура ЗаполнитьНастройкиККМ(ОфлайнОборудование, НастройкиОборудования) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		АдресМагазина = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Выборка.Склад, ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы"));
		...
		Для каждого Оплата Из ТаблицыДанных.Оплаты Цикл
			...
			Если Оплата.ТипОплаты = "ПлатежнаяКарта" Тогда
				...
				Если НЕ ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
					СообщениеОбОшибке = НСтр("ru='Не найден эквайринговый терминал, соответсвующий кассе ККМ. Оплата картами не заполнена.'");
