...
Функция СериализацияЗаказПокупателя(Данные, КоличествоОбъектов)
	...
	Для каждого СтрокаТЧ Из Данные.Запасы Цикл
		...
		Если НужноПересчитыватьСуммы Тогда
			...
			ДобавляемаяСтрока.Discount = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(
	...
	Если НужноПересчитыватьСуммы Тогда
		...
		ПередаваемыйОбъект.Discount = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(
			СуммаСкидкиНаценки,
			Данные.Курс,
			1,
			Данные.Кратность,
...
Функция СериализацияНастрокиНалогооблажения(КоличествоОбъектов)
...
Процедура СериализоватьИДобавитьСведенияОНастрокахНалогооблаженияВОбъектXDTO(ЗаписьСообщения, ЗаписьXML, ВозвращаемыйСписок, УзелОбмена, НомерСообщенияОчереди, КоличествоОбъектов) Экспорт
	
	ПередаваемыйОбъект = СериализацияНастрокиНалогооблажения(КоличествоОбъектов);
	Если ПередаваемыйОбъект <> Неопределено Тогда
		ВозвращаемыйСписок.objects.Добавить(ПередаваемыйОбъект);
	КонецЕсли;
	
КонецПроцедуры // СериализоватьИДобавитьСведенияОНастрокахНалогооблаженияВОбъектXDTO()
