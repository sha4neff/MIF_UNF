...
Процедура ДобавитьИнформациюОНабореПриПолученииДанныхНоменклатуры(СтруктураДанные) Экспорт
	...
	ИначеЕсли СтруктураДанные.Свойство("ВидОперации") Тогда
		СписокСтатусов = НоменклатураВДокументахСервер.СоответствиеВидаОпреацииИлиХозОперацииСтатусуПартии(, СтруктураДанные.ВидОперации);
	...
	Если СтруктураДанные.ЭтоНабор Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СтруктураПолей = Новый Структура("Номенклатура, Характеристика, ЕдиницаИзмерения, Коэффициент, Количество, ДоляСтоимости, СтавкаНДС, ВесЕдиницыТовара, ОбъемЕдиницыТовара, ТипНоменклатуры, ТипНоменклатурыЗапас, ТипНоменклатурыУслуга, Спецификация, Партия, Цена, Склад, Ячейка, ФиксированнаяСтоимость, СтранаПроисхождения");
			...
			Если СтруктураДанные.Свойство("СуммаВключаетНДС")
				И ((СтруктураДанные.СуммаВключаетНДС И НЕ Выборка.ЦенаВключаетНДС)
					...
					Цена = УправлениеНебольшойФирмойСервер.ПересчитатьСуммуПриИзмененииФлаговНДС(Цена, СтруктураДанные.СуммаВключаетНДС, Выборка.СтавкаНДС);
			КонецЕсли;
...
Процедура ОбновитьСоставНабораВТабличнойЧасти(НоменклатураНабора, ХарактеристикаНабора, НомерВариантаКП = Неопределено, КоличествоНаборов, СоставНабора, Объект, ИмяТабличнойЧасти = "Запасы") Экспорт
	...
	Если СоставНабора.Количество()>0 Тогда
		ПерваяСтрока = СоставНабора[0];
		ЗаменаВыполнена = Ложь;
		// При замене набора в форме редактирования состава строки будут содержать новые значения
		Если ПерваяСтрока.Свойство("НоменклатураНабора") И ЗначениеЗаполнено(ПерваяСтрока.НоменклатураНабора) И ПерваяСтрока.НоменклатураНабора<>НоменклатураНабора Тогда
			НоменклатураНабора = ПерваяСтрока.НоменклатураНабора;
			Если ДобавленныеСтроки.Количество()>0 Тогда
				ДобавленныеСтроки[0].НоменклатураНабора = НоменклатураНабора;
				ЗаменаВыполнена = Истина;
			КонецЕсли;
		КонецЕсли; 
		Если ПерваяСтрока.Свойство("ХарактеристикаНабора") И ЗначениеЗаполнено(ПерваяСтрока.ХарактеристикаНабора) И ПерваяСтрока.ХарактеристикаНабора<>ХарактеристикаНабора Тогда
			ХарактеристикаНабора = ПерваяСтрока.ХарактеристикаНабора;
			Если ДобавленныеСтроки.Количество()>0 Тогда
				ДобавленныеСтроки[0].ХарактеристикаНабора = ХарактеристикаНабора;
				ЗаменаВыполнена = Истина;
			КонецЕсли;  
		КонецЕсли;
		Если ЗаменаВыполнена Тогда
			...
			ТЧ.Свернуть("НоменклатураНабора, ХарактеристикаНабора"+?(ДобавленныеСтроки[0].Свойство("НомерВариантаКП"), ", НомерВариантаКП", ""), "Количество");
			Объект.ДобавленныеНаборы.Загрузить(ТЧ);
			ДобавленныеСтроки.Очистить();
		КонецЕсли; 
	КонецЕсли;
	
	СтрокиТЧ = Объект[ИмяТабличнойЧасти].НайтиСтроки(СтруктураОтбора);
	...
	СпособРасчетаЦеныНабора = НоменклатураНабора.СпособРасчетаЦеныНабора;
	СохранятьЦены = (СпособРасчетаЦеныНабора=Перечисления.СпособыРасчетаЦеныНабора.СкладыватьИзЦенКомплектующих);
	ЭтоЗаказ = (ТипЗнч(Объект.Ссылка)=Тип("ДокументСсылка.ЗаказПокупателя") И Объект.ВидОперации<>Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд);
	...
	КонецЕсли;
	Если ЭтоЗаказНаряд И Объект.Свойство("ПоложениеВидаРабот") И Объект.ПоложениеВидаРабот=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
	...
	Если ЭтоЗаказНаряд Тогда
		...
		Для каждого ЭлементСостава Из СоставНабора Цикл
			Если НЕ ЭлементСостава.Свойство("ТипНоменклатуры") ИЛИ НЕ ЭлементСостава.Свойство("ФиксированнаяСтоимость") ИЛИ ЭлементСостава.ТипНоменклатуры<>Перечисления.ТипыНоменклатуры.Работа Тогда
				Продолжить;
			КонецЕсли;
			...
			ИначеЕсли Объект.Свойство("ПоложениеВидаРабот") И Объект.ПоложениеВидаРабот=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
				...
				Если НомерВариантаКП<>Неопределено Тогда
					СтруктураОтбора.Вставить("НомерВариантаКП", НомерВариантаКП);
				КонецЕсли;
				НайденныеСтроки = Объект[ИмяТабличнойЧасти].НайтиСтроки(СтруктураОтбора);
	...
	Для каждого Стр Из СтрокиТЧ Цикл
		Если ИндексСтрокиВставки=Неопределено Тогда
			ИндексСтрокиВставки = Объект[ИмяТабличнойЧасти].Индекс(Стр);
		КонецЕсли; 
		СтрБуфер = БуферЗначений.Добавить();
		ЗаполнитьЗначенияСвойств(СтрБуфер, Стр);
		Объект[ИмяТабличнойЧасти].Удалить(Стр);
	КонецЦикла;
	
	РассчитатьЦеныСоставаНабора(НоменклатураНабора, ХарактеристикаНабора, НомерВариантаКП, КоличествоНаборов, СоставНабора, Объект);
	
	// Добавление строк в табличную часть
	ДобавлятьСтроки = (ИндексСтрокиВставки=Неопределено ИЛИ ИндексСтрокиВставки>=Объект[ИмяТабличнойЧасти].Количество());
	Для каждого ЭлементСостава Из СоставНабора Цикл
		...
		Иначе
			НоваяСтрока = Объект[ИмяТабличнойЧасти].Вставить(ИндексСтрокиВставки);
		...
		Если СтрокиБуфер.Количество()>0 Тогда
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокиБуфер[0], , "Номенклатура, Характеристика"+?(НоваяСтрока.Свойство("ЕдиницаИзмерения"), ", ЕдиницаИзмерения", ""));
		Иначе
			...
			Если НоваяСтрока.Свойство("Спецификация") Тогда
				Если НоваяСтрока.Свойство("Характеристика") Тогда
					СтруктураПолей.Вставить("Спецификация", УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(НоваяСтрока.Номенклатура, НоваяСтрока.Характеристика));
				Иначе
					СтруктураПолей.Вставить("Спецификация", УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(НоваяСтрока.Номенклатура));
				КонецЕсли;
			КонецЕсли;
		...
		Если НоваяСтрока.Свойство("КартинкаНабора") И ДобавленнаяСтрока<>Неопределено Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		УправлениеНебольшойФирмойКлиентСервер.РассчитатьСуммыВСтрокеТЧ(НоваяСтрока, ПараметрыРасчета);
		НаборыКлиентСервер.ЗаполнитьРеквизитыНовойСтроки(Объект[ИмяТабличнойЧасти], НоваяСтрока);
...
Процедура РассчитатьЦеныСоставаНабора(НоменклатураНабора, ХарактеристикаНабора, НомерВариантаКП = Неопределено, КоличествоНаборов, СоставНабора, Объект) Экспорт
	...
	Если УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокументаПоСсылке("ВидЦен", Объект.Ссылка) Тогда
		...
		Если УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокументаПоСсылке("ПроцентСкидкиПоДисконтнойКарте", Объект.Ссылка) Тогда
	...
	Для каждого СтрокаТЧ Из СоставНабора Цикл
		...
		СтрокаТЧ.Вставить("НоменклатураНабора", НоменклатураНабора);
		СтрокаТЧ.Вставить("ХарактеристикаНабора", ХарактеристикаНабора);
		СтрокаТЧ.Вставить("Цена", 0);
		Если НомерВариантаКП<>Неопределено Тогда
			СтрокаТЧ.Вставить("НомерВариантаКП", НомерВариантаКП);
		Иначе
			СтрокаТЧ.Вставить("НомерВариантаКП", 0);
		КонецЕсли; 
		СтрокаТЧ.Вставить("КоличествоНаборов", КоличествоНаборов);
		...
		Если НЕ СтрокаТЧ.Свойство("СтавкаНДС") Тогда
			Если Объект.Свойство("НалогообложениеНДС") И НЕ Объект.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(НоменклатураСтроки.ВидСтавкиНДС) Тогда
				СтрокаТЧ.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(НоменклатураСтроки.ВидСтавкиНДС, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса())));
			Иначе
				СтрокаТЧ.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(Объект.Организация.ВидСтавкиНДСПоУмолчанию, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса())));
			КонецЕсли;
		КонецЕсли; 
		
	КонецЦикла;
	
	Если СтруктураДанных.Свойство("ВидЦен") Тогда
		УправлениеНебольшойФирмойСервер.ПолучитьЦеныТабличнойЧастиПоВидуЦен(СтруктураДанных, СоставНабора);
		Если СтруктураДанных.Свойство("ОкругленияНаборов") И СтруктураДанных.ОкругленияНаборов.Количество()>0 Тогда
			НаборыКлиентСервер.ОкруглениеЦеныНабора(, , , СоставНабора, СтруктураДанных.ОкругленияНаборов[0].Округление);
		КонецЕсли; 
	КонецЕсли;
...
Процедура КомпоноватьТабличнуюЧастьПоНаборам(ДанныеОбъектовПечати, ИмяТабличнойЧасти, Ошибки = Неопределено, ПроверятьРазделители = Ложь) Экспорт
	...
	Для каждого ОбъектПечати Из ДанныеОбъектовПечати Цикл
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			Для ИндекСостава = 0 По СоставНабора.Количество()-1 Цикл
				ИндексСтроки = ОбъектПечати[ИмяТабличнойЧасти].Индекс(СоставНабора[ИндекСостава]);
				Если ИндексСтроки=ИндексПервойСтроки+ИндекСостава Тогда
					Продолжить;
				КонецЕсли;
				ОбъектПечати[ИмяТабличнойЧасти].Сдвинуть(СоставНабора[ИндекСостава], ИндексПервойСтроки+ИндекСостава-ИндексСтроки);
			КонецЦикла;
		КонецЦикла;
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			ДанныеСтроки.Вставить("ЕдиницаИзмеренияКоэфициент", 1);
			Если ЕстьВариантыКП Тогда
			...
			// Представление номенклаткры
			ПредставлениеНабора = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(СтрокаНабора.ЗапасНабора, СтрокаНабора.ХарактеристикаНабора, СтрокаНабора.АртикулНабора);
			...
			// Единица измерения
			Для каждого ИмяПараметра Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПоляЕдиницаИзмерения+?(СтрокаНабора.ТипНоменклатурыНабора=Перечисления.ТипыНоменклатуры.Запас, ", ВидУпаковки", "")) Цикл
				ДанныеСтроки.Вставить(ИмяПараметра, СтрокаНабора.ЕдиницаИзмеренияНабора);
			КонецЦикла;
			Для каждого ИмяПараметра Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПоляКодЕдиницыИзмерения) Цикл
