...
Процедура ПриСозданииФормыНастроекЯндексКассы(Форма, Группа, Префикс, ДополнительныеНастройки) Экспорт
	...
	Поле.ПодсказкаВвода = "ООО НКО ""Яндекс.Деньги"" - Будет создан автоматически";

	ИмяРеквизита = Префикс + "ЭквайринговыйТерминал";
	Поле = Элементы.Добавить(ИмяРеквизита, ТипПолеФормы, ГруппаНастройкиУчета);
	Поле.Вид = ВидПоляФормы.ПолеВвода;
	Поле.ПутьКДанным = ИмяРеквизита;
	Поле.Подсказка = НСтр("ru = 'Виртуальный эквайринговый терминал для операций по Яндекс.Кассе
								|Оставьте это поле пустым и виртуальный эквайринговый терминал будет создан автоматически'");
	Поле.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Поле.АвтоОтметкаНезаполненного = Ложь;
	Поле.ПодсказкаВвода = " Яндекс.Касса - Будет создан автоматически";
	...
	Поле.Вид = ВидПоляФормы.ПолеВвода;
	Поле.ПутьКДанным = ИмяРеквизита;
	Поле.Подсказка = НСтр("ru = 'Касса ККМ для пробития чеков оплат, которые осуществляются через Яндекс.Кассу'");
	Поле.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Поле.АвтоОтметкаНезаполненного = Истина;
	Поле.ПодсказкаВвода = "Касса ККМ";
...
Процедура ПриПроверкеЗаполненияОснованияПлатежа(Знач ОснованиеПлатежа, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОснованиеПлатежа.ИдентификаторПлатежа) Тогда
		...
		СообщениеОбОшибке.Текст = НСтр("ru = 'Данный документ был создан раньше, чем интеграция с Яндекс.Кассой появилась в УНФ.
	...
	Если НЕ Отказ Тогда
		...
		Если ЕстьОплатаПоДокументу(ОснованиеПлатежа) Тогда
			...
			СообщениеОЧастичнойОплате.Текст = НСтр("ru = 'Внимание! По данному документу есть аванс или оплата. 
											|Яндекс.Касса не поддерживает частичные оплаты, поэтому будет отправлен счет на полную сумму документа.'");
...
Процедура ЗаполнитьДанныеОснованияПлатежа(Знач ОснованиеПлатежа, ДанныеОснованияПлатежа) Экспорт
	...
	Если ВыборкаШапка.Следующий() Тогда
		...
		СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(ВыборкаШапка.Организация, ВыборкаШапка.Дата,, ВыборкаШапка.БанковскийСчет);
		СведенияОКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(ВыборкаШапка.Контрагент, ВыборкаШапка.Дата);
...
Процедура ЗаполнитьКонтактнуюИнформациюОснованияПлатежа(Знач ОснованиеПлатежа, КонтактнаяИнформация) Экспорт
	...
	СведенияОКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Контрагент, ТекущаяДатаСеанса());
...
Процедура ПриФормированииСпискаПолучателейСообщения(Знач ОснованиеПлатежа, Знач ВариантОтправки, Получатели) Экспорт
	...
	Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОснованиеПлатежа, "Контрагент");
	Телефон = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Контрагент, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
	АдресЭП = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Контрагент, Справочники.ВидыКонтактнойИнформации.EmailКонтрагента);
...
Процедура ПриЗагрузкеОперацийПоЯндексКассе(Знач Операции, Результат, Отказ) Экспорт
	...
	Для каждого Операция Из ОперацииКЗагрузке Цикл
		...
		Если Операция.ВидОперации = "Оплата" Тогда
			...
			ДанныеЗаполнения.Вставить("ЭквайрерБеретКомиссиюЗаВозвратДС", Операция.ЭквайринговыйТерминал.ЭквайрерБеретКомиссиюЗаВозвратДС);
			...
			Если СуммаОснованияПлатежа <> ДокументОплаты.СуммаДокумента Тогда
				...
				МассивСуммКомисси = РаспределитьПропорционально(ДокументОплаты.СуммаКомиссииПоДоговору, МассивКоэффициентов);
			...
			Если ДатыЗапретаИзменения.ИзменениеЗапрещено(ДокументОплаты,, ТекстСообщения) Тогда
				ЗаписьЖурналаРегистрации(НСтр("ru='ИнтеграцияСЯндексКассойУНФ.ПриЗагрузкеОперацийПоЯндексКассе'",
					...
					,
					ТекстСообщения);
				Результат.КоличествоНезагруженыхОпераций = Результат.КоличествоНезагруженыхОпераций + 1;
				Продолжить;
			КонецЕсли;
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru='ИнтеграцияСЯндексКассойУНФ.ПриЗагрузкеОперацийПоЯндексКассе'",
					...
					Метаданные.ОбщиеМодули.ИнтеграцияСЯндексКассойУНФ,
					,
					НСтр("ru='Ошибка проведения документа ""Операция по платежным картам"" при загрузке операций из Яндекс.Кассы'",
						ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПредопределенныеШаблоныСообщений(Шаблоны) Экспорт 
	...
	Для каждого ТипОснования Из СписокТиповОснований Цикл
		...
		Шаблон.Вставить("Наименование", НСтр("ru = 'Счет на оплату через Яндекс.Кассу (Синоним)'"));
		...
		ТекстШаблона = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстШаблона,
						...
						НСтр("ru='Сумма:'"),
						НСтр("ru='Для оплаты счета перейдите на страницу Яндекс.Кассы, нажав на кнопку'"));
...
Функция НайтиИлиСоздатьЭквайераПоУмолчанию(Организация, СоздаватьЕслиНеНайден = Ложь)
	...
	Иначе
		...
		СоздаваемыйКонтрагент.Комментарий = НСтр("ru = 'Создан автоматически при настройке подключения к Яндекс.Кассе'");
...
Функция НайтиИлиСоздатьЭквайринговыйТерминалПоУмолчанию(СтруктураПараметров)
	...
	НовыйТерминал.Наименование = НСтр("ru = 'Яндекс.Касса'");
	...
	СтрокаНовыйВидПлатежнойКарты.ВидПлатежнойКарты = НСтр("ru = 'Яндекс.Касса'");
