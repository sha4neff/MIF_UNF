...
Процедура ПриСрабатыванииСобытия2(Менеджер, Событие, Предмет, Источник = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ВключенныеЗадачи = ВключечнныеЗадачи(Менеджер, Событие);
...
Функция НовыйСлужебныйПользовательАссистента()
	...
	Попытка
		...
		ОписаниеПользователяИБ.Роли.Добавить(Метаданные.Роли.ИспользованиеМетодовПровайдераТелефонии.Имя);     // todo
...
Функция ВключечнныеЗадачи(Менеджер, Событие) Экспорт
