...
//
// Для новых объектов и при изменении из списка, изменения в СистемаВзаимодействия записываются регламентым заданием (отложенно)
...
//
// Параметры:
//  ДанныеСообщения - Структура - См. функцию ОбсужденияСервер.НовыйДанныеСообщения()
//
...
// Получает объект-владец обсуждения системы взаимодействия по контексу обсуждения.
//
// Параметры:
...
// Получает контекстное обсуждение системы взаимодейсвие по указанному объекту
...
Функция ПолучитьКонтекстноеОбсуждение(ОбъектСсылка) Экспорт
	...
	Обсуждение.Заголовок = "История " + ОбъектСсылка;
...
Процедура ОбсужденияПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") Тогда
		...
		ДобавитьСообщение(Сообщение, Источник.Ссылка,,Истина);
	...
	Если НЕ ПустаяСтрока(Сообщение) Тогда
		...
		Если ИнформационнаяБазаЗарегистрирована Тогда
			...
			Иначе // запись регламентным заданием
				ДобавитьСообщение(Сообщение, Источник.Ссылка, ,Истина);
			КонецЕсли;
		ИначеЕсли УправлениеНебольшойФирмойПовтИсп.ИспользоватьЧатВОбсуждениях() Тогда //запись в регистр Обсуждения
			ДобавитьСообщение(Сообщение, Источник.Ссылка,,Истина);
		КонецЕсли; 
...
Функция ОбсужденияПоОбъекту(Объект, ЗаПериод=Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		HTMLСсылкаДляСтарыхКомментариев(Выборка.Пользователь),
		ТекстКомментария);
		Результат = Результат+Символы.ПС;
...
Процедура ДобавитьСообщение(
	...
	ПользовательАвторСообщения	= ?(НЕ ЗначениеЗаполнено(Пользователь), Пользователи.АвторизованныйПользователь(), Пользователь);
	...
	Если ИнформационнаяБазаЗарегистрирована Тогда
		...
		ДанныеСообщения.Текст = Комментарий;
		ДанныеСообщения.Дата = МоментВремени;
	...
	Иначе
		...
		Если НЕ Системный Тогда
			...
			Для каждого СсылкаУпоминание Из МассивСсылокУпоминаний Цикл
				...
				ЗаписатьДанныеВРегистрОбсуждения(КомментарийУпоминание, СсылкаУпоминание, МоментВремени, Системный, ПользовательАвторСообщения);
			КонецЦикла;
		КонецЕсли;
		...
		// Записать основное сообщение
		ЗаписатьДанныеВРегистрОбсуждения(Комментарий, Ссылка, МоментВремени, Системный, ПользовательАвторСообщения);
		
		СформироватьНапоминанияПользователям(Ссылка, ПользовательАвторСообщения, Комментарий, ПользователиРассылки);
...
Процедура ДобавитьЗаписьВводНаОсновании(Источник)
	...
	Для каждого стр Из спДокументовОснований Цикл
		...
		ДобавитьСообщение(Сообщение, ДокументОснование,,Истина);
	КонецЦикла; 
...
Функция HTMLСсылка(Ссылка) Экспорт
	...
	Если ИнформационнаяБазаЗарегистрирована Тогда
		Если УправлениеНебольшойФирмойПовтИсп.ДоступныМеханизмы8_3_11() Тогда
			Возврат ПолучитьНавигационнуюСсылку(Ссылка);
		Иначе
			Возврат Строка(Ссылка);
		КонецЕсли;
	ИначеЕсли УправлениеНебольшойФирмойПовтИсп.ИспользоватьЧатВОбсуждениях() Тогда
...
Функция HTMLСсылкаДляСтарыхКомментариев(Ссылка) Экспорт
...
Процедура ЗаписатьДанныеВРегистрОбсуждения(Комментарий, Ссылка, МоментВремени, Системный, Пользователь)
...
// Получает контекстное обсуждение системы взаимодейсвие по навигационной ссылке
// В случае если такое обсуждение еще не создано - создает его.
//
// Параметры:
...
//   - ОбсуждениеСистемыВзаимодействия
//
Функция ПолучитьКонтекстноеОбсуждениеПоНавигационнойСсылке(НавигационнаяСсылка, Заголовок = "История ") Экспорт
...
Процедура ОформитьСистемныйТекст(Текст)
	...
	Текст = Новый ФорматированнаяСтрока("#" + Текст, Новый Шрифт(,,,Истина));
...
Функция НайтиСокращения(Знач ТекстСообщения, массивСокращений)
	
	НачалоНССокращения = СтрНайти(ТекстСообщения, "e1cib/data/Справочник.СообщенияОбсуждений");
	Если НачалоНССокращения > 0 Тогда
		
		текстСправа = Сред(ТекстСообщения, НачалоНССокращения);
		ПервыйПробел = СтрНайти(текстСправа, " ");
		конецНССокращения = ?(ПервыйПробел=0, СтрДлина(текстСправа), ПервыйПробел);
		текстНС = Сред(текстСправа, 1, конецНССокращения);
		
		ссылкаСправочника = ПолучитьСсылкуИзНавигационной(текстНС);
		Если НЕ ТипЗнч(ссылкаСправочника) = Тип("Строка") Тогда
		
			массивСокращений.Добавить(ссылкаСправочника);
			
			НайтиСокращения(Сред(текстСправа, конецНССокращения+1), массивСокращений);
...
// Задает длину текстового сообщения, начиная с которой оставщееся сообщение обрезается до трёх точек, 
...
Функция ПреобразоватьНавигационныеСсылки(Комментарий)
	...
	МассивСтрок = СтрРазделить(Комментарий, Символы.ПС, Ложь);
	Комментарий = "";
	
	Для каждого строкаКомментария Из МассивСтрок Цикл
	
		Остаток = строкаКомментария;
		...
		Пока Позиция>0 Цикл
			...
			Если НЕ (СтрНайти(СловоПосле, "e1cib/list/") > 0 
				ИЛИ ТипЗнч(СсылкаНаОбъект) = Тип("Строка")) 
				Тогда
		...
		Пока Индекс>0 Цикл
			ПредставленияСсылокТекст = ПредставленияСсылок[Индекс-1];
			Если ПредставленияСсылокТекст = Неопределено Тогда
				Представление = СсылкаНаОбъект;
			Иначе	
				Представление = ПредставленияСсылокТекст.Текст;
			КонецЕсли; 
		...
		Если НЕ МассивСтрок[МассивСтрок.Количество()-1] = строкаКомментария Тогда
			// Если это не последняя строка, добавим перенос строки
...
Функция ПолучитьДанныеФайлаСправочникаПоНавигационнойСсылке(СтрокаНавигационнойСсылкиСправочника) Экспорт
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(СсылкаПрисоединенныеФайлы,	Неопределено);
		...
		Пока ВыборкаОбъектПользователь.Следующий() Цикл
			...
			Пока ВыборкаЗапись.Следующий() Цикл
				...
				Если ВыборкаЗапись.Системный Тогда
					...
					НайтиСокращения(Текст, МассивСокращений);
				КонецЕсли;
			...
			Если НЕ ЕстьОшибки Тогда
				...
				Если МассивСокращений.Количество() > 0 Тогда
					Для каждого элСокращения Из МассивСокращений Цикл
						ПолноеСообщение = элСокращения.ПолучитьОбъект();
						ПолноеСообщение.ИдентификаторСообщения = НовоеСообщение.Идентификатор;
						ПолноеСообщение.Записать();
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
