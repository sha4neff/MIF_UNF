...
Функция КодМаркировкиДляПередачиИСМП(СтрокаКодаМаркировки, ПараметрыНормализации) Экспорт
	...
	Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
		Или (СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
		И СтрокаКодаМаркировки.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC) Тогда
...
Функция НоваяТаблицаПоискаКодаМаркировкиВПуле() Экспорт
...
Процедура ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(ДанныеСтроки, КодыМаркировки) Экспорт
...
Функция РезультатПоискаВПулеКодовМаркировки(КодыМаркировки, Поля = "") Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныеПоля", ТекстПолей);
...
Процедура УстановитьШтрихкодДляПечатиККТ(Коллекция)
	...
	Если ПолныйКодМОТП.Количество() > 0 Тогда 
		...
		ШтрихкодированиеМОТП.УстановитьШтрихкодДляПечатиККТ(ПолныйКодМОТП);
