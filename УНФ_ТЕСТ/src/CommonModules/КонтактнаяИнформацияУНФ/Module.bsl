...
Процедура ПриСозданииПриЧтенииНаСервере(Форма, Объект = Неопределено, ШиринаПоляВида = 8) Экспорт
	...
	ДобавитьРеквизитыФормыКонтактнойИнформации(Форма, ЕстьКолонкаИдентификаторСтрокиТабличнойЧасти);
...
Процедура ПередЗаписьюНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Для Каждого ДанныеКИ Из Форма.КонтактнаяИнформация Цикл
		Если Форма.ЕстьКолонкаИдентификаторСтрокиТабличнойЧасти Тогда
			УправлениеКонтактнойИнформацией.ЗаписатьКонтактнуюИнформацию(ТекущийОбъект, ДанныеКИ.Значение, ДанныеКИ.Вид, ДанныеКИ.Тип, ДанныеКИ.ИдентификаторСтрокиТабличнойЧасти);
		Иначе
			УправлениеКонтактнойИнформацией.ЗаписатьКонтактнуюИнформацию(ТекущийОбъект, ДанныеКИ.Значение, ДанныеКИ.Вид, ДанныеКИ.Тип);
		КонецЕсли;
	КонецЦикла;
...
// Функция получает XML по предсталению адреса
//
//  ЗначенияПолей - Строка - XML, Описание полей контактной информации.
//  Представление - Строка - Представление контактной информации. Используется, если невозможно определить значение из
//                           параметра ЗначенияПолей
...
Процедура ПрочитатьСвойстваВидовКонтактнойИнформации(Форма, ГруппаВидовКИ, ЕстьКолонкаИдентификаторСтрокиТабличнойЧасти)
...
Функция НоваяПорцияВсегдаВыводимыеВидыКонтактнойИнформации(Форма, Объект, ИдентификаторСтрокиТабличнойЧасти)
	...
	Запрос.УстановитьПараметр("ТаблицаКИ", Объект.КонтактнаяИнформация.Выгрузить(Новый Массив,"Вид,Тип,Представление,Значение"));
	...
	Для каждого КоллекцияПоТипу Из КоллекцияПоТипам Цикл
		...
		Пока ВыборкаОрганизаций.Следующий() Цикл
			...
			Пока ВыборкаПоВидам.Следующий() Цикл
				...
				АдресСтруктура = РаботаСАдресами.СведенияОбАдресе(ВыборкаПоВидам.Значение);
