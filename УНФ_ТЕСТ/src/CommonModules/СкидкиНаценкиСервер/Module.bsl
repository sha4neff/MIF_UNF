...
Функция ПроверитьНеобходимостьПересчетаАвтоматическихСкидок(Действие, КолонкаТЧ) Экспорт
	...
	ИначеЕсли СтрНайти(Действие, "Контрагент") > 0 Тогда
		...
		НаборЗаписей.Прочитать();
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			Если ЗаписьНабора.ЕстьСкидкиСПолучателямиКонтрагенты Тогда
				ТребуетсяПересчетАвтоматическихСкидок = Истина;
			КонецЕсли;
		КонецЦикла;
...
Функция ИнтервалНакопленияОбъемаПродаж(Условие, ПараметрыРасчета)
	...
	ИначеЕсли Условие.ВидПериода = Перечисления.ВидыПериодовДляНакопительныхСкидок.Прошлый Тогда
		...
		Если Условие.Периодичность = Перечисления.Периодичность.Год Тогда
			ДатаНачала = УправлениеНебольшойФирмойКлиентСервер.РассчитатьДатуОкончанияПериода(
...
Функция ТекстЗапросаТаблицаУсловияПредоставления()
	...
	|	ВЫБОР
	|		КОГДА Условия.УсловиеПредоставления.УсловиеПредоставления В (ЗНАЧЕНИЕ(Перечисление.УсловияПредоставленияСкидокНаценок.ЗаРазовыйОбъемПродаж), ЗНАЧЕНИЕ(Перечисление.УсловияПредоставленияСкидокНаценок.ЗаНакопленыйОбъемПродаж))
...
Функция РассчитатьДеревоСкидокНаценок(ПараметрыРасчета, ВходныеПараметры) Экспорт
	...
	Если ПараметрыРасчета.Свойство("ДисконтнаяКарта")
		...
		Пока ВыборкаУсловияПредоставления.Следующий() Цикл
			Если ВыборкаУсловияПредоставления.УсловиеПредоставления = Перечисления.УсловияПредоставленияСкидокНаценок.ЗаНакопленыйОбъемПродаж Тогда
	...
	Пока ВыборкаУсловияПредоставления.Следующий() Цикл
		...
		Если ВыборкаУсловияПредоставления.УсловиеПредоставления = Перечисления.УсловияПредоставленияСкидокНаценок.ЗаНакопленыйОбъемПродаж
