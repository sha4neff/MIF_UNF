...
Функция ИтогиПоДню(ДеньВывода, Кратность, РабочиеПериоды, Ресурс, РасписаниеЗагрузки, ДокументыРасшифровки, МатрицаИнтервалов, НайденныеСтроки = Неопределено) Экспорт
	...
	ТаблицаРабочихИнтревалов = Новый ТаблицаЗначений;
	ТаблицаРабочихИнтревалов.Колонки.Добавить("НачалоИнтервала");
	ТаблицаРабочихИнтревалов.Колонки.Добавить("ОкончаниеИнтервала");
	ТаблицаРабочихИнтревалов.Колонки.Добавить("КоличествоЯчеекВИнтервале");
	ТаблицаРабочихИнтревалов.Колонки.Добавить("ПоДокументу");
	...
	Для Каждого РабочийПериод Из РабочиеПериодыЗаДень Цикл
		...
		Пока НачалоПериода <= ОкончаниеПериода Цикл
			...
			НоваяСтрока = ТаблицаРабочихИнтревалов.Добавить();
	...
	ВремяНачалаИнтервала = ВремяИнтервала;
	...
	ТаблицаРабочихИнтревалов.Сортировать("НачалоИнтервала Возр");
	...
	Для Каждого РабочийИнтервал Из ТаблицаРабочихИнтревалов Цикл;
...
Функция МаксимумПоЗагрузке(ДокументыРасшифровки, НачалоИнтервала, ОкончанияИнтервала, КоличествоЯчеекВИнтервале, ТаблицаИнтервалаПо5Мин) Экспорт
	...
	Для Каждого СтрокаДокументаРасшифровки Из ДокументыРасшифровки Цикл
		...
		НомерЯчейкиОкончанияИнтревала = НомерЯчейкиНачалаИнтервала + (КоличествоЯчеек-1);
		НомерЯчейкиОкончанияИнтревала = ?(НомерЯчейкиОкончанияИнтревала > КоличествоЯчеекВИнтервале, КоличествоЯчеекВИнтервале, НомерЯчейкиОкончанияИнтревала);
		
		НоваяСтрока = ТаблицаИнтервалаПо5Мин.Добавить();
		
		Пока НомерЯчейкиНачалаИнтервала <= НомерЯчейкиОкончанияИнтревала Цикл
...
Функция ЕстьОшибкиЗаполнения(ВыбранныеРесурсы, Объект) Экспорт
	...
	Для Каждого СтрокаРесурс Из ВыбранныеРесурсы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаРесурс.РесурсПредприятия) Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указан ресурс.'");
		...
		Если Не ЗначениеЗаполнено(СтрокаРесурс.Мощность) Тогда
			ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указано значение загрузки.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			Объект,
			ТекстСообщения,
		...
		Если НачалоДня(СтрокаРесурс.Старт) = НачалоДня(СтрокаРесурс.Финиш) 
			...
			Если Не ЗначениеЗаполнено(СтрокаРесурс.ВидЗавершения) Тогда
				ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указан вид окончания повторов.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				Объект,
				ТекстСообщения,
			...
			Если Не ЗначениеЗаполнено(СтрокаРесурс.ЗавершатьПосле) Тогда
				ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указано значение окончания повторов.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				Объект,
				ТекстСообщения,
...
Функция РазложитьСтрокиПоРасписанию(ТаблицаРасписания) Экспорт
	...
	Для Каждого СтрокаРасписания Из ТаблицаРасписания Цикл
		...
		Если СтрокаРасписания.ВидПовтора = Перечисления.ВидыПовторовРасписания.Еженедельно Тогда
			...
			КоличествоОбрабатываемыхДнейНедели = МассивДнейНедели.Количество();
			ТекущийНомерДняНедели = ДеньНедели(НачалоИнтервала);
...
Процедура ПерезаполнитьСлужебныеРеквизитыТаблицыРесурсов(РесурсыПредприятия) Экспорт
	...
	Для Каждого СтрокаРесуров Из РесурсыПредприятия Цикл
		
		СтрокаРесуров.КонтролироватьШаг = СтрокаРесуров.РесурсПредприятия.КонтролироватьШагИнтервалаВДокументах;
		СтрокаРесуров.КратностьПланирования = СтрокаРесуров.РесурсПредприятия.КратностьПланирования;
		
		ВыбранныеДниНедели = ПредставлениеДнейНедели(СтрокаРесуров);
		
		ДополнениеПоМесяцуГоду = "";
		
		Старт = СтрокаРесуров.Старт;
		Финиш = СтрокаРесуров.Финиш;
		...
		Если СтрокаРесуров.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.Ежемесячно")
			...
			Если ЗначениеЗаполнено(ДатаПовторения) Тогда
				...
				+ Строка(ДатаПовторения) + "-ое "+ ПолучитьМесяцПоНомеру(СтрокаРесуров.НомерМесяца)+".","");
			ИначеЕсли ЗначениеЗаполнено(ДеньНеделиМесяца) Тогда
				
				Если ЭтоПоследняяНеделяМесяца(Старт) Тогда
					ДополнениеПоМесяцуГоду = ", в послед. " + СоответствиеНомераДнюНедели(ДеньНеделиМесяца)+ " месяца.";
				Иначе
					НомерНеделиМесяца = НеделяГода(Старт)-НеделяГода(НачалоМесяца(Старт))+1;
					ДополнениеПоМесяцуГоду = " "+СоответствиеНомераДнюНедели(ДеньНеделиМесяца) + " кажд. " +Строка(НомерНеделиМесяца)+ "-й" + " недели";
				КонецЕсли;
		...
		//Повторы
		СтрокаРесуров.ПовторыДоступны = Ложь;
		СтрокаРесуров.ПериодРазличный = Ложь;
		
		Если ЗначениеЗаполнено(Старт) И ЗначениеЗаполнено(СтрокаРесуров.Финиш) Тогда
			
			СтрокаРесуров.ПериодРазличный = ?(Не НачалоДня(Старт) = НачалоДня(СтрокаРесуров.Финиш), Истина, Ложь);
			
			Если НачалоДня(Старт) = НачалоДня(СтрокаРесуров.Финиш) Тогда
				СтрокаРесуров.ПовторыДоступны = Истина;
			КонецЕсли;
		...
		Если ДлительностьВМинутах >= 1440 Тогда
			СтрокаРесуров.Дни = Цел(ДлительностьВМинутах/1440);
		КонецЕсли;
		
		СтрокаРесуров.Время = Дата(1,1,1)+((ДлительностьВМинутах - СтрокаРесуров.Дни*1440)*60);
		
		// Расшифровка счетчика
		
		Если СтрокаРесуров.ВидЗавершения = Перечисления.ВидыЗавершенияПовторовРасписания.ПоСчетчику
...
//
Процедура СформироватьЗаписиКолендаряСотрудникаПоРесурсам(ДокументСсылка, ТаблицаДвижений, Отказ) Экспорт
	...
	Для Каждого СтрокаРасписания Из ТаблицаДвижений Цикл
		...
		Если Не НайденныеСтроки.Количество() Тогда
			...
			Исключение
				...
				ТекстСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Ошибка записи календаря сотрудника'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ДокументСсылка, ТекстСообщения);
				ЕстьОшибка = Истина;
				Прервать;
			КонецПопытки;
		...
		Иначе
			...
			Исключение
				ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
				ШаблонСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Ошибка установки пометки на удаление записи календаря сотрудника %ЗаписьКалендаря%'");
				ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%ЗаписьКалендаря%", Строка(ОбъектЗаписьКалендаря));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ДокументСсылка, ТекстСообщения);
				ЕстьОшибка = Истина;
				Прервать;
			КонецПопытки;
...
Процедура ПометитьНаУдалениеЗаписиКалендаряСотрудника(ДокументСсылка, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
			ШаблонСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Ошибка установки пометки на удаление записи календаря сотрудника %ЗаписьКалендаря%'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%ЗаписьКалендаря%", Строка(Выборка.Ссылка));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ДокументСсылка, ТекстСообщения);
...
Процедура КонтрольПревышенияЗагрузки(ПакетДанныхРесурсов, ТаблицаРесурсов, Объект)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРесурсов Цикл
		...
		Если ЗначениеЗагрузки>Мощность Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для %Ресурс% в период с %НачалоПериода% по %ОкончаниеПериода% есть превышение загрузки. %РасшифровкаЗагрузки%: %ТекущаяЗагруза%'");
			...
			Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%ТекущаяЗагруза%", Строка(ЗагрузкаЗаИнтервал));
