	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЧекЕГАИСВозврат") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
			...
			Если РеквизитыОснования.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя Тогда
				...
				СерийныйНомерККМ = УправлениеНебольшойФирмойСервер.СерийныйНомерККМ(РеквизитыОснования.ЧекККМКассаККМ);
...
Функция СтруктураРеквизитыРасчетаСтатусаОформления()
	...
	Реквизиты.Вставить("ТорговыйОбъект", Справочники.СтруктурныеЕдиницы.ПустаяСсылка());
...
Процедура РассчитатьСтатусОформленияДокументаЕГАИС(Источник, Отказ) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОприходованиеЗапасов")
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.СписаниеЗапасов") 
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") 
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") 
		ИЛИ (ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная")
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ВидОперации") = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику) Тогда
		...
		|	Таблица.Автор          КАК Ответственный,
		|	Таблица.СтруктурнаяЕдиница КАК ТорговыйОбъект,
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПеремещениеЗапасов")
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПеремещениеЗапасов")
		...
		|	Таблица.Номер            КАК Номер,
		|	Таблица.Автор            КАК Ответственный,
		|	Таблица.СтруктурнаяЕдиницаПолучатель КАК ТорговыйОбъект,
		|	Таблица.Организация      КАК Контрагент
		|ИЗ
		|	" + ДокументОснование.Метаданные().ПолноеИмя() + " КАК Таблица
		|ГДЕ
...
Процедура ПриОпределенииЗапросаТоварыДокументаОснования(МетаданныеДокументаОснования, МетаданныеДокументаЕГАИС, ТекстЗапроса, ДополнительныеПараметрыЗапроса) Экспорт
	...
	Если МетаданныеДокументаОснования = Метаданные.Документы.ОприходованиеЗапасов Тогда
		...
		|	И ОприходованиеЗапасовЗапасы.Номенклатура.АлкогольнаяПродукция";		
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.СписаниеЗапасов Тогда
		...
		|	И СписаниеЗапасовЗапасы.Номенклатура.АлкогольнаяПродукция";		
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ПересортицаЗапасов Тогда
		...
		|	И ПересортицаЗапасовЗапасы.Номенклатура.АлкогольнаяПродукция
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ПересортицаЗапасовЗапасы.Ссылка В (&МассивДокументов)
		|	И ПересортицаЗапасовЗапасы.НоменклатураОприходование.АлкогольнаяПродукция";
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ПеремещениеЗапасов Тогда
		...
		|	И ПеремещениеЗапасовЗапасы.Номенклатура.АлкогольнаяПродукция";
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.РасходнаяНакладная Тогда
		...
		|	И РасходнаяНакладнаяЗапасы.Номенклатура.АлкогольнаяПродукция";
...
Процедура ОбработатьОснованияЕГАИС(Параметры) Экспорт
	...
	|	ДокументПриходнаяНакладная.Ссылка КАК ПриходнаяНакладная
	|ИЗ
	...
	|		ПО ДокументПриходнаяНакладная.ДокументОснование = ДокументТТНВходящаяЕГАИС.Ссылка
	|			И (ДокументТТНВходящаяЕГАИС.ДокументОснование = НЕОПРЕДЕЛЕНО
	|				ИЛИ ДокументТТНВходящаяЕГАИС.ДокументОснование = ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяСсылка)
	|				ИЛИ ДокументТТНВходящаяЕГАИС.ДокументОснование = ЗНАЧЕНИЕ(Документ.ПеремещениеЗапасов.ПустаяСсылка))";
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", СтрокаРезультата.ПриходнаяНакладная);
			...
			ДокументОбъектУНФ = СтрокаРезультата.ПриходнаяНакладная.ПолучитьОбъект();
			Если ДокументОбъектУНФ = Неопределено Тогда
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ДокументОбъектЕГАИС.ДокументОснование) Тогда
				ДокументОбъектЕГАИС.ДокументОснование = СтрокаРезультата.ПриходнаяНакладная;
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъектЕГАИС);
			КонецЕсли;
			
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъектУНФ);
