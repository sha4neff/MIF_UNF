...
Процедура ВыполнитьПодключениеКассовыхАппратов(ИнформацияПоКассовымАппаратам, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Начало обмена перерегистрация из ноды.'", КодЯзыка));
	КонецЕсли;
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Завершение обмена перерегистрация из ноды.'", КодЯзыка));
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьПотоковыйОбменДаннымиССерверомШтрихМ(Настройки)
	...
	Соединение = HTTPСоединение(Настройки);
	Если Соединение.HTTPСоединение = Неопределено Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Установка соедиенения'", КодЯзыка),
	...
	Если НЕ ИмяРеквизитаОтвета = "fetch_documents_response" Тогда
		...
		Иначе
			СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Ошибка записи кассового аппарата: %1'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СокрЛП(ДополнительныеПараметры.РеквизитыКассы.Ссылка));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
