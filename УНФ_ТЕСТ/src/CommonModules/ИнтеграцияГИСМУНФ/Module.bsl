...
Процедура ОбработкаЗаполненияУведомленияОбИмпортеГИСМ(УведомлениеОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриходнаяНакладнаяСерийныеНомера.СерийныйНомер.Наименование КАК НомерКиЗ,
	...
	|ГДЕ
	|	ПриходнаяНакладнаяСерийныеНомера.Ссылка = &ДокументОснование";
...
Процедура ТекстЗапросаПоПроблемнымПоступлениям(ИмяНабораДанных, ОтборОрганизация, ОтборКонтрагент, ТекстЗапроса) Экспорт
	...
	Если ИмяНабораДанных = "КиЗ" Тогда
		...
		ТекстОтбораПоНоменклатуре        = "СерийныеНомераПриходныхДокументов.КиЗГИСМ";
	ИначеЕсли ИмяНабораДанных = "МаркированныеТовары" Тогда
		...
		ТекстОтбораПоНоменклатуре        = "СерийныеНомераПриходныхДокументов.ВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыМаркировки.МаркируемаяПродукция)";
	КонецЕсли;
	
	ТекстОтбораДокументыПоступления = "";
	ТекстОтбораДокументыГИСМ        = "";
	Если ЗначениеЗаполнено(ОтборОрганизация) Тогда
		ТекстОтбораДокументыПоступления = ТекстОтбораДокументыПоступления + " И СерийныеНомераПриходныхДокументов.Организация = &Организация ";
	...
	Если ЗначениеЗаполнено(ОтборКонтрагент) Тогда
		ТекстОтбораДокументыПоступления = ТекстОтбораДокументыПоступления + " И СерийныеНомераПриходныхДокументов.Контрагент = &Контрагент ";
		ТекстОтбораДокументыГИСМ        = ТекстОтбораДокументыГИСМ + " И ТабличнаяЧастьДокументаГИСМ.Ссылка.Контрагент = &Контрагент ";
	КонецЕсли;
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	СерийныеНомераПриходныхДокументов.Организация КАК Организация,
	|	СерийныеНомераПриходныхДокументов.Контрагент КАК Контрагент,
	|	СерийныеНомераПриходныхДокументов.НомерКиЗ КАК НомерКИЗ,
	|	СУММА(СерийныеНомераПриходныхДокументов.КоличествоВДокументахПоступления) КАК КоличествоВДокументахПоступления,
	|	СерийныеНомераПриходныхДокументов.Ссылка КАК Ссылка
	...
	|		РегистрСерийныеНомера.СерийныйНомер.НомерКиЗГИСМ КАК НомерКиЗ,
	|		1 КАК КоличествоВДокументахПоступления,
	|		РегистрСерийныеНомера.Номенклатура.КиЗГИСМ КАК КиЗГИСМ,
	|		РегистрСерийныеНомера.Номенклатура.ВидМаркировки КАК ВидМаркировки
	|	ИЗ
	|		РегистрНакопления.СерийныеНомера КАК РегистрСерийныеНомера
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная КАК ПриходнаяНакладная
	|			ПО РегистрСерийныеНомера.Регистратор = ПриходнаяНакладная.Ссылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		NULL,
	|		РегистрСерийныеНомера.СерийныйНомер.НомерКиЗГИСМ,
	|		1,
	|		РегистрСерийныеНомера.Номенклатура.КиЗГИСМ,
	|		РегистрСерийныеНомера.Номенклатура.ВидМаркировки
	|	ИЗ
	|		РегистрНакопления.СерийныеНомера КАК РегистрСерийныеНомера
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдер КАК ПриходныйОрдер
	|			ПО РегистрСерийныеНомера.Регистратор = ПриходныйОрдер.Ссылка) КАК СерийныеНомераПриходныхДокументов,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	СерийныеНомераПриходныхДокументов.Организация,
	|	СерийныеНомераПриходныхДокументов.Контрагент,
	|	СерийныеНомераПриходныхДокументов.НомерКиЗ,
	|	СерийныеНомераПриходныхДокументов.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура РассчитатьСтатусПоступленияПоДокументу(Источник, ДанныеЗаписи)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РегистрСерийныеНомера.Регистратор КАК ДокументПоступления,
	|	СерииНоменклатуры.НомерКиЗГИСМ КАК НомерКиЗ
	|ПОМЕСТИТЬ ВсеДокументыПоступленияПоКиЗ
	|ИЗ
	|	РегистрНакопления.СерийныеНомера КАК РегистрСерийныеНомера
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СерийныеНомера КАК СерииНоменклатуры
	|		ПО РегистрСерийныеНомера.СерийныйНомер = СерииНоменклатуры.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная КАК ПоступлениеТоваровУслуг
	|		ПО РегистрСерийныеНомера.Регистратор = ПоступлениеТоваровУслуг.Ссылка
	|			И (&Организация = ПоступлениеТоваровУслуг.Организация)
	|			И (&Контрагент = ПоступлениеТоваровУслуг.Контрагент)
	|ГДЕ
	|	СерииНоменклатуры.НомерКиЗГИСМ В
...
Процедура ИнициализироватьДанныеДокументаПеремаркировкиТоваров(ДокументСсылкаПеремаркировка,СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасы.Период КАК ДатаСобытия,
	|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Расход) КАК Операция,
	|	ТаблицаЗапасы.СписываемаяСерия КАК СерийныйНомер,
	|	&Организация КАК Организация,
	...
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СерийныеНомера КАК СерийныйНомерКИЗа
	...
	// Сформируем путую таблицу проводок.
	УправлениеНебольшойФирмойСервер.СформироватьТаблицуПроводок(ДокументСсылкаПеремаркировка, СтруктураДополнительныеСвойства);
