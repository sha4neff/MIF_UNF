...
Процедура РасчетДанныхПоФормуле(Знач ФормулаСтрокой, СтруктураОперандов, РасчетныеДанные, ОкруглятьВБольшуюСторону = Неопределено, ПравилоОкругления = Неопределено) Экспорт
	...
	Попытка
		...
		РасчитанаяСумма = ОбщегоНазначения.ВычислитьВБезопасномРежиме(ФормулаСтрокой);
		
		Если ЗначениеЗаполнено(РасчитанаяСумма)
			И ЗначениеЗаполнено(ОкруглятьВБольшуюСторону)
			И ЗначениеЗаполнено(ПравилоОкругления) Тогда
			
			РасчетныеДанные.Сумма = ОкруглитьЧислоПоПравилу(РасчитанаяСумма, ОкруглятьВБольшуюСторону, ПравилоОкругления);
			
		Иначе
			РасчетныеДанные.Сумма = РасчитанаяСумма;
		КонецЕсли;
