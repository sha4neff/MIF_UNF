...
Функция ИнициализироватьПараметрыКодаМаркировки(ЭлементДанных = Неопределено, ВидПродукции = Неопределено, ВыполнятьРазбор = Истина) Экспорт
	...
	Если ИсточникДанных["productGroup"] <> Неопределено И ЗначениеЗаполнено( ИсточникДанных["productGroup"]) Тогда
		 ПараметрыКодаМаркировки.ВидПродукции = ИнтерфейсИСМПСлужебный.ТоварнаяГруппа(ИсточникДанных["productGroup"]);
	КонецЕсли;
	...
	Если ИсточникДанных["countChildren"] <> Неопределено
		...
		Для Каждого СвойстваВложеннойУпаковки Из ИсточникДанных["childrenMap"] Цикл
			...
			ПараметрыКодаМаркировкиВложенный = ИнициализироватьПараметрыКодаМаркировки(СвойстваВложеннойУпаковки,
...
Функция СтатусКодаМаркировки(Знач ЗначениеПоиска) Экспорт
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") Тогда
		...
		ИначеЕсли ЗначениеПоиска = "FTS_RESPOND_WAITING" Тогда
			Возврат Перечисления.СтатусыКодовМаркировкиИСМП.ОжидаетПподтверждениеФТС;
	...
	Иначе
		...
		ИначеЕсли ЗначениеПоиска = Перечисления.СтатусыКодовМаркировкиИСМП.ОжидаетПподтверждениеФТС Тогда
...
Функция ВидОперации(Знач ЗначениеПоиска) Экспорт
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") Тогда
		...
		ИначеЕсли ЗначениеПоиска = "LP_SHIP_GOODS_CROSSBORDER" Тогда
			Возврат Перечисления.ВидыОперацийИСМП.ПриемкаТрансграничнаяТорговля;
...
//
Функция ВидУпаковки(Знач ЗначениеПоиска) Экспорт
