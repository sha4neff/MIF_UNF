...
Процедура ВыполнитьАвтоматическоеНачислениеИСписание()
	...
	|	ДОБАВИТЬКДАТЕ(Контрагенты.ДатаРождения, ГОД, РАЗНОСТЬДАТ(Контрагенты.ДатаРождения, НАЧАЛОПЕРИОДА(&ТекущийПериод, ДЕНЬ), ГОД) + 1) КАК ДеньРождения_СледущийГод
	...
	|			И (НачисленияБонусныхБаллов.Период МЕЖДУ ДОБАВИТЬКДАТЕ(&ТекущийПериод, ДЕНЬ, -ВТ_КартыИПрограммы.ДнейПередДнемРождения) И НАЧАЛОПЕРИОДА(&ТекущийПериод, ДЕНЬ))
	...
	|					КОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ТекущийПериод, ДЕНЬ), ВТ_ДниРождения.ДеньРождения_СледущийГод, ДЕНЬ) <= ВТ_КартыИПрограммы.ДнейПередДнемРождения
	|						ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ВТ_ДниРождения.ДеньРождения_СледущийГод, ДЕНЬ, ВТ_КартыИПрограммы.ДнейПослеДняРождения), ДЕНЬ)
	|				КОНЕЦ
	...
	|							ИЛИ РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ТекущийПериод, ДЕНЬ), ВТ_ДниРождения.ДеньРождения_СледущийГод, ДЕНЬ) <= ВТ_КартыИПрограммы.ДнейПередДнемРождения
...
Процедура РаспределитьОплатуБонусамиКакСкидку(Объект, ТаблицаОплат = Неопределено) Экспорт
	...
	Для Каждого СтрокаЗапаса Из Запасы Цикл
		...
		СуммаБонусовСтроки = Мин(СуммаБонусовСтроки, СуммаКРаспределению);
