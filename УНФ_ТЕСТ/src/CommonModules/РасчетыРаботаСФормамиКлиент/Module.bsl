...
&НаКлиенте
Функция ПоместитьТаблицуЗначенийВМассив(ТаблицаРеквизитФормы) Экспорт
	
	МассивСтрок = Новый Массив;
	СтруктураСтроки = Новый Структура;
	
	Для Каждого ТекущаяСтрока Из ТаблицаРеквизитФормы Цикл
		Для Каждого ТекущаяКолонка Из ТаблицаРеквизитФормы.Колонки Цикл
...
Функция ДополнитьПараметрыДляОткрытияФормыПомощника(ЭтоОплата, пФорма, СтруктураПараметров) Экспорт
	...
	Иначе
		...
		// В накладных в реквзиитах Курс и Кратность храниться информация для валюты расчетов по договору,
...
&НаКлиенте
Процедура РаспределитьСуммуПлатежаПослеРаспределения(пФорма, РезультатЗакрытия, ДопПараметры) Экспорт
	
	НужноЗаполнитьТаблицуПросмотраНаСервере = Истина;
	
	СтруктураВидыИзменяемыхДокументов = ЗаполнитьРасшировкуПлатежаПоДеревуРаспределения(пФорма, РезультатЗакрытия, ДопПараметры);
	
	Если НЕ ДопПараметры.ЭтоЗачетПредоплаты Тогда
		НастроитьЭлементыДляИзменяемыхДокументов(Ложь, пФорма, СтруктураВидыИзменяемыхДокументов);
	КонецЕсли;
...
Функция ЕстьТаблицаДокументовДляИзменения(Ссылка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику") 
		ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
&НаКлиенте
Функция ЗаполнитьРасшировкуПлатежаДеньгиПоДеревуРаспределения(пФорма, РезультатЗакрытия, ДопПараметры) Экспорт
	
	пФорма.Модифицированность = Истина;
	пФорма.Объект.РасшифровкаПлатежа.Очистить();
	пФорма.ТаблицаДокументовДляИзменения.Очистить();
	...
	ЗаполнитьРасшировкуПлатежаДеньгиПоДеревуРаспределенияРекурсивно(пФорма, ЭлементыДерева, СтруктураВидыИзменяемыхДокументов, СоответствиеСобираемыеДанные, РезультатЗакрытия);
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	// Распределим остаток.
	ОстатокПослеРаспределения = (пФорма.Объект.СуммаДокумента - пФорма.Объект.РасшифровкаПлатежа.Итог("СуммаПлатежа"));
	Если ОстатокПослеРаспределения > 0 Тогда
		...
		Для Каждого ТекущийЭлементДерева Из ЭлементыДерева Цикл
			Если ТекущийЭлементДерева.ПолеГруппировки = "Договор"
				...
				РасшифровкаПлатежаСуммаПлатежаПриИзмененииЗавершениеНаКлиенте(
					СтрокаРасшифровки,
					пФорма.Объект.Дата,
					пФорма.Объект.ВалютаДенежныхСредств,
					пФорма.Курс,
					пФорма.Кратность,
		...
		Если ОстатокПослеРаспределения > 0 Тогда
			ЗаполнитьСтрокуРасшифровкиДеньгиПоНераспределенномуОстатку(пФорма,
...
&НаКлиенте
Процедура ТаблицаДляПросмотраОбработкаРасшифровки(пФорма, Элемент, Расшифровка, СтандартнаяОбработка, СтруктураПараметровРасчетов, ЭтоЗачетПредоплаты = Ложь) Экспорт
	...
	Если СтрНайти(Расшифровка, "ПереходитьВРежимРедактирования") <> 0 Тогда
		...
		КонецЕсли;
		пФорма.ВариантЗаполненияРасшифровки = СтруктураПараметровРасчетов.ВариантЗаполненияРасшифровкиВручную;
		Попытка
			Если ЭтоЗачетПредоплаты Тогда
				пФорма.Элементы.Предоплата.ТекущаяСтрока = пФорма.Объект.Предоплата[ИндексСтроки].ПолучитьИдентификатор();
			Иначе
			...
			Попытка
				Если ЭтоЗачетПредоплаты Тогда
					ТекущаяКолонка = пФорма.Элементы["Предоплата"+ИмяПоля];
				Иначе
					ТекущаяКолонка = пФорма.Элементы["РасшифровкаПлатежа"+ИмяПоля];
				КонецЕсли;
			Исключение
			...
			Если ТекущаяКолонка <> Неопределено Тогда
				Если ЭтоЗачетПредоплаты Тогда
					пФорма.Элементы.Предоплата.ТекущийЭлемент = ТекущаяКолонка;
					пФорма.Элементы.Предоплата.ИзменитьСтроку();
				Иначе
		...
		Если Элемент.ТекущаяОбласть.Имя = "СуммаПлатежа" Тогда
			Если ЭтоЗачетПредоплаты Тогда
				пФорма.Элементы.Предоплата.ТекущийЭлемент = ?(СтруктураПараметровРасчетов.УчетВалютныхОпераций,
					пФорма.Элементы.ПредоплатаСуммаПлатежа,
					пФорма.Элементы.ПредоплатаСуммаРасчетов
				);
			Иначе
...
Процедура ОбработатьИзменениеДоговораКонтрагента(пФорма, СтрокаТабличнойЧасти = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) Тогда
		...
		КонецЕсли;
		СтрокаТабличнойЧасти.СтавкаНДС = РасчетыРаботаСФормамиВызовСервера.СтавкаНДСПоОрганизациияНалогообложениеНДСИСпособуЗаполнения(пФорма.Объект.Организация, пФорма.Объект.Дата, СтруктураДанные.СпособЗаполненияСтавкиНДС);
...
Процедура ПриИзмененииКурсаВСтрокеПредоплаты(Форма) Экспорт

	СтрокаТабличнойЧасти = Форма.Элементы.Предоплата.ТекущиеДанные;
	Объект = Форма.Объект;
...
&НаКлиенте
Процедура ЗаполнитьРасшировкуПлатежаДеньгиПоДеревуРаспределенияРекурсивно(пФорма, ЭлементыДерева, СтруктураВидыИзменяемыхДокументов, СоответствиеСобираемыеДанные, РезультатЗакрытия)
	
	Для Каждого ТекущийЭлементДерева Из ЭлементыДерева Цикл
		ЗаполнитьСтрокуРасшифровкиДеньгиПоЭлементуДереваРаспределения(пФорма, ТекущийЭлементДерева, СтруктураВидыИзменяемыхДокументов, СоответствиеСобираемыеДанные, РезультатЗакрытия);
		ЗаполнитьРасшировкуПлатежаДеньгиПоДеревуРаспределенияРекурсивно(пФорма, ТекущийЭлементДерева.ПолучитьЭлементы(), СтруктураВидыИзменяемыхДокументов, СоответствиеСобираемыеДанные, РезультатЗакрытия);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Функция ЗаполнитьРасшировкуПлатежаПоДеревуРаспределения(пФорма, РезультатЗакрытия, ДопПараметры)
	
	пФорма.Объект.Предоплата.Очистить();
	пФорма.ТаблицаДокументовДляИзменения.Очистить();
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	СтруктураВидыИзменяемыхДокументов = Новый Структура("ЕстьСуществующийЗачет, ЕстьДокументыПозжеТекущего", Ложь, Ложь);
	ЗаполнитьРасшировкуПлатежаПоДеревуРаспределенияРекурсивно(пФорма, ЭлементыДерева, СтруктураВидыИзменяемыхДокументов, ДопПараметры);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
&НаКлиенте
Процедура ЗаполнитьРасшировкуПлатежаПоДеревуРаспределенияРекурсивно(ЭтаФорма, ЭлементыДерева, СтруктураВидыИзменяемыхДокументов, ДопПараметры)
	...
	Для Каждого ТекущийЭлементДерева Из ЭлементыДерева Цикл
		...
		ЗаполнитьРасшировкуПлатежаПоДеревуРаспределенияРекурсивно(ЭтаФорма, ТекущийЭлементДерева.ПолучитьЭлементы(), СтруктураВидыИзменяемыхДокументов, ДопПараметры);
	КонецЦикла;
...
Процедура ЗаполнитьСтрокуРасшифровкиДеньгиПоЭлементуДереваРаспределения(пФорма, СтрокаРаспределения, СтруктураВидыИзменяемыхДокументов, СоответствиеСобираемыеДанные, РезультатЗакрытия)
	...
	Если НЕ СтрокаРаспределения.Выбран
		...
		// Теперь добавим информацию о зачёте предоплатуы в других документах.
...
Процедура ЗаполнитьСтрокуРасшифровкиДеньгиПоНераспределенномуОстатку(пФорма, СуммаНераспределенногоОстатка, РезультатЗакрытия, СоответствиеСобираемыеДанные)
	...
	Если ЗначениеЗаполнено(РезультатЗакрытия.ДоговорПоУмолчанию) Тогда
		...
		ТекущаяВалютаРасчетов = УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(РезультатЗакрытия.ДоговорПоУмолчанию, "ВалютаРасчетов");
...
Процедура ЗаполнитьСтрокуРасшифровкиПоЭлементуДереваРаспределения(ЭтаФорма, СтрокаРаспределения, СтруктураВидыИзменяемыхДокументов, ДопПараметры)
	...
	ИначеЕсли СтрокаРаспределения.Выбран
		И (СтрокаРаспределения.ДокументПозжеТекущего ИЛИ СтрокаРаспределения.ЭтоСуществующийЗачет) Тогда
		// Сначала добавим информацию о предоплате.
		Если НЕ СтрокаРаспределения.ЭтоСуществующийЗачет ИЛИ ИзменилисьИсходныеЗначения(СтрокаРаспределения) Тогда
			// Теперь добавим информацию о зачёте предоплатуы в других документах.
			СтрокаИзменения = ЭтаФорма.ТаблицаДокументовДляИзменения.Добавить();
