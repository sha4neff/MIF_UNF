...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	Если ТипЗнч(Получатель) <> Тип("ПланОбменаСсылка.ОбменУправлениеНебольшойФирмойБухгалтерия20")
		И ТипЗнч(Получатель) <> Тип("ПланОбменаСсылка.ОбменУправлениеНебольшойФирмойБухгалтерия30")
		И ТипЗнч(Получатель) <> Тип("ПланОбменаСсылка.СинхронизацияДанныхЧерезУниверсальныйФормат") Тогда
		Возврат;
	КонецЕсли;
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Получатель, "ВариантНастройки,ИспользоватьОтборПоВидамДокументов, РучнойОбмен, ВидыДокументов");
	...
	Если ЗначенияРеквизитов.ВариантНастройки = "КабинетКлиента" Тогда
		МетаданныеРегистрируютсяВручную.Добавить(Метаданные.Справочники.ХарактеристикиНоменклатуры);
		МетаданныеРегистрируютсяВручную.Добавить(Метаданные.РегистрыСведений.ЦеныНоменклатуры);
		МетаданныеРегистрируютсяВручную.Добавить(Метаданные.РегистрыСведений.ОстаткиТоваров);
	КонецЕсли;
