	...
	Для Каждого ИмяТабличнойЧасти Из МассивИменТабличныхЧастей Цикл
		...
		Для Каждого СтрокаЗапасы Из Объект[ИмяТабличнойЧасти] Цикл
			...
			Если ПроверятьЗаполнениеХарактеристикВТабЧасти
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1%", Строка(Номенклатура));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
...
Процедура ЗаполнитьЗначенияРезервовИОстатокЗаказНаПроизводство(Объект, СтруктураДанные = Неопределено, НомерВариантаКП = 0, НоменклатураОтбора = Неопределено, РезервированиеЗапасов = Ложь) Экспорт
	...
	РасчитатьКоличествоПоСтрокамСогласноКоэфициентам(ТаблицаНоменклатуры);
	...
	СтрукрутраПараметровОтбора = Новый Структура("ССкладом, БезСклада, БезСкладаСЗаказом", ПараметрыОтбораДляПоискаОстатков, ПараметрыОтбораБезСклада, ПараметрыОтбораБезСкладаБезПартии);
	ЗаполнитьНачальныеДанныеДляОбработкиТабличнойЧастиЗаказНаПроизводство(Объект, МассивЗаказов, СтрокиОбходаЗапасов, ТаблицаНоменклатуры, СтрукрутраПараметровОтбора);
...
//
Процедура ЗаполнитьНачальныеДанныеДляОбработкиТабличнойЧастиЗаказНаПроизводство(Объект, МассивЗаказов, СтрокиОбходаЗапасов, ТаблицаНоменклатуры, СтрукрутраПараметровОтбора)
	...
	ПараметрыОтбораБезСклада = СтрукрутраПараметровОтбора.БезСклада;
	ПараметрыОтбора = СтрукрутраПараметровОтбора.ССкладом;
	ПараметрыОтбораБезСкладаБезПартииСЗаказом = СтрукрутраПараметровОтбора.БезСкладаСЗаказом;
	...
	// Дозаполним по отгрузкам без резервов
	Для Каждого СтрокаТабличнойЧасти Из СтрокиОбходаЗапасов Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) 
			Или ТипЗнч(СтрокаТабличнойЧасти.Номенклатура) = Тип("Строка") Тогда Продолжить КонецЕсли;
			
		НеОтгруженоПерваяИтерация = СтрокаТабличнойЧасти.НеОтгружено;
...
Процедура ЗаполнитьЗначенияРезервовИОстатокНоменклатурыВТабличнойЧасти(Объект, СтруктураДанные = Неопределено, НомерВариантаКП = 0, НоменклатураОтбора = Неопределено, РезервированиеЗапасов = Ложь, ИмяТЧ = "Запасы") Экспорт
	...
	СтрукрутраПараметровОтбора = Новый Структура("ССкладом, БезСклада", ПараметрыОтбора, ПараметрыОтбораБезСклада);
	ЗаполнитьНачальныеДанныеДляОбработкиТабличнойЧасти(Объект, СтрокиОбходаЗапасов, ТаблицаНоменклатуры, СтрукрутраПараметровОтбора);
...
//
Процедура ЗаполнитьНачальныеДанныеДляОбработкиТабличнойЧасти(Объект, СтрокиОбходаЗапасов, ТаблицаНоменклатуры, СтрукрутраПараметровОтбора)
...
Процедура ЗаполнитьЗначенияОстатковНоменклатурыВТабличнойЧастиЧекККМ_РМК(Объект, СтруктураДанные = Неопределено,  НоменклатураОтбора = Неопределено) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из СтрокиОбходаЗапасов Цикл
		...
		ВРезерве = РезервНаСкладахСКоэффициетом + СтрокаОстатка.СписаноРезерваНакладной;
...
Процедура ЗаполнитьКоличествоПоОстаткамИРезервамРасходнаяНакладная(Объект, ПараметрыОтбора = Неопределено, СтруктураОтбораНоменклатуры = Неопределено, ИмяТЧ = "Запасы") Экспорт
	...
	Для Каждого СтрокаТабЗапасы Из СвернутаяТаблицаЗапасы Цикл
		...
		Если КоличествоДляРаспределения > 0 И МассивСтрокОстаткиОбщие.Количество() Тогда
			...
			ПоследняяСтрока = ПоследнняяСтрокаМассива;
...
//
Функция СписокДокументовПоЗаказу(Объект, ТипОперации, СтруткураПараметров, ИмяТЧ = "Запасы", СостояниеВыполнения = Неопределено, ТипДокумента = Неопределено) Экспорт
	...
	Если СтруткураПараметров.Свойство("ВидыЗаполненияПоЗаказНаряду") Тогда
		КакВЗаказеПокупателя = СтруткураПараметров.ВидыЗаполненияПоЗаказНаряду.КакВЗаказеПокупателя;
		КакВРасходнойНакладной = СтруткураПараметров.ВидыЗаполненияПоЗаказНаряду.КакВРасходнойНакладной;
	Иначе
...
Процедура ЗаполнитьПризнакТипаЗапасНоменклатуры(Объект, ИмяТЧ = "Запасы") Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТЧ] Цикл
		СтрокаТабличнойЧасти.ТипНоменклатурыЗапас = ?(СтрокаТабличнойЧасти.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас, Истина, Ложь);
	КонецЦикла;
...
// Устарела. См. ОбновитьНастройкиФормыВыбораНоменклатурыПриИзмененииРеквизита
Процедура ОбновитьНастрйокиФормыВыбораНоменклатурыПриИзмененииРеквизита(Форма, ВидДокумента, ИмяТЧ,
	НастройкиФормыВыбораНоменклатуры, ДополнениеНаименованияНоменклатуры = "") Экспорт

	ОбновитьНастройкиФормыВыбораНоменклатурыПриИзмененииРеквизита(Форма, ВидДокумента, ИмяТЧ,
		НастройкиФормыВыбораНоменклатуры, ДополнениеНаименованияНоменклатуры);
...
//
Процедура ПроверитьПереопределитьПоложениеСклада(Объект, ИмяТЧ = "Запасы", ИмяКолокниСклад = "СтруктурнаяЕдиница", ИмяРеквизитаСклад = "СтруктурнаяЕдиница") Экспорт
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ИмяКолокниСклад = "СтруктурнаяЕдиницаРезерв";
	...
	Если РазрешитьСкладыВТабличныхЧастях Тогда
		...
		Если Не УчетПоЯчейкам Тогда
			...
			Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТЧ] Цикл
				...
				Если ЗначениеСклада = Неопределено Тогда
					...
					ЗначениеСклада = СтрокаТабличнойЧасти[ИмяКолокниСклад];
				...
				Если ЗначениеЗаполнено(СтрокаТабличнойЧасти[ИмяКолокниСклад]) И НЕ СтрокаТабличнойЧасти[ИмяКолокниСклад] = ЗначениеСклада Тогда
		...
		Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТЧ] Цикл
			...
			Если  ЗначениеЗаполнено(СтрокаТабличнойЧасти[ИмяКолокниСклад]) И НЕ СтрокаТабличнойЧасти[ИмяКолокниСклад] = ЗначениеСклада Или НЕ СтрокаТабличнойЧасти.Ячейка = Объект.Ячейка Тогда
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект[ИмяТЧ][0][ИмяКолокниСклад]) Тогда
			Объект[ИмяРеквизитаСклад] = Объект[ИмяТЧ][0][ИмяКолокниСклад];
...
// Преобразаует текст запроса к регистру штрихкоды если миграция штрихкодов не завершена
//
Процедура ПреобразоватьТекстЗапросаРегистрШтрихкодыНоменклатуры(ТекстЗапроса) Экспорт
	
	Если Константы.МиграцияШтрихкодовВыполнена.Получить() Тогда
		Возврат
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрСведений.ШтрихкодыНоменклатуры", "РегистрСведений.УдалитьШтрихкодыНоменклатуры");
...
Функция ТекстЗапросаОстаткиИРезервыВразрезеЗаказов()
	...
	|	ВтНоменклатураБезСклада.Партия КАК Партия,
	|	Запасы.ЗаказПокупателя КАК ЗаказПокупателя
	|ПОМЕСТИТЬ РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам
	|ИЗ
	|	ВтНоменклатураБезСклада КАК ВтНоменклатураБезСклада
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы КАК Запасы
	|		ПО ВтНоменклатураБезСклада.НоменклатураЗапасов = Запасы.Номенклатура
	...
	|	ЕСТЬNULL(ОстаткиРезервыОбщие.РезервНаСкладах, 0) - ЕСТЬNULL(РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам.ЗарезервированоЗаказом, 0) КАК РезервНаСкладах,
	|	ЕСТЬNULL(РезервСделанныйЗаказом.ЗарезервированоЗаказом, 0) КАК ЗарезервированоЗаказом,
	|	ОстаткиВРазрезеСкладов.РезервТекСклад КАК РезервТекСкладОбщий,
	|	ЕСТЬNULL(ОтгрузкаПоРезервам.ОтгрузкаРезерва, 0) КАК ОтгрузкаРезерва,
	|	ЕСТЬNULL(РезервСделанныйЗаказомПоВсемСкладам.ЗарезервированоЗаказом, 0) КАК ЗарезервированоЗаказомВсеСклады,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам КАК РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам
	|		ПО ВтНоменклатура.НоменклатураЗапасов = РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам.НоменклатураЗапасов
	|			И ВтНоменклатура.ЗаказПокупателя = РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам.ЗаказПокупателя
	|			И ВтНоменклатура.Характеристика = РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам.Характеристика
	|			И ВтНоменклатура.Партия = РезервСделанныйЗаказомВРазерезеЗаказовПоСкладам.Партия";
...
Процедура РасчитатьКоличествоПоСтрокамСогласноКоэфициентам(ТаблицаНоменклатуры)
