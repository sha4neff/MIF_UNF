...
Функция ПолучитьСуммуПлатежаСтроки(ДокументОбъект, ТекущаяСтрока, ЭтоИзменениеДокумента)
	...
	Если НЕ ЭтоИзменениеДокумента И ТекущаяСтрока.ЭтоЗачет Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
			...
			ТекущаяСуммаПлатежа = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(
...
Функция ПолучитьПредставлениеВалюты(Валюта)
	...
	СтруктураРеквизитовВалюты = УправлениеНебольшойФирмойСервер.ЗначенияРеквизитовОбъекта(Валюта, МассивРеквизитовВалюты);
...
Процедура РаспределитьДанныеИзТблицыСуществующихДокументов(Реквизит4Отбора, Значение4Отбора,
	Документ,
			...
			Для Каждого СтрокаСИзменениями Из ТаблицаДокументовДляИзменения Цикл
				...
				Если СтрокаСИзменениями.ВариантИзмененияДанных = Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Изменение Тогда
					...
					Если НужноРаспределить > 0 И НайденныеСтроки.Количество() > 0 Тогда
						НайденнаяСтрока = НайденныеСтроки[НайденныеСтроки.Количество()-1]; // Можно было и 1-ую взять, это особой роли не играет, т.к. это должно вызывать ошибку,
...
Функция ДокументЯвляетсяЗаказом(пДокумент) Экспорт
	...
	Возврат (ТипЗнч(пДокумент) = Тип("ДокументСсылка.ЗаказПокупателя") И ТипЗнч(УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(пДокумент, "ВидЗаказа")) = Тип("СправочникСсылка.ВидыЗаказовПокупателей"))
...
Функция ПолучитьТабличныйДокументОплатаПоДокументуОплаты(Документ, ФормироватьПоДвижениям, Модифицированность, ТаблицаДокументовДляИзменения = Неопределено,
	...
	РаспределитьДанныеИзТблицыСуществующихДокументов(Реквизит4Отбора, Значение4Отбора,
		Документ,
		ТаблицаДокументовДляИзменения, ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа
	);
	...
	Если ТаблицаДокументовДляИзменения <> Неопределено Тогда
		Для Каждого ТекущаяСтрока Из ТаблицаДокументовДляИзменения Цикл
			Если ТекущаяСтрока.ВариантИзмененияДанных <> Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Добавление Тогда // См. вызов функции РаспределитьДанныеИзТблицыСуществующихДокументов
				Продолжить;
			КонецЕсли;
...
//	ТабличныйДокумент - табличный документ с реквизитами расшифровки платежа
//
Функция ПолучитьТабличныйДокументОплатаПоДокументуОтгрукзи(Документ, пФормироватьПоДвижениям, Модифицированность, ТаблицаДокументовДляИзменения = Неопределено,
	ВыводитьДанныеИзШапкиДокумента = Ложь) Экспорт
	...
	// Получим данные для вывода расшифровки платежа
	ПредставлениеРасшифровкиПлатежа = Объект.Предоплата.Выгрузить();
	ПредставлениеРасшифровкиПлатежа.Очистить();
	...
	ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа = РасчетыРаботаСФормамиВызовСервера.ПолучитьДанныеОбОплатеПоДокументуОтгрузки(Объект.Ссылка, ФормироватьПоДвижениям);
	Если Ложь Тогда ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа = Новый ТаблицаЗначений КонецЕсли;
	
	ПредставлениеРасшифровкиПлатежа.Очистить();
	Если НЕ ФормироватьПоДвижениям Тогда
		Для Каждого ТекущаяСтрока Из Объект.Предоплата Цикл
			НоваяСтрокаПредставления = ПредставлениеРасшифровкиПлатежа.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПредставления, ТекущаяСтрока);
	...
	Если НЕ ФормироватьПоДвижениям И НЕ Документ.Ссылка.Пустая() Тогда
		Для Каждого ТекущаяСтрока Из ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа Цикл
			...
			// В этом цикле обарабатываем только оплату другими документами.
			Если ТекущаяСтрока.ПризнакАванса Тогда
				Продолжить;
			КонецЕсли;
			Оплачено = Оплачено + ТекущаяСтрока.СуммаРасчетов;
	...
	Для Каждого ТекущаяСтрока Из ПредставлениеРасшифровкиПлатежа Цикл
		...
		Если НЕ ВыводилиЗаголовок Тогда
			ВыводилиЗаголовок = Истина;
			//ТабличныйДокументКарточка.Вывести(ОбластьГруппаВерхнегоУровня); //, 1,, Истина);
			
			Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
				ВывестиШапку(Ложь, Объект.КонтрагентПолучатель, ТабличныйДокументКарточка, СтруктураОбластейШапки);
			Иначе
				ВывестиШапку(Ложь, Объект.Контрагент, ТабличныйДокументКарточка, СтруктураОбластейШапки);
			КонецЕсли;
		...
		Для Каждого ТекущаяСтрока Из ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа Цикл
			...
			Если НЕ ВыводилиЗаголовок Тогда
				...
				Если НЕ ВыводилиШапку Тогда
					...
					Объект.КонтрагентПолучатель,
					Объект.Контрагент), ТабличныйДокументКарточка, СтруктураОбластейШапки);
