...
// Процедура выполняет обновление текстовых дайджестов процессов
// Параметры:
//		Нет
Процедура ОбновитьДайджестыПроцессов() Экспорт
	...
	Запрос=Новый Запрос("ВЫБРАТЬ
	                    ...
	                    |ГДЕ
	                    |	КП_СсылкиПроцессов.БизнесПроцесс.ПометкаУдаления = ЛОЖЬ
	                    |ИТОГИ ПО
	                    |	БизнесПроцесс");
...
Функция ПолучитьДанныеПечати(БизнесПроцесс, ФайлСсылка) Экспорт
	...
	СтруктураИспользуемыхОбластейМакета=ПолучитьОписаниеОбластейМакетаБизнесПроцесса(ФайлСсылка);
...
Функция ПолучитьДанныеОбъекта(СсылкаНаОбъект, ФайлСсылка)
	...
	Запрос=Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
						 ...
						 |	КП_ПараметрыШаблоновФайлов.ИсточникДанных.ТочкаТаблицыБизнесПроцесса КАК ТочкаТаблицыБизнесПроцесса,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипДанных=Перечисления.КП_ТипыДанныхТегов.Функция 
			...
			ИЛИ ТипДанных=Перечисления.КП_ТипыДанныхТегов.ПараметрБизнесПроцесса 
		...
		ИначеЕсли ТипДанных=Перечисления.КП_ТипыДанныхТегов.ТаблицаБизнесПроцесса Тогда
			...
			Если Выборка.НомерСтрокиТабличнойЧасти=0 Тогда
				...
				ДанныеИсточника=КП_ОбщееСервер.ПолучитьДанныеИсточника(Выборка.ИсточникДанных, СсылкаНаОбъект, ИмяИсточника, ДанныеОбъекта);	
...
&НаСервере
Функция ПолучитьОписаниеОбластейМакетаБизнесПроцесса(ФайлСсылка)
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	СтандартнаяОбработка=Ложь;
	Представление=Данные.Наименование+" № "+СокрЛП(Данные.Номер)+НСтр("ru=' от ';en=' ';")+Формат(Данные.Дата, "ДФ='dd.MM.yyyy HH:mm'");
