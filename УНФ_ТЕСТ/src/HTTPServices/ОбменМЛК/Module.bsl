...
Функция VerifyEmailPost(Запрос)
	...
	КодПодтверждения = ПолучитьСгенирированныйКод();
...
Функция ПолучитьМенееЗагруженныйРесурс(ДлительностьУслуг, ДатаЗаписи, ТекущаяДатаСеанса)
	...
	|ПОМЕСТИТЬ втСвободныеДатаИВермя
	|ИЗ
	|	втУчетЗанятыхИнтервалов КАК втУчетЗанятыхИнтервалов
	|ГДЕ
	|	втУчетЗанятыхИнтервалов.ОсновнаяМощностьРесурса > втУчетЗанятыхИнтервалов.СуммаМощностиИнтервала
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втСвободныеДатаИВермя.Ресурс КАК Ресурс,
	|	втСвободныеДатаИВермя.ВремяНачалаИнтервала КАК ВремяНачалаИнтервала
	|ПОМЕСТИТЬ втПотенциальныеРесурсы
	|ИЗ
	|	втСвободныеДатаИВермя КАК втСвободныеДатаИВермя
	|ГДЕ
	|	втСвободныеДатаИВермя.ВремяНачалаИнтервала = &ДатаЗаписи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПолучитьИсториюЗаписей(ИсключающаяДатаС, ИсключающаяДатаПо, Контрагент)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен Тогда
...
Функция ПолучитьСгенирированныйКод()
...
Функция СоздатьЗаказ(СтруктураЗапроса)
	...
	Для каждого Строка Из СтруктураЗапроса.Корзина Цикл
		...
		Если Номенклатура.ЭтоНабор Тогда
			СтруктураДанные = Новый Структура;
			СтруктураДанные.Вставить("Организация", УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(НовыйОбъект.Организация));
