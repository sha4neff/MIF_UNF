...
Функция СписокСвободныхSKUВДиапазоне(НижняяГраница, ВерхняяГраница, РазмерСписка)
	...
	Если КоличествоЗанятыхSKU = 0 Тогда
		...
		НачалоДиапозонаПоиска = НижняяГраница;
		КонецДиапозонаПоиска  = НачалоДиапозонаПоиска + РазмерСписка - 1;
	...
	Иначе
		...
		Если КоличествоПробельныхSKU = 0 Тогда // пробелов нет
			...
			НачалоДиапозонаПоиска = МаксимальныйЗанятыйSKU + 1;
			КонецДиапозонаПоиска  = МаксимальныйЗанятыйSKU + РазмерСписка;
		...
		Иначе
			...
			Если КоличествоПробельныхSKU > РазмерСписка Тогда
				...
				КонецДиапозонаПоиска = МаксимальныйЗанятыйSKU;
			Иначе
				
				КонецДиапозонаПоиска = МаксимальныйЗанятыйSKU + РазмерСписка - КоличествоПробельныхSKU;
			КонецЕсли;
	...
	КонецДиапозонаПоиска = ?(КонецДиапозонаПоиска > ВерхняяГраница, ВерхняяГраница, КонецДиапозонаПоиска);
	
	МассивДоступныхSKU = СоздатьМассивДоступныхSKU(НачалоДиапозонаПоиска, КонецДиапозонаПоиска, РазмерСписка);
	
	Возврат МассивДоступныхSKU;
	
КонецФункции

Функция СоздатьМассивДоступныхSKU(НачалоДиапозона, КонецДиапозона, РазмерСписка)
	
	ТаблицаSKU = Новый ТаблицаЗначений;
	ТаблицаSKU.Колонки.Добавить("SKU", Новый ОписаниеТипов("Число"));
	
	КодSKU = НачалоДиапозона;
	
	Пока НЕ КодSKU > КонецДиапозона Цикл
