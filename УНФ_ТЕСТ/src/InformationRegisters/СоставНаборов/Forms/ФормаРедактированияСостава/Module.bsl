...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НоменклатураНабора, "ТипНоменклатуры, СпособРасчетаЦеныНабора, ИспользоватьХарактеристики");
	...
	Если ТипНоменклатуры=Перечисления.ТипыНоменклатуры.Запас Тогда
		...
		Элементы.ЗаголовокСостав.Заголовок = СтрЗаменить(Элементы.ЗаголовокСостав.Заголовок, ":", НСтр("ru = ' (может содержать запасы и услуги):'"));
	ИначеЕсли ТипНоменклатуры=Перечисления.ТипыНоменклатуры.Работа Тогда
		...
		МассивТипов.Добавить(Перечисления.ТипыНоменклатуры.Услуга);
		Элементы.ЗаголовокСостав.Заголовок = СтрЗаменить(Элементы.ЗаголовокСостав.Заголовок, ":", НСтр("ru = ' (может содержать работы и услуги):'"));
	Иначе
		Отказ = Истина;
		Возврат;
	КонецЕсли; 
	...
	Если НЕ СохранятьИзменения И НЕ РежимВыбора Тогда
		...
		Для каждого ИмяЭлемента Из ИзменяемыеЭлементы Цикл
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяЭлемента, "ПредупреждениеПриРедактировании", ТекстПредупрежденияИзмененияСостава());
		КонецЦикла; 
	КонецЕсли;
	...
	Если СохранятьИзменения Тогда
		КлючСохраненияПоложенияОкна = "РегистрСведений.СоставНаборов.Форма.ФормаРедактированияСостава/РежимСохраненияИзменений";
	...
	//Характеристики
	Если НЕ ЭтаФорма.СохранятьИзменения
		Тогда
...
Процедура СоставНабораПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ СохранятьИзменения И НЕ РежимВыбора И Количество>1 Тогда
...
Процедура СоставНабораПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		СтрокаТабличнойЧасти = Элемент.ТекущиеДанные;
		СтрокаТабличнойЧасти.ХарактеристикаНабора = ?(СохранятьИзменения, ТекущаяХарактеристика, НоваяХарактеристикаНабора);
		СтрокаТабличнойЧасти.Отображать = Истина;
		СтрокаТабличнойЧасти.Общий = (СохранятьИзменения И ТекущаяХарактеристика=Неопределено);
	КонецЕсли; 
...
Процедура СоставНабораПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ ОтменаРедактирования И СохранятьИзменения И ТекущаяХарактеристика=Неопределено И НоваяСтрока Тогда
	...
	Если НЕ ОтменаРедактирования И НЕ СоставИзменен И НЕ СохранятьИзменения И НЕ РежимВыбора Тогда
...
Процедура СоставНабораПередУдалением(Элемент, Отказ)
	...
	Если СохранятьИзменения И ЗначениеЗаполнено(ТекущаяХарактеристика) Тогда
...
Процедура ПриИзмененииНоменклатурыСостава(Идентификатор = Неопределено)
	...
	Если СтруктураДанные.ТипНоменклатуры<>ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") ИЛИ СтруктураДанные.ФиксированнаяСтоимость Тогда
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Отказ = Ложь;
	Если НЕ СохранятьИзменения И НЕ РежимВыбора И СоставНабора.Количество()=0 Тогда
	...
	Для каждого СтрокаТабличнойЧасти Из СоставНабора Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Количество) Тогда
			...
			Отказ);
		КонецЕсли; 
		Если Элементы.СоставНабораЕдиницаИзмерения.Видимость И НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
...
Процедура УправлениеФормой(Форма)
	...
	КонецЕсли;
	ВидимостьХарактеристик = ((Форма.СохранятьИзменения ИЛИ Форма.РежимВыбора) И Форма.ИспользоватьХарактеристики);
	ОткрытаИзДокумента = (НЕ Форма.СохранятьИзменения И НЕ Форма.РежимВыбора);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, 
		"ГруппаКопирование",				
		"Видимость", 
		ЗначениеЗаполнено(Форма.НоменклатураНабора) И Форма.СохранятьИзменения); 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, 
	...
	Если Форма.РежимВыбора Тогда
		...
		КонецЕсли;
	КонецЕсли;
	// Конец Режим_ВыборХарактеристики
	
	// Режим_РедактированиеСостава
	Если НЕ Форма.СохранятьИзменения И НЕ Форма.РежимВыбора Тогда
		ИзменяемыеЭлементы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("СоставНабораНоменклатура,СоставНабораХарактеристика,СоставНабораЕдиницаИзмерения");
		Для каждого ИмяЭлемента Из ИзменяемыеЭлементы Цикл
	...
	КонецЕсли;
	// Конец Режим_РедактированиеСостава
...
Процедура ЗаполнитьСоставНабора()
	...
	Пока ВыборкаПорядок.Следующий() Цикл
		...
		Пока ВыборкаХарактеристики.Следующий() Цикл
			...
			КонецЕсли;
			Если НЕ РежимВыбора ИЛИ ВыборкаХарактеристики.ХарактеристикаНабора<>Неопределено Тогда
				Характеристики.Добавить(ВыборкаХарактеристики.ХарактеристикаНабора, ЗаголовокХарактеристики, НЕ ЗначениеЗаполнено(ВыборкаХарактеристики.ХарактеристикаНабора));
			КонецЕсли; 
...
Процедура СвернутьДубли()
	...
	КоличествоСтрок = СоставНабора.Количество();
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураДанные.Номенклатура, "ЕдиницаИзмерения, ИспользоватьСерийныеНомера, ИспользоватьХарактеристики, ИспользоватьПартии, ФиксированнаяСтоимость, ТипНоменклатуры");
...
Процедура СохранитьИзменения()
	...
	Если СохранятьИзменения Тогда
		...
		Оповестить(НаборыКлиент.ИмяСобытияИзменениеСоставаНабора(), , НоменклатураНабора);
	...
	Иначе
		...
		Оповестить(НаборыКлиент.ИмяСобытияИзменениеСоставаНабора(), СтруктураВозврата, ?(ТипЗнч(ВладелецФормы)=Тип("ФормаКлиентскогоПриложения"), ВладелецФормы.УникальныйИдентификатор, Неопределено));
...
Процедура ДобавитьТипыНоменклатуры(СтруктураВозврата)
	...
	МассивНоменклатиуры = Новый Массив;
	Для каждого Элемент Из СтруктураВозврата.СоставНабора Цикл
		Если ЗначениеЗаполнено(Элемент.Номенклатура) И МассивНоменклатиуры.Найти(Элемент.Номенклатура)=Неопределено Тогда
			МассивНоменклатиуры.Добавить(Элемент.Номенклатура);
		КонецЕсли; 
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивНоменклатиуры", МассивНоменклатиуры);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	Номенклатура.Ссылка В(&МассивНоменклатиуры)";
...
Процедура ПереместитьСтрокуТаблицы(Направление)
	...
	КонецЕсли;
	КрайняяСтрока = СоставНабора.НайтиПоИдентификатору(КрайняяСтрокаИдентификатор);
...
// Заполняет признаки использования характиристик на основании которых будет строится условное оформление
//
&НаСервере
