...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТолькоПросмотр = НЕ (УправлениеДоступом.ЕстьРоль("ПолныеПрава") ИЛИ УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеПодсистемыНалоги"));
...
Процедура НастроитьОтображениеСписка()
	...
	Для Каждого КомандаНастройки Из НастройкиКнопок Цикл
		...
		Иначе
			УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(СписокЗадач, КомандаНастройки.Ключ, КомандаНастройки.Значение, Ложь);
		КонецЕсли;
	КонецЦикла;
