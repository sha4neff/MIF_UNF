...
Процедура ОбновитьЗаписьКалендаряПриЗаписиИсточника(ЗаписиПоИсточнику) Экспорт
	...
	ЗаписьКалендаря.Календарь		= ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.КалендариСотрудников.КалендарьНалогов");
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоТочныйРасчет И Завершено Тогда
		СуммаНалога = 0;
	КонецЕсли;
	
	ПредставлениеСуммыНалога = ?(ЭтоТочныйРасчет,"","~")+Строка(СуммаНалога)+НСтр("ru = ' руб.'");
КонецПроцедуры
