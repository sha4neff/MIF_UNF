...
Процедура ОК(Команда)
	...
	Иначе
		ПараметрыЗакрытия = Новый Структура("ЭквайринговыйТерминал, ВидПлатежнойКарты, НомерПлатежнойКарты, НомерЧекаЭТ, СсылочныйНомер, Сумма, ВидОплаты",
...
Процедура ДобавитьОплатуКартой(Команда)
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ВладелецФормы, ПоказыватьПредупреждение) Тогда
