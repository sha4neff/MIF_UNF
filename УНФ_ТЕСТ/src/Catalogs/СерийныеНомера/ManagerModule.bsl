...
Функция СрокДействияГарантии(СерийныйНомер, ДатаПроверки) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СерийныеНомераГарантии.Регистратор КАК Регистратор,
	...
	|	ДОБАВИТЬКДАТЕ(СерийныеНомераГарантии.ДатаСобытия, МЕСЯЦ, Товары.ГарантийныйСрок) КАК ГарантияДо,
	...
	|		ПО СерийныеНомераГарантии.Номенклатура = Товары.Ссылка
	|ГДЕ
