...
Процедура ЗаполнитьСписокПолейОснований()
	...
	Для Каждого ОписаниеПоля Из ОписаниеПолей Цикл
		...
		//{kp-unf.ru
		Если Объект.ПолноеИмяОснования="Задача.КП_Задача" Тогда
			РеквизитОснования=МетаданныеИсточника.РеквизитыАдресации["Исполнитель"];
			ОписаниеПоля.СписокПолейОснования.Добавить(РеквизитОснования.Имя, "Исполнитель (пользователь)");
			ОписаниеПоля.СписокПолейОснования.Добавить("Наименование", "Наименование задачи");
			ЭлементАвтора=ОписаниеПоля.СписокПолейОснования.НайтиПоЗначению("Автор");
			Если ЭлементАвтора<>Неопределено Тогда
				ЭлементАвтора.Представление="Автор (пользователь)";
			КонецЕсли;
		КонецЕсли;
		//kp-unf.ru}
