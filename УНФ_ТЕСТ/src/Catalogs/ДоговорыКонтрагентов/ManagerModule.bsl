...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОплатуКартами") Тогда
		...
		Результат.Добавить("РасчетКомиссиВОтчетеЭквайера");
	КонецЕсли; 
...
Процедура ПриДобавленииПараметровШаблона(НазначениеШаблона, ПараметрыШаблона) Экспорт
	...
	Если НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагента
		...
		ПараметрыДоговора = ШаблоныПечатиОфисныхДокументов.ДобавитьГруппуПараметровШаблона(ПараметрыШаблона, НСтр("ru='Договор контрагента'"));
	...
	ИначеЕсли НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаСчет Тогда
		...
		ПараметрыСчета = ШаблоныПечатиОфисныхДокументов.ДобавитьГруппуПараметровШаблона(ПараметрыШаблона, НСтр("ru='Счет на оплату'"));
...
Процедура ПриПолученииЗначенийПараметровШаблона(ОбъектПечати, НазначениеШаблона, ОписаниеПараметров, ДополнительныеПараметры) Экспорт
	...
	Если НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаЗаказ
		...
		Документы.ЗаказПокупателя.ПриПолученииЗначенийПараметровШаблона(ДополнительныеПараметры.ЗаказПокупателя, НазначениеШаблона, ОписаниеПараметров, ДополнительныеПараметры);
		
	ИначеЕсли НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаСчет Тогда
		
		Документы.СчетНаОплату.ПриПолученииЗначенийПараметровШаблона(ДополнительныеПараметры.СчетНаОплату, НазначениеШаблона, ОписаниеПараметров, ДополнительныеПараметры);
...
Процедура ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода) Экспорт
	...
	Если ПараметрыПечати.НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаЗаказ
		...
		Документы.ЗаказПокупателя.ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода);
	...
	ИначеЕсли ПараметрыПечати.НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаСчет Тогда
		...
		Документы.СчетНаОплату.ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода);
...
Процедура ЗаполнитьЗначенияПараметровДоговора(ОбъектПечати, ОписаниеПараметров, ДополнительныеПараметры)
	...
	ЗаполнитьЗначенияПараметровОрганизации(ОписаниеПараметров, "ДоговорыКонтрагентов.Организация.", ДанныеДоговора.ДоговорыКонтрагентовОрганизация);
	...
	ШаблоныПечатиОфисныхДокументов.ЗаполнитьЗначенияКонтактнойИнформации(ОписаниеПараметров, "ДоговорыКонтрагентов.Владелец.", ДанныеДоговора.ДоговорыКонтрагентовВладелец);
...
Процедура ЗаполнитьЗначенияПараметровОрганизации(ОписаниеПараметров, Префикс, Организация)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Форма договора (устарела) (HTML)'");
	...
	КомандаПечати.Представление = НСтр("ru = 'Форма договора (устарела) (Microsoft word)'");
	КомандаПечати.СписокФорм = "";
	КомандаПечати.ФункциональныеОпции = "ДоговорыКонтрагентовТехническиеКомандыПечати";
	
	Если РольДоступна("ПолныеПрава") ИЛИ РольДоступна("ДобавлениеИзменениеПодсистемыБиллинг") Тогда
		КомандаПечати = КомандыПечати.Добавить();
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ПечатнаяФормаДоговорDOC = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ДоговорКонтрагента_DOC");
	Если ПечатнаяФормаДоговорDOC <> Неопределено Тогда
		ПечатнаяФормаДоговорDOC.ОфисныеДокументы = ПечатьДоговорКонтрагента_DOC(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати);
	КонецЕсли;
	
	ПечатнаяФормаДоговорHTML = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ДоговорКонтрагента_HTML");
	Если ПечатнаяФормаДоговорHTML <> Неопределено Тогда
		ПечатнаяФормаДоговорHTML.ОфисныеДокументы = ПечатьДоговорКонтрагента_HTML(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати);
	КонецЕсли;
...
Функция ПечатьДоговорКонтрагента_DOC(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати = Неопределено) Экспорт
	
	Расширение = "doc";
	Возврат ПечатьДоговорКонтрагентаБланкДоговора(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати, Расширение);
	
КонецФункции

Функция ПечатьДоговорКонтрагента_HTML(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати = Неопределено) Экспорт
...
Функция ПечатьДоговорКонтрагентаБланкДоговора(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати, Расширение)
	...
	Для каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Иначе
			Договор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектПечати, "Договор");
		КонецЕсли;
		
		ПечатнаяФормаДоговора = УправлениеНебольшойФирмойФормированиеПечатнойФормыДоговора.ПолучитьHTMLСформированногоДоговора(
			Договор, ДокументОткрытия,, Истина, Расширение);
		
		ИмяФайла = СтрШаблон(НСтр("ru='Договор контрагента - %1.%2'"), ОбъектПечати, Расширение);
		НовыйФайл = ПолучитьИмяВременногоФайла();
		
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(ПечатнаяФормаДоговора);
		ТекстовыйДокумент.Записать(НовыйФайл);
		ДвоичныеДанные = Новый ДвоичныеДанные(НовыйФайл);
		
		ОфисныеДокументы.Вставить(ПоместитьВоВременноеХранилище(ДвоичныеДанные), ИмяФайла);
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	// Интеркампани
	Если Параметры.Свойство("ВидОперации") И Параметры.Отбор.Свойство("Владелец") Тогда
		Если Параметры.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтНашейОрганизации
			ИЛИ Параметры.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации Тогда
			Параметры.Отбор.Вставить("Организация", Параметры.Отбор.Владелец);
			Параметры.Отбор.Вставить("ЭтоДоговорПередчиТоваровМеждуОрганизациями", Истина);
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	Если НастройкиЗагрузкиДанных.ФиксированныйШаблон Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "УИДВладелец", "УИД Владельца", ОписаниеТиповСтрока50, ОписаниеТиповСтрока50);
	КонецЕсли; 
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Наименование", "Договор (наименование)", ОписаниеТиповСтрока150, ОписаниеТиповКолонка, "Договор", 1, , Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерДоговора", "Договор (номер)", ОписаниеТиповСтрока50, ОписаниеТиповКолонка, "Договор", 2, , Истина);
	
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ВладелецИНН", "Контрагент (ИНН)", ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Владелец", 1);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ВладелецНаименование", "Контрагент (наименование)", ОписаниеТиповСтрока150, ОписаниеТиповКолонка, "Владелец", 2);
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Организации");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Организация", "Организация", ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("ПеречислениеСсылка.ВидыДоговоров");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ВидДоговора", "Вид договора", ОписаниеТиповСтрока50, ОписаниеТиповКолонка);
	
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаДоговора", "Дата договора", ОписаниеТиповСтрока25, ОписаниеТиповДата);	
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СрокДействия", "Срок действия", ОписаниеТиповСтрока25, ОписаниеТиповДата);
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Валюты");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ВалютаРасчетов", "Валюта расчетов", ОписаниеТиповСтрока11, ОписаниеТиповКолонка);
	
	ОписаниеТиповБулево = Новый ОписаниеТипов("Булево");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "РасчетыВУсловныхЕдиницах", "Расчеты в условных единицах", ОписаниеТиповСтрока11, ОписаниеТиповБулево);
	...
	Если НастройкиЗагрузкиДанных.ОписаниеДополнительныхРеквизитов.Количество() > 0 Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, ИмяПоля, "Дополнительные реквизиты", ОписаниеТиповСтрока150, ОписаниеТиповСтрока11, , , , , , Истина, Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_ДоговорыКонтрагентов);
