...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СопостовлениеПолей" Тогда
...
Процедура ОбновитьЭлементыФормЗаявок()
	...
	Для ИндексГруппы = 0 По СозданныеФормы[ИндексТекущейФормы(ЭтотОбъект)].ПоляКонструктора.Количество()-1 Цикл
		Если Команды.Найти("ПолеСопостовленияУНФ_"+ИндексГруппы) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		УдаляемыеКоманды.Добавить(Команды.Найти("ПолеСопостовленияУНФ_"+ИндексГруппы));
	КонецЦикла;
	...
	ЭлементЗаголовокКонструктора.ПодсказкаВвода = "Заголовок";
	...
	ЭлементОписаниеКонструктора.ПодсказкаВвода = "Описание";
	...
	ЭлементТекстПослеОтправкиОтвета.ПодсказкаВвода = "Текст после успешной отправки ответа";
...
Процедура ОбновитьЭлементыКонструктора(ПоляКонструктораТекущее, ИндексТекущейФормы)
	...
	Для Каждого ПолеКонструктора Из ПоляКонструктораТекущее Цикл
		...
		ГруппаПолеКонструктора = Элементы.Добавить("ГруппаПолеКонструктура_" + ИндексПоляКонструктора, Тип("ГруппаФормы"), Элементы.ГруппаПоляКонструктора);
		...
		Если ПолеКонструктора.ТипПоля = ТипСтрока() ИЛИ ПолеКонструктора.ТипПоля = ТипАбзац() ИЛИ ПолеКонструктора.ЭтоТовар Тогда
			...
			ДекорацияПолеУНФКоманда = ЭтаФорма.Команды.Добавить("ПолеСопостовленияУНФ_" + ИндексПоляКонструктора);
...
Процедура Подключаемый_КопированиеКодаДляСайтаНажатие(Элемент)
	ТекущаяСтрока = СозданныеФормы[ИндексТекущейФормы(ЭтотОбъект)];
	ЭлектронноеВзаимодействиеСлужебныйКлиент.СкопироватьВБуферОбмена(ТекущаяСтрока.КодДляСайта, НСтр("ru = 'Код для сайта скопирован'"));
КонецПроцедуры
...
Процедура Подключаемый_КопированиеСсылкиНажатие(Элемент)
	...
	ТекущаяСтрока = СозданныеФормы[ИндексТекущейФормы(ЭтотОбъект)];
	ЭлектронноеВзаимодействиеСлужебныйКлиент.СкопироватьВБуферОбмена(ТекущаяСтрока.СсылкаНаФорму, НСтр("ru = 'Ссылка на форму скопирована'"));
...
Процедура ДобавитьПоляВариантов(ИндексПоляКонструктора, ИндексТекущейФормы)
	...
	Для Каждого Вариант Из ВариантыОтветовПоля Цикл
		...
		ГруппаВариантОтвета = Элементы.Добавить("ГруппаПоле_" + ИндексПоляКонструктора + "_ВариантОтвета_"+ИндексВарианта, Тип("ГруппаФормы"), Элементы["ГруппаПолеКонструктура_"+ИндексПоляКонструктора]);
...
Процедура ОбновитьЭлементыСозданныхФорм()
	...
	Для Каждого КонструкторФорм Из СозданныеФормы Цикл
		...
		ДекорацияУдалитьФорму.Подсказка = "Удалить форму";
...
Процедура Подключаемый_ОткрытьФормуСопоставления(Команда)
	...
	ОткрытьФорму("Справочник.КонтактныеФормыGoogle.Форма.ФормаСопостовления",ПараметрыКонструктора);
