...
Процедура ИерархияИзменить(Команда)
	...
	Если ТекущиеДанные = Неопределено
		ИЛИ ТипЗнч(ТекущиеДанные.ГруппаСотрудников) <> Тип("СправочникСсылка.Сотрудники")
...
Процедура ИерархияСоздатьГруппу(Команда)
	...
	ТекущиеДанные = Элементы.ОтборИерархия.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено
		ИЛИ ТекущиеДанные.ГруппаСотрудников = ИерархияБригады() Тогда
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("Справочник.Сотрудники.ФормаГруппы",
		Новый Структура("ЗначенияЗаполнения, ЭтоГруппа", ЗначенияЗаполнения, Истина),
		Элементы.СписокСотрудники);
...
Процедура ИерархияСкопировать(Команда)
	...
	ОткрытьФорму("Справочник.Сотрудники.ФормаГруппы",
		Новый Структура("ЗначениеКопирования, ЭтоГруппа", ТекущиеДанные.ГруппаСотрудников, Истина),
		Элементы.СписокСотрудники);
...
Процедура ИерархияУстановитьПометкуУдаления(Команда)
	...
	ПоказатьОповещениеПользователя(
		ТекстОповещения,
		ПолучитьНавигационнуюСсылку(ТекущиеДанные.ГруппаСотрудников),
		Элементы.ОтборИерархия.ТекущиеДанные.ГруппаСотрудников,
...
Процедура ОтборИерархияНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Если СтрокаИерархии = Неопределено
		Или СтрокаИерархии.ГруппаСотрудников = ИерархияСотрудники()
		Или СтрокаИерархии.ГруппаСотрудников = ИерархияБригады()
		Или СтрокаИерархии.ГруппаСотрудников = ИерархияБезГруппы() Тогда
...
Процедура ОтборИерархияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	НоваяГруппа = ?(СтрокаИерархии.ГруппаСотрудников = ИерархияБезГруппы(), ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка"), СтрокаИерархии.ГруппаСотрудников);
...
Процедура УстановитьКнопкуПоУмолчанию(Форма)
	...
	Элементы = Форма.Элементы;
	Если Элементы.ГруппаБригадыСотрудники.ТекущаяСтраница = Элементы.ГруппаБригады Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокБригадыВыбрать", "КнопкаПоУмолчанию", Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокСотрудникиВыбрать", "КнопкаПоУмолчанию", Истина);
	КонецЕсли; 
