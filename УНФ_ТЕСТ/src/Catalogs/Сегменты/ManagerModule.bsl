...
Функция ПолучитьДоступныеПравилаОтбора() Экспорт
	...
	НовоеПравило = ГруппаЗаказы.Строки.Добавить();
	НовоеПравило.Имя = "ЗаказыПокупателейВариантЗавершения";
	НовоеПравило.Представление = НСтр("ru='Завершение заказа'");
	НовоеПравило.ЭтоГруппа = Ложь;
	НовоеПравило.МножественноеИспользование = Ложь;
	НовоеПравило.СвойстваЗначения.Вставить("ОграничениеТипа", Новый ОписаниеТипов("ПеречислениеСсылка.ВариантыЗавершенияЗаказа"));
	ДобавитьВидыСравнений(НовоеПравило, "Равно");
	...
	ГруппаДополнительно = ГруппаЗаказы.Строки.Добавить();
	ГруппаДополнительно.Имя = "ЗаказНарядыДополнительно";
	ГруппаДополнительно.Представление = НСтр("ru = 'Уточнения заказ-нарядов'");
	ГруппаДополнительно.ЭтоГруппа = Истина;
	
	НовоеПравило = ГруппаДополнительно.Строки.Добавить();
	НовоеПравило.Имя = "ЗаказНарядыСостояниеЗаказа";
	НовоеПравило.Представление = НСтр("ru = 'Состояние заказ-наряда'");
	НовоеПравило.ЭтоГруппа = Ложь;
	НовоеПравило.МножественноеИспользование = Ложь;
	НовоеПравило.СвойстваЗначения.Вставить("ОграничениеТипа", Новый ОписаниеТипов("СправочникСсылка.СостоянияЗаказНарядов"));
	НовоеПравило.СвойстваЗначения.Вставить("ВыборГруппИЭлементов", ГруппыИЭлементы.Авто);
	ДобавитьВидыСравнений(НовоеПравило, "Равно,НеРавно,ВСписке,НеВСписке");
...
Функция СформироватьЗапросПоПравилам(Сегмент) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СегментыИспользуемыеПравила.Имя КАК Имя,
		|	СегментыИспользуемыеПравила.Настройка КАК Настройка,
		|	СегментыИспользуемыеПравила.КлючДинамическогоПравила КАК КлючДинамическогоПравила
		|ИЗ
		|	Справочник.Сегменты.ИспользуемыеПравила КАК СегментыИспользуемыеПравила
		|ГДЕ
		|	СегментыИспользуемыеПравила.Ссылка В(&Ссылка)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Пока ВыборкаПравил.Следующий() Цикл
		...
		ИначеЕсли СтрНачинаетсяС(ВыборкаПравил.Имя, "ЗаказыПокупателей")
			ИЛИ СтрНачинаетсяС(ВыборкаПравил.Имя, "ЗаказНаряды") Тогда
			...
			ИначеЕсли ВыборкаПравил.Имя = "ЗаказыПокупателейКоличествоПериод" Тогда
				...
				Если ЗначениеЗаполнено(НастройкаПравила.Значение.ДатаОкончания) Тогда
					...
					Запрос.УстановитьПараметр(ВыборкаПравил.Имя + "Конец",  НастройкаПравила.Значение.ДатаОкончания);
				КонецЕсли;
			ИначеЕсли ВыборкаПравил.Имя = "ЗаказыПокупателейСостояниеЗаказа"
				ИЛИ ВыборкаПравил.Имя = "ЗаказНарядыСостояниеЗаказа" Тогда
				...
				Запрос.УстановитьПараметр(ВыборкаПравил.Имя, НастройкаПравила.Значение);
			ИначеЕсли ВыборкаПравил.Имя = "ЗаказыПокупателейВариантЗавершения" Тогда
				ОтборВложенногоЗапроса.Добавить(УсловиеСравнения("ЗаказПокупателя.ВариантЗавершения", НастройкаПравила.ВидСравнения, ВыборкаПравил.Имя));
				Запрос.УстановитьПараметр(ВыборкаПравил.Имя, НастройкаПравила.Значение);
			КонецЕсли;
			
		ИначеЕсли СтрНачинаетсяС(ВыборкаПравил.Имя, "СчетаНаОплату") Тогда
		...
		ИначеЕсли СтрНачинаетсяС(ВыборкаПравил.Имя, "ПродажиНоменклатура") Тогда
			
			ТаблицаПродажиИмя = "ПродажиНоменклатура" + НомерПравила;
			
			НовыйИсточник = Оператор.Источники.Добавить(Тип("ВложенныйЗапросСхемыЗапроса"), ТаблицаПродажиИмя);
			НовыйИсточник.Источник.Запрос.УстановитьТекстЗапроса(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			НовыйИсточник.Соединения.Очистить();
			Оператор.Источники[0].Соединения.Добавить(ТаблицаПродажиИмя, "Контрагенты.Ссылка = " + ТаблицаПродажиИмя + ".Контрагент");
			Оператор.Источники[0].Соединения[0].ТипСоединения = ТипСоединенияСхемыЗапроса.ЛевоеВнешнее;
			
			Если НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
				ИЛИ НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии
				ИЛИ НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
				ИЛИ НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
				ОтборЗапроса.Добавить("НЕ ПродажиНоменклатура" + НомерПравила + ".Контрагент ЕСТЬ NULL");
			Иначе
				ОтборЗапроса.Добавить("ПродажиНоменклатура" + НомерПравила + ".Контрагент ЕСТЬ NULL");
			КонецЕсли;
			
			ОтборВложенногоЗапроса = НовыйИсточник.Источник.Запрос.Операторы[0].Отбор;
			
			Если ВыборкаПравил.Имя = "ПродажиНоменклатураНоменклатура"
				ИЛИ ВыборкаПравил.Имя = "ПродажиНоменклатураГруппаНоменклатуры"
				ИЛИ ВыборкаПравил.Имя = "ПродажиНоменклатураКатегорияНоменклатуры" Тогда
				
				Если ВыборкаПравил.Имя = "ПродажиНоменклатураНоменклатура" Тогда
					ПравилоПолеСравнения = "Продажи.Номенклатура";
				ИначеЕсли ВыборкаПравил.Имя = "ПродажиНоменклатураГруппаНоменклатуры" Тогда
					ПравилоПолеСравнения = "Продажи.Номенклатура.Родитель";
				ИначеЕсли ВыборкаПравил.Имя = "ПродажиНоменклатураКатегорияНоменклатуры" Тогда
					ПравилоПолеСравнения = "Продажи.Номенклатура.КатегорияНоменклатуры";
				КонецЕсли;
				
				Если НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
					ИЛИ НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно Тогда
					ПравилоВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				ИначеЕсли НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
					ИЛИ НастройкаПравила.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке Тогда
					ПравилоВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
				Иначе
					ПравилоВидСравнения = НастройкаПравила.ВидСравнения;
				КонецЕсли;
				
				ОтборВложенногоЗапроса.Добавить(УсловиеСравнения(ПравилоПолеСравнения, ПравилоВидСравнения, ВыборкаПравил.Имя + НомерПравила));
				Запрос.УстановитьПараметр(ВыборкаПравил.Имя + НомерПравила, НастройкаПравила.Значение);
		...
		ИначеЕсли СтрНачинаетсяС(ВыборкаПравил.Имя, "ПродажиВыручка") Тогда
