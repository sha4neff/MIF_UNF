...
Процедура НекачественныйЛидПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Лиды.Форма.ФормаНекачественыйЛидКанбан",,,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ТипЗнч(Элемент.ТекущаяСтрока) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		...
		ЛидАктивнойСтроки = ?(Элемент.ТекущиеДанные = Неопределено, Неопределено, Элемент.ТекущиеДанные.Ссылка);
		Если ЛидАктивнойСтроки <> ТекущийЛид Тогда
			ТекущийЛид = ЛидАктивнойСтроки;
...
Процедура ПланировщикПриСменеТекущегоПериодаОтображения(Элемент, ТекущиеПериодыОтображения, СтандартнаяОбработка)
	...
	Если ТекущиеПериодыОтображения[0].Начало > Планировщик.ТекущиеПериодыОтображения[0].Конец Тогда
		...
		ОбновитьЭлементыПланировщика("Скролл","СкроллВперед");
	ИначеЕсли ТекущиеПериодыОтображения[0].Начало < Планировщик.ТекущиеПериодыОтображения[0].Начало Тогда
		...
		Если ТекущийЭкранКанбан = 0 Тогда
			ОбновитьЭлементыПланировщика("Создание");
		Иначе
			ОбновитьЭлементыПланировщика("Скролл","СкроллНазад");
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор, "ВариантЗавершения");
...
Процедура УстановитьОтборПоРезультату()
	...
	ОтборНеУстановлен = ЭлементыОтбора.Количество() = 0 ИЛИ НЕ ЭлементыОтбора[0].Использование
		ИЛИ ЭлементыОтбора[0].ПравоеЗначение.НайтиПоЗначению(Перечисления.ВариантЗавершенияРаботыСЛидом.ПустаяСсылка()) <> Неопределено;
	...
	Если ОтборНеУстановлен Тогда
		...
		Отбор.Добавить(Перечисления.ВариантЗавершенияРаботыСЛидом.ПустаяСсылка());
...
Процедура ОбновитьИзмеренияПланировщика()
	...
	ФильтрПоСостоянию = "";
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор, "СостояниеЛида");
	Если ЭлементыОтбора.Количество() <> 0
		И (ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") И ЭлементыОтбора[0].ПравоеЗначение.Количество() <> 0) Тогда
...
&НаСервере
Процедура ОбновитьЭлементыПланировщика(Операция, НаправлениеСкролла = Неопределено)
	...
	КоличествоЛидовПоСостояниям = КоличествоЭлементовПоСостояниямВКанбане();
	...
	Для Каждого Состояние Из Планировщик.Измерения[0].Элементы Цикл
		...
		ИначеЕсли Операция = "Скролл" И НаправлениеСкролла = "СкроллВперед" Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Операция = "Скролл" И НаправлениеСкролла = "СкроллНазад" Тогда
...
&НаСервере
Функция ТекстЗапросаКанбан(Операция, НаправлениеСкролла, Количество = 14)
...
&НаСервере
Функция КоличествоЭлементовПоСостояниямВКанбане()
...
Процедура ОбновитьПанельКонтактнойИнформацииСервер()
	...
	КонтактнаяИнформацияПанельУНФ.ОбновитьДанныеПанели(ЭтотОбъект, ТекущийЛид);
