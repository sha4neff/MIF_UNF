...
Функция ПолучитьПодчиненныеПоказатели(Показатель) Экспорт
	...
	ВыборкаПодчиенныхПоказателей = Запрос.Выполнить().Выбрать();

	Пока ВыборкаПодчиенныхПоказателей.Следующий() Цикл
		МассивПодчиненныхПоказателей.Добавить(ВыборкаПодчиенныхПоказателей.Ссылка);
	КонецЦикла;
