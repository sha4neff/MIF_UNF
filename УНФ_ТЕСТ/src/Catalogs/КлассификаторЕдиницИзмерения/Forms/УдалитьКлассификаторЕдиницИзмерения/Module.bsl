...
Процедура ИскатьСтрокуВТаблице(ИскатьВперед)
	...
	Если НайденнаяОбласть = Неопределено Тогда
		...
		Если НайденнаяОбласть = Неопределено Тогда
			...
			ТекстСообщения = НСтр("ru = 'Единица измерения не найдена'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ВыполнитьВыбор(ТекущаяОбласть, ЗакрытьФорму = Истина)
	...
	Если НЕ ПустаяСтрока(ТекстСообщения) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "Классификатор");
		ЗакрытьФорму = Ложь;
		Возврат;
	...
	Если ЕдИзм <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Единица измерения была добавлена раннее.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура СоздатьЕдиницуИзмеренияПоКлассификатору(ЗначенияЗаполнения)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Добавление единиц измерения из ОКЕИ'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
