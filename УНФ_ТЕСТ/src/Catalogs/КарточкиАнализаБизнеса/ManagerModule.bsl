...
// Формирует набор поставляемых карточек в обработчике обновления или при первоначальном заполненнии информационной базы
//
//
Процедура СформироватьНаборПоставляемыхКарточек() Экспорт
	...
	КарточкаДебеторкаКредиторка = Новый Структура;
	КарточкаДебеторкаКредиторка.Вставить("Имя", 						"ДебеторкаКредиторка");
	КарточкаДебеторкаКредиторка.Вставить("ИмяПоказателя", 				"Дебиторская задолженность");
	КарточкаДебеторкаКредиторка.Вставить("ИмяПоказателяСравнение", 		"Кредиторская задолженность");
	КарточкаДебеторкаКредиторка.Вставить("ПерваяСтрокаЗаголовка", 		"Дебеторка");
	КарточкаДебеторкаКредиторка.Вставить("ВтораяСтрокаЗаголовка", 		"Кредиторка");
	КарточкаДебеторкаКредиторка.Вставить("ВидОтчета", 					Перечисления.ВидыФинансовыхОтчетов.Баланс);
	КарточкаДебеторкаКредиторка.Вставить("ВидКарточки", 				Перечисления.ВидыКарточекАнализаБизнеса.КонтрольПоказателей);
	КарточкаДебеторкаКредиторка.Вставить("ПоказательБизнеса", 			Справочники.ПоказателиБизнеса.ПустаяСсылка());
	КарточкаДебеторкаКредиторка.Вставить("ВариантПроверки", 			Перечисления.ВариантыПроверокАнализаБизнеса.СравнениеСПоказателем);
	КарточкаДебеторкаКредиторка.Вставить("ВидСравненияЗначений", 		Перечисления.ВидСравненияЗначений.БольшеИлиРавно);
	КарточкаДебеторкаКредиторка.Вставить("ЗначениеДляСравнения", 		Справочники.ПоказателиБизнеса.ПустаяСсылка());
	КарточкаДебеторкаКредиторка.Вставить("Период", 						Перечисления.ДоступныеПериодыОтчета.Квартал);
	КарточкаДебеторкаКредиторка.Вставить("ТекстПоложительныйРезультат", ПолучитьОписаниеИзМакета(ШаблонИмениМакетаХорошо,   КарточкаДебеторкаКредиторка));
	КарточкаДебеторкаКредиторка.Вставить("ТекстОтрицательныйРезультат", ПолучитьОписаниеИзМакета(ШаблонИмениМакетаПлохо,    КарточкаДебеторкаКредиторка));
	КарточкаДебеторкаКредиторка.Вставить("ТекстОписаниеПроверки", 		ПолучитьОписаниеИзМакета(ШаблонИмениМакетаОписание, КарточкаДебеторкаКредиторка));
	
	СформироватьКарточкуПоПараметрам(КарточкаДебеторкаКредиторка);
...
Процедура СформироватьТекстSVGСравнениеСПоказателем(ДанныеКарточки)
	...
	СтрокаSVG = СтрЗаменить(СтрокаSVG, "Заголовок2", 	   ДанныеКарточки.ВтораяСтрокаЗаголовка);
	СтрокаSVG = СтрЗаменить(СтрокаSVG, "М_1",			   ПредставлениеПериода);
	СтрокаSVG = СтрЗаменить(СтрокаSVG, "зн_1",			   СимволСравнения);
	СтрокаSVG = СтрЗаменить(СтрокаSVG, "СуммаПоказатель1", ПолучитьПредставлениеСуммы(ДанныеКарточки.СуммаПоказателя, ДанныеКарточки.ЭтоПроцент));
...
Процедура ЗаполнитьСуммыЦелиРекурсивно(ДанныеКарточки, Показатель, ТаблицаДанных)
	...
	Если ТипПоказателя = Перечисления.ТипыПоказателейБизнеса.Группа Тогда
		...
		ПодиченныеПоказатели = Справочники.ПоказателиБизнеса.ПолучитьПодчиненныеПоказатели(Показатель);
		
		Для каждого ПодчиненныйПоказатель Из ПодиченныеПоказатели Цикл
...
Процедура СформироватьКарточкуПоПараметрам(ПараметрыКарточки)
	...
	// Проверям не создана ли карточка по этому показателю ранее
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КарточкиАнализаБизнеса.Ссылка КАК Ссылка
