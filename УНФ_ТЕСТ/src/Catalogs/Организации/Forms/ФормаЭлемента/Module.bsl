...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьСписокРежимовНалогообложения(ЭтотОбъект);
	...
	Для Каждого ЗначениеСоответствия Из СоответствиеСтавок Цикл
		Если ЗначениеЗаполнено(ЗначениеСоответствия.Значение) Тогда
			Элементы.ВидСтавкиНДСПоУмолчанию.СписокВыбора.Добавить(ЗначениеСоответствия.Ключ,
				Строка(ЗначениеСоответствия.Значение));
		КонецЕсли;
	КонецЦикла;
	Элементы.ДекорацияСтавкиНДСПодсказка.Видимость = Объект.ВидСтавкиНДСПоУмолчанию = ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.Общая")
		ИЛИ Объект.ВидСтавкиНДСПоУмолчанию = ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.ОбщаяРасчетная");	
КонецПроцедуры
...
Процедура СистемаНалогообложенияПриИзменении(Элемент)
	...
	Если СистемаНалогообложенияПредставление = 0 Тогда
		КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.Общая");
		КлючСистемыНалогообложения.ПлательщикУСН = Ложь;
	ИначеЕсли СистемаНалогообложенияПредставление = 1 Тогда
		КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.Упрощенная");
	...
	ИначеЕсли СистемаНалогообложенияПредставление = 2 Тогда
		...
		КлючСистемыНалогообложения.ПлательщикУСН = Ложь;
		КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ОсобыйПорядок");
	...
	Если КлючСистемыНалогообложения.ПлательщикУСН 
		Или КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ОсобыйПорядок") Тогда
...
Процедура ГоловнаяОрганизацияПриИзменении(Элемент)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИНН", 										"Доступность", НЕ ЗначениеЗаполнено(Объект.ГоловнаяОрганизация));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЦифровойИндексОбособленногоПодразделения",	"Видимость", ЗначениеЗаполнено(Объект.ГоловнаяОрганизация));
...
Процедура ИсторияСтраховыхВзносов(Команда)
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		ОткрытьФорму("РегистрСведений.ПрименяемыеТарифыСтраховыхВзносов.ФормаСписка",  Новый Структура("Отбор", Новый Структура("Организация",Объект.Ссылка)));
	КонецЕсли;
...
Процедура ПримененияПатентов(Команда)
	ОткрытьФорму("РегистрСведений.ПримененияПатенты.ФормаСписка", Новый Структура("Отбор", Новый Структура("Организация",Объект.Ссылка)));
КонецПроцедуры
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", ПредопределенноеЗначение("Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_Организации"));
...
Процедура НовыйТарифСтраховыхВзносов(Команда)
	...
	ОткрытьФорму("РегистрСведений.ПрименяемыеТарифыСтраховыхВзносов.ФормаЗаписи",  Новый Структура(
...
Процедура УправлениеФормой(Форма)
	...
	Иначе
		...
		Элементы.ПрименяетсяПатент.Видимость 		= Форма.КлючСистемыНалогообложения.СистемаНалогообложения <> ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ОсобыйПорядок");
	...
	КонецЕсли;
	Элементы.ПлательщикЕНВД.Видимость = Форма.КлючСистемыНалогообложения.СистемаНалогообложения <> ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ОсобыйПорядок");
	
	
	ЭтаОрганизацияЯвляетсяГоловной = ОрганизацияЯвляетсяГоловной(Объект.Ссылка);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГоловнаяОрганизация", "Видимость", ЭтоЮридическоеЛицо И НЕ ЭтаОрганизацияЯвляетсяГоловной);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСписокРежимовНалогообложения(Форма)
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	Элементы.СистемаНалогообложения.СписокВыбора.Очистить();
	Элементы.СистемаНалогообложения.СписокВыбора.Добавить(0,"Общая");
	Элементы.СистемаНалогообложения.СписокВыбора.Добавить(1,"Упрощенная");
	Элементы.СистемаНалогообложения.СписокВыбора.Добавить(2,"Только ЕНВД");
	Если Не (Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо")) Тогда
		Элементы.СистемаНалогообложения.СписокВыбора.Добавить(3,"Только патент");
	Иначе
		Если Форма.КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ОсобыйПорядок")
			И Форма.КлючСистемыНалогообложения.ПрименяетсяПатент Тогда
			Форма.КлючСистемыНалогообложения.СистемаНалогообложения = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.Общая");
...
Процедура УстановитьПодсказкуПоТарифу()
	...
	Элементы.ДекорацияОписаниеТарифа.Заголовок = СтрШаблон(
		НСтр("ru = 'Действует с %5 г.(ПФР - %1%4, ФСС - %2%4, ФФОМС - %3%4)'"),
		СрезПоТарифу.ПФР,
		СрезПоТарифу.ФСС,
		СрезПоТарифу.ФФОМС,
		"%",
		Формат(КлючТарифаСтраховыхВзносов.Период,"ДФ=dd.MM.yyyy"));
...
Функция ОрганизацияЯвляетсяГоловной(ОрганизацияСсылка)
	...
	Запрос.Текст = "ВЫБРАТЬ ИСТИНА ИЗ Справочник.Организации КАК СпрОрганизации ГДЕ СпрОрганизации.ГоловнаяОрганизация = &ОрганизацияСсылка";
...
Процедура ЗаполнитьРеквизитыПоИНННаСервере(ИНН, ОписаниеОшибки = "")
	...
	Если ЭтоЮрЛицо Тогда
		...
		Если РеквизитыОрганизации.ЮридическийАдрес <> Неопределено Тогда
			...
			РеквизитыОрганизации.ЮридическийАдрес.Вставить("Значение", РеквизитыОрганизации.ЮридическийАдрес.КонтактнаяИнформация);
		...
		Если РеквизитыОрганизации.Телефон <> Неопределено Тогда
			...
			РеквизитыОрганизации.Телефон.Вставить("Значение", РеквизитыОрганизации.Телефон.КонтактнаяИнформация);
			КонтактнаяИнформацияУНФ.ЗаполнитьЗначениеКонтактнойИнформации(
				ЭтотОбъект,
...
&НаСервере
Процедура ЗаполнитьСведенияОНалоговойИнспекцииПоАдресу(ЗначениейАдреса, ОписаниеОшибки = "")
	
	ЭтоФизЛицо = Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
	
	Если НЕ ЗначениеЗаполнено(ЗначениейАдреса) Тогда
		Возврат;
	КонецЕсли;
	
	СведенияОНалоговомОрганеПоАдресу = ДанныеАдресногоКлассификатора.НалоговыйОрганПоАдресу(ЗначениейАдреса);
