...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ТипЗнч(Элемент.ТекущаяСтрока) <> Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		...
		КонтактАктивнойСтроки = ?(Элемент.ТекущиеДанные = Неопределено, Неопределено, Элемент.ТекущиеДанные.Ссылка);
		Если КонтактАктивнойСтроки <> ТекущийКонтакт Тогда
			ТекущийКонтакт = КонтактАктивнойСтроки;
...
Процедура ОтборСвязиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УстановитьОтборПоДействительнымСвязям(ВыбранноеЗначение);
...
Процедура ОтборСвязиИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	Если Текст = "" Тогда
		ОтборСвязи = "";
		УстановитьОтборПоДействительнымСвязям(ОтборСвязи);
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьМеткуОтбора(МеткаИД)
	...
	ЭлементыОтбораКомпоновщикНастроек = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор, "Контрагент");
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.Отбор, "Контрагент");
	
	Если (ЭлементыОтбораКомпоновщикНастроек.Количество() = 0 ИЛИ (ТипЗнч(ЭлементыОтбораКомпоновщикНастроек[0].ПравоеЗначение) = Тип("Массив") 
		И ЭлементыОтбораКомпоновщикНастроек[0].ПравоеЗначение.Количество() = 0) ИЛИ НЕ ЭлементыОтбораКомпоновщикНастроек[0].Использование)
		И (ЭлементыОтбора.Количество() = 0 ИЛИ (ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") 
		И ЭлементыОтбора[0].ПравоеЗначение.Количество() = 0)) Тогда
		Список.Параметры.Элементы.Найти("ЕстьСвязи").Использование = Ложь;
	КонецЕсли;
...
Процедура СохранитьНастройкиОтборовПоСвязям()
	...
	Если Список.Параметры.Элементы.Найти("ЕстьСвязи").Значение И Список.Параметры.Элементы.Найти("ЕстьСвязи").Использование Тогда
		ХранилищеСистемныхНастроек.Сохранить("УстановленПараметрЕстьСвязи", "УстановленПараметрЕстьСвязи_СписокКонтактныеЛица",Истина);
	Иначе
		ХранилищеСистемныхНастроек.Сохранить("УстановленПараметрЕстьСвязи", "УстановленПараметрЕстьСвязи_СписокКонтактныеЛица",Ложь);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьОтборПоДействительнымСвязям(ВыбранноеЗначение)
	
	МассивОтбора = Новый Массив;
	Если НЕ ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		УправлениеНебольшойФирмойКлиентСервер.УдалитьЭлементОтбораСписка(Список,"ЕстьСвязи");
	ИначеЕсли ВыбранноеЗначение = "ЕстьСвязи" Тогда
		МассивОтбора.Добавить(Истина);
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ЕстьСвязи", МассивОтбора, Истина, ВидСравненияКомпоновкиДанных.ВСписке);
	ИначеЕсли ВыбранноеЗначение = "НетСвязей" Тогда
		МассивОтбора.Добавить(Ложь);
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ЕстьСвязи", МассивОтбора, Истина, ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
...
Процедура ВосстановитьНастройкиОтборовПоСвязям()
	...
	УстановленПараметрЕстьСвязи = ХранилищеСистемныхНастроек.Загрузить("УстановленПараметрЕстьСвязи", "УстановленПараметрЕстьСвязи_СписокКонтактныеЛица");
	Если ЗначениеЗаполнено(УстановленПараметрЕстьСвязи) И УстановленПараметрЕстьСвязи Тогда
		Список.Параметры.УстановитьЗначениеПараметра("ЕстьСвязи", Истина);
	КонецЕсли;
	...
	ОтборПоКонтрагентам = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор,"Контрагент");
	Если ОтборПоКонтрагентам.Количество() = 0 И НЕ ЗначениеЗаполнено(ОтборСвязи) Тогда
		УправлениеНебольшойФирмойКлиентСервер.УдалитьЭлементОтбораСписка(Список,"ЕстьСвязи");
	КонецЕсли;
