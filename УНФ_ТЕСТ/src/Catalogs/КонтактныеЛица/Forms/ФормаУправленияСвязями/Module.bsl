...
Процедура ОК(Команда)
	...
	Если ИзмененыСвязиСКонтрагентами Тогда
		...
		НСтр("ru='Изменение связей с контрагентами'"),
		,
		СтрШаблон(НСтр("ru='Cвязи изменены для 
			|%1 '"), СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(ВыделенныеКонтакты.Количество(), ПараметрыПредмета)),
...
Процедура ОбновитьЭлементыКонтрагентов()
	...
	Для Каждого Контрагент Из ДанныеКонтрагентов Цикл
		...
		Если ИндексКонтрагента > 0 Тогда
			...
			ПолеКонтрагент.ПодсказкаВвода = "Связать с контрагентом";
...
Процедура ЗаписатьДанныеПоСвязямВРегистр()
	...
	// Разорвать связи с контрагентаим
