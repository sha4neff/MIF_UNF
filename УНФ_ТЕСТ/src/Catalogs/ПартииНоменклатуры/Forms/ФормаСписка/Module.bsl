...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("РежимВыбора") И Параметры.РежимВыбора Тогда
		...
		Иначе
			ВидОперации = ?(Параметры.Свойство("ОперацияДокумента") И ЗначениеЗаполнено(Параметры.ОперацияДокумента), Параметры.ОперацияДокумента, Неопределено);
			СтатусПартии = НоменклатураВДокументахСервер.СоответствиеВидаОпреацииИлиХозОперацииСтатусуПартии(ЭтаФорма, ВидОперации);
		КонецЕсли;
...
УстановитьГруппировкиДинмическогоСпискаСогласноОтбору();
КонецПроцедуры
	...
	Если НЕ СтрокаТабличнойЧасти=Неопределено И СтрокаТабличнойЧасти.Свойство("РодительскаяГруппировкаСтроки") И  НЕ Копирование Тогда
		...
		Если Не Элементы.Список.РежимВыбора Тогда
			...
			Иначе
				...
				Для Каждого СтрокаМоссива Из СтрокиПоСтатусам Цикл
					МассивСтатусов.Добавить(СтрокаМоссива.Метка);
				КонецЦикла;
...
Процедура Подключаемый_ПриИзмененииДопРеквизитаСТипомЧислоНачалоИнтервала()
	...
	РеквизитОснование = СтрЗаменить(ТекущийЭлемент.Имя,"_НачИнт","");
	ЗначениеОкончаниеИнтервала = ЭтаФорма[РеквизитОснование + "_КонИнт"];
	ЗначениеТекРеквизита = ЭтаФорма[ТекущийЭлемент.Имя];
	
	Если ЗначениеЗаполнено(ЗначениеТекРеквизита)
		Тогда
		УстановитьОтборДинамическогоСписка(РеквизитОснование,ЗначениеТекРеквизита,,ВидСравненияКомпоновкиДанных.БольшеИлиРавно,Ложь);
	Иначе
...
Процедура Подключаемый_ПриИзмененииДопРеквизитаСТипомЧислоОкончаниеИнтервала()
	...
	РеквизитОснование = СтрЗаменить(ТекущийЭлемент.Имя,"_КонИнт","");
	ЗначениеНачалоИнтервала = ЭтаФорма[РеквизитОснование + "_НачИнт"];
	ЗначениеТекРеквизита = ЭтаФорма[ТекущийЭлемент.Имя];
	
	Если ЗначениеЗаполнено(ЗначениеТекРеквизита)
		Тогда
		УстановитьОтборДинамическогоСписка(РеквизитОснование,ЗначениеТекРеквизита,,ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, Ложь);
	Иначе
...
&НаСервере
Процедура УстановитьГруппировкиДинмическогоСпискаСогласноОтбору()
	...
	Если ЭтаФорма.Параметры.Свойство("Отбор") //Форма еще не открыта
		...
		Если НаименованиеГруппировки = ""
			Тогда
			УстановитьГруппировкиДинмическогоСписка(СписокСтатусов, СписокВладельцев);
		КонецЕсли;
	Иначе
		УстановитьГруппировкиДинмическогоСписка(СписокСтатусов, СписокВладельцев);
	КонецЕсли;
...
//
&НаСервере
Процедура УстановитьГруппировкиДинмическогоСписка(СтатусПартии = Неопределено, ВладельцыПартии = Неопределено)
	...
	Если УровниГруппировок.КоличествоВладельцевПартии > 1 И УровниГруппировок.КоличествоСтатусов > 1
		...
		Элементы.Наименование.Заголовок = "Статус / Владелец партии / Партия";
	...
	ИначеЕсли УровниГруппировок.КоличествоВладельцевПартии = 1 И УровниГруппировок.КоличествоСтатусов > 1
		...
		Элементы.Наименование.Заголовок = "Статус / Партия";
	...
	ИначеЕсли УровниГруппировок.КоличествоВладельцевПартии > 1 И УровниГруппировок.КоличествоСтатусов = 1
		...
		Элементы.ВладелецПартии.Видимость = Ложь;
		Элементы.Наименование.Заголовок = "Владелец партии / Партия";
...
// Добавляет поле отбора динамического списка для дполнительного реквизита на форму.
//
&НаСервере
Процедура ДобавитьПолеОтбораДополнительногоРеквизита(ИмяСвойства, ЗаголовокПоля, ОписаниеТиповПоля, НаименованиеСвойства, СвойствоОбъекта = Неопределено) //
	...
	Если ОписаниеТиповПоля = ОписаниеТиповЧ
		...
		ИмяПоляНачалоИнтревала = ИмяПоля+"_НачИнт"; //Начало интревала
		
		НовыеРеквизитыФормы = Новый Массив;
		НовыйРеквизит = Новый РеквизитФормы(ИмяПоляНачалоИнтревала, ОписаниеТиповПоля, , ЗаголовокПоля, Истина);
		НовыеРеквизитыФормы.Добавить(НовыйРеквизит);
		ИзменитьРеквизиты(НовыеРеквизитыФормы);
		
		НовоеПолеФормы = ЭтаФорма.Элементы.Добавить(ИмяПоляНачалоИнтревала, Тип("ПолеФормы"),НоваяГруппаФормы);
		НовоеПолеФормы.ПутьКДанным = ИмяПоляНачалоИнтревала;
		НовоеПолеФормы.Вид = ВидПоляФормы.ПолеВвода;
		НовоеПолеФормы.РастягиватьПоГоризонтали = Истина;
		НовоеПолеФормы.Заголовок = ЗаголовокПоля + " от";
		НовоеПолеФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
		
		Элементы[ИмяПоляНачалоИнтревала].УстановитьДействие("ПриИзменении","Подключаемый_ПриИзмененииДопРеквизитаСТипомЧислоНачалоИнтервала");
		
		ДобавитьПараметрОтбораВСписок(НаименованиеСвойства, ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
		
		ИмяПоляОкончаниеИнтервала = ИмяПоля+"_КонИнт"; // Окончание интревала
		...
		НовоеПолеФормы.Вид = ВидПоляФормы.ПолеВвода;
		НовоеПолеФормы.РастягиватьПоГоризонтали = Истина;
		НовоеПолеФормы.Заголовок = "до";
		НовоеПолеФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
		
		Элементы[ИмяПоляОкончаниеИнтервала].УстановитьДействие("ПриИзменении","Подключаемый_ПриИзмененииДопРеквизитаСТипомЧислоОкончаниеИнтервала");
	...
	Иначе
		...
		Если ОписаниеТиповБулево = ОписаниеТиповПоля
			...
			НовоеПолеФормы.СписокВыбора.Добавить(0,"Все");
			НовоеПолеФормы.СписокВыбора.Добавить(1,"Да");
			НовоеПолеФормы.СписокВыбора.Добавить(2,"Нет");
		...
		ИначеЕсли ОписаниеТиповДопЗначение = ОписаниеТиповПоля Или ОписаниеТиповДопЗначениеИерархия = ОписаниеТиповПоля
			...
			РеквизитДляСвязи = Новый РеквизитФормы("РевизитДляСвязи_"+ИмяПоля, ОписаниеТиповДопРеквизиты, , ЗаголовокПоля, Истина); 
			НовыеРеквизитыФормы.Добавить(НовыйРеквизит);
			...
			ЭтаФорма["РевизитДляСвязи_"+ИмяПоля] = СвойствоОбъекта.Ссылка;
			
			ПараметрСвязи = Новый СвязьПараметраВыбора("Отбор.Владелец", "РевизитДляСвязи_"+ИмяПоля);
			...
			ДобавитьПараметрОтбораВСписок(НаименованиеСвойства, ?(ОписаниеТиповС = ОписаниеТиповПоля, ВидСравненияКомпоновкиДанных.Содержит, ВидСравненияКомпоновкиДанных.ВСписке));
...
Процедура УстановитьОтборДинамическогоСписка(ЗаголовокПоля, ПравоеЗначение = Неопределено, ИспользованиеОтбора = Истина, ВидСравненияОтбора, УстановитьМетку = Истина, ЭтоДопРеквизит = Истина, ИмяПоляОтбора = "")
	...
	ИмяРеквизитаПоля = СтрЗаменить(ЗаголовокПоля,"ДопРекв_","");
