...
Функция ПолучитьДанныеТарифа()
	...
	Если Объект.ДатаНачалаОсуществленияДеятельности < Дата('20190701') Тогда
		ДействуетС = Дата('20150701');
	Иначе
		ДействуетС = Дата('20190701');
	КонецЕсли;
	
	ОтборТарифа = Новый Структура("ВидТорговойДеятельности, ЗонаТарифа, ДействуетС",КодВидаТорговойДеятельности , ЗонаТарифа, ДействуетС);
