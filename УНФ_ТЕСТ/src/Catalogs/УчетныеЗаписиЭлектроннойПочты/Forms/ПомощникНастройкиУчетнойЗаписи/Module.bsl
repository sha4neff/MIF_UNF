...
Процедура СоздатьУчетнуюЗапись()
	...
	Попытка
		...
		// УНФ
		УстановитьОсновнуюУчетнуюЗапись(УчетнаяЗаписьСсылка);
		// Конец УНФ
...
&НаСервереБезКонтекста
Процедура УстановитьОсновнуюУчетнуюЗапись(УчетнаяЗапись) Экспорт
	...
	Если ЗначениеЗаполнено(
		УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ОсновнаяУчетнаяЗаписьЭлектроннойПочты")) Тогда
		Возврат;
	КонецЕсли;
