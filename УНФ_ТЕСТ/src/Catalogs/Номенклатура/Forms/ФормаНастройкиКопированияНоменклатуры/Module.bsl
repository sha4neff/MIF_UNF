...
Процедура УправлениеФормой(Форма, ИмяСписка = "")
	...
	Если ПустаяСтрока(ИмяСписка) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаЕдиницыИзмерения", "Видимость", Форма.ЕстьЕдиницыИзмеренияВОбъектеКопирования);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаХарактеристики", "Видимость", Форма.ЕстьИндивидуальныеХарактеристикиВОбъектеКопирования);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСпецификации", "Видимость", Форма.ЕстьСпецификацииВОбъектеКопирования);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСоставНаборов", "Видимость", Форма.ЕстьСоставыНаборовВОбъектеКопирования);
	КонецЕсли; 
	
	Если ПустаяСтрока(ИмяСписка) ИЛИ ИмяСписка="ЕдиницыИзмерения" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЕдиницыИзмеренияПометка", "ТолькоПросмотр", Форма.РежимКопированияЕдиницИзмерения<>2);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЕдиницыИзмерения", "Доступность", Форма.РежимКопированияЕдиницИзмерения<>0);
	КонецЕсли;
	Если ПустаяСтрока(ИмяСписка) ИЛИ ИмяСписка="Характеристики" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ХарактеристикиПометка", "ТолькоПросмотр", Форма.РежимКопированияХарактеристик<>2);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Характеристики", "Доступность", Форма.РежимКопированияХарактеристик<>0);
	КонецЕсли; 
	Если ПустаяСтрока(ИмяСписка) ИЛИ ИмяСписка="Спецификации" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СпецификацииПометка", "ТолькоПросмотр", Форма.РежимКопированияСпецификаций<>2);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Спецификации", "Доступность", Форма.РежимКопированияСпецификаций<>0);
	КонецЕсли; 
	Если ПустаяСтрока(ИмяСписка) ИЛИ ИмяСписка="СоставНаборов" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставНаборовПометка", "ТолькоПросмотр", Форма.РежимКопированияСоставаНаборов<>2);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставНаборов", "Доступность", Форма.РежимКопированияСоставаНаборов<>0);
	КонецЕсли; 
...
Процедура ЗаполнитьСписки()
	...
	СоставНаборов.Загрузить(Результат.Получить(3).Выгрузить());
...
Процедура ПроверитьПереносСвязанныхДанных(ВыводитьСообщения = Ложь)
	...
	// Часть спецификаций и состава наборов может быть недоступна для копирования, если не копируется характеристика-владелец
	Если РежимКопированияХарактеристик=0 Тогда
	...
	ИначеЕсли РежимКопированияХарактеристик=2 Тогда
		...
		Если НекопируемыеЗначения.Количество()=0 Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		Если ЕстьСпецификацииВОбъектеКопирования И РежимКопированияСпецификаций>0 И НекопируемыеЗначения.Количество()>0 Тогда
			Если РежимКопированияСпецификаций=1 Тогда
				РежимКопированияСпецификаций = 2;
				ТребуетсяОбновлениеСпецификаций = Истина;
			КонецЕсли; 
			Для каждого СтрокаТабличнойЧасти Из Спецификации Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Характеристика) ИЛИ НекопируемыеЗначения.Найти(СтрокаТабличнойЧасти.Характеристика)=Неопределено Тогда
					СтрокаТабличнойЧасти.Недоступно = Ложь;
					Продолжить;
				КонецЕсли; 
				СнятьПометку("Спецификации", СтрокаТабличнойЧасти, ВыводитьСообщения);
			КонецЦикла;
		КонецЕсли;
		Если ЕстьСоставыНаборовВОбъектеКопирования И РежимКопированияСоставаНаборов>0 И НекопируемыеЗначения.Количество()>0 Тогда
			Если РежимКопированияСоставаНаборов=1 Тогда
				РежимКопированияСоставаНаборов = 2;
				ТребуетсяОбновлениеСоставаНаборов = Истина;
			КонецЕсли; 
			Для каждого СтрокаТабличнойЧасти Из СоставНаборов Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Значение) ИЛИ НекопируемыеЗначения.Найти(СтрокаТабличнойЧасти.Значение)=Неопределено Тогда
					СтрокаТабличнойЧасти.Недоступно = Ложь;
					Продолжить;
				КонецЕсли; 
				СнятьПометку("СоставНаборов", СтрокаТабличнойЧасти, ВыводитьСообщения);
...
Процедура СнятьПометку(ИмяТаблицы, СтрокаТабличнойЧасти, ВыводитьСообщения)
	...
	Если СтрокаТабличнойЧасти.Пометка Тогда
		...
		Если ВыводитьСообщения Тогда
			...
			ИмяТаблицы+"["+ЭтотОбъект[ИмяТаблицы].Индекс(СтрокаТабличнойЧасти)+"].Пометка");
		КонецЕсли; 
	КонецЕсли; 
