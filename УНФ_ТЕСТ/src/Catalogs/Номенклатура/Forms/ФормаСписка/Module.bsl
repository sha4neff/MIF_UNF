...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ОтборВидПродукцииИС") Тогда
		...
		Для Каждого ВидПродукцииИС Из Параметры.ОтборВидПродукцииИС Цикл
			
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаВидыПродукции, "ВидПродукцииИС", ВидПродукцииИС, ВидСравненияКомпоновкиДанных.Равно, НСтр("ru='Вид продукции ИС'"), Истина);
	...
	УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(СписокЗапасы, 
			"ПодконтрольнаяПродукцияВЕТИС", Параметры.ПодконтрольнаяПродукцияВЕТИС, Истина, ВидСравненияКомпоновкиДанных.Равно);
	КонецЕсли;
	...
	ДобавлениеИзменениеПодсистемыЗакупки = РольДоступна("ДобавлениеИзменениеПодсистемыЗакупки");
	ДобавлениеИзменениеПодсистемыПродажи = РольДоступна("ДобавлениеИзменениеПодсистемыПродажи");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПереходНаНовуюФорму Тогда
		ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаИнформацияОПереходеНаСтаруюФорму", Новый Структура, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура СписокЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "КартинкаКорзина" Или (ЭтоПодбор И Не Поле.Имя = "КартинкаОстатки") Тогда
		СтандартнаяОбработка = Ложь;
		ДобавитьВКорзинуСписок(Истина);
		Возврат;
	ИначеЕсли Поле.Имя = "КартинкаОстатки" Тогда
		СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура ВернутьсяНаСтаруюФормуПодбора(Команда)
	
	Режим = РежимДиалогаВопрос.ДаНет;
	
	ПараметрыОповещения = Новый Структура();
	
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаВернутьсяНаСтаруюФорму", ЭтотОбъект, ПараметрыОповещения);
	ПоказатьВопрос(Оповещение, НСтр("ru='В следующих релизах старая форма подбора будет заменена на текущую. Продолжить?'"), Режим, 0);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияВопросаВернутьсяНаСтаруюФорму(Результат, ПараметрыОповещения) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		УправлениеНебольшойФирмойСервер.УстановитьЗначениеКонстанты("ИспользоватьНовуюФормуПодбораНоменклатуры", Ложь);
		
		Оповестить("СменитьФормуПобора", Ложь, 
...
Процедура ВосстановитьНастройкиФормы(СтруктураНастроек, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураНастроек.НастройкиПоиска = 0 И НастройкиПоиска = 1 Тогда
...
Процедура ЗаполнитьСопутствующиеТоварыДляДерева(Ссылка)
	...
	|			И СопутствующиеТовары.ХарактеристикаCопутствующегоТовара = ОстаткиТоваров.Характеристика
	|ГДЕ
...
Процедура УстановитьОтборПоТипуНоменклатуры()
	...
	ОбновитьЗаголовокФильтров(Ложь);
...
Процедура СохранитьНастройки(ИзФормы = Ложь)
	...
	СтруктураПоиска = Новый Структура("Элемент");
	
	ИмяКлючаОбъекта = "СписокНоменклатураОбщиеНастройки";
	
	ХранилищеСистемныхНастроек.Сохранить(ИмяКлючаОбъекта,
	ИмяКлючаОбъекта+"_" + "ИнформацияПоНоменклатуре", Элементы.ИнформацияПоНоменклатуреОбщаяГруппа.Скрыта());
	
	ХранилищеСистемныхНастроек.Сохранить(ИмяКлючаОбъекта,
	ИмяКлючаОбъекта+"_" + "ГруппаОтборы", Элементы.Отборы.Скрыта());
	
	ХранилищеСистемныхНастроек.Сохранить(ИмяКлючаОбъекта,
	ИмяКлючаОбъекта+"_" + "ПраваяПанельСвернута", Элементы.ДекорацияРазвернутьОтборы.Видимость);
	...
	Если РежимВыбора Тогда
		...
		|ПоискВХарактеристиках,ПоискВШтрихКодах,ПоискВДопСведениях,ОстаткиВидимость,ИсторияПоискаДерево,ИсторияПоискаСписок,ПодбиратьВесьОстаток" + ДополнениеСтрокиНастроек;
	ИначеЕсли ЭтоПодбор Тогда
		...
		|ПоискВХарактеристиках,ПоискВШтрихКодах,ПоискВДопСведениях,ИсторияПоискаДерево,ИсторияПоискаСписок,ПодбиратьВесьОстаток";
	Иначе
		СтрокаНастроек = "ПоказыватьЦены,ПоказыватьОстатки,ЗапрашиватьКоличество,ЗапрашиватьЦену,БыстрыеТовары,СравниватьНоменклатуруПоСвойствам,ПоказыватьНеПрямыеАналоги,
		...
		|ПоискВХарактеристиках,ПоискВШтрихКодах,ПоискВДопСведениях,ОстаткиВидимость,ИсторияПоискаДерево,ИсторияПоискаСписок,ПодбиратьВесьОстаток";
	КонецЕсли;
	...
	Для Каждого Элемент Из МассивНастроек Цикл
		...
		СтруктураПоиска.Элемент = Элемент;
		НайденнаяНастройка = ЗначенияНастроек.НайтиСтроки(СтруктураПоиска);
		
		Если Не НайденнаяНастройка.Количество() Тогда
			ХранилищеСистемныхНастроек.Сохранить(ИмяКлючаОбъекта,
			ИмяКлючаОбъекта+"_" + Элемент, ЭтаФорма[Элемент]);
			Продолжить
		КонецЕсли;
			
		Если НайденнаяНастройка[0].Значение = ЭтаФорма[Элемент] Тогда Продолжить КонецЕсли;
	...
	Для каждого Элемент Из МассивНастроек Цикл
		...
		Если Не НайденнаяНастройка.Количество() Тогда
			ХранилищеСистемныхНастроек.Сохранить(ИмяКлючаОбъектаФормаВыбора,
			ИмяКлючаОбъектаФормаВыбора+"_" + Элемент, ЭтаФорма[Элемент]);
			Продолжить
		КонецЕсли;
...
Процедура ВосстановитьНастройки()
	...
	|ИсторияПоискаДерево,ИсторияПоискаСписок";
	...
	ГруппыИнформацияПоНоменклатуреСкрыта = ХранилищеСистемныхНастроек.Загрузить(ИмяКлючаОбъекта, ИмяКлючаОбъекта+"_" + "ИнформацияПоНоменклатуре");
	...
	ГруппыОтборыСкрыта = ХранилищеСистемныхНастроек.Загрузить(ИмяКлючаОбъекта, ИмяКлючаОбъекта+"_" + "ГруппаОтборы");
	Если ГруппыОтборыСкрыта = Неопределено Или ГруппыОтборыСкрыта Тогда
		Элементы.Отборы.Скрыть();
	КонецЕсли;
	
	ПоложениеПравойПанелиСвернута = ХранилищеСистемныхНастроек.Загрузить(ИмяКлючаОбъекта, ИмяКлючаОбъекта+"_" + "ПраваяПанельСвернута");
	
	Если РежимВыбора Тогда
		СтрокаНастроек = "ПоказыватьЦены,ПоказыватьОстатки,ОтборОстатки,ОтборОрганизация,ОтборСклад,ДействиеПриПоискеПоШК,НастройкиПоиска,
		...
		|ИсторияПоискаДерево,ИсторияПоискаСписок,ПодбиратьВесьОстаток";
	...
	Для каждого Элемент Из МассивНастроек Цикл
		...
		Если Значение=Неопределено Тогда
			...
			Если Элемент = "ПоказыватьЦены" Тогда ЭтаФорма[Элемент] = Истина КонецЕсли;
	...
	МассивНастроек = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаНастроек, , Истина, Истина);
	Для Каждого Элемент Из МассивНастроек Цикл
		Значение = ХранилищеСистемныхНастроек.Загрузить(ИмяКлючаОбъектаФормаВыбора, ИмяКлючаОбъектаФормаВыбора+"_" + Элемент);
		Если Значение=Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ЭтаФорма[Элемент] = Значение;
	...
	// Удалить после окончательного перехода
	Если ЭтоПодбор Тогда
		ПереходНаНовуюФорму = ПоказыватьИнформациюОПереходеНаСтаруюФорму();
		Если ПереходНаНовуюФорму Тогда
			ХранилищеСистемныхНастроек.Сохранить("ПереходНаНовуюФормуПодбора", "ПереходНаНовуюФормуПодбора", Ложь);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьНоменклатуруВКорзину(СтруктураВыбора)
	...
	Если РазныеВалюты Тогда
		...
		Иначе
			ДатаПересчета = УправлениеНебольшойФирмойВызовСервера.ДатаСеанса();
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Подбор товаров'")
		,
		,
		НСтр("ru = 'Товар "+СтруктураВыбора.Номенклатура+","+СтруктураВыбора.Характеристика+" добавлен в корзину'"), БиблиотекаКартинок.ТележкаМаленькая);
...
&НаКлиенте
Процедура ДобавитьСтрокуВКорзину(ВыделеннаяСтрока, Склад, СтрокаДобавлена = Истина)
	...
	Если Не ЗначениеЗаполнено(Склад) Тогда
		...
		Если ТекущаяСтраницаНоменклатуры = "СписокЗапасы" И ПоказыватьОстатки 
...
Процедура ДобавитьВКорзинуСписок(ДобавитьВКорзину = Истина, ИмяТаблицыДобавления = "")
	...
	МассивНоменклатурыКДобавлению = Новый Массив;
...
Процедура ДобавитьУдалитьСтрокиИзКорзины(ДобавитьВКорзину = Неопределено, СписокВладельцев = Неопределено, ИмяТаблицыДобавления = Неопределено)
	...
	Если СписокСтрокДобавленияВКорзину.Количество() Тогда
		...
		ДобавитьВКорзину(МассивСтрок, Склад);
...
&НаСервере
Функция ПоказыватьИнформациюОПереходеНаСтаруюФорму()
	
	Значение = Ложь;
	
	Если ЭтоПодбор Тогда
		
		СтрокаНастроек = "ПереходНаНовуюФормуПодбора";
		ИмяКлючаОбъекта = "ПереходНаНовуюФормуПодбора";
		МассивНастроек = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаНастроек, , Истина, Истина);
		
		Значение = ХранилищеСистемныхНастроек.Загрузить(ИмяКлючаОбъекта, ИмяКлючаОбъекта);
...
Процедура УстановитьНастройкиФормыПриСоздании(Параметры)
	...
	ДоступныПродажи = Пользователи.ЭтоПолноправныйПользователь() Или РольДоступна("ДобавлениеИзменениеПодсистемыПродажи");
...
Процедура УстановитьВидимостьИДоступностьПриСоздании()
	...
	Если Параметры.Свойство("ПоказыватьКолонкуРезерв") Тогда
...
Процедура УстановитьПараметрыЗапросовСписков()
	...
	Если ОтборОстатки = 1 И ЗначениеЗаполнено(ОтборСклад) Тогда
		...
		УстановитьПараметрСписка(СписокЗапасы, "Склад", ОтборСклад);
	Иначе
	...
	Если ОтборОстатки = 1 И ЗначениеЗаполнено(ОтборОрганизация) Тогда
		...
		УстановитьПараметрСписка(СписокЗапасы, "Организация", ОтборОрганизация);
	Иначе
...
Процедура УстановитьВидимостьИДоступность()
	...
	Если ЭтоПодбор Тогда
		...
		Если НеПоказыватьИнформациюПоСкладуИРезервам  Тогда
...
Процедура ПереопределитьТекстЗапросаСписокЗапасы()
	...
	Если ОтборПоЦенам И ЦенаВСписке = 0 Тогда //Медленный способ с двумя левыми соединениями
		...
		Иначе
			...
			"ПО СправочникНоменклатура.Ссылка = НаличиеФайлов.ОбъектСФайлами",
			"ПО СправочникНоменклатура.Ссылка = НаличиеФайлов.ОбъектСФайлами
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(СрезПоследних(&ДатаСреза, Актуальность И ВидЦен = &ВидЦен И Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяССылка)) КАК ЦеныНоменклатурыСрезПоследних
			|		ПО ЦеныНоменклатурыСрезПоследних.Номенклатура = СправочникНоменклатура.Ссылка");
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьЗаголовокФильтров(СохранятьНастройки = Истина)
	
	Если СохранятьНастройки И Не Элементы.СписокЗапасы.РежимВыбора И Не ЭтоПодбор Тогда 
		СохранитьНастройки() 
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьДанныеПоНоменклатуреСервер(Номенклатура, Характертистика = Неопределено, ЕдиницаИзмерения = Неопределено)
...
Процедура ОбновитьПанельОстатков(Номенклатура, ЕдиницаИзмерения)
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ОстаткиТоваров.Организация = &Организация", "");
	КонецЕсли;
...
Процедура СписокЗапасыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Если ПоказыватьЦены Тогда
		ВывестиЦены(Строки, Ссылки, ВидЦен, ПоказыватьЦены, ДатаСреза, РаботаСЗакупками);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ВывестиЦены(Строки, Ссылки, ВидЦен, ПоказыватьЦены, ДатаСреза, РаботаСЗакупками = Ложь)
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковПолный(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|	И ВЫБОР
	|			КОГДА &ОтборПоОрганизации
	|				ТОГДА ОстаткиТоваров.Организация = &Организация
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &ОтборПоСкладу
	|				ТОГДА ОстаткиТоваров.СтруктурнаяЕдиница = &СтруктурнаяЕдиница
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ТИПЗНАЧЕНИЯ(ОстаткиТоваров.СтруктурнаяЕдиница) = ТИП(Справочник.СтруктурныеЕдиницы)
	|
	...
	|				ВЫБОР
	|						КОГДА &ОтборПоОрганизации
	|							ТОГДА ГрафикДвиженияЗапасов.Организация = &Организация
	|						ИНАЧЕ ИСТИНА
	|					КОНЕЦ
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТВывод.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ""<характеристика не указана>""
	|		ИНАЧЕ ВТВывод.Характеристика
	|	КОНЕЦ КАК ХарактеристикаПоказатель,
	|	ВТВывод.Характеристика.Артикул КАК Артикул,
	|	ВЫБОР
	|		КОГДА ВТВывод.Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|			ТОГДА ""<партия не указана>""
	|		ИНАЧЕ ВТВывод.Партия
	...
	|		ПО ВТВывод.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
	|			И ВТВывод.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|ГДЕ
	|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) >= &ЦенаОт
	|	И ВЫБОР
	|			КОГДА &ОтборПоЦенам
	|				ТОГДА ВЫБОР
	|						КОГДА &ЦенаДо > 0
	...
	|ПО
	|	ХарактеристикаПоказатель,
	|	ПартияПоказатель, СтруктурнаяЕдиница";
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковБезПартий(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|		ИНАЧЕ ВТВывод.Характеристика
	|	КОНЕЦ КАК ХарактеристикаПоказатель,
	|	ВТВывод.Характеристика.Артикул КАК ХарактеристикаАртикул,
	|	ВТВывод.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	&ДопПоле1,
	|	&ДопПоле2,
	|	&ДопПоле3 КАК ДопПоле3,
	...
	|	МАКСИМУМ(КоличествоДокументовЗаказано)
	|ПО
	|	ХарактеристикаПоказатель, СтруктурнаяЕдиница";
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковБезХарактеристик(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|					И Актуальность) КАК ЦеныНоменклатурыСрезПоследних
	|		ПО ВТВывод.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЕстьОстаток УБЫВ,
	|	ПартияПоказатель,
...
Функция ДополнитьТекстЗапросаИнформациейПоЯчейкам(ТекстЗапроса, ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры)
	...
	ПодстрокаПоискаСтрокиЗамены = ?(Не ИспользуютсяХарактеристикиНоменклатуры, "ВТВывод.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура", "И ВТВывод.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура");
...
Процедура ВывестиДеревоОстатков(Ссылка, ДеревоРезультат, ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры
	...
	Если ИспользуютсяХарактеристикиНоменклатуры И Не ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ДеревоРезультат.Следующий() Цикл
			...
			ВыборкаПоСкладам = ?(УчетПоЯчейкам, ДеревоРезультат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам),ДеревоРезультат.Выбрать());
	...
	ИначеЕсли ИспользуютсяХарактеристикиНоменклатуры И ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ДеревоРезультат.Следующий() Цикл
			...
			Пока ВыборкаПоПартиям.Следующий() Цикл
				...
				ВыборкаПоСкладам = ?(УчетПоЯчейкам, ВыборкаПоПартиям.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам),ВыборкаПоПартиям.Выбрать());
...
Функция СписокДокументовПоНоменклатуре(ТипОперации, СтруткураПараметров)
	...
	Запрос.УстановитьПараметр("ИспользуютсяХарактеристики", СтруткураПараметров.Номенклатура.ИспользоватьХарактеристики И СтруткураПараметров.ОтборПоХарактеристике);
...
&НаКлиенте
Процедура КартинкаСвободныеОстаткиСвернутьНажатие(Элемент)
	Элементы.ГруппаОстаткиДинСписка.Видимость = Ложь;
	Элементы.КартинкаСвободныеОстаткиРазвернуть.Видимость = Истина;
	Элементы.КартинкаСвободныеОстаткиСвернуть.Видимость = Ложь;
КонецПроцедуры
...
Процедура ПоказатьСписокВыбораДокументов(Элемент)
	...
	Если ПолныеПрава Тогда
		...
		СписокДокументов.Добавить("СчетНаОплату", "Счет на оплату",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		СписокДокументов.Добавить("ЗаказПокупателя", "Заказ покупателя",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		
		Если ИспользоватьПодсистемуРаботы Тогда
			СписокДокументов.Добавить("ЗаказНаряд", "Заказ-наряд",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		КонецЕсли;
		
		СписокДокументов.Добавить("РасходнаяНакладная", "Расходная накладная",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		СписокДокументов.Добавить("АктВыполненныхРабот", "Акт выполненных работ",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		
		Если УчетРозничныхПродаж Тогда
			СписокДокументов.Добавить("ЧекККМ", "Чек ККМ",,БиблиотекаКартинок.ВидБизнесаРозничнаяТорговля);
		КонецЕсли;
		
		СписокДокументов.Добавить("ЗаказПоставщику", "Заказ поставщику",,БиблиотекаКартинок.РазделЗакупки);
		СписокДокументов.Добавить("ПриходнаяНакладная", "Приходная накладная",,БиблиотекаКартинок.РазделЗакупки);
		
		ПоказатьВыборИзМеню(Оповещение, СписокДокументов, Элемент);
		Возврат;
	КонецЕсли;
	
	Если ДобавлениеИзменениеПодсистемыЗакупки Тогда
		СписокДокументов.Добавить("ЗаказПоставщику", "Заказ поставщику",,БиблиотекаКартинок.РазделЗакупки);
		СписокДокументов.Добавить("ПриходнаяНакладная", "Приходная накладная",,БиблиотекаКартинок.РазделЗакупки);
	КонецЕсли;
	
	Если ДобавлениеИзменениеПодсистемыПродажи Тогда
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаКорзины, ПараметрыРасчета = Неопределено)
	...
	УправлениеНебольшойФирмойКлиент.РассчитатьСуммыВСтрокеТЧ(СтрокаКорзины, ПараметрыРасчета);
...
Процедура ПоказатьВСписке(Команда)
	...
	Если ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.КатегорииНоменклатуры") Тогда
		Если НЕ Элементы.ОтборКатегории.ТекущиеДанные.Представление = НСтр("ru = '<Все категории>'") Тогда
			ПоказатьВСпискеСервер(Ссылка, "КатегорииНоменклатуры");
		КонецЕсли;
	Иначе
		Если НЕ Элементы.ОтборИерархия.ТекущиеДанные.Представление = НСтр("ru = '<Все группы>'") Тогда
...
Функция ПолучитьДанныеПоШтрихкодамВМассиве(Штрихкоды, СтруктурнаяЕдиница = Неопределено)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерийныеНомера КАК СерийныеНомера
	|		ПО (&СерийныеНомераИспользуются)
	|			И КодыТоваровSKU.Номенклатура = СерийныеНомера.Владелец
	|			И (КодыТоваровSKU.Номенклатура.ИспользоватьСерийныеНомера)
...
Процедура ПоискПоПодстроке()
	...
	Если ПустаяСтрока(СтрокаРасширенногоПоиска) Тогда
		...
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(ЭтаФорма[ТекущаяСтраницаНоменклатуры], 
		"Номенклатура", Неопределено, Ложь, ВидСравненияКомпоновкиДанных.ВСписке);
	...
	УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(ЭтаФорма[ТекущаяСтраницаНоменклатуры], 
	"Номенклатура", РезультатПоиска, Истина, ВидСравненияКомпоновкиДанных.ВСписке);
...
Процедура ПолнотекстовыйПоискНаКлиенте()
	...
	Иначе
		...
		Если Не ПодборПоискНеУдачный Тогда
			...
			УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(ЭтаФорма[ТекущаяСтраницаНоменклатуры], 
			"Номенклатура", ?(Использование, РезультатПоиска.Номенклатура, Неопределено), Использование, ВидСравненияКомпоновкиДанных.ВСписке);
