...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "КатегорияНоменклатуры, ТипНоменклатуры, ИспользоватьХарактеристики");
	...
	Если Параметры.Свойство("КопироватьЕдиницыИзмерения", КопироватьЕдиницыИзмерения)
		...
		Если КопироватьЕдиницыИзмерения Тогда
			ОбъектыСтрокой = ОбъектыСтрокой + ?(ПустаяСтрока(ОбъектыСтрокой), "", ", ") + НСтр("ru = 'единиц измерения'");
		КонецЕсли; 
		Если КопироватьХарактеристики Тогда
			ОбъектыСтрокой = ОбъектыСтрокой + ?(ПустаяСтрока(ОбъектыСтрокой), "", ", ") + НСтр("ru = 'характеристик'");
		КонецЕсли; 
		Если КопироватьСпецификации Тогда
			ОбъектыСтрокой = ОбъектыСтрокой + ?(ПустаяСтрока(ОбъектыСтрокой), "", ", ") + НСтр("ru = 'спецификаций'");
		КонецЕсли; 
		Если КопироватьСоставНаборов Тогда
			ОбъектыСтрокой = ОбъектыСтрокой + ?(ПустаяСтрока(ОбъектыСтрокой), "", ", ") + НСтр("ru = 'состава наборов'");
		КонецЕсли; 
...
Процедура ВыбранныеОбъектыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя="ВыбранныеОбъектыСообщение" Тогда
		...
		Если КопироватьЕдиницыИзмерения Тогда
			...
			ОткрытьФорму("Справочник.ЕдиницыИзмерения.ФормаСписка", ПараметрыОткрытия, ЭтотОбъект, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли КопироватьХарактеристики Тогда
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("СкрытьРеквизиты", Истина);
			ПараметрыОткрытия.Вставить("Отбор", Новый Структура);
			ПараметрыОткрытия.Отбор.Вставить("Владелец", СтрокаТабличнойЧасти.Значение);
			ОткрытьФорму("Справочник.ХарактеристикиНоменклатуры.ФормаСписка", ПараметрыОткрытия, ЭтотОбъект, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли КопироватьСпецификации Тогда
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("Отбор", Новый Структура);
			ПараметрыОткрытия.Отбор.Вставить("Владелец", СтрокаТабличнойЧасти.Значение);
			ОткрытьФорму("Справочник.Спецификации.ФормаСписка", ПараметрыОткрытия, ЭтотОбъект, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли КопироватьСоставНаборов Тогда
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("НоменклатураНабора", СтрокаТабличнойЧасти.Значение);
			ЗначенияРеквизитов = ЗначенияРеквизитов(СтрокаТабличнойЧасти.Значение, "СпособРасчетаЦеныНабора, ИспользоватьХарактеристики");
			...
			ОткрытьФорму("РегистрСведений.СоставНаборов.Форма.ФормаРедактированияСостава", ПараметрыОткрытия, ЭтотОбъект, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли; 
	КонецЕсли; 
...
Процедура Подбор(Команда)
	...
	ОткрытьФорму("Справочник.Номенклатура.ФормаВыбора", СтруктураОткрытия, ЭтотОбъект, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Копировать(Команда)
	...
	Для Каждого СтрокаТабличнойЧасти Из ВыбранныеОбъекты Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Значение) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ВыбранныеОбъекты[%1].Значение", НСтр("ru = 'Не выбран объект копирования'"), "ВыбранныеОбъекты.Значение", Индекс, НСтр("ru = 'Не выбран объект копирования в строке %1'"));
		КонецЕсли; 
	КонецЦикла;
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.ЕстьСвязьСХарактеристиками 
		И Форма.КопироватьСпецификации 
		...
		Если ТабПроверки.Количество()>1 Тогда
			...
			Для каждого СтрокаКопирования Из Значения Цикл
				...
				Если Прав(СтрокаКопирования.Представление, 1)=")" Тогда
					СтрокаКопирования.Представление = Лев(СтрокаКопирования.Представление, СтрДлина(СтрокаКопирования.Представление)-1)
					+СтрШаблон(", %1)", Строка(СтрокаКопирования.Характеристика));
				Иначе
					СтрокаКопирования.Представление = СтрокаКопирования.Представление+СтрШаблон(" (%1)", Строка(СтрокаКопирования.Характеристика));
				КонецЕсли; 
			КонецЦикла; 
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ЗаполнитьПараметрыВыбора()
	...
	ЕстьСвязьСХарактеристиками = (КопироватьСпецификации ИЛИ КопироватьСоставНаборов) И ИспользоватьХарактеристики;
	...
	Если ЗначениеЗаполнено(Номенклатура) И (ТребуетсяКопированиеХарактеристик ИЛИ КопироватьСпецификации) Тогда
	...
	Если НЕ КопироватьИзВыбранных И ИспользоватьХарактеристики И (КопироватьСпецификации ИЛИ КопироватьСоставНаборов) Тогда
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.ИспользоватьХарактеристики", Истина));
	КонецЕсли;
...
Процедура ВыполнитьКопированиеДанных()
	...
	Если КопироватьСпецификации ИЛИ КопироватьСоставНаборов Тогда
		МассивНоменклатуры = Новый Массив;
		Для каждого СтрокаТабличнойЧасти Из ВыбранныеОбъекты Цикл
			МассивНоменклатуры.Добавить(СтрокаТабличнойЧасти.Значение);
		КонецЦикла; 
	...
	Для каждого СтрокаТабличнойЧасти Из Значения Цикл
		...
		Если СтрокаТабличнойЧасти.ТипЗначения="СоставНаборов" Тогда
			КопироватьСоставНаборов(СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Характеристика, (СтрокаТабличнойЧасти.Значение=Неопределено), Владельцы, СоответствиеХарактеристик, КопироватьИзВыбранных);
		КонецЕсли; 
	КонецЦикла;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ru='Копирование связанной информации.'", УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Номенклатура, Номенклатура, ТекстСообщения);
...
Процедура ЗагрузитьНастройкиОтображенияПодсказок()
	...
	НастройкиОтображенияПодсказок = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить("КопированиеСвязаннойИнформации", "НастройкиОтображенияПодсказок", Новый Структура);
...
Процедура СохранитьНастройкиОтображенияПодсказок()
	...
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить("КопированиеСвязаннойИнформации", "НастройкиОтображенияПодсказок", НастройкиОтображенияПодсказок);
