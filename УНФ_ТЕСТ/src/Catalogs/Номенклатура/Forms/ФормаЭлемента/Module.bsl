...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// Наборы
	Если ИмяСобытия=НаборыКлиент.ИмяСобытияИзменениеСоставаНабора() 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	КонецЕсли; 
	Если Объект.ЭтоНабор И Наборы.Количество()>0 Тогда
		Объект.ЭтоНабор = Ложь;
	КонецЕсли; 
	// Конец Наборы
...
Процедура ТипНоменклатурыПриИзменении(Элемент)
	...
	Если ЭтоСертификат() Тогда
		...
		Объект.ИспользоватьСерийныеНомера = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьСерийныеНомера");
	КонецЕсли;
...
Процедура КатегорияНоменклатурыПриИзмененииНаСервере()
	...
	Если ФОУчетТабачнойПродукции И Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда
		Объект.ТабачнаяПродукция = Объект.КатегорияНоменклатуры.ТабачнаяПродукция;
	КонецЕсли;
	
	ФОУчетОбувнойПродукции = ИнтеграцияИСМПВызовСервера.ВестиУчетМаркируемойПродукции(Перечисления.ВидыПродукцииИС.Обувь);
	Если ФОУчетОбувнойПродукции И Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда
		Объект.ОбувнаяПродукция = Объект.КатегорияНоменклатуры.ОбувнаяПродукция;
	КонецЕсли;
...
Процедура ИспользоватьСерийныеНомераПриИзменении(Элемент)
	...
	// Подарочные сертификаты
	Если ЭтоСертификат() Тогда
		Если Не МожноИзменитьПараметрСертификата("ИспользоватьСерийныеНомера") Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьСоставНабора(Команда)
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("НоменклатураНабора", Объект.Ссылка);
	СтруктураПараметров.Вставить("СпособРасчетаЦеныНабора", Объект.СпособРасчетаЦеныНабора);
	СтруктураПараметров.Вставить("ИспользоватьХарактеристики", Объект.ИспользоватьХарактеристики);
	ОткрытьФорму("РегистрСведений.СоставНаборов.Форма.ФормаРедактированияСостава", СтруктураПараметров, ЭтотОбъект);
...
Процедура УправлениеФормой()
	...
	Элементы.ИспользоватьСерийныеНомера.Видимость = (ЭтоЗапас() ИЛИ ЭтоСертификат()) И НЕ Объект.ЭтоНабор;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаГабариты", 			"Видимость", 	ЭтоЗапас() ИЛИ Объект.ЭтоНабор);
	...
	Элементы.СпособПополнения.Видимость = ЭтоЗапас() И НЕ Объект.ЭтоНабор;
	УстановитьВидимостьПриИзмененииСпособаПополнения(ЭтотОбъект);
	Элементы.СтранаПроисхождения.Видимость = ЭтоЗапас();
	Элементы.СрокПополнения.Видимость = ЭтоЗапас() И НЕ Объект.ЭтоНабор;
	Элементы.НормаВремени.Видимость = ЭтоОперация() И НЕ Объект.ЭтоНабор;
	Элементы.ГарантийныйСрок.Видимость = ЭтоЗапас() И НЕ Объект.ЭтоНабор;
	Элементы.ВыписыватьГарантийныйТалон.Видимость = ЭтоЗапас() И НЕ Объект.ЭтоНабор;
	...
	// Наборы
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтоНабор",	"Доступность",	НЕ Объект.Весовой);
	
	Если ФОИспользуютсяНаборы Тогда
		ВходитВСоставыНаборов = (Наборы.Количество()>0);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНаборы",	    		"Видимость",     ЭтоЗапас() ИЛИ ЭтоРабота());
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтоНабор", 	    			"Видимость",	 НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИзменитьСоставНабора",	    "Доступность",   Объект.ЭтоНабор И ЗначениеЗаполнено(Объект.Ссылка));
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИзменитьСоставНабора",	    "Видимость",	 НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СпособРасчетаЦеныНабора",   "Доступность",	 РазрешеноРедактированиеЦен И Объект.ЭтоНабор И НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СпособРасчетаЦеныНабора",   "Видимость",	 НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВариантПечатиНабора", 	    "Доступность",	 Объект.ЭтоНабор И НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВариантПечатиНабора", 	    "Видимость",	 НЕ ВходитВСоставыНаборов);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Наборы", 	    			"Видимость",	 ВходитВСоставыНаборов);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНаборы",	    		"Видимость",     Ложь);
	КонецЕсли; 
	// Конец Наборы
	...
	Если ФОИспользуютсяСертификаты Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТипНоминала",					"Доступность", Объект.ИспользоватьСерийныеНомера);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЧастичноеПогашение",			"Доступность", Объект.ИспользоватьСерийныеНомера);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСрокПериод",				"Доступность", Объект.ИспользоватьСерийныеНомера);
...
Процедура ЭтоНаборПриИзменении(Элемент)
	...
	Если НЕ МожноИзменитьПризнакНабора() Тогда
		Возврат;
	КонецЕсли; 
	...
	НоменклатураВДокументахКлиент.ОбновитьЗаголовкиГрупп(ЭтаФорма, "НаборКомплект");
	
КонецПроцедуры

&НаСервере
Функция МожноИзменитьПризнакНабора()
...
&НаСервереБезКонтекста
Функция ЕстьСпецификацииПартииСерийныеНомера(Номенклатура)
	...
	СтрукутраВозврата.Вставить("ЕстьСпецификации", НЕ Результат[0].Пустой());
	СтрукутраВозврата.Вставить("ЕстьПартии", НЕ Результат[1].Пустой());
	СтрукутраВозврата.Вставить("ЕстьСерийныеНомера", НЕ Результат[2].Пустой());
	СтрукутраВозврата.Вставить("ЕстьАналоги", НЕ Результат[3].Пустой());
	СтрукутраВозврата.Вставить("ЕстьУправлениеЗапасами", НЕ Результат[4].Пустой());
...
Процедура ЗаполнитьЗначенияОбъектаПоУмолчанию()
	...
	Объект.ИспользоватьСерийныеНомера = (Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат"))
										И ПолучитьФункциональнуюОпцию("ИспользоватьСерийныеНомера");
...
Процедура ЗаполнитьРеквизитыПравДоступа()
	...
	РазрешенПросмотрПродаж = РольДоступна("ПолныеПрава") 
...
Процедура УстановитьНастройкуФормы(ЗначениеНастройки, ИмяНастройки)
	УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(ЗначениеНастройки, ИмяНастройки, Пользователи.ТекущийПользователь());
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыКопирования()
	...
	ЕстьСоставыНаборовВОбъектеКопирования = Не Результат.Получить(3).Пустой();
...
Процедура КопироватьДополнительныеДанныеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КопироватьСоставНаборов И НЕ Объект.ЭтоНабор Тогда
		...
		ЭтоНаборПриИзменении(Элементы.ИспользоватьХарактеристики);
	КонецЕсли; 
	
	СтруктураДанных = Новый Структура("ЕдиницыИзмерения, Характеристики, Спецификации, СоставНаборов");
