...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолныеПрава Тогда
		Элементы.ГруппаОфрмитьДокумент.Доступность = ДобавлениеИзменениеПодсистемыЗакупки Или ДобавлениеИзменениеПодсистемыПродажи;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокКнопкиСозданияДокумента()
	...
	Если Не ЗначениеЗаполнено(ТипДокументаПоУмолчанию) Тогда
		...
		Элементы.СоздатьДокумент.Заголовок = "Заказ покупателя";
	КонецЕсли;
	...
	Если ТипДокументаПоУмолчанию = "СчетНаОплату" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Счет на оплату";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "ЗаказНаряд" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Заказ-наряд";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "РасходнаяНакладная" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Расходная накладная";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "АктВыполненныхРабот" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Акт выполненных работ";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "ЗаказПоставщику" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Заказ поставщику";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "ПриходнаяНакладная" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Приходная накладная";
		Возврат;
	КонецЕсли;
	
	Если ТипДокументаПоУмолчанию = "ЧекККМ" Тогда
		Элементы.СоздатьДокумент.Заголовок = "Чек ККМ";
		Возврат;
	КонецЕсли;
