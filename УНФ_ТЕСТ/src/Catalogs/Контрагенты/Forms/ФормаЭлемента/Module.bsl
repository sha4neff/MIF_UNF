...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "УстановитьСвязь_Дубли" И (Источник <> Неопределено И Источник.ВладелецФормы = ЭтотОбъект) Тогда
		...
		Если ДанныеКонтактныхЛиц.Количество() > 1 ИЛИ ЗначениеЗаполнено(ДанныеКонтактныхЛиц[0].Наименование) Тогда
			НовоеКонтактноеЛицо = ДанныеКонтактныхЛиц.Добавить();
		Иначе
			НовоеКонтактноеЛицо = ДанныеКонтактныхЛиц[0];
		КонецЕсли;
...
Процедура НаименованиеКонтактногоЛица_ПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ДанныеКонтактныхЛиц[ИндексВыбранногоКонтактногоЛица].Наименование) Тогда
		ПоказатьСкрытьНадписьОДубляхНаименованияКЛ(ИндексВыбранногоКонтактногоЛица, Ложь);
	КонецЕсли;
...
Процедура УстановитьВидимостьИДоступностьМобильноеПриложениеИРМК()
	...
	ИначеЕсли УправлениеНебольшойФирмойУправлениеДоступомПовтИсп.ЕстьПрофильРабочееМестоКассира() Тогда
