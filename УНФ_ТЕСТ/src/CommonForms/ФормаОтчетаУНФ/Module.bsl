...
Процедура ПериодПриИзмененииСервер()
	...
	Если НастройкиОтчета.ПоказыватьГруппуСтрокиНаФормеОтчета Тогда
...
Процедура МодифицироватьСхему(СхемаКомпоновкиДанных)
	...
	Поле = ОтчетыУНФ.ПолеСхемы(СхемаКомпоновкиДанных, ИмяГруппировкиДинамическийПериод());
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		"ФормаОтчетаУНФ.УстановитьПараметрыИОтборыРасшифровки",
...
// При группировке "ГоризонтальнаяЕслиВозможно" когда горизонтально не влезает,
// платформа просто размещает элементы вертикально.
// Здесь реализован перенос на следующую строку только невлезающих элементов.
&НаСервере
