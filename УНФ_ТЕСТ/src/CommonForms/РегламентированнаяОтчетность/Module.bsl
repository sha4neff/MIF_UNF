			...
			Иначе
				...
				Если ЗначениеЗаполнено(КонтролирующийОрган) Тогда
					...
					Иначе
						...
						КонтекстЭДОКлиент.ПодтвердитьОтправкуОтчета(Ссылка, ОписаниеОповещения);
					КонецЕсли;
				Иначе
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если ТекущиеДанныеНаименованиеОтчета = "Движение средств по счету в банке за пределами РФ" Тогда
				...
				Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв2"
					И НЕ РеглОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФПредставляетсяВЭлектронномВиде() Тогда
					ПоказатьПредупреждение(, НСтр(
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Сообщение о наличии транспортных средств и (или) земельных участков"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.сооб.трансп.земля")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о выдаче справки о состоянии расчетов по налогам и сборам" Тогда
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(ПроверяемыйОбъект) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Представление = "Движение средств по счету в банке за пределами РФ" Тогда
				Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв2"
					И НЕ РеглОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФПредставляетсяВЭлектронномВиде() Тогда
					ПоказатьПредупреждение(, НСтр(
...
&НаСервере
Функция РеглОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФПредставляетсяВЭлектронномВиде()
	
	Возврат ТекущаяДатаСеанса() >= '20201215';
