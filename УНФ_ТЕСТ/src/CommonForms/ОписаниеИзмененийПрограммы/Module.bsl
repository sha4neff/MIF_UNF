...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//{kp-unf.ru Внимание - закомментированный блок перенесен ниже (см. вставку ниже по тексту процедуры)
	...
	//КонецЕсли;
	ОбновленияТиповойКонфигурацииНебыло=(ДокументОписаниеОбновлений.ВысотаТаблицы=0);
	//kp-unf.ru}
	...
	//{kp-unf.ru
	Если ДокументОписаниеОбновлений.ВысотаТаблицы = 0 Тогда
		Если ОбновленияТиповойКонфигурацииНебыло Тогда
			Текст = НСтр("ru = 'Конфигурация успешно обновлена'");
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Конфигурация успешно обновлена на версию %1'"), Метаданные.Версия);
		КонецЕсли;
		ДокументОписаниеОбновлений.Область("R1C1:R1C1").Текст = Текст;
