...
Процедура ИзменитьДобавитьСтрокуВКорзину(СтрокаСпискаТоваров, СтрокаХарактеристикПартий, СтрокаКорзины, Количество, Цена, СвободныйОстаток, ЕдИзмерения = Неопределено)
	...
	Иначе
		...
		Если ПересчитатьВВалюту Тогда
			...
			ПредварительнаяЦена = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(ПредварительнаяЦена, КурсВалютаВидаЦен, КурсВалютаДокумента, КратностьВалютаВидаЦен, КратностьВалютаДокумента);
...
Функция ЗаписатьПодборВХранилище() 
	...
	Если СтруктураНастроек.ЗапоминатьТекущуюИерархию Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
			...
			УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(КатегорияНоменклатуры, "ГруппаОтбора");
			
		Иначе
			
			УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(Неопределено, "ГруппаОтбора");
...
Процедура ОбновитьОтборПоГруппеДинамическихСписков(ПрименитьОтбор, ЭлементОтбора)
	...
	Если ПрименитьОтбор Тогда
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(СписокЗапасов, "Номенклатура.Родитель", ЭлементОтбора, Истина, ВидСравненияКомпоновкиДанных.ВИерархии);
			
		Иначе
			
			УправлениеНебольшойФирмойКлиентСервер.УдалитьЭлементОтбораСписка(СписокЗапасов, "Номенклатура.Родитель");
			УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(СписокЗапасов, "Номенклатура.КатегорияНоменклатуры", ЭлементОтбора, Истина, ВидСравненияКомпоновкиДанных.ВИерархии);
...
&НаСервере
Процедура СохранитьУслвоиеОтображенияПодсветкиПерехода()
	ХранилищеСистемныхНастроек.Сохранить("НеОтображатьПодсветкуПерехода", "НеОтображатьПодсветкуПерехода", Истина);
	ХранилищеСистемныхНастроек.Сохранить("ПереходНаНовуюФормуПодбора", "ПереходНаНовуюФормуПодбора", Истина);
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрокаНастроек = "НеОтображатьПодсветкуПерехода";
	ИмяКлючаОбъекта = "НеОтображатьПодсветкуПерехода";
	МассивНастроек = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаНастроек, , Истина, Истина);
	
	НеОтображатьПодсветкуПереходаНаНовуюФорму = Ложь;
	...
	Для каждого Элемент Из МассивНастроек Цикл
		...
		Если Значение=Неопределено Тогда
			Продолжить;
		КонецЕсли;
		НеОтображатьПодсветкуПереходаНаНовуюФорму = Значение;
	КонецЦикла;
	
	Если НеОтображатьПодсветкуПереходаНаНовуюФорму Тогда
		Элементы.ГруппаПерейтиНаНовуюФормуПодбора.ЦветФона = ЦветаСтиля.ЦветФонаФормы;
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияПерейтиНаНовуюФормуНажатие(Элемент)
	
	СохранитьУслвоиеОтображенияПодсветкиПерехода();
	
	Оповестить("СменитьФормуПобора", Истина, 
	?(УникальныйИдентификаторФормыВладельца = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"), Неопределено, УникальныйИдентификаторФормыВладельца));
	
	УправлениеНебольшойФирмойСервер.УстановитьЗначениеКонстанты("ИспользоватьНовуюФормуПодбораНоменклатуры", Истина);
