	...
	Для Каждого СтрокаДерева Из ДеревоСкидок.Строки Цикл
		...
		Иначе
			...
			Если ВсеУсловияВыполнены Тогда
				...
				ДанныеСкидки = ВходящееДеревоСкидок.ТаблицаСкидкиНаценки.НайтиСтроки(Новый Структура("КлючСвязи, СкидкаНаценка", КлючСвязи, СтрокаДерева.СкидкаНаценка));
				Если ДанныеСкидки.Количество() = 0 Тогда
					СтрокаДерева.НеПримениласьПоУсловиямСовместногоПрименения = Истина;
				Иначе
					СтрокаДерева.Комментарий = ДанныеСкидки[0].Комментарий;
				КонецЕсли;
