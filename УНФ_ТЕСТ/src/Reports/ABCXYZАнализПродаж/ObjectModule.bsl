...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиОтчета.ПоляСебестоимости = "ВаловаяПрибыль, Себестоимость";
	...
	Для Каждого ТекСтрокаРеквизит Из Форма.ПоляНастроек.ПолучитьЭлементы() Цикл
		...
		Для Каждого ТекСтрокаЭлемент Из ТекСтрокаРеквизит.Элементы Цикл
			...
			Если СтрНачинаетсяС(ТекСтрокаРеквизит.Поле, "Количество") Тогда
				// Для предотвращения переноса устанавливается заголовок с использованием нерастяжимых пробелов:
				ТекЭлемент.Заголовок = НСтр("ru = 'Количество периодов анализа'");
