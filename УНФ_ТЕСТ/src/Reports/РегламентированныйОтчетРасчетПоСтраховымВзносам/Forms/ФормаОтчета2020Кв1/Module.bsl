...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Если ВидПечати <> "ПоказатьРеестр" Тогда
		...
		ТекстСообщения = "";
		Если ТекДата() >= '20210101' Тогда
			
			Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
				НомКорр = ТабличныйДокумент.Области.НомерКорректировки.Значение;
			Иначе
				НомКорр = СтруктураДанныхТитульный.НомерКорректировки;
			КонецЕсли;
			
			ГодРасчета = Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
			
			Если ГодРасчета > 2020 Тогда
				
				ТекстСообщения = "Расчет по форме, утвержденной приказом ФНС России от 18.09.2019 № ММВ-7-11/470@,"
				+ " не представляется за периоды " + Формат(ГодРасчета, "ЧГ=") + " года";
				
			ИначеЕсли НомКорр = 0 Тогда
				
				РасчетЗа2020Год = КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = КонецГода('20201231');
				
				Если РасчетЗа2020Год Тогда
					
					ТекстСообщения = "С 01.01.2021 не представляется первичный расчет по форме, утвержденной"
					+ " приказом ФНС России от 18.09.2019 № ММВ-7-11/470@, за расчетный период 2020 год";
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	Возврат ОбъектОтчета.ВыполнитьДокументированиеОбъект2020();
