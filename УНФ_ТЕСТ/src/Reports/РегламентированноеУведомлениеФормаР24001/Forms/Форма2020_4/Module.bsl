...
Процедура ОбработкаАдреса(Область, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяОблКодРегиона) Тогда
		ОблКодРегиона = ПредставлениеУведомления.Области.Найти(ИмяОблКодРегиона);
		...
		КонецЕсли;
	КонецЕсли;
	ПараметрыОпределенияАдреса.Вставить("РегионНаименование", РегионНаименование);
	
	АдресВФорматеФИАС = РегламентированнаяОтчетностьКлиент.АдресВФорматеФИАСПоДаннымСтраницыРеглОтчета(
		ПредставлениеУведомления, ПараметрыОпределенияАдреса);
...
Процедура ЗаполнитьАдресВФорматеФИАСНаСтраницеРеглОтчета(СтраницаРаздела, ПараметрыЗаполнения)
	...
	СтруктураАдреса = РегламентированнаяОтчетность.АдресВФорматеФИАС(ПараметрыЗаполнения.Адрес, ДополнительныеПараметры);
