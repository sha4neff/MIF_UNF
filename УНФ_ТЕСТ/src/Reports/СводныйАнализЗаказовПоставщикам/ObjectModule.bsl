...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	ЗначениеПараметра = НастройкиСКД.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СостояниеПоступления"));
	Если ЗначениеПараметра.Использование И ЗначениеЗаполнено(ЗначениеПараметра.Значение) Тогда
		ЗначениеПоУмолчанию = ЗначениеПараметра.Значение;
	Иначе
		ЗначениеПоУмолчанию = "Все заказы";
	КонецЕсли; 
	...
	"Все заказы,Непоступившие,Поступившие частично,Не поступившие полностью,Поступившие полностью",
	ЗначениеПоУмолчанию);
	
	ЗначениеПараметра = НастройкиСКД.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СостояниеОплаты"));
	Если ЗначениеПараметра.Использование И ЗначениеЗаполнено(ЗначениеПараметра.Значение) Тогда
		ЗначениеПоУмолчанию = ЗначениеПараметра.Значение;
	Иначе
		ЗначениеПоУмолчанию = "Все заказы";
	...
	"Все заказы,Неоплаченные,Оплаченные частично,Без полной оплаты,Оплаченные полностью",
	ЗначениеПоУмолчанию);
