...
Процедура ДиаграммаНовыеПоПериодамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Новые лиды за период'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ДиаграммаПоМенеджерамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Лиды по менеджерам'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ОбновитьДиаграммуНовые()
	...
	Запрос.Текст = 
		...
		|	СУММА(ВЫБОР
		|			КОГДА Лиды.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец
		|				ТОГДА 1
		|			ИНАЧЕ 0
	...
	Если ОтборМенеджеры.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.Ответственный В (&ОтборМенеджеры)";
		Запрос.УстановитьПараметр("ОтборМенеджеры", ОтборМенеджеры);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборИсточникПривлеченияПокупателей) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.ИсточникПривлечения = &ОтборИсточник";
		Запрос.УстановитьПараметр("ОтборИсточник", ОтборИсточникПривлеченияПокупателей);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборСостояние) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.СостояниеЛида = &ОтборСостояние";
		Запрос.УстановитьПараметр("ОтборСостояние", ОтборСостояние);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборРезультат) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.ВариантЗавершения = &ОтборРезультат";
		Запрос.УстановитьПараметр("ОтборРезультат", ОтборРезультат);
	КонецЕсли;
...
Процедура ОбновитьДиаграммуНовыеПоПериодам()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	НАЧАЛОПЕРИОДА(Лиды.ДатаСоздания, НЕДЕЛЯ) КАК ДатаСоздания,
		...
		|	Лиды.ПометкаУдаления = ЛОЖЬ
		|	И Лиды.ЭтоГруппа = ЛОЖЬ
		|	И Лиды.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаСоздания
		|ИТОГИ
	...
	Если ЗначениеЗаполнено(ОтборТег) Тогда
		ОтборЗапроса.Добавить("Лиды.Теги.Тег = &ОтборТег");
		Запрос.УстановитьПараметр("ОтборТег", ОтборТег);
	КонецЕсли;
	
	
	Если ОтборМенеджеры.Количество() > 0 Тогда
		ОтборЗапроса.Добавить("Лиды.Ответственный В (&ОтборМенеджеры)");
		Запрос.УстановитьПараметр("ОтборМенеджеры", ОтборМенеджеры);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборИсточникПривлеченияПокупателей) Тогда
		ОтборЗапроса.Добавить("Лиды.ИсточникПривлечения = &ОтборИсточник");
		Запрос.УстановитьПараметр("ОтборИсточник", ОтборИсточникПривлеченияПокупателей);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборСостояние) Тогда
		ОтборЗапроса.Добавить("Лиды.СостояниеЛида = &ОтборСостояние");
		Запрос.УстановитьПараметр("ОтборСостояние", ОтборСостояние);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборРезультат) Тогда
		ОтборЗапроса.Добавить("Лиды.ВариантЗавершения = &ОтборРезультат");
		Запрос.УстановитьПараметр("ОтборРезультат", ОтборРезультат);
	КонецЕсли;
	...
	Пока ВыборкаДатаСоздания.Следующий() Цикл
		...
		Подсказка = НСтр("ru = 'Создано '") + ВыборкаДатаСоздания.Новый + НСтр("ru = ' лидов '") + Текст;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	Запрос.Текст = 
		...
		|	Лиды.Ответственный КАК Ответственный,
		|	Лиды.Ответственный.Представление КАК ОтветственныйПредставление
		|ИЗ
		|	Справочник.Лиды КАК Лиды
		|ГДЕ
		|	Лиды.ПометкаУдаления = ЛОЖЬ
		...
		|СГРУППИРОВАТЬ ПО
		|	Лиды.Ответственный,
		|	Лиды.Ответственный.Представление
		|
		|УПОРЯДОЧИТЬ ПО
		|	Лиды УБЫВ,
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru = ', лидов: '") + ВыборкаОтветственные.Лиды;
			ДиаграммаПоМенеджерам.УстановитьЗначение(Точка, Серия, ВыборкаОтветственные.Лиды, ВыборкаОтветственные.Ответственный, Подсказка);
		...
		Иначе
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru = ', лидов: '") + (СводноеЗначение.Значение + ВыборкаОтветственные.Лиды);
			...
			ДиаграммаПоМенеджерам.УстановитьЗначение(Точка, Серия, СводноеЗначение.Значение + ВыборкаОтветственные.Лиды, Расшифровка, Подсказка);
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ru = '<Менеджер не указан>'"))
				+ НСтр("ru = ', лидов: '") + ВыборкаОтветственные.Лиды;
...
Функция СформироватьРасшифровкуНовых(Признак)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Отчет.АнализБазыЛидов.ПФ_MXL_КонтрагентыРасшифровка");
	...
	Запрос.Текст = 
		...
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Новый,
		|	Лиды.Ответственный КАК Менеджер,
		|	Лиды.ДатаСоздания КАК Создан,
		|	Лиды.СостояниеЛида КАК СостояниеЛида,
		|	Лиды.ВариантЗавершения КАК Результат,
		|	ПРЕДСТАВЛЕНИЕ(Лиды.Ссылка) КАК ПредставлениеЛида,
		|	ПРЕДСТАВЛЕНИЕ(Лиды.Ответственный) КАК ПредставлениеМенеджера
		|ИЗ
		|	Справочник.Лиды КАК Лиды
		|ГДЕ
		|	Лиды.ПометкаУдаления = ЛОЖЬ
	...
	Если ВРег(Признак) = ВРег("Новые") Тогда
		ОтборЗапроса.Добавить("Лиды.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец");
	ИначеЕсли ВРег(Признак) = ВРег("Существующие") Тогда
		ОтборЗапроса.Добавить("НЕ (Лиды.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец)");
	КонецЕсли;
...
Функция СформироватьРасшифровкуНовыхЗаПериод(ДатаРасшифровки)
	...
	Запрос.Текст = 
		...
		|	Лиды.ПометкаУдаления = ЛОЖЬ
		|	И Лиды.ЭтоГруппа = ЛОЖЬ
		|	И НАЧАЛОПЕРИОДА(Лиды.ДатаСоздания, НЕДЕЛЯ) = &ДатаРасшифровки
		|	И Лиды.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец
		|
		|УПОРЯДОЧИТЬ ПО
		|	Создан УБЫВ,
...
Функция СформироватьРасшифровкуПоМенеджерам(Менеджеры)
	...
	Запрос.Текст = 
		...
		|	Лиды.ПометкаУдаления = ЛОЖЬ
		|	И Лиды.ЭтоГруппа = ЛОЖЬ
		|	И Лиды.Ответственный В(&Менеджеры)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Создан УБЫВ,
		|	Лиды.Наименование";
