...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Форма.Элементы.ЗапомнитьНастройки.Видимость = Ложь;
	...
	// Организация
	СтрокаОписания = Форма.СтрокаОписанияПоляВызов("Параметр", "Организация");
	Если СтрокаОписания<>Неопределено Тогда
		СтрокаОписания.НестандартныйОбработчик = Истина;
	КонецЕсли; 
	БезОрганизаций = (НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") 
	ИЛИ Константы.УчетПоКомпании.Получить() 
	ИЛИ ЗначениеЗаполнено(ПараметрДоговор.Значение));
	Если БезОрганизаций Тогда
		Если СтрокаОписания<>Неопределено Тогда
			Для каждого ОписаниеЭлемента Из СтрокаОписания.Элементы Цикл
				Форма.Элементы[ОписаниеЭлемента.Ключ].Видимость = Ложь;
			КонецЦикла; 
		КонецЕсли; 
		Форма.Элементы.СтраницыНастройки.Видимость = Ложь;
	Иначе
		Если СтрокаОписания<>Неопределено Тогда
			Для каждого ОписаниеЭлемента Из СтрокаОписания.Элементы Цикл
				Элемент = Форма.Элементы[ОписаниеЭлемента.Ключ];
				Если Элемент.Вид=ВидПоляФормы.ПолеВвода Тогда
					Элемент.ПодсказкаВвода = НСтр("ru = 'Организация: Все'");
	...
	// Акт сверки в валюте
	ВалютаДоговора = ВалютаДоговора(ПараметрКонтрагент.Значение, ПараметрДоговор.Значение, ПараметрОрганизация.Значение);
	БезВалюты = (ВалютаДоговора = Константы.ВалютаУчета.Получить());
	Если БезВалюты Тогда
		НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("ВалютаДоговора", Справочники.Валюты.ПустаяСсылка());
	Иначе
	...
	Если БезВалюты Тогда
		НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра(Стр.Поле, "ВВалютеУчета");
	Иначе
	...
	Элемент.Видимость = НЕ БезВалюты;
	Стр.Элементы.Вставить(Элемент.Имя, Элемент.ПутьКДанным);
	
	Форма.Элементы.СтраницыНастройки.Видимость = НЕ БезВалюты ИЛИ НЕ БезОрганизаций;
	
КонецПроцедуры

Процедура ПриИзмененииНестандартногоРеквизита(Тип, ИмяПоля, СтруктураЗначений, НастройкиСКД, Форма, ИмяЭлемента) Экспорт
	
	Если Тип="Параметр" И ИмяПоля="Организация" Тогда
		
		ПараметрКонтрагент = НастройкиСКД.ПараметрыДанных.Элементы.Найти("Контрагент");
		ПараметрДоговор = НастройкиСКД.ПараметрыДанных.Элементы.Найти("Договор");
		Стр = Форма.СтрокаОписанияПоляВызов("Параметр", "Организация");
		...
		Если ПустаяСтрока(ИмяРеквизита) Тогда
			Возврат;
		КонецЕсли; 
		Организация = СтруктураЗначений[ИмяРеквизита];
		НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("Организация", Организация);
		ВалютаДоговора = ВалютаДоговора(ПараметрКонтрагент.Значение, ПараметрДоговор.Значение, Организация);
		
		Стр = Форма.СтрокаОписанияПоляВызов("Параметр", "Режим");
		Если Стр=Неопределено Тогда
			Возврат;
		КонецЕсли; 
		БезВалюты = (ВалютаДоговора = Константы.ВалютаУчета.Получить());
		Если БезВалюты Тогда
			НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("ВалютаДоговора", Справочники.Валюты.ПустаяСсылка());
			НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("Режим", "ВВалютеУчета");
		Иначе
			НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("ВалютаДоговора", ВалютаДоговора);
			НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("Режим", Форма["Режим"]);
		КонецЕсли;
		Для каждого ОписаниеЭлемента Из Стр.Элементы Цикл
			Форма.Элементы[ОписаниеЭлемента.Ключ].Видимость = НЕ БезВалюты;
		КонецЦикла; 
