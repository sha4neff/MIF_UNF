...
// Процедура - Обработчик обновления настроек формы отчета
//
// Параметры:
//  НастройкиОтчета	 - Структура - Настройки отчета, подробнее см. процедуру ОтчетыУНФ.ИнициализироватьНастройкиОтчета
//  НастройкиСКД	 - НастройкиКомпоновкиДанных - Настройки компоновки данных отчета
//  Форма			 - УправляемаяФорма - Общая форма отчета
//
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	СтруктураОтбора.Вставить("Поле", "ВариантРасчетаПлановойСебестоимости");
	...
	Если Идентификаторы.Количество()>0 Тогда
		...
		ВариантРасчетаПлановойСебестоимости = ЗначениеПараметра("ВариантРасчетаПлановойСебестоимости", НастройкиСКД);
		Для каждого ОписаниеРеквизита Из СтрПараметр.Реквизиты Цикл
			СписокВыбора = ОбщегоНазначения.СкопироватьРекурсивно(Форма.Элементы[ОписаниеРеквизита.Ключ].СписокВыбора);
			Форма.Элементы[ОписаниеРеквизита.Ключ].Вид = ВидПоляФормы.ПолеПереключателя;
			Для каждого ЭлементСписка Из СписокВыбора Цикл
				Форма.Элементы[ОписаниеРеквизита.Ключ].СписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
			КонецЦикла; 
		КонецЦикла; 
		Если ВариантРасчетаПлановойСебестоимости<>2 Тогда
...
// Процедура - обработчик события изменения нестандартного реквизита на форме отчета
//
// Параметры:
//  Тип					 - Строка - Тип поля (Фильтр / Параметр / Показатель / ГруппаПоказателей / Группировка / Реквизит)
//  ИмяПоля				 - Строка - Имя поля
//  СтруктураЗначений	 - Структура - Структура содержит значения изменяемых полей
//  НастройкиСКД		 - НастройкиКомпоновкиДанных - Настройки компоновки данных отчета
//  Форма				 - УправляемаяФорма - Общая форма отчета, в которой произошло событие
//  ИмяЭлемента			 - Строка - Имя изменяемого элемента формы
//
Процедура ПриИзмененииНестандартногоРеквизита(Тип, ИмяПоля, СтруктураЗначений, НастройкиСКД, Форма, ИмяЭлемента) Экспорт
	
	Если Тип="Параметр" И ИмяПоля="ВариантРасчетаПлановойСебестоимости" Тогда
		СтрПараметр = Форма.СтрокаОписанияПоляВызов("Параметр", "ВариантРасчетаПлановойСебестоимости");
		Если СтрПараметр<>Неопределено Тогда
			...
			Если Стр<>Неопределено Тогда
				Для каждого ОписаниеРеквизита Из Стр.Реквизиты Цикл
					ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ОписаниеРеквизита.Ключ, "Видимость", СтруктураЗначений[ИмяРеквизита]=2);
				КонецЦикла; 
			КонецЕсли; 
...
Процедура СформироватьТаблицыВыпускаИПлановыхЗатрат(ТаблицаВыпуска, ТаблицаПлановыхЗатратНаВыпуск, ЗначенияПараметровМКД)
	...
	|			ИЛИ СоставВыпуска.Спецификация <> ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка))";
...
Процедура РазузловатьУзлыСпецификаций(ТаблицаУзловСпецификацийКРазузлованию, ТаблицаПлановыхЗатратНаВыпуск)
	...
	|	ТаблицаУзловСпецификацийКРазузлованию.ТипСтрокиСостава КАК ТипСтрокиСостава
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СправочникСпецификации.Владелец КАК НоменклатураУзел,
	|	СправочникСпецификации.Ссылка КАК СпецификацияУзла,
	|	СправочникСпецификации.ХарактеристикаПродукции КАК ХарактеристикаУзла,
	...
	|ИЗ
	|	Вт_ТаблицаУзловСпецификацийКРазузлованию КАК СпецификацийКРазузлованию
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Спецификации КАК СправочникСпецификации
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Спецификации.Состав КАК СпецификацииСостав
	|			ПО СправочникСпецификации.Ссылка = СпецификацииСостав.Ссылка
	|		ПО СпецификацийКРазузлованию.СпецификацияУзла = СправочникСпецификации.Ссылка
	|			И СпецификацийКРазузлованию.НоменклатураУзел = СправочникСпецификации.Владелец
	|			И СпецификацийКРазузлованию.ХарактеристикаУзла = СправочникСпецификации.ХарактеристикаПродукции
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СправочникСпецификации.Владелец,
	|	СправочникСпецификации.Ссылка,
	|	СправочникСпецификации.ХарактеристикаПродукции,
	...
	|			ПО СправочникСпецификации.Ссылка = СпецификацииОперации.Ссылка
	|		ПО СпецификацийКРазузлованию.СпецификацияУзла = СправочникСпецификации.Ссылка
	|			И СпецификацийКРазузлованию.НоменклатураУзел = СправочникСпецификации.Владелец
	|			И СпецификацийКРазузлованию.ХарактеристикаУзла = СправочникСпецификации.ХарактеристикаПродукции
	|ГДЕ
	|	НЕ СправочникСпецификации.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НоменклатураУзел,
	|	ХарактеристикаУзла,
