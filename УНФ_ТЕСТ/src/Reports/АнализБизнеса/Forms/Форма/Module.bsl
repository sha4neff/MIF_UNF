...
Процедура ОбновитьОтчеты()
	...
	ЗаполнитьСписокАктивныхПериодов(Элементы.АктивныйПериод.СписокВыбора, ПериодыПланирования, Отчет.Периодичность); 
...
&НаСервере
Процедура ЗаполнитьСписокАктивныхПериодов(СписокВыбора, ПериодыПланирования, Периодичность)
	...
	Для каждого Период Из ПериодыПланирования Цикл
		...
		СписокВыбора.Добавить(Период.Значение, ПериодПредставление);
	КонецЦикла;
...
Функция СформироватьSVGПоСтроке(ШаблонSVG)
	...
	Возврат ПоместитьВоВременноеХранилище(ДвоичДанные, УникальныйИдентификатор);
