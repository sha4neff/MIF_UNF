...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["ВоронкаПродажОбщая"].Рекомендуемый = Истина;
	НастройкиВариантов["ВоронкаПродажПоЛидам"].Рекомендуемый = Ложь;
	НастройкиВариантов["ВоронкаПродажПоПокупателям"].Рекомендуемый = Ложь;
	НастройкиВариантов["ВоронкаПродаж"].Рекомендуемый = Ложь;
...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	УдалитьЭлементыФильтров(Форма);	
	ДобавитьПолеВыбораСравнения(НастройкиСКД, Форма);	
	УправлениеВидимостьюВариантовВоронки(Форма, ВидВоронки);
	
	Идентификатор = СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", "");
	ГруппаНастроекПараметров = Форма.Элементы.Вставить("ГруппаПараметр" + Идентификатор, Тип("ГруппаФормы"), Форма.Элементы.ГруппаПараметрыЭлементы);
	ГруппаНастроекПараметров.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаНастроекПараметров.ОтображатьЗаголовок = Истина;
	ГруппаНастроекПараметров.Отображение = ОтображениеОбычнойГруппы.СлабоеВыделение;
	ГруппаНастроекПараметров.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаНастроекПараметров.Заголовок = НСтр("ru = 'Настройки'");
	...
	Форма.Элементы.ГруппаФильтры.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	Форма.Элементы.ГруппаПараметрыЭлементы.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	Форма.Элементы.ГруппаПараметрыЭлементы.Заголовок = НСтр("ru = 'Настройки'");
	...
	ГруппаФильтрВидОперацииВидЗаказа.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	ГруппаФильтрВидОперацииВидЗаказа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	ФильтрыВоронки = ФильтрыВоронки(Форма, НастройкиОтчета, НастройкиСКД, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа, ДобавляемыеРеквизиты, ИменаРеквизитов);
	
	Если Форма.Элементы.Найти("ФильтрПодразделение") <> Неопределено Тогда 
		Форма.Элементы["ФильтрПодразделение"].Видимость = ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам();
		Форма.Элементы["ФильтрПодразделение_Иерархия"].Видимость = Форма.Элементы.Найти("ФильтрПодразделение") <> Неопределено И ВидВоронки =  Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам();
	КонецЕсли;
	...
	ДобавитьФильтрыВоронки(Форма, ВидВоронки, ФильтрыВоронки, НастройкиСКД);
	
	УстановитьВидимостьЭлементов(Форма.Элементы, ВидВоронки);
...
Процедура ПриИзмененииНестандартногоРеквизита(Тип, ИмяПоля, СтруктураЗначений, НастройкиСКД, Форма, ИмяЭлемента) Экспорт
	...
	Если Тип = "Параметр" И (ИмяПоля = "ВидВоронки" ИЛИ ИмяПоля = "ВариантВоронки") Тогда
		
		НастройкиОтчета = Новый Структура;
		НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра(ИмяПоля, Форма[ИмяПоля]);
		ВариантВоронки = Отчеты.ВоронкаПродаж.ВариантВоронки(КомпоновщикНастроек.Настройки);
		
		Если ВариантВоронки = ВариантВоронкиПоЗаказам() Тогда
			Форма.Элементы.ГруппаФильтры.Скрыть();
			Возврат;
		КонецЕсли;
		
		Форма.Элементы.ГруппаФильтры.Показать();
...
Процедура УстановитьВидимостьЭлементов(Элементы, ВидВоронки)
	
	Элементы["ДобавитьФильтр"].Видимость = ВидВоронки <>  Отчеты.ВоронкаПродаж.ВидВоронкиОбщая();
	Элементы["СтраницыНастройки"].Видимость = Истина;
		
КонецПроцедуры

Процедура УправлениеВидимостьюВариантовВоронки(Форма, ВидВоронки)
	...
	Для Каждого ТекСтрПолеНастройки Из Форма.ПоляНастроек.ПолучитьЭлементы() Цикл
		...
		Если ТекСтрПолеНастройки.Поле = "ВариантВоронки" И ТекСтрПолеНастройки.Заголовок = "Вариант воронки" Тогда
			ТекСтрПолеНастройки.Скрыт = Истина;
			Форма.Элементы[ТекСтрПолеНастройки.ИмяГруппы].Видимость = Ложь;
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ТекЭлемент Из ТекСтрПолеНастройки.Элементы Цикл
			...
			Если ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() Тогда
				...
				ЭлементПоЛидам = Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.НайтиПоЗначению("ПоЛидам");
				Если ЭлементПоЛидам <> Неопределено Тогда
					Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.Удалить(ЭлементПоЛидам);
				КонецЕсли;
			...
			Если ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЛидам() Тогда
				...
				ЭлементПоПокупателям = Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.НайтиПоЗначению("ПоПокупателям");
				Если ЭлементПоПокупателям <> Неопределено Тогда
					Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.Удалить(ЭлементПоПокупателям);
				КонецЕсли;
				Возврат;
			...
			Если ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиОбщая() Тогда
				...
				ЭлементПоГруппам = Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.НайтиПоЗначению("ПоЛидам");
				Если ЭлементПоГруппам <> Неопределено Тогда
					Форма.Элементы[ТекЭлемент.Значение].СписокВыбора.Удалить(ЭлементПоГруппам);
				КонецЕсли;
...
Процедура ДобавитьПолеВыбораСравнения(НастройкиСКД, Форма)
	
	СписокВыбора = Новый СписокЗначений;
	
	СписокВыбора.Добавить("ПоЗаказам", "Не сравнивать");
	СписокВыбора.Добавить("ПоЛидам", "Лид");
	СписокВыбора.Добавить("ПоПокупателям", "Покупатель");
	СписокВыбора.Добавить("ПоМенеджерам", "Менеджер");
	СписокВыбора.Добавить("ПоПодразделениям", "Подразделение");
	СписокВыбора.Добавить("ПоИсточникам", "Источник привлечения");
...
Процедура ДобавитьЭлементДляВыбораПараметра(НастройкиСКД, Форма, ГруппаНастроекПараметров, ИмяРеквизита, Заголовок, ПоложениеЗаголовка, СписокВыбора, ЗначениеПоУмолчанию)
	
	ЭлементПараметра = Форма.Элементы.Найти(ИмяРеквизита);
	
	Если ЭлементПараметра <> Неопределено Тогда
		
		ЭлементПараметра.СписокВыбора.Очистить();
		Для Каждого ТекЭлементВыбора Из СписокВыбора Цикл
			ЭлементПараметра.СписокВыбора.Добавить(ТекЭлементВыбора.Значение, ТекЭлементВыбора.Представление);
		КонецЦикла;
		
		ЗначениеРеквизита = НастройкиСКД.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяРеквизита)).Значение;
		Возврат;
	...
	Элемент = Форма.Элементы.Добавить(ИмяРеквизита, Тип("ПолеФормы"), ГруппаНастроекПараметров);
	Элемент.Вид = ВидПоляФормы.ПолеВвода;
	Элемент.ПоложениеЗаголовка = ПоложениеЗаголовка;
	Элемент.РежимВыбораИзСписка = Истина;
	Элемент.ПутьКДанным = ИмяРеквизита;
	Для Каждого ТекЭлементВыбора Из СписокВыбора Цикл
		Элемент.СписокВыбора.Добавить(ТекЭлементВыбора.Значение, ТекЭлементВыбора.Представление);
	КонецЦикла;
...
Процедура УдалитьЭлементыФильтров(Форма)
	
	УдаляемыеЭлементы = Новый Массив;
	
	Для ИндексГруппы = 0 По Форма.Элементы.ГруппаФильтры.ПодчиненныеЭлементы.Количество()-1 Цикл
		
		Если Форма.Элементы.ГруппаФильтры.ПодчиненныеЭлементы[ИндексГруппы].Имя = "ДобавитьФильтр" 
			ИЛИ Форма.Элементы.ГруппаФильтры.ПодчиненныеЭлементы[ИндексГруппы].Имя = "ГруппаФильтрЭлементы" Тогда
			Продолжить;
		КонецЕсли;
		УдаляемыеЭлементы.Добавить(Форма.Элементы.ГруппаФильтры.ПодчиненныеЭлементы[ИндексГруппы]);
...
Функция ФильтрыВоронки(Форма, НастройкиОтчета, НастройкиСКД, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа, ДобавляемыеРеквизиты, ИменаРеквизитов)
	
	ФильтрыВоронки = Новый Структура;
	
	ДобавитьФильтрВидОперации(Форма, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа,ДобавляемыеРеквизиты, ИменаРеквизитов, ФильтрыВоронки, НастройкиСКД);
	ДобавитьФильтрВидЗаказа(Форма, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа, ДобавляемыеРеквизиты, ИменаРеквизитов, ФильтрыВоронки);

	Если Не МенееДвухИсточниковВСправочникеИсточникиПривлеченияПокупателей() И НЕ УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, "ИсточникПривлечения")
		...
		ФильтрыВоронки.Вставить("СтрИсточник", СтрИсточник);
		
	КонецЕсли;

	Если Не МенееДвухПокупателейВСправочникеКонтрагенты() И ВидВоронки <> Отчеты.ВоронкаПродаж.ВидВоронкиПоЛидам() 
		И ВидВоронки <> Отчеты.ВоронкаПродаж.ВидВоронкиОбщая() И Форма.Элементы.Найти("ФильтрПокупатель") = Неопределено
		И НЕ УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, "Контрагент") Тогда
		...
		ФильтрыВоронки.Вставить("СтрКонтрагент", СтрКонтрагент);
	КонецЕсли;
	
	Если Не МенееДвухЛидовВСправочникеЛиды() И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЛидам() 
		И Форма.Элементы.Найти("ФильтрЛид") = Неопределено
		И НЕ УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, "Лид") Тогда
		СтрЛид = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Лид", Новый ОписаниеТипов("СправочникСсылка.Лиды"), НСтр("ru = 'Лид'"), "Поле");
		СтрЛид.НестандартныйОбработчик = Ложь;
		СтрЛид.Создан = Истина;
		СтрЛид.Поле = "Лид";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЛид", НСтр("ru = 'Лид'"), СтрЛид.ТипЗначения, СтрЛид, ИменаРеквизитов);
		
		ФильтрыВоронки.Вставить("СтрЛид", СтрЛид);
	КонецЕсли;
	
	Если Не МенееДвухЭлементовВСправочникеСотрудники() И Форма.Элементы.Найти("ФильтрМенеджер") = Неопределено 
		И НЕ УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, "Ответственный") Тогда
		...
		ФильтрыВоронки.Вставить("СтрОтветственный", СтрОтветственный);
		
	КонецЕсли;
	
	Если Не МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() 
		И ПолучитьФункциональнуюОпцию("УчетПоНесколькимПодразделениям") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрПодразделение") = Неопределено 
		И НЕ УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, "Подразделение") Тогда
		...
		ФильтрыВоронки.Вставить("СтрСтруктурнаяЕдиницаПродажи", СтрСтруктурнаяЕдиницаПродажи);
	КонецЕсли;
	
	Возврат ФильтрыВоронки;
	
КонецФункции

Процедура ДобавитьФильтрыВоронки(Форма, ВидВоронки, ФильтрыВоронки, НастройкиСКД)
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрЗаказыПокупателей") = Неопределено Тогда
		ДобавитьФлажок("ФильтрЗаказыПокупателей", ФильтрыВоронки.СтрВидОперации, Форма);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") 
		И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() И Форма.Элементы.Найти("ФильтрВидЗаказаПокупателя") = Неопределено Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказаПокупателя", НСтр("ru = 'Вид заказа покупателя'"), ФильтрыВоронки.СтрВидЗаказаПокупателя, Форма);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрЗаказНаряды") = Неопределено Тогда
		ДобавитьФлажок("ФильтрЗаказНаряды", ФильтрыВоронки.СтрВидОперации, Форма);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрВидЗаказНаряда") = Неопределено Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказНаряда", НСтр("ru = 'Вид заказ-наряда'"), ФильтрыВоронки.СтрВидЗаказНаряда, Форма);
	КонецЕсли;
	
	Если ФильтрыВоронки.Свойство("СтрКонтрагент") Тогда
		ПолеКонтрагент = НастройкиСКД.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ФильтрыВоронки.СтрКонтрагент.Поле));
		ОтчетыУНФ.СоздатьФильтрПолеВвода(ФильтрыВоронки.СтрКонтрагент, Форма, Форма.Элементы, ПолеКонтрагент.ПолучитьПараметрыВыбора(), ПолеКонтрагент.ПолучитьСвязиПараметровВыбора());
	КонецЕсли;
	
	Если ФильтрыВоронки.Свойство("СтрЛид") Тогда
		ПолеЛид = НастройкиСКД.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ФильтрыВоронки.СтрЛид.Поле));
		ОтчетыУНФ.СоздатьФильтрПолеВвода(ФильтрыВоронки.СтрЛид, Форма, Форма.Элементы, ПолеЛид.ПолучитьПараметрыВыбора(), ПолеЛид.ПолучитьСвязиПараметровВыбора());
	КонецЕсли;
	
	Если ФильтрыВоронки.Свойство("СтрОтветственный") Тогда
		ПолеОтветственный = НастройкиСКД.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ФильтрыВоронки.СтрОтветственный.Поле));
		ОтчетыУНФ.СоздатьФильтрПолеВвода(ФильтрыВоронки.СтрОтветственный, Форма, Форма.Элементы, ПолеОтветственный.ПолучитьПараметрыВыбора(), ПолеОтветственный.ПолучитьСвязиПараметровВыбора());
	КонецЕсли;
	
	Если ФильтрыВоронки.Свойство("СтрИсточник") Тогда
		ПолеИсточник = НастройкиСКД.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ФильтрыВоронки.СтрИсточник.Поле));
		ОтчетыУНФ.СоздатьФильтрПолеВвода(ФильтрыВоронки.СтрИсточник, Форма, Форма.Элементы, ПолеИсточник.ПолучитьПараметрыВыбора(), ПолеИсточник.ПолучитьСвязиПараметровВыбора());
	КонецЕсли;

	Если ФильтрыВоронки.Свойство("СтрСтруктурнаяЕдиницаПродажи") Тогда
		ПолеПодразделение = НастройкиСКД.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ФильтрыВоронки.СтрСтруктурнаяЕдиницаПродажи.Поле));
		ОтчетыУНФ.СоздатьФильтрПолеВвода(ФильтрыВоронки.СтрСтруктурнаяЕдиницаПродажи, Форма, Форма.Элементы, ПолеПодразделение.ПолучитьПараметрыВыбора(), ПолеПодразделение.ПолучитьСвязиПараметровВыбора());
	КонецЕсли;

КонецПроцедуры

Процедура ДобавитьФильтрВидОперации(Форма, ВидВоронки, ГруппаФильтр, ДобавляемыеРеквизиты,ИменаРеквизитов, ФильтрыВоронки, НастройкиСКД)
	
	Если НЕ (ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрЗаказыПокупателей") = Неопределено И Форма.Элементы.Найти("ФильтрЗаказНаряды") = Неопределено) Тогда
		Возврат;
	КонецЕсли;
	...
	СтрВидОперации.ИмяГруппы = ГруппаФильтр.Имя;
	...
	Для Каждого ЭлементОтбора Из НастройкиСКД.Отбор.Элементы Цикл
		...
		Если Строка(ЭлементОтбора.ЛевоеЗначение) <> "ВидОперации" Тогда
			Продолжить;
		КонецЕсли;
		
		Если ЭлементОтбора.ПравоеЗначение = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу Тогда
			СтрВидОперации.Реквизиты.ФильтрЗаказыПокупателей = Истина;
		ИначеЕсли ЭлементОтбора.ПравоеЗначение = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			СтрВидОперации.Реквизиты.ФильтрЗаказНаряды = Истина;
		Иначе
			СтрВидОперации.Реквизиты.ФильтрЗаказыПокупателей = Истина;
			СтрВидОперации.Реквизиты.ФильтрЗаказНаряды = Истина;
		КонецЕсли;
	...
	ФильтрыВоронки.Вставить("СтрВидОперации", СтрВидОперации);
	
КонецПроцедуры

Процедура ДобавитьФильтрВидЗаказа(Форма, ВидВоронки, ГруппаФильтр, ДобавляемыеРеквизиты, ИменаРеквизитов, ФильтрыВоронки)
	
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам()) Тогда
		
		СтрВидЗаказаПокупателя = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидЗаказа", Новый ОписаниеТипов, НСтр("ru = 'Вид заказа'"), "Поле");
		СтрВидЗаказаПокупателя.НестандартныйОбработчик = Ложь;
		СтрВидЗаказаПокупателя.Создан = Истина;
		СтрВидЗаказаПокупателя.ИмяГруппы = ГруппаФильтр.Имя;
		СтрВидЗаказаПокупателя.Поле = "ВидЗаказа";
		ФильтрыВоронки.Вставить("СтрВидЗаказаПокупателя", СтрВидЗаказаПокупателя);

	КонецЕсли;
	
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам()) Тогда
		
		СтрВидЗаказНаряда = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидЗаказа", Новый ОписаниеТипов, НСтр("ru = 'Вид заказа'"), "Поле");
		СтрВидЗаказНаряда.НестандартныйОбработчик = Ложь;
		СтрВидЗаказНаряда.Создан = Истина;
		СтрВидЗаказНаряда.ИмяГруппы = ГруппаФильтр.Имя;
		СтрВидЗаказНаряда.Поле = "ВидЗаказа";
		
		ФильтрыВоронки.Вставить("СтрВидЗаказНаряда", СтрВидЗаказНаряда);
		
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам()
		И Форма.Элементы.Найти("ФильтрВидЗаказНаряда") = Неопределено Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказНаряда", НСтр("ru = 'Вид заказ-наряда'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказНарядов"), СтрВидЗаказНаряда, ИменаРеквизитов);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") И ВидВоронки = Отчеты.ВоронкаПродаж.ВидВоронкиПоЗаказам() 
		И Форма.Элементы.Найти("ФильтрВидЗаказаПокупателя") = Неопределено Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказаПокупателя", НСтр("ru = 'Вид заказа покупателя'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказовПокупателей"), СтрВидЗаказаПокупателя, ИменаРеквизитов);
	КонецЕсли;
...
Функция УстановленПользовательскийФильтр(НастройкиОтчета, НастройкиСКД, ПолеФильтра)
	
	ЭтоПользовательскийВариант = НастройкиОтчета.ВариантСсылка.Пользовательский;
	...
	Для Каждого ЭлементОтбора Из НастройкиСКД.Отбор.Элементы Цикл
		ФильтрУстановлен = Строка(ЭлементОтбора.ЛевоеЗначение) = ПолеФильтра;
		
		Если ФильтрУстановлен Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ФильтрУстановлен;
...
Процедура ВывестиВоронкуПоЗаказам(ДокументРезультат, МакетОтчета, ВариантВоронки, ДанныеРасшифровки, ОбъектВоронки, ПолеГруппировки = Неопределено)
	...
	Если РезультатыЗапросаВоронкаПродаж[2].Пустой() Тогда
		ТекстОтстутствуютДанные = НСтр("ru = 'Отсутствуют заказы, у которых было изменено состояние в течение заданного периода.'");
		ВывестиОбластьОтсутствуютДанные(ДокументРезультат, МакетОтчета, НСтр("ru = 'Заказы покупателей'"), ТекстОтстутствуютДанные);
		Возврат;
	КонецЕсли;
...
Процедура ВывестиВоронкуПоЛидам(ДокументРезультат, МакетОтчета, ВариантВоронки, ДанныеРасшифровки, ОбъектВоронки = Неопределено, ПолеГруппировки = Неопределено)
	...
	Если НетДанныхВВоронке Тогда
		ТекстОтсутствутДанные = НСтр("ru = 'Отсутствуют лиды, у которых было изменено состояние в течение заданного периода.'");
		ВывестиОбластьОтсутствуютДанные(ДокументРезультат, МакетОтчета, НСтр("ru = 'Лиды'"), ТекстОтсутствутДанные);
		ДанныеРасшифровки.Настройки.ДополнительныеСвойства.Вставить("ЦветаГрадиентаЗаказы", Неопределено);
	КонецЕсли;
