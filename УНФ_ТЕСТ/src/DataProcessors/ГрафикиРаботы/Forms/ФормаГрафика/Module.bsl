...
Процедура ПолеПредставлениеГрафикаНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли СписокГрафиковГиперСсылка.Количество() > 1
		...
		Оповещение = Новый ОписаниеОповещения("ПослеВыбораГрафика", ЭтаФорма, ПараметрыОповещения);
		
		СписокГрафиковГиперСсылка.ПоказатьВыборЭлемента(Оповещение,НСтр("ru = 'Графики за период'"),ЭтаФорма);
	КонецЕсли;
...
Функция РедактированиеДоступно()
	...
	СписокПрофилейДоступа = Новый СписокЗначений;
	
	СписокПрофилейДоступа.Добавить(Справочники.ПрофилиГруппДоступа.НайтиПоНаименованию("Зарплата"));
	СписокПрофилейДоступа.Добавить(Справочники.ПрофилиГруппДоступа.НайтиПоНаименованию("Производство"));
	СписокПрофилейДоступа.Добавить(Справочники.ПрофилиГруппДоступа.НайтиПоНаименованию("Продажи"));
	СписокПрофилейДоступа.Добавить(Справочники.ПрофилиГруппДоступа.Администратор);
	
	Если Не СписокПрофилейДоступа.Количество() Тогда Возврат Ложь КонецЕсли;
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("СписокПрофилейДоступа", СписокПрофилейДоступа);
	...
	|	И ГруппыДоступаПользователи.Ссылка.Профиль В(&СписокПрофилейДоступа)";
	...
	Для Каждого СтрокаДанных Из ДанныеГрафика Цикл
		...
		Для НомерДня = 1 По 31 Цикл
			...
			Иначе
				...
				ПарамерыОтбора = Новый Структура("ЭтоОтклонение, ДатаДня", Истина, ДатаДня);
				ЕстьОтклоненияПоДню = ИзмененныеДни.НайтиСтроки(ПарамерыОтбора);
...
Процедура ЗаполнитьДанныеГрафика(НомерГода = Неопределено)
	...
	Для Каждого СтрокаОтклений Из ТаблицаОтклонений Цикл
		
		НомерДняСтрокой = Строка(День(СтрокаОтклений.ДатаДня));
		
		ИмяПоляДень = "День" + НомерДняСтрокой;
		ИмяПоляИзменен = "Изменен"+НомерДняСтрокой;
		
		СтрокаДанныхГрафика = ДанныеГрафика[Месяц(СтрокаОтклений.ДатаДня) - 1];
		
		СтрокаДанныхГрафика[ИмяПоляДень] = СтрокаОтклений.Длительность;
		СтрокаДанныхГрафика.ИтогЧасы = СтрокаДанныхГрафика.ИтогЧасы + СтрокаОтклений.Длительность;
...
Процедура ЗафиксироватьИзмененныйПериодВыделенныеДни(Результат, ДопПараметрыОповещения) Экспорт
	...
	Если Не Результат = Неопределено И Результат.Свойство("Периоды") Тогда
		...
		Для Каждого ВыделеленныйДеньСписка Из ДопПараметрыОповещения.СписокДней Цикл
			
			ВыделеленныйДень = ВыделеленныйДеньСписка.Значение;
			
			ОчиститьСтрокиИзмененныхДней(ВыделеленныйДень);
			
			Для Каждого СтрокаПериод Из Результат.Периоды Цикл
				
				НоваяСтрока = ИзмененныеДни.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПериод);
				НоваяСтрока.ДатаДня = ВыделеленныйДень;
				НоваяСтрока.Длительность = СтрокаПериод.Длительность;
				НоваяСтрока.ЭтоОтклонение = Истина;
			...
			Если Не ЧасовПоДню = 0 Тогда
				СтрокаДанных = ДанныеГрафика[Месяц(ВыделеленныйДень)-1];
				
				НомерДня = День(ВыделеленныйДень);
...
&НаСервере
Процедура ДополнитьТаблчинуюЧасть(ИмяТЧПриемник, ТЧИсточник, ГрафикРаботы)
...
Процедура ОбновитьДанныеГрафика(ГрафикРаботы, МассивОбработанныхГрафиков)
	...
	ДополнитьТаблчинуюЧасть("ШаблоныПоГоду", ГрафикРаботы.ШаблоныПоГоду, ГрафикРаботы);
	ДополнитьТаблчинуюЧасть("РасписаниеРаботы", ГрафикРаботы.РасписаниеРаботы, ГрафикРаботы);
	ДополнитьТаблчинуюЧасть("Перерывы", ГрафикРаботы.Перерывы, ГрафикРаботы);
	ДополнитьТаблчинуюЧасть("ДополнительныеНастройкиЗаполнения", ГрафикРаботы.ДополнительныеНастройкиЗаполнения, ГрафикРаботы);
	ДополнитьТаблчинуюЧасть("ПериодыДополнительныхНастроекЗаполнения", ГрафикРаботы.ПериодыДополнительныхНастроекЗаполнения, ГрафикРаботы);
	ДополнитьТаблчинуюЧасть("ПериодыГрафика", ГрафикРаботы.ПериодыГрафика, ГрафикРаботы)
...
Процедура СформироватьПредставлениеПериода()
	...
	ПредставлениеГрафика = ПредствалениеГрафика(МассивДат);
...
&НаКлиенте
Функция ПредствалениеГрафика(МассивДат)
