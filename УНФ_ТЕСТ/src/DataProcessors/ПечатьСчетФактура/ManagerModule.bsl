...
Процедура УчестьРазличияСводногоСФ(ДанныеОбъекта, ДанныеПечати, ЭтоУПД)
	...
	Для Каждого Строка Из ДанныеОбъекта.ТаблицаПокупателей Цикл
		...
		КПП = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(Строка.СведенияОПокупателе, "КПП,", Ложь);
		ИНН_КПП_Покупателя = ИНН_КПП_Покупателя + ?(ПустаяСтрока(ИНН_КПП_Покупателя), "", "; ")
						+ УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(Строка.СведенияОПокупателе, "ИНН,", Ложь)
		...
		ПредставлениеГрузополучателя = ПредставлениеГрузополучателя + ?(ПустаяСтрока(ПредставлениеГрузополучателя), "",
			"; ") + УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(Строка.СведенияОГрузополучателе,
...
Функция УниверсальноеПредставлениеСубъекта(СведенияОСубъекте, ПредставлениеИННиКПП = "")
	...
	ПредставлениеСубъекта = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОСубъекте,
		"ПолноеНаименование,");
...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ЭтоУПД) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		СведенияОГрузоотправителе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(ДанныеОбъекта.Грузоотправитель,
			ДанныеОбъекта.ДатаДокумента, , );
		СведенияОГрузополучателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(ДанныеОбъекта.Грузополучатель,
			ДанныеОбъекта.ДатаДокумента, , );
		...
		ДанныеПечати.Вставить("АдресПоставщика", УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			СведенияОПоставщике, "ЮридическийАдрес,"));

		ДанныеПечати.Вставить("ПредставлениеПокупателя", УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			СведенияОПокупателе, "ПолноеНаименование,"));
		ДанныеПечати.Вставить("АдресПокупателя", УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
		...
		Если ЕстьЗапасы Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеОбъекта.Грузоотправитель) Тогда
				...
				ПредставлениеГрузоотправителя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			...
			ПредставлениеГрузополучателя = ?(ПустаяСтрока(АдресДоставки),
				...
				"ПолноеНаименование, ФактическийАдрес,"), УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
		...
		Если ОбластиМакета.ОбластьПодвал <> Неопределено Тогда
			...
			ДанныеПечати.Вставить("Свидетельство", УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
				СведенияОПоставщике, "Свидетельство,"));

			КоличествоСтраницПрописью = УправлениеНебольшойФирмойСервер.ФормаМножественногоЧисла(НСтр("ru = 'листе'"),
	...
	Если СчетФактура.СводныйКомиссионный Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			...
			Для Каждого СтрокаПокупателя Из СтрокаТаблицы.ТаблицаПокупателей Цикл
				...
				СтрокаПокупателя.СведенияОПокупателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(
					СтрокаПокупателя.Покупатель, СтрокаТаблицы.ДатаДокумента, , );
				СтрокаПокупателя.СведенияОГрузополучателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(
					СтрокаПокупателя.Покупатель, СтрокаТаблицы.ДатаДокумента, , );
...
Функция РазобратьДокументыНаСчетаФактуры(МассивОбъектов, ЭтоУПД, ЭтоВходящийДокумент = Ложь) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Иначе
			...
			СтруктураДанных = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуру(ОбъектПечати,
				ЭтоВходящийДокумент);
