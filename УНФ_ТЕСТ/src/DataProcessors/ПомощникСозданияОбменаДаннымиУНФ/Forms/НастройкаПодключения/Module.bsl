...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// УНФ.
	УНФ_РазделениеВключено = РаботаВМоделиСервиса.РазделениеВключено();
	Если УНФ_РазделениеВключено Тогда
		УНФ_АдресПриложения = Параметры.УНФ_АдресПриложения;
		УНФ_НомерОбласти = Параметры.УНФ_НомерОбласти;
	Иначе
		ВидПодключения          = "Интернет";
		ИнтернетВебАдрес        = Параметры.УНФ_АдресПриложения;
		ИнтернетИмяПользователя = Параметры.УНФ_ИмяПользователя;
		ИнтернетПароль          = Параметры.УНФ_Пароль;
...
Процедура ПриОткрытии(Отказ)
	...
	// УНФ.
	// Разделение выключено.
	// Автоматическое заполнение реквизитов подключения.
	// Переход "Далее" к проверке подключения.
	Если НЕ УНФ_РазделениеВключено Тогда
		// 1. Основные реквизиты заполнены в ПриСоздании.
		ПриИзмененииВидПодключения();
		ИзменитьПорядковыйНомерПерехода(+1); // Выбран вид подключения. Далее.
		ИзменитьПорядковыйНомерПерехода(+1); // Заполнены параметры подключения. Далее.
		Элементы.ДекорацияУНФСостояниеПроцесса.Заголовок = НСтр("ru='Проверка подключения..'");
	КонецЕсли;
	// КонецУНФ.
...
Процедура ПриЗавершенииПолученияСпискаПриложенийДляПодключения()
	...
	// УНФ.
	// Разделение включено.
	// Список приложений подготовился из МС.
	// Переход "Далее" к проверке подключения.
	ИзменитьПорядковыйНомерПерехода(+1);
	// КонецУНФ
...
Процедура ПриЗавершенииПроверкиПодключенияOnline()
	...
	Если ПроверкаПодключенияВыполнена Тогда
		...
		// УНФ.
		// Разделение включено.
		// Прошла проверка подключения к выбранному приложению.
		// Названия приложений остаются по умолчанию.
		// Переход "Далее" к сохранению настроек.
		Элементы.ДекорацияУНФСостояниеПроцесса.Заголовок = НСтр("ru='Подключение..'");
		ИзменитьПорядковыйНомерПерехода(+1); // Названия приложений подставлены.. Далее.
		// КонецУНФ.
	Иначе
		ИзменитьПорядковыйНомерПерехода(-1);
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Не удалось подключиться к программе. Попробуйте еще раз.'")
		КонецЕсли;
...
Функция Подключаемый_СтраницаСпособНастройкиПодключения_ПриПереходеДалее(Отказ)
	...
	// УНФ
	Если УНФ_РазделениеВключено Тогда
		СпособНастройкиПодключения = "ПриложениеИзСписка";
	КонецЕсли;
	// КонецУНФ
	...
	Если СпособНастройкиПодключения = "ПриложениеИзСписка" Тогда
		...
		// УНФ
		// Режим разделения.
		// Список приложений подготовлен.
		// Автоматический выбор нужного по номеру области.
		Строки = ПриложенияВСервисе.НайтиСтроки(
			Новый Структура("ОбластьДанных", УНФ_НомерОбласти));
