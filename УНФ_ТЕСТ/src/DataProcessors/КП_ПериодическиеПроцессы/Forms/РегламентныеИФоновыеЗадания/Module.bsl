...
Процедура УдалитьРегламентноеЗаданиеВыполнитьНаСервере(Знач Идентификатор)
	...
	КП_ПериодическиеЗадания.УдалитьПараметрыПроцесса(Идентификатор);
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗадание(Знач Действие)
	...
	//	ОткрытьФорму("Обработка.КП_ПериодическиеПроцессы.Форма.РегламентноеЗадание", ПараметрыФормы, ЭтотОбъект);
	//КонецЕсли;
...
&НаСервере
Функция ПолучитьПолноеИмяФормы()
		
	ИмяВызываемойФормы="РегламентноеЗадание";
	ИмяОбработки = РеквизитФормыВЗначение("Объект").Метаданные().ПолноеИмя();

	ПолноеИмяФормы=ИмяОбработки + ".Форма."+ИмяВызываемойФормы;
...
Процедура ЗагрузитьРегламентныеЗадания(ИдентификаторЗадания = Неопределено, ОбновитьНезаметно = Ложь)
	...
	КонецЕсли;
	ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания);
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	СформироватьТаблицуРегламентныхЗаданий(ПараметрыДлительнойОперации, ПараметрыВыполнения);
	
	Элементы.СтраницыОтложеннаяЗагрузкаРегламентныхЗаданий.ТекущаяСтраница = Элементы.СтраницаРегламентныеЗадания;	
	ОбработатьРезультат(ПараметрыВыполнения);
	
	//Результат=ПолучитьИзВременногоХранилища(ПараметрыВыполнения.АдресРезультата);
	
	//ЗагрузитьРегламентныеЗаданияЗавершение(ПараметрыВыполнения, Неопределено);
...
&НаСервере
Процедура СформироватьТаблицуРегламентныхЗаданий(Параметры, АдресХранилища)
	...
	МетаданныеЗадания=Метаданные.РегламентныеЗадания.КП_ПериодическийПроцесс;
	ОтборЗаданий=Новый Структура("Метаданные", МетаданныеЗадания);
	ТекущиеЗадания = РегламентныеЗадания.ПолучитьРегламентныеЗадания(ОтборЗаданий);
	...
	Для Каждого ОбъектМетаданных Из Метаданные.РегламентныеЗадания Цикл
		Если ОбъектМетаданных.Имя<>"КП_ПериодическийПроцесс" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьСвойстваРегламентногоЗадания(Приемник, ЗаданиеИсточник)
	...
	Приемник.Наименование = РегламентныеЗаданияСлужебный.ПредставлениеРегламентногоЗадания(ЗаданиеИсточник);
	...
	Если СвойстваПоследнегоФоновогоЗадания = Неопределено Тогда
		Приемник.ДатаНачала          = РегламентныеЗаданияСлужебный.ТекстНеОпределено();
		Приемник.ДатаОкончания       = РегламентныеЗаданияСлужебный.ТекстНеОпределено();
		Приемник.СостояниеВыполнения = РегламентныеЗаданияСлужебный.ТекстНеОпределено();
	Иначе
...
Процедура ОбработатьРезультат(ПараметрыЗадания)
	...
	Результат = ПолучитьИзВременногоХранилища(ПараметрыЗадания);
	ОтключенныеЗадания.Очистить();
	...
	ТаблицаПериодическихПроцессов=Результат.Таблица.Скопировать();
	ТаблицаПериодическихПроцессов.Очистить();
	
	Для Каждого СтрокаТЧ Из Результат.Таблица Цикл
		Если СтрокаТЧ.ИмяЗадания<>"КП_ПериодическийПроцесс" Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока=ТаблицаПериодическихПроцессов.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
		
	КонецЦикла;
	
	//ЗначениеВРеквизитФормы(Результат.Таблица, "ТаблицаРегламентныеЗадания");
	ТаблицаРегламентныеЗадания.Загрузить(ТаблицаПериодическихПроцессов);
