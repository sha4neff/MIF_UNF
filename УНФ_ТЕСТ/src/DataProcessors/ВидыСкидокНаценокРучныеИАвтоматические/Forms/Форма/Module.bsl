...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьАвтоматическиеСкидкиНаценки Тогда
		...
		Если ВариантСовместногоПримененияСкидок.Пустая() Тогда
			...
			Иначе
				Отказ = Истина;
				ОбщегоНазначения.СообщитьПользователю("Автоматические скидки не настроены! Обратитесь к пользователю с правом редактирования цен в документах!");
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыРазмещения.Источники = ВидыСкидокНаценок.Отбор.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных("Ссылка")).ТипЗначения;
	...
	Если Параметры.Свойство("ДисконтныеКарты") И Параметры.ДисконтныеКарты Тогда
		...
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(ЭтаФорма.АвтоматическиеСкидки, 
			"ЕстьУсловияПоДК", Истина, Истина, ВидСравненияКомпоновкиДанных.Равно);
	КонецЕсли;
...
Процедура АвтоматическиеОкругленияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОткрытьФорму("Справочник.АвтоматическиеСкидки.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", Новый Структура("СпособПредоставления", ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.Округление"))));
