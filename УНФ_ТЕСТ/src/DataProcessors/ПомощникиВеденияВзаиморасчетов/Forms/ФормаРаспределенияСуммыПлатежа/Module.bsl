...
Процедура ДеревоРаспределенияСуммыПлатежаВыбранПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено
		...
		// Пользователь уже все равспределил.
		// Установка флага не приведет к изменению знаения в колонке "",
...
Процедура ОбработатьАктивизациюСтрокиДерева()
	...
	Иначе
		...
		Содержимое.Добавить(Новый ФорматированнаяСтрока(""+ТекущиеДанныеДерева.УжеОплачено+" "+ТекущиеДанныеДерева.ВалютаОбъектаПредставление,, ЦветТекстаПравойПанели,, "РасшфироватьОплачено"));
...
Процедура УчестьДанныеРасшифровкиПлатежаИСвязанныхДокументов()
	...
	// 1. Сначала обработоем таблицу документов для изменения.
	Для Каждого ТекущаяСтрока Из ТаблицаДокументовДляИзменения Цикл
	...
	// Во 2-ую очередь зачтём расшифровку платежа.
	Для Каждого ТекущаяСтрока Из ТаблицаРасшифровкаПлатежа Цикл
...
Функция ПолучитьТекстЗапросаДляРаспределенияСуммыПлатежа()
	...
	|		ИЛИ &ЭтоЗаказ
	|			И ВременнаяТаблицаДанные.Документ.МоментВремени > &МоментВремениТекущегоЗаказа КАК ДокументПозжеТекущего,
...
Процедура УстановитьЗаголовкиКолонокДереваРаспределенияПлатежа()
	...
	Для Каждого ТекущаяСтрока Из ТаблицаГруппировокПравойПанели Цикл
		Если ТекущаяСтрока.Выбран Тогда
			...
			Иначе
				ЗаколовокКолокиОбъект = ЗаколовокКолокиОбъект + " / ";
			КонецЕсли;
			ПолеГруппировки = ПолучитьСтрокуДляГруппировкиПравойПанели(ТекущаяСтрока.Группировка, Истина);
			ЗаколовокКолокиОбъект = ЗаколовокКолокиОбъект + ?(ПолеГруппировки = "СчетНаОплату", НСтр("ru = 'Счет на оплату'"), ПолеГруппировки);
		КонецЕсли;
	КонецЦикла;
	Элементы.ДеревоРаспределенияСуммыПлатежаОбъект.Заголовок = ЗаколовокКолокиОбъект;
...
&НаКлиенте
Процедура ОткрытьФормуРедактированияДополнительныхРеквизитовЗавершение(Результат, ДополнительнчеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		
		ТекущаяСтрока = ДеревоРаспределенияСуммыПлатежа.НайтиПоИдентификатору(ДополнительнчеПараметры.ИдентификаторСтроки);
		ПредыдщееЗначениеСуммыПлатежа = ТекущаяСтрока.СуммаПлатежа;
		...
		Если ПредыдщееЗначениеСуммыПлатежа <> ТекущаяСтрока.СуммаПлатежа Тогда
...
Процедура РаспределитьСуммуПлатежаРекурсивно(СтрокаРодитель, СуммаКРаспределению, ОбрабатыватьДолги, ОбнулятьСуммуПодчиненных, ПервыйПроходПриЗачетеПредоплаты = Ложь, НакопленнаяСумма = 0)
	...
	Для Каждого ТекущаяСтрока Из СтрокиДляРаспределения Цикл
		...
		ИначеЕсли ЭтоЗачетПредоплаты
			...
			// При этом анализируем общеий остаток, т.к. можно зачесть оплату без заказа под отгрузку по заказу!
...
Процедура ДеревоРаспределенияСуммыПлатежаСчетНаОплатуНачалоВыбораЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		ТекущиеДаннные = Элементы.ДеревоРаспределенияСуммыПлатежа.ТекущиеДанные;
		Если ТекущиеДаннные <> Неопределено Тогда
			ТекущиеДаннные.СчетНаОплату = ВыбранноеЗначение;
			ОбновитьСтрокуТаблицыПоСтрокеДереваНаКлиенте(ТекущиеДаннные);
		КонецЕсли;
	КонецЕсли;
