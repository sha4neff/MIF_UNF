...
Процедура ПодключитьОбсужденияНажатие(Элемент)
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗавершитьПодключениеОбсуждений", ЭтотОбъект);
	ОткрытьФорму("Обработка.РегистрацияВСистемеВзаимодействияУНФ.Форма",,,,,,ОповещениеОЗакрытии);
КонецПроцедуры
...
Процедура УстановитьПризнакиВРаботе(ИдентификаторГруппы = Неопределено)
	...
	Элементы.ВРаботе_ОповещениеКлиентаОЗаписиНаУслугу.Видимость = Справочники.ЗадачиАссистентаПоНапоминаниямОЗаписи.ЕстьЗадачиВРаботе();
