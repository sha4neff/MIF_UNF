...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТолькоПросмотрФормы = ДоступенПрофиль("Только просмотр");
	...
	ЭтаФорма.Заголовок = НСтр("ru = 'Планировщик ресурсов'");
	...
	Если Не НомерПодсистемы = 0 Тогда 
		...
		Если НомерПодсистемы = 3 Тогда 
			...
			ЭтаФорма.Заголовок = "Журнал записи";
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПолучитьФункциональнуюОпцию("ПланироватьЗагрузкуРесурсовПредприятия") 
		ИЛИ (Не РольДоступна("ПолныеПрава") И Не ДоступенПрофиль("Производство")) Тогда
...
Процедура ЗагрузкаРесурсовВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ТипЗнч(РасшифровкаЯчейки) = Тип("Структура")
		...
		ВремяОкончания = ПериодПозиционирования + (Область.Право - Область.Лево-1)*300;
...
Процедура ИспользоватьВариантыПриИзменении(Элемент)
	...
	Если Не ИспользоватьВарианты Тогда
		...
		Элементы.ВариантОтображения.СписокВыбора.Добавить(1, "Вариант 1(основной)");
...
Процедура КомандаПереименоватьВариант(Команда)
	...
	ОткрытьФорму("Обработка.ПланировщикРесурсов.Форма.ФормаПереименоватьВариант", ПараметрыОткрытия,ЭтотОбъект,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ВыбранныеРесурсы(Команда)
	
	ПараметрыОткрытия = Новый Структура("ВыбранныеРесурсы", ВыбранныеРесурсы);
	
	ПараметрыОповещения = Новый Структура();
	
	Оповещение = Новый ОписаниеОповещения("ОбработатьДанныеКорзины", ЭтотОбъект, ПараметрыОповещения);
	
	ОткрытьФорму("ВнешняяОбработка.ПланировщикРесурсов.Форма.ФормаВыбранныеРесурсы",ПараметрыОткрытия,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура КомандаПодобратьВКорзину(Команда)
	...
	Иначе
		...
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаПодбоИзКонтесктногоМеню", ЭтаФорма, ПараметрыОповещения);
...
Процедура ВывестиТабличныйДокумент(МассивДат = Неопределено, ОткрытиеФормы = Ложь, ВыводСКалендаря = Ложь)
	...
	СтруктураШагаПланирования = СтруткрутраИнтервалаПланирования();
	...
	ПакетДанныхРесурсов = ПолучитьРасписаниеЗагрузкиРесурсов(ТаблицаРесурсовПредприятия.ВыгрузитьКолонку("Ресурс"), РесурсыИДокументы.СписокДокументов);
...
Процедура ВывестиРасписаниеПоМесяцам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов,
	...
	КоличествоСтрокВОбласти = МаксимальноеКоличествоЗадействованныхРесурсовВЕдиницуПериода(ТаблицаРесурсовПредприятия.ВыгрузитьКолонку("Ресурс"));
	...
	Для Каждого СтрокаГод Из ТЗГодыПериода Цикл
		...
		ДлинаСтрокиОкрашивания = ?(НайденныеСтрокиМесяцы.Количество()>3, 2, НайденныеСтрокиМесяцы.Количество()-1);
		...
		Для Каждого СтрокаПериод Из НайденныеСтрокиМесяцы Цикл
			...
			Для Каждого РесурсПредприятия Из ТаблицаРесурсовПредприятия Цикл
				...
				НайденныеСтрокиРесурсы = РасписаниеЗагрузкиГод.НайтиСтроки(ПараметрыОтбора);
...
Процедура ВывестиРасписаниеПоДням(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов
	...
	Для Каждого РесурсПредприятия Из ТаблицаРесурсовПредприятия Цикл
		...
		Для Каждого ДеньСписка Из СписокДатПериодаДляФормированияГрафикаПоДням Цикл
			...
			ДокументыРасшифровки = ПланированиеРесурсовУНФ.ТаблицаДокументовЗаПериод(НайденныеСтроки, ДеньСписка.Значение, ДеньСписка.Значение, КратностьИнтервала, Истина);
			
			СтруктураИтоговЗаДень = ПланированиеРесурсовУНФ.ИтогиПоДню(ДеньСписка.Значение, КратностьПланирования, РабочиеПериоды
			,Ресурс, РасписаниеЗагрузки, ДокументыРасшифровки, МатрицаИнтервалов, НайденныеСтроки);
			...
			СтруктураРасшифровки = Новый Структура("Ресурс, День, ВидРасшифровки, ОстатокПоЗагрузке"
			, Ресурс, ДеньСписка.Значение, "КратноИнтервалам", ОстатокПоЗагрузке);
			ОбластьОбъединения.Расшифровка = СтруктураРасшифровки;
			
			ОбластьОбъединения.Объединить();
			
			ОбновитьОтображениеОбластиСогласноЗагрузке(ОбластьОбъединения, ЗагрузкаРесурса, ВозможнаяЗагрузкаРесурсаВДень,,
			,СтруктураИтоговЗаДень.ЕстьПревышение,,СтруктураИтоговЗаДень,ЭтоРабочийПериод);
		...
		Если ЕстьРасшифровка Тогда
			...
			ЗагрузкаРесурсов.Вывести(ОбластьРесурс,НомерГруппы,"Расшифровка по дукемнтам"+Строка(Ресурс), Ложь);
...
Процедура ВывестиРасписаниеКратноИнтервалам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов
	...
	Для Каждого РесурсПредприятия Из ТаблицаРесурсовПредприятия Цикл
		...
		КратностьПланированияРесурса = РесурсПредприятия.КратностьПланирования*60;
		...
		Для Каждого ДеньСписка Из ОбрабатываемыйСписокДат Цикл
			...
			КоличествоЯчеекДляОбъединенияРасшифровки = 0;
			КоличествоЯчеекДляОбъединенияИнтервалыСовпадаютСКратностью = 0;
			...
			Пока Интервал <= КоличествоПериодовЗаДеньСписка Цикл
				...
				Если КоличествоЯчеекДляОбъединения >=1 Тогда
					...
					ДлинаПериода = (ОкончанияИнтервала - НачалоИнтервала)/60;
		...
		Если ЕстьРасшифровка Тогда
			...
			ЗагрузкаРесурсов.Вывести(ОбластьРесурс,НомерГруппы,"Расшифровка по дукемнтам"+Строка(РесурсПредприятия.Ресурс), Ложь);
...
Процедура ВывестиДокументыРасшифровки(ДокументыРасшифровки,ИнтервалыВыводаПоДокументам,СоответствиеДокументовРасшифровкиСтрокам
	...
	Для Каждого ДокументРасшифровки Из ДокументыРасшифровки Цикл
		...
		КоличествоЯчеекВИнтервале = (ОкончанияИнтервала - НачалоИнтервала)/60/5;
		...
		Если СтрокиПоДокументу.Количество() Тогда
			...
			Если СтрокаПоДокументу.КоличествоЯчеек = 0 Тогда
				...
				КоличествоИнтерваловВывода = ИнтервалыВыводаПоДокументам.Количество();
				Если КоличествоИнтерваловВывода Тогда
					ПоследнийВыведенныйИнтервал = ИнтервалыВыводаПоДокументам[КоличествоИнтерваловВывода-1];
					ЕстьПересечение = ?(ДокументРасшифровки.ВремяНачала < ПоследнийВыведенныйИнтервал.ОкончаниеПредыдущегоИнтервалаЗагрузки, Истина, Ложь);
				КонецЕсли;
		...
		ОбластьУточенияПунктиром = ОбластьРасшифровкаПоДокументам.Область(СтрокаРасшифровки,3+ДопСмещениеКолонки,СтрокаРасшифровки
		,КолонкаОкончанияИнтервала);
		ОбластьУточенияПунктиром.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.РедкийПунктир);
		ОбластьУточенияПунктиром.ЦветРамки = WebЦвета.СеребристоСерый;
...
&НаСервере
Функция МассивДокументовСПересечениямиЗаВесьПериод(ДокументыПоРесурсу, ТаблицаРаспиания)
	...
	Для Каждого СтрокаДокументов Из МассивСсылок Цикл
		...
		НайденныеСтроки = ТаблицаРаспиания.НайтиСтроки(ПараметрыОтбора);
...
Функция МаксимумПоЗагрузке(ДокументыРасшифровки, НачалоИнтервала, ОкончанияИнтервала, КоличествоЯчеекВИнтервале, ТаблицаИнтервалаПо5Мин)
	...
	Для Каждого СтрокаДокументаРасшифровки Из ДокументыРасшифровки Цикл
		...
		Пока НомерЯчейкиНачалаИнтервала <= НомерЯчейкиОкончанияИнтервала Цикл
			...
			ИмяКолокни = "Колонка"+Строка(НомерЯчейкиНачалаИнтервала);
			
			НоваяСтрока[ИмяКолокни] = СтрокаДокументаРасшифровки.Загрузка;
...
&НаСервере
Функция ПолучитьЗначениеИнтервала(СписокКрастностейРесурсов)
	
	Первое = СписокКрастностейРесурсов[0].Значение;
	
	Второе = ?(СписокКрастностейРесурсов.Количество() = 1,Первое, СписокКрастностейРесурсов[1].Значение);
	
	СписокРекурсии = Новый СписокЗначений;
	...
	Если Остаток > 0 Тогда
		...
		СписокРекурсии.Добавить(Первое);
		СписокРекурсии.Добавить(Второе);
		СписокРекурсии.СортироватьПоЗначению(НаправлениеСортировки.убыв);
		
		Если СписокКрастностейРесурсов.Количество()>2 Тогда
			СписокКрастностейРесурсов.Удалить(СписокКрастностейРесурсов[0]);
			СписокКрастностейРесурсов.Удалить(СписокКрастностейРесурсов[0]);
		КонецЕсли;
		
		НОД = ПолучитьЗначениеИнтервала(СписокРекурсии);
	...
	Иначе
		...
		Если СписокКрастностейРесурсов.Количество()>2 Тогда
			...
			Если СписокКрастностейРесурсов[0].Значение%Второе > 0
				Тогда
				СписокРекурсии.Очистить();
				СписокРекурсии.Добавить(СписокКрастностейРесурсов[0].Значение);
				СписокРекурсии.Добавить(Второе);
				СписокРекурсии.СортироватьПоЗначению(НаправлениеСортировки.убыв);
				НОД = ПолучитьЗначениеИнтервала(СписокРекурсии);
...
&НаСервере
Функция СтруткрутраИнтервалаПланирования()
...
Функция СтруктураДанныхСобытийПоКонтактам(СписокРесурсов, СписокДокументов)
	...
	СтурктураВозврата = Новый Структура("СписокРесурсов, СписокДокументов",МассивРесурсов, МассивДокументов);
...
Функция ПолучитьГрафикРаботыРесурса(СписокРесурсов, ДатаГрафика)
	...
	Для Каждого СтрокаРезультат Из Результат Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаРезультат.НачалоРабочегоПериодаЗаДень) Тогда Продолжить КонецЕсли;
		
		ДоступнаяЗагрузка = ?(СтрокаРезультат.ДоступнаяЗагрузка - Цел(СтрокаРезультат.ДоступнаяЗагрузка)>0
		,Цел(СтрокаРезультат.ДоступнаяЗагрузка)+1, СтрокаРезультат.ДоступнаяЗагрузка);
		
		СтруктураЭлементаПериода = Новый Структура("НачалоПериода, ОкончаниеПериода, ДоступнаяЗагрузка, КратностьПланирования, КонтролироватьШаг,"
		,СтрокаРезультат.НачалоРабочегоПериодаЗаДень, СтрокаРезультат.ОкончаниеРабочегоПериодаЗаДень
		,СтрокаРезультат.ДоступнаяЗагрузка, СтрокаРезультат.КратностьПланирования, СтрокаРезультат.КонтролироватьШаг);
...
Функция СтруктураРесурсовИДокументов()
	...
	СписокДокументовПоВыбраннымКонтактам = Новый СписокЗначений;
...
Процедура УдалитьМеткуОтбора(МеткаИД)
	...
	Если Не ДанныеМеток.Количество() Тогда
		Элементы.ГруппаСтраницаОтбор.Заголовок = "Отбор";
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиФормы()
	...
	Элементы.ДекорацияПодобрано.Заголовок = "Подобрано: " + Строка(ВыбранныеРесурсы.Количество());
...
&НаСервере
Функция СформироватьСписокВыбораВремениПереходаПоКоординатам(КоличествоПериодов, СтрокаТЧ = Неопределено, НачалоИнтервала)
...
&НаСервере
Функция СформироватьСписокВыбораВремениПериодов()
...
Процедура НастроитьОтображениеКорзины(ОткрытиеФормы = Ложь, ОчищатьВыделениеПриОбновленииПериодов = Истина)
	...
	Элементы.ДекорацияПодобрано.Заголовок = "Подобрано: " + Строка(КоличествоПодобранных);
...
Функция СоответствиеДнейНедели()
	...
	СоответствиеВовзрата = Новый Соответствие;
	
	СоответствиеВовзрата.Вставить(1, "пн.");
	СоответствиеВовзрата.Вставить(2, "вт.");
	СоответствиеВовзрата.Вставить(3, "ср.");
	СоответствиеВовзрата.Вставить(4, "чт.");
	СоответствиеВовзрата.Вставить(5, "пт.");
	СоответствиеВовзрата.Вставить(6, "сб.");
	СоответствиеВовзрата.Вставить(7, "вс.");
	
	Возврат СоответствиеВовзрата;
...
&НаСервере
Функция ДоступенПрофиль(ИмяРоли)
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("Пользователь", Пользователи.АвторизованныйПользователь());
	Запрос.УстановитьПараметр("Профиль", Справочники.ПрофилиГруппДоступа.НайтиПоНаименованию(ИмяРоли));
	...
	Для Каждого ДатаПериода Из СписокДатПериода Цикл
		...
		Для Каждого СтрокаРесурс Из РесурсыПоСтрокам Цикл 
			...
			Если ПредварительнаяОчистка Тогда 
				...
				КоординатаКолонкиНачалаПозиционирования = Формат(ТекИнтервалДаты.КоординатаНачала,"ЧГ=");
				КоординатаКолонкиОкончанияПозиционирования = Формат(ТекИнтервалДаты.КоординатаОкончания,"ЧГ=");
				
				Область = ЗагрузкаРесурсов.Область(НомерСтрокиРесурса-1,КоординатаКолонкиНачалаПозиционирования,НомерСтрокиРесурса-1,КоординатаКолонкиОкончанияПозиционирования);
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				НачалоВыделяемогоИнтервала = Час(НайденнаяСтрока.Старт)*60+Минута(НайденнаяСтрока.Старт)-ОтклонениеНачалаДня;
				КоординатаКолонкиНачалаПозиционированияЧислом = ТекИнтервалДаты.КоординатаНачала + НачалоВыделяемогоИнтервала/МинимальныйИнтервал;
				КоординатаКолонкиНачалаПозиционирования = Формат(КоординатаКолонкиНачалаПозиционированияЧислом,"ЧГ=");
				...
				КоординатаКолонкиОкончанияПозиционированияЧислом = ТекИнтервалДаты.КоординатаНачала + ОкончаниеВыделяемогоИнтервала/МинимальныйИнтервал;
				КоординатаКолонкиОкончанияПозиционирования = Формат(КоординатаКолонкиОкончанияПозиционированияЧислом,"ЧГ=");
				...
				Область = ЗагрузкаРесурсов.Область(НомерСтрокиРесурса-1,КоординатаКолонкиНачалаПозиционирования,НомерСтрокиРесурса-1,КоординатаКолонкиОкончанияПозиционирования-1);
...
Процедура ПриАктивизацииТабличногоДокумента()
	...
	НужноПерезаполнениеСпискаПозиционирования = Ложь;
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаПодбоИзКонтесктногоМеню(Результат, ПараметрыОповещения) Экспорт
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
