	...
	Для Каждого ТекущиеДанные Из ВыбранныеРесурсы Цикл
		...
		Если ТекущиеДанные.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.Ежемесячно")
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.ДатаПовторения) Тогда
				...
				ДополнениеПоМесяцуГоду = ?(ЗначениеЗаполнено(ТекущиеДанные.ДатаПовторения), ", каждое " 
										+ Строка(ТекущиеДанные.ДатаПовторения) + "-ое "+ПланированиеРесурсовУНФКлиент.ПолучитьМесяцПоНомеру(ТекущиеДанные.НомерМесяца)+".","");
			ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.ДеньНеделиМесяца) Тогда
				...
				Если ПланированиеРесурсовУНФКлиент.ЭтоПоследняяНеделяМесяца(ТекущиеДанные.НачалоПериода) Тогда
					ДополнениеПоМесяцуГоду = ", в послед. " + ПланированиеРесурсовУНФКлиент.СоответствиеНомераДнюНедели(ТекущиеДанные.ДеньНеделиМесяца)+ " месяца.";
				Иначе
				НомерНеделиМесяца = НеделяГода(ТекущиеДанные.НачалоПериода)-НеделяГода(НачалоМесяца(ТекущиеДанные.НачалоПериода))+1;
				ДополнениеПоМесяцуГоду = " "+ПланированиеРесурсовУНФКлиент.СоответствиеНомераДнюНедели(ТекущиеДанные.ДеньНеделиМесяца) + " кажд. " +Строка(НомерНеделиМесяца)+ "-й" + " недели";
				КонецЕсли;
		...
		Междометие = ?(ТекущиеДанные.ВидПовтора = "Еженедельно","каждую", "каждый");
...
Процедура КонтрольНаСервере()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРесурсов Цикл
		...
		КоличестоЯчеекВИнтервале = (ОкончаниеПериода - СтрокаТаблицы.НачалоПериода)/300;
		ЗагрузкаЗаИнтервал = ПланированиеРесурсовУНФ.МаксимумПоЗагрузке(ДокументыЗаПериод, СтрокаТаблицы.НачалоПериода, ОкончаниеПериода, КоличестоЯчеекВИнтервале, МатрицаИнтервалов);
		...
		Если (ЗагрузкаЗаИнтервал+СтрокаТаблицы.Загрузка)>Мощность Тогда
			...
			Если ЗагрузкаЗаИнтервал = 0 Тогда
				Сообщение.Текст = НСтр("ru = 'Для %Ресурс% в период с %НачалоПериода% по %ОкончаниеПериода% текущее значение загрузки превышает допустимую. Доступная загрузка %ТекущаяЗагруза%'");
				ЗагрузкаЗаИнтервал = Мощность;
			Иначе
				Сообщение.Текст = НСтр("ru = 'Для %Ресурс% в период с %НачалоПериода% по %ОкончаниеПериода% загрузка с учетом текущей превышает допустимую. Доступная загрузка %ТекущаяЗагруза%'");
...
Функция ЕстьОшибкиЗаполнения()
	...
	Для Каждого СтрокаРесурс Из ВыбранныеРесурсы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаРесурс.НачалоПериода) Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указано начало периода(Старт)'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			ЭтотОбъект,
			ТекстСообщения,
		...
		Если Не ЗначениеЗаполнено(СтрокаРесурс.ОкончаниеПериода) Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указано окончание периода(Финиш).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			ЭтотОбъект,
			ТекстСообщения,
		...
		Если НачалоДня(СтрокаРесурс.НачалоПериода) = НачалоДня(СтрокаРесурс.ОкончаниеПериода) 
			И ЗначениеЗаполнено(СтрокаРесурс.ВидПовтора) И Не СтрокаРесурс.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.НеПовторять") Тогда
			
			Если Не ЗначениеЗаполнено(СтрокаРесурс.ВидЗавершения) Тогда
				ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указан вид завершения.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
			...
			Если Не ЗначениеЗаполнено(СтрокаРесурс.ЗавершатьПосле) Тогда
				ТекстСообщения = НСтр("ru = 'В строке №%Номер% табл. части не указан параметр окончания повторов.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
