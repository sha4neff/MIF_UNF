...
Процедура ВводНачальныхОстатковТоварыЗапасыПередУдалением(Элемент, Отказ)
	...
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(ВводНачальныхОстатковТовары.СерийныеНомера, СтрокаТабличнойЧасти,
...
Процедура ЗаполнитьПоФормуле()
	...
	Обработки.ФормированиеЦенНоменклатуры.ПодготовитьОперандыИДанные(ПараметрыРасчета, ТаблицаОперандов);
	...
	ЦенообразованиеФормулыСервер.РасчитатьДанныеКоллекции(КоллекцияНоменклатуры, ТаблицаОперандов, ПараметрыРасчетаКоллекции, Ложь, Истина);
...
Процедура УстановитьВидимостьКурсов()
	...
	Иначе
		КурсыСебестоимости = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ВалютаСебестоимости, ВалютаУчета, ДатаОстатков); 
	...
	Если НЕ ЗначениеЗаполнено(ВалютаПродажи) Тогда
		...
		КратностьПродажи = 1;
	Иначе
		КурсыПродажи = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ВалютаПродажи, ВалютаУчета, ДатаОстатков); 
...
Функция ПересчитатьИзВалютыВНациональнуюВалюту(СуммаВал, ВалютаДоговор, ДатаКурса) Экспорт
	...
	Если ЗначениеЗаполнено(ВалютаДоговор) Тогда
		...
		СтруктураКурсовВалют = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(Валюта, Валюта, ДатаКурса);
