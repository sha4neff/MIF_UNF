...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Формат2019 = ПолучитьФормат2019Года(ДокументОтчетности);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СозданоЗаявлениеНаПодключениеКЭДОСКО" Тогда
		ПроверитьВозможностьОтправки();
	ИначеЕсли ИмяСобытия = "Запись_СведенияОСтраховомСтажеЗастрахованныхЛицСЗВ_СТАЖ" И Источник = ДокументОтчетности Тогда
		Формат2019 = ПолучитьФормат2019Года(ДокументОтчетности);
	КонецЕсли;
...
Функция ПолучитьДанныеФайлаНаСервере(ДокументОтчетности, УникальныйИдентификатор)
	...
	Возврат УчетСтраховыхВзносов.ПолучитьДанныеФайла(ДокументОтчетности, УникальныйИдентификатор);
КонецФункции
...
&НаКлиенте
Процедура ДокументОтчетностиФормат2019ПриИзменении(Элемент)
	УстановитьФормат2019Года(ДокументОтчетности, Формат2019);
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция ПолучитьФормат2019Года(ДокументОтчетности)
	Если Не ЗначениеЗаполнено(ДокументОтчетности) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОтчетности, "Формат2019");
	
КонецФункции

&НаСервереБезКонтекста
Процедура УстановитьФормат2019Года(ДокументОтчетности, Формат2019)
	Если Не ЗначениеЗаполнено(ДокументОтчетности) Тогда
		Возврат;
	КонецЕсли;
	
	ДокументОбъект = ДокументОтчетности.ПолучитьОбъект();
	ДокументОбъект.Формат2019 = Формат2019;
