...
Процедура Подбор(Команда)
	...
	#Если МобильныйКлиент Тогда
		...
		ИмяФормыПодбора = "Обработка.ПодборПечатьЭтикетокИЦенников.Форма.КорзинаЦенаОстатокРезервХарактеристика_МК";
		
	#Иначе
		
		ИмяФормыПодбора = "Обработка.ПодборПечатьЭтикетокИЦенников.Форма.КорзинаЦенаОстатокРезервХарактеристика";
		
	#КонецЕсли
	
	ОткрытьФорму(ИмяФормыПодбора, ЗаполнитьПараметрыПодбора(), ЭтаФорма, Истина, , , ОписаниеОповещенияПриЗакрытииПодбора, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьПоляТаблицы(ЗаполняемыеПоля)
	...
	Запрос.УстановитьПараметр("Товары",			Объект.Товары.Выгрузить(, "Номенклатура, Характеристика, Партия"));
