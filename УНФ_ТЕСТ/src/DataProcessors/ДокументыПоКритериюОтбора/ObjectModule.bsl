...
Процедура ЗаполнитьТаблицуЗапросов(ТаблицаЗапросов, 
	...
	Для Каждого СтрокаКорня Из ДеревоДокументов.Строки Цикл
		...
		Для Каждого СтрокаРеквизитТЧ Из СтрокаКорня.Строки Цикл
			...
			ТекстУсловия = ""; ТекстУсловияПоВидам = ""; ТекстУсловияКэша = "";
			...
			Если НЕ РольДоступна("ПолныеПрава") И УточняемыеОбъекты[СтрокаКорня.Документ.Имя] <> Неопределено Тогда
				ВремТекстУсловия = СтрЗаменить(ШаблонУсловияШапки, "%ПолеПараметрШапки%", "ВидОперации");
				ВремТекстУсловия = СтрЗаменить(ВремТекстУсловия, "= &Параметр", "В (&МассивДопустимыхВидовОпераций" + СтрокаКорня.Документ.Имя + ")");
				ТекстУсловияПоВидам = ТекстУсловияПоВидам + ?(ПустаяСтрока(ТекстУсловияПоВидам),"", " И ") + ВремТекстУсловия;
			КонецЕсли;
