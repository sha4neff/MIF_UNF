...
Процедура ЗаполнитьПоФормуле(Результат)
	...
	Если ТЧНоменклатура.Количество() > КэшЗначений.КоличествоСтрокПриемлевогоБыстродействия Тогда
		...
		РасчитатьДанныеКоллекцииВДлительнойОперации(ПараметрыФормированияЦен);
...
Процедура ДобавитьНоменклатуруВДлительнойОперации(СтруктураТаблицДанных, ПараметрыКопирования)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Копирование полученых строк в табличные части обработки'");
...
&НаСервере
Процедура РасчитатьДанныеКоллекцииВДлительнойОперации(ПараметрыПроцедуры)
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Если СтрокиПереходаТекущие.Количество() = 0 Тогда
		ЗаписьЖураналаЕслиНеОпределенаСтраница("ПорядковыйНомерПереходаПриИзменении", "<В этой функции не используется>");
...
Процедура ВыполнитьОбработчикиСобытийПерехода(Знач ЭтоПереходДалее)
	...
	Если СтрокиПереходаТекущие.Количество() = 0 Тогда
		ЗаписьЖураналаЕслиНеОпределенаСтраница("ВыполнитьОбработчикиСобытийПерехода", СтрокаПереходаВСтроку(СтрокаПерехода));
		ВызватьИсключение НСтр("ru = 'Не определена страница для отображения.'");
	КонецЕсли;
...
&НаСервере
Процедура ЗаписьЖураналаЕслиНеОпределенаСтраница(ИмяФункции, СтрокаПерехода)
	
	КомменатрийЖурнала = "ИмяФункции = " + ИмяФункции
		+ Символы.ПС
	...
	ЗаписьЖурналаРегистрации(
		"Не определена страница для отображения",
...
Процедура ВыполнитьОбработчикДлительнойОперации()
	...
	Если СтрокиПереходаТекущие.Количество() = 0 Тогда
		ЗаписьЖураналаЕслиНеОпределенаСтраница("ВыполнитьОбработчикДлительнойОперации", "<В этой функции не используется>");
		ВызватьИсключение НСтр("ru = 'Не определена страница для отображения.'");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВидыЦенИспользование", "Формат", "БЛ=пропустить; БИ=расчитывается");
