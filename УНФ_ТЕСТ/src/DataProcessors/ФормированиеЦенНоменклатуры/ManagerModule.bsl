...
Процедура ПолучитьДеревоВидовЦен(ДеревоРезультата, МассивВидовЦен, ВключаяЦеныНоменклатуры = Истина, ВключаяЦеныКонтрагентов = Истина) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ВидыЦен.Ссылка КАК ВидЦен, Неопределено КАК Использование, Представление(ВидыЦен.Ссылка) КАК Представление, ИСТИНА КАК ЭтоВидЦенНоменклатуры, 0 КАК Картинка, ЛОЖЬ КАК РазрешитьВыбирать, ВидыЦен.БазовыйВидЦен КАК БазовыйВидЦен, ВЫБОР КОГДА ВидыЦен.ТипВидаЦен = Значение(Перечисление.ТипыВидовЦен.ДинамическийФормула) ТОГДА ""Формула"" КОГДА ВидыЦен.ТипВидаЦен = Значение(Перечисление.ТипыВидовЦен.ДинамическийПроцент) ТОГДА ""Динамический"" ИНАЧЕ ""Статический"" КОНЕЦ КАК Вид, ВидыЦен.ИдентификаторФормул КАК ИдентификаторФормул
...
Процедура РассчитатьНовыеЦеныПоФормуле(ПараметрыРасчета) Экспорт
	...
	Для каждого ЭлементСоответствия Из ПараметрыРасчета.ВидыЦенКПересчету Цикл
		...
		ЦенообразованиеФормулыСервер.РасчитатьДанныеКоллекции(КоллекцияНоменклатуры, ТаблицаОперандов, ПараметрыРасчетаКоллекции, Истина, Истина);
