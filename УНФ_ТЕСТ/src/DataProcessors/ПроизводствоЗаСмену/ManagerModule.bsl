...
Функция ВыполнитьЭтапСборкаЗапасов(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	Если СтрокиПродукция.Количество()=0 Тогда
		...
		УправлениеНебольшойФирмойКлиентСервер.ЗаполнитьКлючСвязи(ДокументОбъект.Продукция, НоваяСтрокаПродукция, "КлючСвязи");
	Иначе
...
Функция ПроверитьСостав(ЗначенияПолей, ТаблицаСостава, Ошибки)
	...
	КонтрольСерийныхНомеров = ПолучитьФункциональнуюОпцию("КонтрольОстатковСерийныхНомеров");
	КонтрольГТД = Константы.КонтролироватьОстаткиПоНомерамГТД.Получить();
	Если НЕ КонтрольСерийныхНомеров И НЕ КонтрольГТД Тогда
		Возврат Истина;
	КонецЕсли; 
	
	СписокСерийныеНомера = Новый СписокЗначений;
	СписокНомераГТД = Новый СписокЗначений;
	Для каждого СтрокаТабличнойЧасти Из ТаблицаСостава Цикл
		Если КонтрольСерийныхНомеров 
			И СтрокаТабличнойЧасти.ИспользоватьСерийныеНомера 
			И СписокСерийныеНомера.НайтиПоЗначению(СтрокаТабличнойЧасти.Номенклатура)=Неопределено Тогда
			СписокСерийныеНомера.Добавить(СтрокаТабличнойЧасти.Номенклатура);
		КонецЕсли;
		Если КонтрольГТД 
	...
	Если СписокСерийныеНомера.Количество()>0 ИЛИ СписокНомераГТД.Количество()>0 Тогда
		...
		Ошибки.Добавить(, Ошибка);
	КонецЕсли; 
	Если СписокСерийныеНомера.Количество()>0 Тогда
		Ошибка = СтрШаблон(НСтр("ru = 'Автоматическое формирование документов производства невозможно при использовании материалов с учетом по серийным номерам (%1)'"), Строка(СписокСерийныеНомера));
		Ошибки.Добавить(, Ошибка);
	КонецЕсли; 
	...
	Возврат (СписокСерийныеНомера.Количество()=0 И СписокНомераГТД.Количество()=0);
...
Функция ВыполнитьЭтапСдельныйНаряд(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	Для каждого СтрокаОперации Из ТаблицаОпераций Цикл
		...
		УправлениеНебольшойФирмойКлиентСервер.ЗаполнитьКлючСвязи(ДокументОбъект.Операции, НоваяСтрокаОперация, "КлючСвязи");
...
Функция ОтменитьЭтапСборкаЗапасов(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	СтрокиВыполненныеЭтапы = ДокументОбъект.ВыполненныеЭтапы.НайтиСтроки(СтруктураПоискаКлючСвязи);
	Если СтрокиВыполненныеЭтапы.Количество()=0 Тогда
		УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(ДокументОбъект.СерийныеНомераПродукция, СтрокаПродукция);
...
Функция ОтменитьЭтапСдельныйНаряд(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	СтрокиОперации = ДокументОбъект.Операции.НайтиСтроки(СтруктураПоиска);
	Для каждого СтрокаОперации Из СтрокиОперации Цикл
		УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(ДокументОбъект.СоставБригады, СтрокаОперации);
		ДокументОбъект.Операции.Удалить(СтрокаОперации);
	КонецЦикла;
...
Функция СоставЭтапа(ЗначенияПолей)
	...
	Если ЗначениеЗаполнено(ЗначенияПолей.ЗаказНаПроизводство) Тогда
		...
		|	ЗаказНаПроизводствоРаспределениеЗапасов.Номенклатура.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера
		|ИЗ
	...
	Иначе	
		...
		|	СпецификацииСостав.Номенклатура.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера
		|ИЗ
