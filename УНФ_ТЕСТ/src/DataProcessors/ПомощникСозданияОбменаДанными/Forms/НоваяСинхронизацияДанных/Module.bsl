...
Процедура ДобавитьКомандуСозданияНовогоОбменаДляВариантаНастройки(
	...
	ОтключитьДоступностьЧтобыПоказатьЧтоУНФУмеетВРИБПоОрганизации(ЭлементСсылка,
		ОписаниеВариантаНастройки.ЗаголовокКомандыДляСозданияНовогоОбменаДанными);
	
КонецПроцедуры

&НаСервере
Процедура ОтключитьДоступностьЧтобыПоказатьЧтоУНФУмеетВРИБПоОрганизации(КнопкаКоманды, ЗаголовокКоманды)
	
	Если ЗаголовокКоманды = ПланыОбмена.ПоОрганизации.ЗаголовокИспользуетсяУчетПоКомпании() Тогда
		КнопкаКоманды.Доступность = Ложь;
	КонецЕсли;
