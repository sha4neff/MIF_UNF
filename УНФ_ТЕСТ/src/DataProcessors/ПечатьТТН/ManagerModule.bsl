...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		СведенияОПоставщике = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.ЮрФизЛицо, Шапка.ДатаДокумента,
			, Шапка.БанковскийСчет);
		СведенияОГрузоотправителе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Грузоотправитель,
		...
		СведенияОГрузополучателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Грузополучатель,
			Шапка.ДатаДокумента);
		...
		ОбластьШапка.Параметры.ГрузоотправительПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			СведенияОГрузоотправителе, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьШапка.Параметры.ГрузополучательПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			СведенияОГрузополучателе, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьШапка.Параметры.ПлательщикПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
			СведенияОПокупателе);
		...
		ИначеЕсли ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ПеремещениеЗапасов") Тогда
			...
			Для Каждого ВыборкаСтрокЗапасы Из ТабличнаяЧастьДокумента Цикл
				...
				ОбластьСтрока.Параметры.ЗапасНаименование = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(
		...
		Если ИтогоСуммаСНДС > 0 Тогда
			ОтпущеноНаСуммуПрописью = ОтпущеноНаСуммуПрописью
									  ...
									  + УправлениеНебольшойФирмойСервер.СформироватьСуммуПрописью(ИтогоСуммаСНДС,
		...
		ПеревозчикПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОПеревозчике,
		...
		Если Шапка.ОрганизацияОплачиваетПеревозку = Перечисления.ДаНет.Да Тогда
			...
			ЗаказчикПеревозокПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОПоставщике,
		...
		ИначеЕсли Шапка.ОрганизацияОплачиваетПеревозку = Перечисления.ДаНет.Нет Тогда
			...
			ЗаказчикПеревозокПредставление = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОПокупателе,
				"ПолноеНаименование, ИНН, ЮридическийАдрес, Телефоны, НомерСчета, Банк, БИК, КоррСчет");
			ЗначенияПараметров.Вставить("ЗаказчикПеревозок", ЗаказчикПеревозокПредставление);
		...
		Если ЗначениеЗаполнено(Шапка.Водитель) Тогда
			...
			Если ДокументыФизическихЛиц.Количество() > 0 Тогда
				...
				ВодительскоеУдостоверение = Нстр("ru = 'Серия '") + Строка(ДокументыФизическихЛиц[0].Серия) + Нстр(
					"ru = ' № '") + Строка(ДокументыФизическихЛиц[0].Номер);
		...
		Если ПустаяСтрока(ПунктПогрузки) Тогда
			...
			ПунктПогрузки = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОГрузоотправителе,
				"ФактическийАдрес");
		...
		Если ПустаяСтрока(ПунктРазгрузки) Тогда
			...
			ПунктРазгрузки = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОГрузополучателе,
				"ФактическийАдрес");
