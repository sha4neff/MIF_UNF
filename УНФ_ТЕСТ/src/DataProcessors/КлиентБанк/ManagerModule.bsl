...
Функция РазобратьБанковскуюВыписку(КонтекстФормы) Экспорт
	...
	Пока ИмпортТекущаяСтрока <= ИмпортКоличествоСтрок Цикл
		...
		Если Лев(ВРег(СокрЛП(Стр)), 14) = "СЕКЦИЯДОКУМЕНТ" Тогда
			...
			Если Тег = "СЕКЦИЯДОКУМЕНТ" Тогда
				...
				Если НЕ ЗагрузитьСекциюДокумента(Результат, НоваяСтрокаДокументов, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ИмпортЗагружаемые, ВыдаватьСообщенияОНеверныхРеквизитахИСекциях) Тогда
					Возврат Результат;
				КонецЕсли;
			Иначе
		...
		ИначеЕсли Лев(ВРег(СокрЛП(Стр)), 8) = "РАСЧСЧЕТ" Тогда
			...
			Если Тег = "РАСЧСЧЕТ" И ЗначениеЗаполнено(Значение) Тогда
				...
				Выборка = ПолучитьВыборкуПоНомеруСчета(Значение, ЕстьОтборПоБанковскомуСчету, Объект.БанковскийСчет);
				Если Выборка.Следующий() Тогда
					НайденныйБанковскийСчет = Выборка.Ссылка;
					Если НЕ ЕстьОтборПоБанковскомуСчету И ЗначениеЗаполнено(Объект.БанковскийСчет)
					   ...
					   Если СокрЛП(Объект.БанковскийСчет.НомерСчета) = Значение Тогда
		...
		ИначеЕсли Лев(ВРег(СокрЛП(Стр)), 10) = "КОНЕЦФАЙЛА" Тогда
			...
			Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
				...
				// банковскому счету.
				Если СокрЛП(СтрокаДокумента.ПлательщикСчет) = СокрЛП(Объект.БанковскийСчет.НомерСчета)
				 ИЛИ СокрЛП(СтрокаДокумента.ПолучательСчет) = СокрЛП(Объект.БанковскийСчет.НомерСчета) Тогда
					РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, ПараметрыВыделенияКолонок, Объект, ТаблицаТаблицДокументов, КонтекстФормы, ПослеЗагрузкиВыписокВ1С);
		...
		ИначеЕсли Не ПустаяСтрока(Стр) Тогда
			ЗагрузитьСтрокуЗаголовка(Стр, ТегиЗаголовка, ИмпортЗаголовок, ИмпортТекущаяСтрока, ВыдаватьСообщенияОНеверныхРеквизитахИСекциях);
		КонецЕсли;
	...
	Результат.Вставить("ИмпортПризнакОбмена",  ИмпортПризнакОбмена);
...
Функция ЗагрузитьСекциюДокумента(Результат, СтрокаДокумента, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ИмпортЗагружаемые, ВыдаватьСообщенияОНеверныхРеквизитахИСекциях)
	...
	Пока Лев(Врег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			ИначеЕсли ВыдаватьСообщенияОНеверныхРеквизитахИСекциях Тогда
				
				// Неправильный реквизит заголовка.
				ТекстСообщения = НСтр("ru = 'Неверный реквизит платежного документа, строка %Импорт%: %СтрокаРазбора%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
		...
		Иначе
			...
			Если ВыдаватьСообщенияОНеверныхРеквизитахИСекциях Тогда
				// Неправильный реквизит заголовка.
				ТекстСообщения = НСтр("ru = 'Нарушена структура платежного документа, строка %Импорт%: %СтрокаРазбора%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
				Результат.ОбработкаПрервана = Истина;
				Результат.ТекстСообщения = ТекстСообщения;
...
Функция ЗагрузитьСекциюРасчСчета(СтрокаРССчета, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ТегиРасчетногоСчета)
	...
	Пока РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Цикл
		...
		Иначе
			// Неправильный реквизит заголовка.
			ТекстСообщения = НСтр("ru = 'Неверный реквизит в секции описания расчетного счета, строка %Импорт%: %СтрокаРазбора%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗагрузитьСтрокуЗаголовка(ТекстСтрокиЗаголовка, ТегиЗаголовка, ИмпортЗаголовок, ИмпортТекущаяСтрока, ВыдаватьСообщенияОНеверныхРеквизитахИСекциях)
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, ПараметрыВыделенияКолонок, Объект, ТаблицаТаблицДокументов, КонтекстФормы, ПослеЗагрузкиВыписокВ1С = Ложь)
