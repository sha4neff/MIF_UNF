...
// Функция создает и возвращает таблицу значений для хранения объектов, которые созданы в процессе загрукзи. Таблица
...
Функция Загрузить(ИмпортЗаголовок, ТаблицаРезультата, ТаблицаПомеченныхНаУдалениеКакДубль) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		Возврат Новый Структура("ЕстьПредупреждения, ОбработаноСтрок, ВсегоСтрок", ЕстьПредупреждения, 0, ДокументыКИмпорту.Количество());
	КонецЕсли;
