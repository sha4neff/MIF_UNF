...
Процедура ВывестиОтчетВзаиморасчеты()
	...
	Если НастройкиКомпоновки.ПараметрыДанных.Элементы.Найти("СтПериод")<>Неопределено Тогда
		НастройкиКомпоновки.ПараметрыДанных.УстановитьЗначениеПараметра("СтПериод", Новый СтандартныйПериод(Параметры.НачалоПериода, КонецДня(Параметры.КонецПериода)));
	КонецЕсли; 
