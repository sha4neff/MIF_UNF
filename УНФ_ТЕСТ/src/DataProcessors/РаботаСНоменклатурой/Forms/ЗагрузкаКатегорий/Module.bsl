...
Процедура ВычислениеКатегорийЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли СтатусЗадания = "Выполнено" 
		...
		ОбработатьКатегорииТоварныхГрупп(Результат.АдресРезультата
			, ТекстВопроса
...
&НаСервере
Процедура НайтиДобавитьУзелВДерево(ПутиККорню, ДеревоКатегорийФормы, ТекущийУзел, ТекущийИдентификатор, ЭтоСозданиеДерева = Истина)
	...
	НайтиДобавитьСтрокуДерева(ПутиККорню, СтрокиДерева, ТекущийИдентификатор, ЭтоСозданиеДерева);
...
&НаСервере
Процедура ОбработатьДеревоПослеДополнения(ДеревоКатегорийФормы, ЗначениеПометки)
	
	Для каждого СтрокаДерева Из ДеревоКатегорийФормы.Строки Цикл
		Если СтрокаДерева.Пометка <> ЗначениеПометки Тогда
			ЗначениеПометки = 2;
		КонецЕсли;
		Если НЕ СтрокаДерева.ЛистоваяКатегория 
			И НЕ СтрокаДерева.ПустаяГруппа Тогда
			КоличествоПодчиненных = СтрокаДерева.Строки.Количество();
			Если КоличествоПодчиненных = 0 Тогда
				ПустаяКатегория = СтрокаДерева.Строки.Добавить();
				ПустаяКатегория.ПустаяГруппа = Истина;
			ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаДерева.КоличествоПодчиненных) Тогда
				СтрокаДерева.КоличествоПодчиненных = КоличествоПодчиненных;
			КонецЕсли;
			ПометкаСтрок = СтрокаДерева.Строки[0].Пометка;
			ОбработатьДеревоПослеДополнения(СтрокаДерева, ПометкаСтрок);
			Если СтрокаДерева.Строки[0].Пометка <> ПометкаСтрок Тогда
				ПометкаСтрок = 2;
			КонецЕсли;
			Если СтрокаДерева.Пометка <> ПометкаСтрок Тогда
				СтрокаДерева.Пометка = ПометкаСтрок;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если УстановленОтборКатегорий Тогда
		Для каждого ТекущийУзел Из ПутиККорню Цикл
			Если ТипЗнч(ТекущийУзел.Значение) <> Тип("Структура") Тогда
				Продолжить;
			КонецЕсли;
			НайтиДобавитьУзелВДерево(ПутиККорню, СлужебнаяКатегорияКорня, ТекущийУзел.Значение, ТекущийУзел.Ключ, Ложь);
		КонецЦикла;
		СлужебнаяКатегорияКорня.ДочерниеПодгружены = Истина;
		ОбработатьДеревоПослеДополнения(ДеревоКатегорийФормы, СлужебнаяКатегорияКорня.Пометка);
