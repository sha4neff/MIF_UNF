...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	//{kp-unf.ru
	ИначеЕсли ИмяСобытия="КП_Задача" ИЛИ ИмяСобытия="КП_КорпоративныйБизнесПроцесс" Тогда
		ПодключитьОбработчикОжидания("КП_МоиЗадачиКлиент", 1, Истина);
	//kp-unf.ru}		
	КонецЕсли;
...
#Область КП_МоиЗадачи //kp-unf.ru

&НаКлиенте
Процедура КП_МоиЗадачиКлиент() Экспорт
...
&НаСервере
Процедура КП_МоиЗадачи()
			
	УстановитьПривилегированныйРежим(Истина);
	
	Если НЕ ПолучитьФункциональнуюОпцию("КП_ИспользоватьПодсистему") Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ПравоДоступа("Чтение",Метаданные.Задачи.КП_Задача) Тогда
		Возврат;
	КонецЕсли;
	
	ВариантРазмещения=Константы.КП_РазмещениеВТекущиеДела.Получить();
	
	Если ВариантРазмещения=2 Тогда
		Элементы.ГруппаНовыеЗадачи.Видимость=Ложь;
	Иначе
		ТекущийПользователь=Пользователи.ТекущийПользователь();	
		
		СписокНовыхЗадач=КП_ЗадачиПроцессов.ПолучитьСписокЗадачПользователя(ТекущийПользователь, "Новые");
		СписокЗадачВРаботе=КП_ЗадачиПроцессов.ПолучитьСписокЗадачПользователя(ТекущийПользователь, "ВРаботе");
		КоличествоЗадачНовые=СписокНовыхЗадач.Количество();
		КоличествоЗадачВРаботе=СписокЗадачВРаботе.Количество();
		КоличествоВсегоЗадач=КоличествоЗадачНовые+КоличествоЗадачВРаботе;
		
		Если КоличествоЗадачНовые>0 Тогда
			Элементы.ГруппаНовыеЗадачи.Видимость=Истина;
			Элементы.ДекорацияНазваниеГруппыНовые.Заголовок="Новые задачи ("+Формат(КоличествоЗадачНовые, "ЧЦ=10; ЧДЦ=0; ЧН=0; ЧГ=")+")";			
			Элементы.КП_МоиЗадачи.Видимость=Истина;
			КП_ОбновитьСписокЗадач(СписокНовыхЗадач, Элементы.ГруппаНовыеЗадачиСписок, КоличествоВсегоЗадач, "Новые");
		Иначе
			Элементы.ГруппаНовыеЗадачи.Видимость=Ложь;
			Элементы.ДекорацияНазваниеГруппыНовые.Заголовок="Новые задачи";
		КонецЕсли;
		Если КоличествоЗадачВРаботе>0 Тогда
			Элементы.ДекорацияНазваниеГруппыВРаботе.Заголовок="Задачи в работе ("+Формат(КоличествоЗадачВРаботе, "ЧЦ=10; ЧДЦ=0; ЧН=0; ЧГ=")+")";			
			Элементы.ГруппаЗадачиВРаботе.Видимость=Истина;
			Элементы.КП_МоиЗадачи.Видимость=Истина;
			КП_ОбновитьСписокЗадач(СписокЗадачВРаботе, Элементы.ГруппаЗадачиВРаботеСписок, КоличествоВсегоЗадач, "ВРаботе");
		Иначе
			Элементы.ГруппаЗадачиВРаботе.Видимость=Ложь;
			Элементы.ДекорацияНазваниеГруппыВРаботе.Заголовок="Задачи в работе";
		КонецЕсли;
		
		Если КоличествоЗадачНовые=0 И КоличествоЗадачВРаботе=0 Тогда
			Элементы.КП_МоиЗадачи.Видимость=Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура КП_ДекорацияОткрытьВсеЗадачиНажатие(Элемент)
	ОткрытьФорму("Обработка.КП_НавигаторЗадач.Форма",,,,,,,РежимОткрытияОкнаФормы.Независимый);
КонецПроцедуры

&НаСервере
Процедура КП_ОбновитьСписокЗадач(СписокЗадач, Группа, КоличествоВсегоЗадач, Суффикс="")
	
	МаксимумЗадачВСписке=7;

	Для Каждого ПочиненныйЭлемент Из Группа.ПодчиненныеЭлементы Цикл
		ИмяЭлемента=ПочиненныйЭлемент.Имя;
		Если ТипЗнч(ПочиненныйЭлемент)=Тип("ГруппаФормы") Тогда
			Продолжить;
		КонецЕсли;
		ПочиненныйЭлемент.Видимость=Ложь;
	КонецЦикла;
	
	КоличествоЗадачВсего=СписокЗадач.Количество();
	НомерЗадачи=1;
	
	ТекущийПользователь=КП_ОбщееСервер.ТекущийПользователь();
	
	ВыводитьАвтораЗадачи=Константы.КП_ТекущиеДелаПоказыватьАвтора.Получить();
	
	Для Каждого ЭлементСписка Из СписокЗадач Цикл
		Если НомерЗадачи>МаксимумЗадачВСписке Тогда
			Прервать;
		КонецЕсли;
		ЗадачаСсылка=ЭлементСписка.Значение;
		Если ВыводитьАвтораЗадачи Тогда
			ЗаголовокЗадачи=СокрЛП(ЗадачаСсылка.Наименование)+" от "+КП_ОбщееСерверПС.ПолучитьФИОПользователя(ЗадачаСсылка.Автор)+" к "+Формат(ЗадачаСсылка.ДатаВыполненияПлан, "ДФ='HH:mm dd.MM.yy'");
		Иначе
			ЗаголовокЗадачи=СокрЛП(ЗадачаСсылка.Наименование)+" к "+Формат(ЗадачаСсылка.ДатаВыполненияПлан, "ДФ='HH:mm dd.MM.yy'");
		КонецЕсли;
		ИдентификаторЗадачи=ЗадачаСсылка.УникальныйИдентификатор();
		
		ИмяЭлемента="Задача_"+СтрЗаменить(ИдентификаторЗадачи, "-", "_")+"_"+Группа.Имя;
		...
		Элемент.Вид = ВидДекорацииФормы.Надпись;
		Элемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
		Элемент.Заголовок = ЗаголовокЗадачи;
		Элемент.АвтоМаксимальнаяШирина = Ложь;
		Элемент.Гиперссылка=Истина;
		
		СтруктураЦвета=КП_ОформлениеСтрокСервер.ПолучитьСтруктуруЦветаОбъекта(ЗадачаСсылка, ТекущийПользователь, Истина);
		Если СтруктураЦвета=Неопределено Тогда
			Если ЗадачаСсылка.ДатаВыполненияПлан<ТекущаяДата() Тогда
				Элемент.ЦветТекста=ЦветаСтиля.ПросроченноеЗадание;
			КонецЕсли;
		Иначе
			//установим  цвет пользователя
			Элемент.ЦветТекста=СтруктураЦвета.ЦветТекста;
			Элемент.ЦветФона=СтруктураЦвета.ЦветФона;
			Если ЗадачаСсылка.ДатаВыполненияПлан<ТекущаяДата() Тогда
				Элемент.ЦветРамки=ЦветаСтиля.ПросроченноеЗадание;
			КонецЕсли;
			Элемент.Шрифт=Новый Шрифт(,,СтруктураЦвета.ШрифтЖирный,СтруктураЦвета.ШрифтНаклонный);			
		КонецЕсли;
		
		Элемент.УстановитьДействие("Нажатие", "Подключаемый_КП_ОбработатьНажатиеНаЗадачу");
		НомерЗадачи=НомерЗадачи+1;
		
	КонецЦикла; //по задачам
		
	Если НомерЗадачи>МаксимумЗадачВСписке Тогда
		//добавим дополнительную ссылку на Навигатор задач
		ИмяЭлемента="Задача_ПоказатьВсеЗадачи_"+Суффикс;
		Элемент=Элементы.Найти(ИмяЭлемента);
		Если Элемент=Неопределено Тогда
			Элемент = Элементы.Добавить(ИмяЭлемента, Тип("ДекорацияФормы"), Группа);
		Иначе
		...
		Элемент.Вид = ВидДекорацииФормы.Надпись;
		Элемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
		Элемент.Заголовок = "Показать все задачи ("+Формат(КоличествоВсегоЗадач, "ЧЦ=; ЧН=0; ЧГ=")+")";
		Элемент.АвтоМаксимальнаяШирина = Ложь;
		Элемент.Гиперссылка=Истина;
		Элемент.УстановитьДействие("Нажатие", "Подключаемый_КП_ОбработатьНажатиеНаЗадачу");		
	...
	//добавим дополнительную ссылку на Панель задач
	ИмяЭлемента="Задача_ПоказатьПанельЗадач_"+Суффикс;
	Элемент=Элементы.Найти(ИмяЭлемента);
	Если Элемент=Неопределено Тогда
		Элемент = Элементы.Добавить(ИмяЭлемента, Тип("ДекорацияФормы"), Группа);
	Иначе
	...
	Элемент.Вид = ВидДекорацииФормы.Надпись;
	Элемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
	Элемент.Заголовок = "Показать панель задач";
	Элемент.АвтоМаксимальнаяШирина = Ложь;
	Элемент.Гиперссылка=Истина;
	Элемент.УстановитьДействие("Нажатие", "Подключаемый_КП_ОбработатьНажатиеНаЗадачу");		
...
&НаКлиенте
Процедура КП_ДекорацияКартинкаНовыеНажатие(Элемент)
	Элементы.ГруппаНовыеЗадачиСписок.Видимость=НЕ Элементы.ГруппаНовыеЗадачиСписок.Видимость;
	Если Элементы.ГруппаНовыеЗадачиСписок.Видимость Тогда
		Элементы.ДекорацияКартинкаНовые.Картинка=БиблиотекаКартинок.СтрелкаВниз;
	Иначе
		Элементы.ДекорацияКартинкаНовые.Картинка=БиблиотекаКартинок.СтрелкаВправоКрасная;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура КП_ДекорацияКартинкаВРаботеНажатие(Элемент)
	Элементы.ГруппаЗадачиВРаботеСписок.Видимость=НЕ Элементы.ГруппаЗадачиВРаботеСписок.Видимость;
	Если Элементы.ГруппаЗадачиВРаботеСписок.Видимость Тогда
		Элементы.ДекорацияКартинкаВРаботе.Картинка=БиблиотекаКартинок.СтрелкаВниз;
	Иначе
		Элементы.ДекорацияКартинкаВРаботе.Картинка=БиблиотекаКартинок.СтрелкаВправоКрасная;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_КП_ОбработатьНажатиеНаЗадачу(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;

	Попытка
		Если Найти(Элемент.Имя, "Задача_ПоказатьВсеЗадачи_")>0 Тогда
			ОткрытьФорму("Обработка.КП_НавигаторЗадач.Форма",,,,,,,РежимОткрытияОкнаФормы.Независимый);
			Возврат;
		КонецЕсли;
	
		Если Найти(Элемент.Имя, "Задача_ПоказатьПанельЗадач_")>0 Тогда
			ОткрытьФорму("Обработка.КП_ПанельЗадач.Форма",,,,,,,РежимОткрытияОкнаФормы.Независимый);
			Возврат;
		КонецЕсли;
	Исключение
		Сообщить("Нарушение прав доступа! Возможно не включен доступ к подсистеме ""Процессы"" в настройке профилей доступа. Обратитесь к администратору программы.");
		Возврат;
	КонецПопытки;
	
	ЗадачаСсылка=КП_ПолучитьЗадачуГиперссылки(Элемент.Имя);
	Если ЗадачаСсылка=Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы=Новый Структура("Ключ", ЗадачаСсылка);
	ОткрытьФорму("Задача.КП_Задача.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
	
КонецПроцедуры

&НаСервере
Функция КП_ПолучитьЗадачуГиперссылки(ИмяЭлемента)
	
	ИдентификаторЗадачи=СтрЗаменить(ИмяЭлемента, "Задача_", "");
	ИдентификаторЗадачи=СтрЗаменить(ИдентификаторЗадачи, "_", "-");
	
	Попытка
		ЗадачаСсылка=Задачи.КП_Задача.ПолучитьСсылку(Новый УникальныйИдентификатор(ИдентификаторЗадачи));
	Исключение
		Сообщить("Не найдена задача из списка");
		Возврат Неопределено;
	КонецПопытки;
	
	Попытка	
		Если ЗадачаСсылка.Пустая() ИЛИ ЗадачаСсылка.ПолучитьОбъект()=Неопределено Тогда
			Сообщить("Не найдена задача из списка");		
			Возврат Неопределено;
		КонецЕсли;
		Возврат ЗадачаСсылка;
	Исключение
		//возможно нет прав доступа
		ТекстОшибки=КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Если Найти(ТекстОшибки, "Нарушение прав доступа!")>0 Тогда
			ТекстОшибки=ТекстОшибки+" Возможно не включен доступ к подсистеме ""Процессы"" в настройке профилей доступа. Обратитесь к администратору программы.";
		КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияНазваниеГруппыНовыеНажатие(Элемент)
	КП_ДекорацияКартинкаНовыеНажатие(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияНазваниеГруппыВРаботеНажатие(Элемент)
	КП_ДекорацияКартинкаВРаботеНажатие(Элемент);
КонецПроцедуры

#КонецОбласти //КП_МоиЗадачи
