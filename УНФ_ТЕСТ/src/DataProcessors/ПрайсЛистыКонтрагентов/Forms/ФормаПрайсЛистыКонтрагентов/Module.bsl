...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(ЭтотОбъект, "УНФ.Обработка.ПрайсЛистыКонтрагентов", "ФормаПрайсЛистыКонтрагентов", Неопределено, ЗаголовокНовости, Ложь, НастрокиЗаполнения, "ПриОткрытии");
...
&НаКлиенте
Процедура ВидЦенКонтагентаСравнитьПриИзменении(Элемент)
	
	УстановитьСвойстваЭлементовДополнительногоОтбора();
	
	Если Объект.ВидЦенКонтагентаСравнить = Ложь Тогда
...
Процедура ОткрыватьПрайсЛистКонтрагентовКакОбработкуПриИзменении(Элемент)
	...
	УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(ОткрыватьПрайсЛистКонтрагентовКакСписок, "ОткрыватьПрайсЛистКонтрагентовКакСписок");
...
Процедура Декорация1Нажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораВидовЦенКонтрагентов", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещени);
...
Процедура Декорация2Нажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораНоменклатурыКомпоновщик", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещени);
...
Процедура Декорация3Нажатие(Элемент)
	...
	ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.Форма.ФормаОДокументе", ПараметрыОткрытия, ЭтаФорма, , , , ОписаниеОповещени);
...
Процедура УстановитьСвойстваЭлементовПоВидуЦенКонтрагентов()
	...
	ИначеЕсли КоличествоСтрок = 1 Тогда
		...
		ТекстрЗаголовка = СокрЛП(Объект.ОтборВидовЦенКонтрагентов[0].ВидЦенКонтрагента);
		
	ИначеЕсли КоличествоСтрок = 2 Тогда
		
		ТекстрЗаголовка = СокрЛП(Объект.ОтборВидовЦенКонтрагентов[0].ВидЦенКонтрагента) + ", " + СокрЛП(Объект.ОтборВидовЦенКонтрагентов[1].ВидЦенКонтрагента);
...
Процедура УстановитьСвойстваЭлементовДополнительногоОтбора()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаУсловиеСравнения", "Доступность", Объект.ВидЦенКонтагентаСравнить);
...
Функция ОбъектВСтруктуру()
	...
	СтруктураДанных.Вставить("ВидЦенКонтагентаСравнить",	Объект.ВидЦенКонтагентаСравнить);
...
Процедура ИзменитьОбъектФормыПоСсылке(НовыеДанныеЭлементаСправочника = Неопределено)
	...
	Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.НастройкиПользователей) Тогда
		...
		УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(Объект.Ссылка, "РабочийПрайсЛистКонтрагентов");
...
Процедура ОбновитьСуществующийПрайсЛист(СсылкаОбновляемого, НовыеДанныеЭлементаСправочника)
	...
	Попытка
		...
		УправлениеНебольшойФирмойСервер.УстановитьНастройкуПользователя(СсылкаОбновляемого, "РабочийПрайсЛистКонтрагентов");
