...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов) Экспорт
	...
	//	И ((ДанныеШтрихкодов[0].Свойство("ДанныеМаркировки")
	//	И ДанныеШтрихкодов[0].ДанныеМаркировки.ЭтоШтрихкодШинИПокрышек) 
	//	ИЛИ 
	//	(ДанныеШтрихкодов[0].Свойство("ЭтоШтрихкодШиныБезМаркировки")
	//	И ДанныеШтрихкодов[0].ЭтоШтрихкодШиныБезМаркировки)) Тогда
	//	ПолученыШтрихкодыМаркировкаШинИПокрышек(ДанныеШтрихкодов);
	//	
	//ИначеЕсли ИспользуетсяРегистрацияРозничныхПродажВЕГАИС
	//	ИЛИ ИспользуетсяМаркировкаТабачнойПродукции
	//	ИЛИ ИспользуетсяМаркировкаОбуви Тогда
...
Функция ОбработатьИзменениеВладельцаДисконтнойКарты(ПустойКонтрагентПередИзменением)
	...
	ИзминиласьДисконтнаяКарта = Ложь;
	
	Если НЕ Объект.ДисконтнаяКарта.Пустая()
		И ДисконтныеКартыВызовСервера.НужноОчиститьДисконтнуюКартуПриСменеВладельца(
		Объект.Контрагент,
		Объект.ДисконтнаяКарта,
		ПустойКонтрагентПередИзменением) Тогда
		
		ИзминиласьДисконтнаяКарта = Истина;
	...
	Возврат ИзминиласьДисконтнаяКарта;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаЗапасы Из Объект.Запасы Цикл
		СтрокаЗапасы.ТипНоменклатурыЗапас = (СтрокаЗапасы.Номенклатура.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас"))
			ИЛИ (СтрокаЗапасы.Номенклатура.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат"));
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ЧекККМ_Проведение_"+Объект.Запасы.Количество()+ РаботаСФормойДокументаКлиентСервер.ПолучитьИмяФормыСтрокой(ЭтотОбъект.ИмяФормы));
	КонецЕсли;
	// СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура ПоискПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	//
	//Если ЭтоШтрихкодШинИПокрышек Тогда
	//	Данные.Добавить(Новый Структура("Штрихкод, Количество, ЭтоШтрихкодШиныБезМаркировки", ТекШтрихкод, 1, Истина));
	//Иначе
...
Процедура ПробитьЧекВыполнить(Команда)
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПробитьЧек"+ РаботаСФормойДокументаКлиентСервер.ПолучитьИмяФормыСтрокой(ЭтотОбъект.ИмяФормы)+"_"+Объект.Запасы.Количество());
	// СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	ИзминиласьДисконтнаяКарта = КонтрагентПриИзмененииНаСервере(ПустойКонтрагентПередИзменением);
	Если ИзминиласьДисконтнаяКарта Тогда
		ПересчитатьДокументНаКлиенте();
...
Функция КонтрагентПриИзмененииНаСервере(ПустойКонтрагентПередИзменением)
	...
	ИзминиласьДисконтнаяКарта = ОбработатьИзменениеВладельцаДисконтнойКарты(ПустойКонтрагентПередИзменением);
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруткураПараметров.Номенклатура = Номенклатура;
	СтруткураПараметров.Характеристика = Характеристика;
	СтруткураПараметров.Партия = Партия;
	...
	СтруткураПараметров.ЕдиницаИзмерения = ТекущиеДанные.ЕдиницаИзмерения;
	СтруткураПараметров.ЗаказПокупателя = Объект.Заказ;
	СтруткураПараметров.НомерВариантаКП = 0;
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыВРезервеВсего 
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СтруткураПараметров.Склад = ?(Элемент.ТекущийЭлемент = Элементы.ЗапасыВРезервеВсего, Неопределено, СтруткураПараметров.Склад);
				
				СписокДокументов = СписокДокументовПоЗаказу("Остаток", СтруткураПараметров);
...
Процедура ЗапасыРазбитьСтроку(Команда)
	...
	Если СтрокаТабличнойЧасти.Количество = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Количество в текущей строке равно нулю.";
		Сообщение.КлючДанных = Объект.Ссылка;
		Сообщение.Сообщить();
		Возврат
	КонецЕсли;
	...
	ПараметрыОповещения = Новый Структура("СтрокаТабличнойЧасти, АвтоматическиеСкидкиРасчитаны", СтрокаТабличнойЧасти, Объект.СкидкиРассчитаны);
...
Процедура РазбитьСтрокуТабличнойЧастиЗавершение(Результат, Параметры) Экспорт
	...
	Если Результат <= 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Количество в новой строке должно быть больше нуля.";
		Сообщение.КлючДанных = Объект.Ссылка;
		Сообщение.Сообщить();
		Возврат
	КонецЕсли;
	...
	Если Результат >= СтрокаТабличнойЧасти.Количество Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Количество в новой строке должно быть меньше чем в текущей.";
		Сообщение.КлючДанных = Объект.Ссылка;
		Сообщение.Сообщить();
		Возврат
	КонецЕсли;
...
Процедура НастроитьЭлементыСтруктурныхЕдиниц()
	...
	Если НастройкаРМК.СтруктурныеЕдиницы.Количество() Тогда
		...
		КонецЦикла;
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаНастройкаДокумента", "Видимость", Ложь);
