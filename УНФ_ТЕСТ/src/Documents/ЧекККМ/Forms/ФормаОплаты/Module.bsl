...
Процедура УстановитьДоступностьНомераСертификатаНаСервере(ИмяЭлемента, ОчищатьНомер = Истина)
	...
	НомерСертификата = Элементы.Найти(ИмяЭлемента).Родитель.ПодчиненныеЭлементы.Найти("НомерСертификата_" + ИндексЭлемента);
	Если Не НомерСертификата = Неопределено Тогда
		Если ОчищатьНомер Тогда
			ВременнаяТаблицаКарт[Число(ИндексЭлемента)].НомерСертификата = "";
			ВременнаяТаблицаКарт[Число(ИндексЭлемента)].ОстатокСертификата = 0;
		КонецЕсли;
		Если ВременнаяТаблицаКарт[Число(ИндексЭлемента)].ПодарочныйСертификат.ИспользоватьСерийныеНомера Тогда
			НомерСертификата.Доступность = Истина;
			ВременнаяТаблицаКарт[Число(ИндексЭлемента)].НоминалСертификата = 0;
		Иначе
			НомерСертификата.Доступность = Ложь;
			ВременнаяТаблицаКарт[Число(ИндексЭлемента)].НоминалСертификата = ВременнаяТаблицаКарт[Число(ИндексЭлемента)].ПодарочныйСертификат.Номинал;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПересчитатьСуммы()
	...
	Если Дельта > 0 И НЕ ЭтоМобильныйКлиент Тогда
		Элементы.ПолученоНаличными.СписокВыбора.Добавить(Дельта, 
			""+Дельта+" "+ВалютаДокумента+" (к оплате)");
	КонецЕсли;
	...
	Если НЕ ЭтоМобильныйКлиент Тогда
		Для Каждого ТекущаяСтрока Из ВременнаяТаблицаКарт Цикл
			...
			Если Дельта > 0 Тогда
				Элементы["ОплатаКартой_"+Индекс].СписокВыбора.Добавить(Дельта, ""+Дельта+" "+ВалютаДокумента+" (к оплате)");
			КонецЕсли;
			Если ТекущаяСтрока.ОстатокСертификата > 0 Тогда
				Элементы["ОплатаКартой_"+Индекс].СписокВыбора.Добавить(ТекущаяСтрока.ОстатокСертификата, ""+ТекущаяСтрока.ОстатокСертификата+" "+ВалютаДокумента+" (остаток)");
			КонецЕсли;
			Если ТекущаяСтрока.НоминалСертификата > 0 Тогда
				Элементы["ОплатаКартой_"+Индекс].СписокВыбора.Добавить(ТекущаяСтрока.НоминалСертификата, ""+ТекущаяСтрока.НоминалСертификата+" "+ВалютаДокумента+" (номинал)");
			КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоМобильныйКлиент И Не НажатаКнопкаПробитьЧек Тогда
		...
		Иначе
			...
			// Если была оплата бонусами, то нужно её удалить
...
Процедура ДобавитьКарту(Команда)
	...
	Если Объект.ЭквайринговыйТерминал.Пустая() Тогда
		ТекстСообщения = НСтр("ru = 'Выберите эквайринговый терминал'");
		Поле = "Объект.ЭквайринговыйТерминал";
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, ТекстСообщения,,, Поле);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьКартуНаСервере(ТекущаяСтрокаИдентификатор = Неопределено)
	...
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	НовыйЭлемент.ПодсказкаВвода = "Номер карты";
...
Процедура ДобавитьСертификатНаСервере(ТекущаяСтрокаИдентификатор = Неопределено)
	...
	НовыйЭлемент.ПодсказкаВвода = "Подарочный сертификат";
	НовыйЭлемент.ВыбиратьТип = Ложь;
	...
	НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	НовыйЭлемент.АвтоМаксимальнаяШирина = Ложь;
	НовыйЭлемент.ПодсказкаВвода = "Номер сертификата";
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "РеквизитСертификатаПриИзменении");
...
Процедура ОК(Команда)
	...
	Если Элементы.ГруппаОплата.ТекущаяСтраница = Элементы.ГруппаОплатаКартой
		...
		Если ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Неопределено, СообщениеОбОшибке);
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Если ТекущаяСтрока.Сумма > 0 Или ТекущаяСтрока.СуммаБонусов > 0 Тогда
			Если ТекущаяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.БанковскаяКарта") Тогда
				...
				Если Не ЗначениеЗаполнено(ТекущаяСтрока.ВидПлатежнойКарты) Тогда
					Отмена = Истина;
					ТекстСообщения = НСтр("ru = 'Заполните вид карты'");
					Поле = "ВидПлатежнойКарты";
					ИмяТабличнойЧасти = "ВременнаяТаблицаКарт";
					УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Неопределено, ТекстСообщения, ИмяТабличнойЧасти, ВременнаяТаблицаКарт.Индекс(ТекущаяСтрока)+1, Поле);
				КонецЕсли;
			...
			ИначеЕсли ТекущаяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.ПодарочныйСертификат") Тогда
				...
				Если Не ЗначениеЗаполнено(ТекущаяСтрока.ПодарочныйСертификат) Тогда
					Отмена = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = "Укажите подарочный сертификат";
					Сообщение.Сообщить();
				Иначе
					Если УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.ПодарочныйСертификат, "ИспользоватьСерийныеНомера") Тогда
						Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСертификата) Тогда
							Отмена = Истина;
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = "Укажите номер сертификата";
							Сообщение.Сообщить();
						КонецЕсли;
						Если НЕ ЭтоМобильныйКлиент И ТекущаяСтрока.Сумма > ТекущаяСтрока.ОстатокСертификата Тогда
							Отмена = Истина;
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = "Превышен остаток средств по сертификату";
							Сообщение.Сообщить();
						КонецЕсли;
					Иначе
						Если НЕ ЭтоМобильныйКлиент И 
							ТекущаяСтрока.Сумма > УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.ПодарочныйСертификат, "Номинал") Тогда
							Отмена = Истина;
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = "Превышен номинал сертификата";
							Сообщение.Сообщить();
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			...
			ИначеЕсли ТекущаяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы") Тогда
				...
				Если ТекущаяСтрока.СуммаБонусов > ТекущаяСтрока.МаксимальнаяСуммаОплатыБонусами Тогда
					Отмена = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = "Превышена максимальная сумма оплаты бонусами";
					Сообщение.Сообщить();
				КонецЕсли;
				...
				Если ТекущаяСтрока.СуммаБонусов > ТекущаяСтрока.ОстатокБонуснойКарты Тогда
					Отмена = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = "Превышен остаток бонусов по карте";
					Сообщение.Сообщить();
				КонецЕсли;
			...
			ИначеЕсли ТекущаяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Кредит") Тогда
				...
				Если Не ЗначениеЗаполнено(ТекущаяСтрока.ДоговорКредита) Тогда
					Отмена = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = "Заполните договор с банком";
					Сообщение.Сообщить();
	...
	Если Команда <> Неопределено Тогда
		Если ЕстьОплатаЭТ() Тогда
			ПоказатьПредупреждение(
				,НСтр("ru = 'Сначала необходимо отменить оплату на экваринговом терминале'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписатьБаллыНаСервере(ТекущаястрокаИдентификатор = Неопределено)
	...
	НоваяСтрока.МаксимальнаяСуммаОплатыБонусами = Окр(СуммаДокумента * ?(МаксимальныйПроцент = 0, 100, МаксимальныйПроцент)/100, 2);;
	...
	Если НоваяСтрока.ОстатокБонуснойкарты = МинимальнаяСумма Тогда
		НовыйЭлемент.СписокВыбора.Добавить(НоваяСтрока.ОстатокБонуснойкарты, "" + НоваяСтрока.ОстатокБонуснойкарты + " (Остаток)");
	ИначеЕсли КОплате = МинимальнаяСумма Тогда
		НовыйЭлемент.СписокВыбора.Добавить(КОплате, "" + КОплате + " (К оплате)");
	ИначеЕсли НоваяСтрока.МаксимальнаяСуммаОплатыБонусами = МинимальнаяСумма Тогда
		НовыйЭлемент.СписокВыбора.Добавить(НоваяСтрока.МаксимальнаяСуммаОплатыБонусами, "" + НоваяСтрока.МаксимальнаяСуммаОплатыБонусами + " (Макс.)");
	КонецЕсли;
...
Процедура ДобавитьКредитНаСервере(ТекущаяСтрокаИдентификатор = Неопределено)
	...
	НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	НовыйЭлемент.АвтоМаксимальнаяШирина = Ложь;
	НовыйЭлемент.ПодсказкаВвода = "Договор с банком";
	НовыйЭлемент.ВыбиратьТип = Ложь;
...
Процедура РаспределениеПродажПриСоздании()
	...
	Если Параметры.Свойство("СписокЧековКОплате")
		...
		Для Каждого ЭлементСписка Из Параметры.СписокЧековКОплате Цикл
			...
			УправлениеНебольшойФирмойСервер.ЗаполнитьТекстовоеПредставлениеЧека(СтрокаСостава, ЭлементСписка.Значение);
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.ЗаполнитьТекстовоеПредставлениеЧека(СтрокаСостава, ДокументОбъект, ФискальныйРегистратор);
...
Функция ВыполнитьПоискСертификатаПоНомеру(Номер)
	...
	ИначеЕсли Результат.Количество() = 1 Тогда
		НоваяСтрока = ВременнаяТаблицаКарт.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Результат[0]);
		НоваяСтрока.ВидОплаты = Перечисления.ВидыБезналичныхОплат.ПодарочныйСертификат;
		НоваяСтрока.Сумма = Мин(Результат[0].Остаток, СуммаДокумента - ВременнаяТаблицаКарт.Итог("Сумма") - Объект.Предоплата.Итог("СуммаРасчетов"));
		ДобавитьСертификатНаСервере(НоваяСтрока);
...
Процедура ДобавитьВыбранныйСертификат(Результат, ДополнительныеПараметры) Экспорт
	...
	НоваяСтрока.ВидОплаты = Перечисления.ВидыБезналичныхОплат.ПодарочныйСертификат;
	НоваяСтрока.Сумма = Мин(Результат.Остаток, СуммаДокумента - ВременнаяТаблицаКарт.Итог("Сумма") - Объект.Предоплата.Итог("СуммаРасчетов"));
	ДобавитьСертификатНаСервере(НоваяСтрока);
	ЗаполнитьОстатокПоСертификату(ВременнаяТаблицаКарт.Индекс(НоваяСтрока));
...
Процедура ЗаполнитьОстатокПоСертификату(ИндексЭлемента)
	...
	ДанныеСертификата = РаботаСПодарочнымиСертификатами.ПолучитьСтруктуруДанныхСертификата(
		ВременнаяТаблицаКарт[Число(ИндексЭлемента)].ПодарочныйСертификат,
		ВременнаяТаблицаКарт[Число(ИндексЭлемента)].НомерСертификата,
		Объект.Ссылка);
	
	ВременнаяТаблицаКарт[Число(ИндексЭлемента)].ОстатокСертификата = ДанныеСертификата.Остаток;
...
Процедура ЗаполнитьСписокВыбораСуммыПредоплаты()
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.ОсталосьОплатить > 0 Тогда
			Элементы.СуммаПредоплаты.СписокВыбора.Добавить(Выборка.ОсталосьОплатить, "" + Выборка.ОсталосьОплатить + " (остаток по заказу)");
...
Функция ПроверкиПередОплатойПройдены()
	...
	Если Объект.СуммаДокумента = 0 Тогда
		...
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтотОбъект, ТекстОшибки);
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОплатитьНаличными(Команда)
	...
	ПроверкиПройедны = ПроверкиПередОплатойПройдены();
	Если Не ПроверкиПройедны Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыбранаДисконтнаяКартаДополнительноНаСервере()
	...
	Если ЭтоБонуснаяКарта Тогда
		СтруктураДанных = РаботаСБонусами.ПолучитьДанныеБонуснойКарты(Объект.ДисконтнаяКарта, Объект.Дата);
		ОстатокБонусов = СтруктураДанных.Остаток;
...
Процедура ПередатьНаТерминал(Команда)
	...
	Если Не ЗначениеЗаполнено(ВидКартыОплатаКартой) Тогда
		ТекстСообщения = НСтр("ru = 'Заполните вид карты'");
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Неопределено, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСуммуПогашенияСертификатовНаСервере()
	...
	Для Каждого СтрокаОплаты Из ВременнаяТаблицаКарт Цикл
		Если СтрокаОплаты.ВидОплаты = Перечисления.ВидыБезналичныхОплат.ПодарочныйСертификат Тогда
			...
			ИначеЕсли СтрокаОплаты.ПодарочныйСертификат.ИспользоватьСерийныеНомера Тогда
