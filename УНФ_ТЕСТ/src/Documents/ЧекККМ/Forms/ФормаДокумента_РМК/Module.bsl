...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	ПолеДляОформления = ?(Не ЭтоМобильныйКилент, "ЗапасыДоступно", "ЗапасыМКДоступно");
...
Процедура СформироватьЗаголовокНаКлиенте(СтруктураСостояниеКассовойСмены)
	...
	ТекстСообщения = НСтр("ru='%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%, %Ответственный%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТорговаяТочка%", СокрЛП(СтруктураСостояниеКассовойСмены.СтруктурнаяЕдиницаПредставление));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСмены%", СокрЛП(СтруктураСостояниеКассовойСмены.НомерКассовойСмены));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВремяИзменения%", Формат(СтруктураСостояниеКассовойСмены.ДатаИзмененияСтатуса,"ДФ=dd.MM.yyyy"));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ответственный%", Строка(Объект.Ответственный));
...
&НаСервере
Функция ЭтоШтрихкодАлкогольнойПродукции(Штрихкод)
	
	Возврат ШтрихкодированиеЕГАИС.ЭтоШтрихкодАлкогольнойПродукции(Штрихкод);
	
КонецФункции // ЭтоШтрихкодАлкогольнойПродукции()

&НаСервере
Функция ЭтоШтрихкодТабачнойПродукции(Штрихкод)
	
	Возврат ШтрихкодированиеМОТП.ЭтоШтрихкодТабачнойПродукции(Штрихкод);
	
КонецФункции // ЭтоШтрихкодТабачнойПродукции()

&НаСервере
Функция ЭтоШтрихкодОбувнойПродукции(Штрихкод)
	
	Возврат ШтрихкодированиеИСМП.ЭтоШтрихкодОбувнойПродукции(Штрихкод);
...
Процедура ПробитьЧек(СформироватьТоварныйЧек = Ложь)
	...
	Если ВключенКонтрольОстатков И НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтаФорма, ПоказыватьПредупреждение) Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если глОперацияЗакрытияКассовойСмены <> Неопределено Тогда
		...
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется закрытие смены'");
		ПараметрыОжидания.ВыводитьПрогрессВыполнения = Истина;
		Оповещение = Новый ОписаниеОповещения("ПриЗавершенииОжидания", ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Элементы.ГруппаПродажаИВозврат.ТекущаяСтраница = Элементы.ГруппаОтложенные Тогда
		Если Источник = "ПодключаемоеОборудование"
			...
			ДисконтныеКарты = ДисконтныеКартыВызовСервера.НайтиДисконтныеКартыПоШтрихкоду(Штрихкод);
	...
	Если Не БылаАвторизация Тогда
		...
		Если Источник = "ПодключаемоеОборудование"
			...
			Если ИмяСобытия = "ScanData" Тогда
				...
				// ++( ГосИС 1.1.1.12 ГосИС_2
				//ЭтоШтрихкодШинИПокрышек = Ложь;
				//ШтриховойКодТовара = МенеджерОборудованияМаркировкаКлиентСервер.РазобратьШтриховойКодТовара(Штрихкод);
				//ЕстьДанныеМаркировки = Ложь;
				//Если ШтриховойКодТовара.Разобран 
				//	И ШтриховойКодТовара.ТипИдентификатораТовара = ПредопределенноеЗначение("Перечисление.ТипыИдентификаторовТовараККТ.КодТовараВФорматеDataMatrixGS1") Тогда
				//	ЕстьДанныеМаркировки = Истина;
				//	РазделительGS1 = МенеджерОборудованияМаркировкаКлиентСервер.РазделительGS1();
				//	ШтрихкодBase64 = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(Штрихкод);
				//	ПараметрыСканирования = ШтрихкодированиеИСКлиент.ПараметрыСканирования(ЭтотОбъект);
				//	//++( ГосИС 1.1.1.12
				//	//КодМаркировки = НормализованныйКодМаркировки(ШтрихкодBase64, ПараметрыСканирования);
				//	КодМаркировки = НормализованныйКодМаркировки(Штрихкод);
				//	//)++
				//	ЭтоШтрихкодАлкогольнойПродукции = ЭтоШтрихкодАлкогольнойПродукции(КодМаркировки);
				//	ЭтоШтрихкодТабачнойПродукции = ЭтоШтрихкодТабачнойПродукции(КодМаркировки);
				//	ЭтоШтрихкодОбувнойПродукции = ЭтоШтрихкодОбувнойПродукции(КодМаркировки);
				//	ШтриховойКодТовара.Вставить("ЭтоШтрихкодАлкогольнойПродукции", ЭтоШтрихкодАлкогольнойПродукции);
				//	ШтриховойКодТовара.Вставить("ЭтоШтрихкодТабачнойПродукции", ЭтоШтрихкодТабачнойПродукции);
				//	ШтриховойКодТовара.Вставить("ЭтоШтрихкодОбувнойПродукции", ЭтоШтрихкодОбувнойПродукции);
				//	Если ИспользуетсяМаркировкаШинИПокрышек
				//		И ШтриховойКодТовара.ТипИдентификатораТовара = ПредопределенноеЗначение("Перечисление.ТипыИдентификаторовТовараККТ.КодТовараВФорматеDataMatrixGS1")
				//		И НЕ ЭтоШтрихкодАлкогольнойПродукции
				//		И НЕ ЭтоШтрихкодТабачнойПродукции
				//		И НЕ ЭтоШтрихкодОбувнойПродукции Тогда
				//		ЭтоШтрихкодШинИПокрышек = Истина;
				//	КонецЕсли;
				//	ШтриховойКодТовара.Вставить("ЭтоШтрихкодШинИПокрышек", ЭтоШтрихкодШинИПокрышек);
				//	ШтриховойКодТовара.Вставить("КодМаркировки", КодМаркировки);
				//	ШтрихкодEAN = ШтриховойКодТовара.EAN;
				//Иначе
				//	НоменклатураПоШтрихкоду = ПолучитьНоменклатуруПоШтрихкоду(Штрихкод);
				//	Если ЗначениеЗаполнено(НоменклатураПоШтрихкоду) Тогда
				//		ЭтоШтрихкодШинИПокрышек = УправлениеНебольшойФирмойВызовСервера.ЗначениеРеквизитаОбъекта(НоменклатураПоШтрихкоду, "ШиныИПокрышки");
				//	КонецЕсли;
				//КонецЕсли;
				//)++
				
				// ++( ГосИС 1.1.1.12 ГосИС_2
				//Если ЭтоШтрихкодШинИПокрышек Тогда
				//	Если ЕстьДанныеМаркировки Тогда
				//		Данные.Добавить(Новый Структура("Штрихкод, Количество, ДанныеМаркировки", ШтрихкодEAN, 1, ШтриховойКодТовара));
				//	Иначе
				//		Данные.Добавить(Новый Структура("Штрихкод, Количество, ЭтоШтрихкодШиныБезМаркировки", Штрихкод, 1, Истина));
				//	КонецЕсли;
				//Иначе
	...
	ИначеЕсли ИмяСобытия = "РМК_Запись_ЧекККМВозврат" Тогда
		...
		Иначе
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = Элементы.ДекорацияЧекККМНаВозврат.Заголовок + "; "+" №"+НомерДокумента+" от "+Формат(Параметр.Дата, "ДФ=dd.MM.yyyy");
		КонецЕсли;
...
Функция ПолучитьНоменклатуруПоШтрихкоду(Штрихкод)
	...
	Возврат РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьНоменклатуруПоШтрихкоду(Новый Структура("Штрихкод, Количество", Штрихкод, 1));
...
Процедура ИнициализацияФормы(пПриСозданииНаСервере = Истина)
	...
	УправлениеНебольшойФирмойСервер.УстановитьОформлениеКолонкиДата(СписокЗаказов);
	УправлениеНебольшойФирмойСервер.УстановитьОформлениеКолонкиДата(СписокОтложенныхЧеков);
...
Процедура ОткрытьСотрудника(Команда)
	...
	УправлениеНебольшойФирмойКлиент.ОткрытьФормуСотрудника(ЭтотОбъект);
...
Процедура ПересчитатьРучныеСкидкиПоВидуСкидки()
	...
	Скидка = УправлениеНебольшойФирмойСервер.ПолучитьПроцентСкдикиПоВидуСкидокНаценок(Объект.ВидСкидкиНаценки);
	Для каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
...
Процедура ЗапасыПередУдалениемОбработчикСобытия(ИмяТЧ, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.КлючСвязи) Тогда
			УправлениеНебольшойФирмойКлиентСервер.ЗаполнитьКлючСвязи(Объект.Запасы, ТекущиеДанные, "КлючСвязи");
		КонецЕсли;
...
Процедура ЗапасыПередУдалениемЗавершение(Результат, ДанныеНабора) Экспорт
	...
	ИначеЕсли Результат=КодВозвратаДиалога.Да Тогда
		...
		Для каждого Стр Из СтрокиНабора Цикл
			РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомера, Стр,
				ИспользоватьСерийныеНомераОстатки);
		КонецЦикла; 
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
		...
		СтруткураПараметров = Новый Структура("ОрганизацияОбъекта, Номенклатура, Характеристика, Партия, Склад, ЕдиницаИзмерения, ЗаказПокупателя, НомерВариантаКП");
		
		СтруткураПараметров.Номенклатура = СтрокаТабличнойЧасти.Номенклатура;
		СтруткураПараметров.Характеристика = СтрокаТабличнойЧасти.Характеристика;
		СтруткураПараметров.Партия = СтрокаТабличнойЧасти.Партия;
		СтруткураПараметров.Склад = ?(ЗначениеЗаполнено(СтрокаТабличнойЧасти.Структурнаяединица),
									  СтрокаТабличнойЧасти.СтруктурнаяЕдиница,
									  Объект.СтруктурнаяЕдиница);
		СтруткураПараметров.ЕдиницаИзмерения = СтрокаТабличнойЧасти.ЕдиницаИзмерения;
		СтруткураПараметров.ЗаказПокупателя =Объект.Заказ;
		СтруткураПараметров.ОрганизацияОбъекта = Объект.Организация;
		
		ПолеДляПроверки = ?(ЭтоМобильныйКилент, Элементы.ЗапасыМККПоступлению, Элементы.ЗапасыКПоступлению);
		...
		Если Элемент.ТекущийЭлемент = ПолеДляПроверки Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ЗапасыПоступления", СтруткураПараметров);
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыСтруктурнаяЕдиницаДляРежимаОстатки Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ОбщиеЗапасы", СтруткураПараметров);
				...
				Если СписокДокументов.Количество() Тогда
					СписокДокументов.Добавить("ОтчетОстаткиНаСкладах", "Остатки на складах (отчет)",, БиблиотекаКартинок.Отчет);
					СписокВыбораСклад.Добавить("ОтчетОстаткиНаСкладах", "Остатки на складах (отчет)",, БиблиотекаКартинок.Отчет);
				КонецЕсли;
...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов) Экспорт
	...
	ИначеЕсли ИспользуетсяРегистрацияРозничныхПродажВЕГАИС Или ИнтеграцияИСМПВызовСервера.ВестиУчетМаркируемойПродукции() Тогда
		...
		ОбработатьКодМаркировки(ДанныеШтрихкодов[0]);
...
&НаСервереБезКонтекста
// ++( ГосИС 1.1.1.12
//Функция НормализованныйКодМаркировки(ШтрихкодBase64, ПараметрыСканирования)
//	
//	Возврат ШтрихкодированиеИС.НормализованныйШтрихкод(ШтрихкодBase64, ПараметрыСканирования);
//	
//КонецФункции
Функция НормализованныйКодМаркировки(КодМаркировки)
	
	Возврат ИнтерфейсИСМП.РазобратьИНормализоватьКодМаркировки(КодМаркировки);
...
&НаКлиенте
Процедура ПолученыШтрихкодыМаркировкаШинИПокрышек(ДанныеШтрихкодов)
	
	Модифицированность = Истина;
	
	НеДобавленныеШтрихкоды		= ЗаполнитьПоДаннымШтрихкодовМаркировкаШинИПокрышек(ДанныеШтрихкодов);
	НеизвестныеШтрихкоды		= НеДобавленныеШтрихкоды.НеизвестныеШтрихкоды;
	ШтрихкодыНекорректногоТипа	= НеДобавленныеШтрихкоды.ШтрихкодыНекорректногоТипа;
...
КонецПроцедуры // ПолученыШтрихкодыМаркировкаШинИПокрышек()
...
Процедура ПоискПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Данные = Новый Массив();
	//НоменклатураПоШтрихкоду = ПолучитьНоменклатуруПоШтрихкоду(ТекШтрихкод);
	//Если ЗначениеЗаполнено(НоменклатураПоШтрихкоду) Тогда
	//	ЭтоШтрихкодШинИПокрышек = УправлениеНебольшойФирмойВызовСервера.ЗначениеРеквизитаОбъекта(НоменклатураПоШтрихкоду, "ШиныИПокрышки");
	//Иначе
	//	ЭтоШтрихкодШинИПокрышек = Ложь;
	...
	//Если ЭтоШтрихкодШинИПокрышек Тогда
	//	Данные.Добавить(Новый Структура("Штрихкод, Количество, РежимУдаления, ЭтоШтрихкодШиныБезМаркировки", ТекШтрихкод, 1, РежимУдаления, Истина));
	//Иначе
...
Процедура ЗаменитьСтрокуЗапасовДаннымиНабора(Форма, СтрокаТабличнойЧасти, СтруктураДанные)
	...
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомера, СтрокаТабличнойЧасти,
		Форма.ИспользоватьСерийныеНомераОстатки);
...
Процедура УстановитьВидимостьПредупрежденияОКассире(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(КассаДляПроверкиОтветственного) Тогда
		...
		РеквизитыКассира = УправлениеНебольшойФирмойСервер.ПолучитьРеквизитыКассира(РеквизитыДокумента);
...
Процедура ПоказатьДублиСтрок(Команда)
	...
	СтрокаТабличнойЧасти = ?(ЭтоМобильныйКилент, Элементы.ЗапасыМК.ТекущиеДанные, Элементы.Запасы.ТекущиеДанные);
	ИмяДекорации = ?(ЭтоМобильныйКилент, "ДекорацияОтборПоДублямМК", "ДекорацияОтборПоДублям");
	ИмяКоманды = ?(ЭтоМобильныйКилент, "ЗапасыМККонтекстноеМенюПоказатьДублиСтрок", "ЗапасыКонтекстноеМенюПоказатьДублиСтрок");
	ИмяТЧ = ?(ЭтоМобильныйКилент, "ЗапасыМК", "Запасы");
...
&НаСервере
Функция СписокДокументовПоЗаказу(ТипОперации, СтруткураПараметров)
	Возврат НоменклатураВДокументахСервер.СписокДокументовПоЗаказу(Объект, ТипОперации,СтруткураПараметров);
КонецФункции
...
Функция ЗагрузитьСледующийРаспределенныйЧек(ЭтоПервыйЧек = Ложь)
	...
	Если КассаККМ <> Объект.КассаККМ Тогда
		...
		УстановитьОрганизациюПоВладельцуККМ();
		СпециальныйНалоговыйРежим = УправлениеНебольшойФирмойСервер.СпециальныйНалоговыйРежим(Объект.Организация, Объект.СтруктурнаяЕдиница, Объект.Дата);
...
Процедура ВосстановитьЗапасыИзРаспределенныхЧеков(СкидкиИзменились = Ложь)
	...
	Для Каждого ЭлементСписка Из СписокРаспределенныхЧеков Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ЭлементСписка.Значение.СерийныеНомера, Объект.СерийныеНомера);
...
Процедура ЗаполнитьГруппуКатегориюПоСтроке(НоваяСтрока, ГруппыКатегории)
	...
	Если ЗначениеЗаполнено(НоваяСтрока.НоменклатураНабора) Тогда
...
Процедура ЗаполнитьРаспределенныеЧеки(ЗапасыДляРаспределения)
	...
	КопияСерийныеНомера				= Объект.СерийныеНомера.Выгрузить();
	...
	Для Каждого СтрокаЗапасов Из ЗапасыДляРаспределения Цикл
		...
		// Серийные номера
		МассивНайденныхСтрок = КопияСерийныеНомера.НайтиСтроки(Новый Структура("КлючСвязи", НоваяСтрокаЗапасов.КлючСвязи));
		Если МассивНайденныхСтрок.Количество() > 0 Тогда
			Для Каждого НайденнаяСтрока Из МассивНайденныхСтрок Цикл
				НоваяСтрока = Объект.СерийныеНомера.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденнаяСтрока);
				КопияСерийныеНомера.Удалить(НайденнаяСтрока);
			КонецЦикла;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(НоваяСтрокаЗапасов.НоменклатураНабора) Тогда
			...
			Если МассивНайденныхСтрок > 0 Тогда
				Для Каждого НайденнаяСтрока Из МассивНайденныхСтрок Цикл
...
Процедура РаспределитьЗапасыПоЧекам()
	...
	ЗапасыДляРаспределения.Сортировать("ДругаяКасса, ДругойРежим");
