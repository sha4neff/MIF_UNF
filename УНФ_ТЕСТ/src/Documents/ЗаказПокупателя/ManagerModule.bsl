...
Процедура СформироватьТаблицаРезервыТоваровОрганизаций(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура ИнициализироватьДанныеДокументаЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) 
	...
	СформироватьТаблицаРезервыТоваровОрганизаций(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтроль(ДокументОбъектЗаказПокупателя, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		// Интеркампани
		Если НЕ ДополнительныеСвойства.ПараметрыДокумента.ЭтоЗаказНаряд Тогда
			Запрос.УстановитьПараметр("МассивОрганизаций",
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДополнительныеСвойства.ДляПроведения.Организация));
		Иначе
			Запрос.УстановитьПараметр("МассивОрганизаций", 
...
Процедура ВывестиСекциюОбсуждение(Макет, Результат, Параметры)
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Если Позиция>0 Тогда
			...
			СсылкаНаОбъект = ОбсужденияУНФ.ПолучитьСсылкуИзНавигационной(ТекстСсылки);
...
Процедура СформироватьЗаказПокупателя(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ДанныеПечати.Вставить("ВсегоНДС", ?(СтруктураИтогов.СуммаНДС = 0 И Не СтруктураИтогов.ЕстьСтавкаНольПроцентов,
			"-", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДС, , "0,00")));
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Для каждого СтрокаРаботы Из Шапка.ТаблицаРаботы Цикл
			...
			Для Каждого СтрИсполнитель Из СтрокиИсполнители Цикл
				ПредставлениеСотрудник = УправлениеНебольшойФирмойСервер.ПолучитьФамилиюИмяОтчество(
					СтрИсполнитель.Фамилия, СтрИсполнитель.Имя, СтрИсполнитель.Отчество);
		...
		ОбластьИтогоРаботы.Параметры.Всего = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаРаботы);
		...
		Если Шапка.ТаблицаЗапасы.Количество() > 0 Тогда
			...
			ОбластьИтогоТовары.Параметры.Всего = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаТовары);
...
Процедура ЗаполнитьПараметрыНДС(Знач ОбластьИтогоНДС, Знач СуммаНДС, Знач ВсегоНДС, Знач Шапка)
	...
	Иначе
		...
		КонецЕсли;
		ОбластьИтогоНДС.Параметры.ВсегоНДС = УправлениеНебольшойФирмойСервер.ФорматСумм(ВсегоНДС);
	КонецЕсли;
...
Процедура СформироватьПриложениеКДоговору(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Для каждого СтрокаРаботы Из Шапка.ТаблицаРаботы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиПриложениеКДоговору(СтрокаРаботы, ДанныеПечати,
				ПараметрыНоменклатуры, СтруктураИтогов);
		...
		Для каждого СтрокаЗапасы Из Шапка.ТаблицаЗапасы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиПриложениеКДоговору(СтрокаЗапасы, ДанныеПечати,
				ПараметрыНоменклатуры, СтруктураИтогов);
		...
		ДанныеПечати.Вставить("ВсегоНДС", ?(СтруктураИтогов.СуммаНДС = 0, "-",
			УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДС)));
		ДанныеПечати.Вставить("ИтоговаяСтрока", ПечатьДокументовУНФ.ИтоговаяСтрока(СтруктураИтогов.Количество,
			СтруктураИтогов.Всего, Шапка.ВалютаДокумента));
		ДанныеПечати.Вставить("СуммаПрописью", РаботаСКурсамиВалют.СформироватьСуммуПрописью(СтруктураИтогов.Всего,
			Шапка.ВалютаДокумента));
...
Процедура СформироватьКалькуляцию(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьПокупатель = Макет.ПолучитьОбласть("Покупатель");
		ПредставлениеПолучателя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбКонтрагенте,
