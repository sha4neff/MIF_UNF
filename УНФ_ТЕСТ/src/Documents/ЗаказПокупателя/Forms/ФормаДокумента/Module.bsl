...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РезервированиеЗапасов Тогда
		...
		КомандаОстаткииИРезервы = Команды.Найти("ОстаткиИРезервы");
		КомандаОстаткииИРезервы.Подсказка = "Показать остатки";
		
		ЭлементЗапасыОстатокСвободно = Элементы.Найти("ЗапасыОстатокСвободно");
		ЭлементЗапасыОстатокСвободно.Подсказка = "Доступно на складе для отгрузки";
		ЭлементЗапасыОстатокСвободно.Заголовок = "Доступно";
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруткураПараметров.Склад = Склад;
	СтруткураПараметров.ЕдиницаИзмерения = ТекущиеДанные.ЕдиницаИзмерения;
	СтруткураПараметров.ЗаказПокупателя =Объект.Ссылка;
	СтруткураПараметров.НомерВариантаКП = ТекущийВариантКП;
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыВРезерве Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				СтруткураПараметров.Склад = Неопределено;
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыРазмещено И ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("Заказано", СтруткураПараметров);
				ПоказатьВыборИзМеню(Оповещение, СписокДокументов, Элемент.ТекущийЭлемент);
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыОстатокОбщий 
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				Если Не ТекущиеДанные.ОстатокОбщий = 0 И РазрешитьСкладыВТабличныхЧастях Тогда
					...
					ПредаставлениеОбщегоОстатка = ОстатокОбщий + " " + Строка(ТекущиеДанные.ЕдиницаИзмерения) + ". <Всего>; Заполнить по остаткам.";
					
					ЭлементСпискаЗначений = Новый СписокЗначений;
					ЭлементСпискаЗначений.Добавить("РаспределитьПоРезервам", ПредаставлениеОбщегоОстатка);
					
					СписокВыбораСклад.Добавить(ЭлементСпискаЗначений[0].Значение, ПредаставлениеОбщегоОстатка);
					СписокДокументов.Добавить(ЭлементСпискаЗначений[0].Значение, ПредаставлениеОбщегоОстатка);
	...
	Иначе
		...
		Если СтрокаТабличнойЧасти.ЕстьДопРеквизитыСпецификации Тогда
			Если СтрокаТабличнойЧасти.КлючСвязи<>0 Тогда
				УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(Объект.РеквизитыПараметрическихСпецификаций, СтрокаТабличнойЧасти);
			КонецЕсли; 
...
Процедура ЗапасыПередУдалением(Элемент, Отказ)
	...
	// ПараметрическиеСпецификации
	УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(Объект.РеквизитыПараметрическихСпецификаций, ТекущиеДанные); 
...
Процедура ЗапасыПараметрыСпецификацииЗавершениеВыбора(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(Объект.РеквизитыПараметрическихСпецификаций, ВыбранноеЗначение); 
...
&НаКлиенте
Процедура ИзменитьРезервЗаполнитьПоОстаткамБезРазбиеня(Команда)
	...
	Для каждого ОписаниеНабора Из ВыделенныеНаборы Цикл
		...
		Для каждого СтрокаСостава Из СтрокиСостава Цикл
			...
			Если Элементы.Запасы.ВыделенныеСтроки.Найти(ИдентификаторСтроки)=Неопределено Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'В группу можно добавлить только набор целиком'"));
				Возврат;
			КонецЕсли; 
		КонецЦикла; 
	КонецЦикла; 
...
Процедура ШапкаТабличнаяЧасть(Команда)
	...
	СтруктураПараметров.Вставить("Доступность", Новый Структура("ПоложениеСкладаВДокументахОтгрузки", РезервированиеЗапасов И НЕ РежимОстаткиИРезервы));
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Запасы.СтруктурнаяЕдиницаРезерв", Справочники.СтруктурныеЕдиницы.ПустаяСсылка());
...
Процедура ОбработатьИзменениеВидаЦенИВалютыРасчетов(ПараметрыДокумента)
	...
	Если ОчиститьДисконтнуюКарту Тогда
		...
		ДисконтныеКартыКлиент.ПоказатьПользователюИнфомрациюОбОчисткеДКИРучныхСкидок();
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если СтруктураДанные.Свойство("Характеристика") Тогда
		СтруктураДанные.Вставить("Спецификация", УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(СтруктураДанные.Номенклатура, СтруктураДанные.Характеристика, ВключаяШаблоны));
	Иначе
		СтруктураДанные.Вставить("Спецификация", УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(СтруктураДанные.Номенклатура, , ВключаяШаблоны));
	КонецЕсли;
				...
				Иначе
					...
					Если НоваяСтрока.ЕстьДопРеквизитыСпецификации Тогда
						Если НоваяСтрока.КлючСвязи<>0 Тогда
							УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(Объект.РеквизитыПараметрическихСпецификаций, НоваяСтрока);
						КонецЕсли; 
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") 
		...
		Если НЕ РезультатЗакрытия.ПерезаполнитьЦены
			...
			УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, ВалютаРасчетовПередИзменением, "Запасы", РезультатЗакрытия.КурсПересчетаЦен);
			// Доставка
...
Процедура УправлениеФормой()
	...
	Элементы.СтруктурнаяЕдиницаРезерв.Видимость			= СкладВШапке И РезервированиеЗапасов;
	Элементы.ВариантыКПОтступ.Видимость					= Элементы.СтруктурнаяЕдиницаРезерв.Видимость И НЕ ОбщегоНазначения.ЭтоМобильныйКлиент();
	Элементы.ЗапасыСтруктурнаяЕдиницаРезервТЧ.Видимость	= Не СкладВШапке И Не РежимОстаткиИРезервы И РезервированиеЗапасов;
	...
	Элементы.ИзменитьРезервЗаполнитьПоОстаткамБезРазбиеня.Доступность = Не ТолькоПросмотр;
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов()
	...
	Иначе
		...
		Элементы.ЗапасыСтруктурнаяЕдиницаРезервТЧ.Видимость = РезервированиеЗапасов И НЕ СкладВШапке;
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасовСервер()
	...
	Иначе
		...
		Элементы.ЗапасыСтруктурнаяЕдиницаРезервТЧ.Видимость = НЕ СкладВШапке И РезервированиеЗапасов;
		Элементы.ЗапасыКонтекстноеМенюГруппаРаботаССтрокамиОбновить.Видимость = Ложь;
...
Процедура ПослеВыбораИзВыпадающегоСпискаТабличнойЧасти(Результат, Параметры) Экспорт
	...
	Если Параметры.ВыбранноеПоле = "ЗапасыОстатокОбщий" Тогда
		...
		Если Не РезервированиеЗапасов Или Не РазрешитьСкладыВТабличныхЧастях Тогда Возврат КонецЕсли;
			...
			Если ИмяТЧ="Запасы" Тогда
				...
				Если Строка.ЕстьДопРеквизитыСпецификации Тогда
					Если Строка.КлючСвязи<>0 Тогда
						УправлениеНебольшойФирмойКлиентСервер.УдалитьСтрокиПоКлючуСвязи(Объект.РеквизитыПараметрическихСпецификаций, Строка);
					КонецЕсли; 
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьПоляДоставки(Знач Объект, Знач ДанныеКонтрагента)
...
Функция РассчитатьСтоимостьДоставкиНаСервере()
	...
	Иначе
		...
		Если ТекущаяВалюта <> Объект.ВалютаДокумента Тогда
			СтруктураКурсы = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ТекущаяВалюта, Объект.ВалютаДокумента, Объект.Дата);
			СтоимостьДоставкиСНДС = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(СтоимостьДоставкиСНДС, СтруктураКурсы.КурсНач, СтруктураКурсы.Курс, СтруктураКурсы.КратностьНач, СтруктураКурсы.Кратность);
			Объект.СебестоимостьДоставки = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(Объект.СебестоимостьДоставки, СтруктураКурсы.КурсНач, СтруктураКурсы.Курс, СтруктураКурсы.КратностьНач, СтруктураКурсы.Кратность);
		КонецЕсли; 
		ОбновитьСуммуНДСДоставкиПриРасчете(ЭтотОбъект);
...
&НаСервере
Процедура ОбработатьУстановкуСпособаДоставкиНаСервере(ОбновлятьДанные = Истина)
...
Функция АдресаДоставки(Контрагент)
	...
	Пока Выборка.Следующий() Цикл
		Если УникальныеЗначения[Выборка.Представление] = Неопределено Тогда
			УникальныеЗначения[Выборка.Представление] = Новый Структура;
			УникальныеЗначения[Выборка.Представление].Вставить("Значение", Выборка.Значение);
			УникальныеЗначения[Выборка.Представление].Вставить("ЗначенияПолей", Выборка.ЗначенияПолей);
		КонецЕсли;
	КонецЦикла;
...
Процедура НоменклатураДоставкиПриИзмененииНаСервере()
	...
	ЗаполнятьСтоимость = ЗаполнятьСтоимостьДоставкиПоЦенеНоменклатуры(ЗаданаФормулаРасчетаСтоимости, ВыбранаСлужбаДоставкиЯндекса, ВариантУчетаДоставки);
...
Процедура ПересчитатьЦенуДоставки(ПредыдущаяВалюта, КурсПересчетаЦен = Неопределено)
	...
	СтруктураКурсы = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ПредыдущаяВалюта, Объект.ВалютаДокумента, Объект.Дата);
	...
	Объект.СтоимостьДоставки = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(Объект.СтоимостьДоставки, СтруктураКурсы.КурсНач, СтруктураКурсы.Курс, СтруктураКурсы.КратностьНач, СтруктураКурсы.Кратность);
	ОбновитьСуммуНДСДоставки();
...
Процедура ЗаменитьСтрокуЗапасовДаннымиНабора(Форма, СтрокаТабличнойЧасти, СтруктураДанные)
	...
	// Обновление отбора для отображения добавленых строк
...
Процедура ОбновитьСоставНабора(НоменклатураНабора, ХарактеристикаНабора, КоличествоНаборов, СоставНабора)
	...
	НаборыСервер.ОбновитьСоставНабораВТабличнойЧасти(НоменклатураНабора, ХарактеристикаНабора, ТекущийВариантКП, КоличествоНаборов, СоставНабора, Объект);
	ЗаполнитьСлужебныеПоляТабличнойЧасти();
	Модифицированность = Истина;
...
Процедура ЗаполнитьДопРеквизитыПоУмолчанию(Идентификатор)
	...
	Если СтрокаТабличнойЧасти.КлючСвязи=0 Тогда
		ИмяТабличнойЧасти = "Запасы";
		СтрокаТабличнойЧасти.КлючСвязи = УправлениеНебольшойФирмойСервер.СоздатьНовыйКлючСвязи(ЭтотОбъект);
	КонецЕсли;
