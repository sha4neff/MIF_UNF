...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СменитьФормуПобора"
		...
		Если МаркерПодбора = "Работы" Тогда
			...
			Если ПодборДляМатериаловВРаботах Тогда
				ЗНМатериалыПодбор(Неопределено);
			Иначе
				РаботыПодбор(Неопределено);
			КонецЕсли;
...
Процедура ЗНМатериалыНоменклатураПриИзменении(Элемент)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.ЗНМатериалы.ВыделенныеСтроки Цикл
		...
		РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераМатериалы, ТекущиеДанныеСтроки,
			ИспользоватьСерийныеНомераОстатки, "КлючСвязиСерийныеНомера");
	КонецЦикла;
...
Процедура ЗаполнитьПоРесурсамДляТекущейРаботы(Команда)
	...
	Если Объект.РесурсыПредприятия.Количество() = 0 Тогда
		...
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр(
			"ru = 'В табличной части ""Задействованные ресурсы"" нет записей.'"));
		Возврат;
...
Процедура ЗНМатериалыПодбор(Команда)
	...
	СписокСтатусов.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыПартий.СобственныеЗапасы"));
	
	ПараметрыПодбора.Вставить("СтатусПартии", СписокСтатусов);
...
Процедура ДобавитьОплатуСертификатом(Команда)
	...
	// Проверим рассчитанность скидок перед добавлнием сертификата
	Если ИспользоватьАвтоматическиеСкидки Тогда
...
Процедура ПредварительныйПросмотрЧекаЗавершение(ИдентификаторУстройстваФР, Параметры) Экспорт
	...
	УправлениеНебольшойФирмойКлиент.ПредпросмотрЧекаДенежныхДокументов(ОбщиеПараметры, ЭтотОбъект,
...
Процедура ЗаполнитьДанныеТаблицыРесурсовНаФорме()
	...
	Для Каждого СтрокаРесурсов Из Объект.РесурсыПредприятия Цикл
		...
		Если СтрокаРесурсов.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.Ежемесячно")
			...
			Если ЗначениеЗаполнено(ДатаПовторения) Тогда
				...
				ДополнениеПоМесяцуГоду = ?(ЗначениеЗаполнено(ДатаПовторения), ", каждое " 
										+ Строка(ДатаПовторения) + "-ое "+ПланированиеРесурсовУНФКлиент.ПолучитьМесяцПоНомеру(СтрокаРесурсов.НомерМесяца)+".","");
		...
		Междометие = ?(СтрокаРесурсов.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.Еженедельно"),"каждую", "каждый");
	...
	Если СтруктураДанные.Свойство("ВидЦен") Тогда
		...
		Если СтруктураДанные.Свойство("ВидРабот") Тогда
			...
			Иначе
				...
				Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДляВидаРабот);
				СтруктураДанные.Вставить("Цена", Цена);
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				Если ЗначениеЗаполнено(СтруктураДанные.ВидЦен) Тогда
					...
					Если ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения)
						...
						СтруктураДанныеНоменклатуры.Вставить("Коэффициент", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеШтрихкода.ЕдиницаИзмерения, "Коэффициент"));
					Иначе
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") 
		...
		Если НЕ РезультатЗакрытия.ПерезаполнитьЦены
			...
			УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, ВалютаРасчетовПередИзменением, "Работы", РезультатЗакрытия.КурсПересчетаЦен);
...
Процедура РедактироватьСоставНабора(СтрокаТабличнойЧасти)
	...
	Для каждого Стр Из СтрокиНабора Цикл
		СтруктураСтроки = Новый Структура("Номенклатура,Характеристика,Количество,ДоляСтоимости,Активная");
		Если ИмяТабличнойЧасти="Запасы" Тогда
...
Процедура ОбновитьСоставНабора(НоменклатураНабора, ХарактеристикаНабора, КоличествоНаборов, СоставНабора)
	...
	НаборыСервер.ОбновитьСоставНабораВТабличнойЧасти(НоменклатураНабора, ХарактеристикаНабора, , КоличествоНаборов, СоставНабора, Объект, ИмяТабличнойЧасти);
	ЗаполнитьСлужебныеПоляТабличнойЧасти(Объект.Запасы, Объект.Работы);
	Модифицированность = Истина;
...
Процедура ОбновитьЦеныНабора(Идентификатор, ИмяТЧ)
	...
	Для каждого СтрокаНабора Из СтрокиНабора Цикл
		...
		КонецЦикла; 
		Если ИмяТЧ="Работы" 
			И Объект.ПоложениеВидаРабот=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти 
	...
	НаборыСервер.РассчитатьЦеныСоставаНабора(СтрокаТабличнойЧасти.НоменклатураНабора, СтрокаТабличнойЧасти.ХарактеристикаНабора, , КоличествоНаборов, СоставНабора, Объект);
	...
	Для каждого ЭлементСостава Из СоставНабора Цикл
		...
		Для каждого СтрокаТабличнойЧасти Из СтрокиТЧ Цикл
			...
			УправлениеНебольшойФирмойКлиентСервер.РассчитатьСуммыВСтрокеТЧ(СтрокаТабличнойЧасти, ПараметрыРасчета);
		КонецЦикла;
	КонецЦикла;
...
Процедура ОбработатьВыборЭлементаТабличнойЧасти(Элемент)
	...
	СтруткураПараметров = Новый Структура("ОрганизацияОбъекта, Номенклатура, Характеристика, Партия, Склад, ЕдиницаИзмерения
	...
	СтруткураПараметров.ВидыЗаполненияПоЗаказНаряду = ВидыЗаполненияПоЗаказНаряду;
	...
	Если ЗаказНарядВСостоянииВыполнения Тогда
		...
		Если Элемент.ТекущийЭлемент = Элементы[ОстатокОбщий]
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ОбщиеЗапасы", СтруткураПараметров, ИмяТЧ, "Отгрузка", "ЗаказНаряд");
		...
		Если Элемент.ТекущийЭлемент = Элементы[ВРезервеВсего]
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СтруткураПараметров.Склад = ?(Элемент.ТекущийЭлемент = Элементы[ВРезервеВсего], Неопределено, СтруткураПараметров.Склад);
				
				СписокДокументов = СписокДокументовПоЗаказу("Остаток", СтруткураПараметров, ИмяТЧ, "Отгрузка", "ЗаказНаряд");
	...
	Иначе
		...
		Если Элемент.ТекущийЭлемент = Элементы[ВРезерве] Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("Остаток", СтруткураПараметров, ИмяТЧ);
		...
		Если Элемент.ТекущийЭлемент = Элементы[Размещено] И ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("Заказано", СтруткураПараметров, ИмяТЧ);
				ПоказатьВыборИзМеню(Оповещение, СписокДокументов, Элемент.ТекущийЭлемент);
		...
		Если Элемент.ТекущийЭлемент = Элементы[ОстатокОбщий]
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ОбщиеЗапасы", СтруткураПараметров, ИмяТЧ);
				...
				Если Не ТекущиеДанные.ОстатокОбщий = 0 И РазрешитьСкладыВТабличныхЧастях Тогда
					...
					ПредаставлениеОбщегоОстатка = ЗначениеОстатокОбщий + " " + Строка(ТекущиеДанные.ЕдиницаИзмерения) + ". <Всего>; Заполнить по остаткам.";
		...
		Если Элемент.ТекущийЭлемент = Элементы[ДатаПоступления] Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ЗапасыПоступления", СтруткураПараметров, ИмяТЧ);
				ПоказатьВыборИзМеню(Оповещение, СписокДокументов, Элемент.ТекущийЭлемент);
...
Процедура РазбитьСтроку()
	...
	ПараметрыОповещения = Новый Структура("СтрокаТабличнойЧасти, АвтоматическиеСкидкиРасчитаны, ИмяТЧ", СтрокаТабличнойЧасти, Объект.СкидкиРассчитаны, ИмяТЧ);
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов(ИмяТЧ = Неопределено, УбратьВидимостьРежима = Ложь, ОбновлятьСостояние = Истина)
	...
	Для Каждого ИмяТЧ Из МассивИменТЧ Цикл
		...
		Если ЗаказНарядВСостоянииВыполнения Тогда
			Элементы.ИзменитьРезервЗаполнитьПоОстаткам.Заголовок = "По остаткам и резервам (все склады)";
			Элементы.ЗНМатериалыИзменитьРезервЗаполнитьПоОстаткам.Заголовок = "По остаткам и резервам (все склады / для всех работ)";
		Иначе
			Элементы.ИзменитьРезервЗаполнитьПоОстаткам.Заголовок = "По остаткам (все склады)";
			Элементы.ЗНМатериалыИзменитьРезервЗаполнитьПоОстаткам.Заголовок = "По остаткам (все склады / все работы)";
		КонецЕсли;
...
Процедура ПослеВыбораИзВыпадающегоСпискаТабличнойЧасти(Результат, Параметры) Экспорт
	...
	Если Параметры.ВыбранноеПоле = "ЗапасыОстатокОбщий" Тогда
		...
		Если Не ФункциональнаяОпцияРезервированиеЗапасов Или Не РазрешитьСкладыВТабличныхЧастях Тогда Возврат КонецЕсли;
		
		Если Результат.Значение = "РаспределитьПоРезервам" Тогда
			
			Если ИмяТЧ = "Запасы" Тогда
				ДатаОтгрузки = ?(Не ЗначениеЗаполнено(ТекущиеДанные.ДатаОтгрузки), Объект.ДатаОтгрузки, ТекущиеДанные.ДатаОтгрузки);
			Иначе
...
&НаСервере
Функция СписокДокументовПоЗаказу(ТипОперации, СтруткураПараметров, ИмяТЧ, ДопПараметр = Неопределено, ТипДокумента = Неопределено)
	Возврат НоменклатураВДокументахСервер.СписокДокументовПоЗаказу(Объект, ТипОперации, СтруткураПараметров, ИмяТЧ, ДопПараметр, ТипДокумента);
КонецФункции
...
Процедура ПриИзмененииКалькуляцииСервер(АдресНастроек)
	...
	КонецЦикла;
	Объект.Материалы.Очистить();
	Объект.СерийныеНомераМатериалы.Очистить();
