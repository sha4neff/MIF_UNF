...
Процедура СоздатьВТПерсональныеДанныеСотрудников(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ФизическиеЛица = Неопределено) Экспорт 
	...
	Если ФизическиеЛица <> Неопределено Тогда
		...
		|	Где СотрудникиОрганизации.ФизическоеЛицо В (&СписокФизическихЛиц)";
	КонецЕсли;
...
Функция СформироватьВыходнойФайл(Ссылка)
	...
	Пока ВыборкаУплаченныхВзносов.Следующий() Цикл 
		...
		УзелУплата = СправкиПоНДФЛ.ДобавитьУзелВДеревоXML(УзелФормы, "Уплата", "");
		СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелУплата, НаборЗаписейТипУплата);
	КонецЦикла;
	...
	ФорматСЗВ_КОРР = СправкиПоНДФЛ.ЗагрузитьФорматНабораЗаписей(ДеревоФормата, "СЗВ_КОРР");
	...
	Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		УзелФормы = СправкиПоНДФЛ.ДобавитьУзелВДеревоXML(УзелПФР, "СЗВ-КОРР", "");
		СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, ФорматСЗВ_КОРР);
		...
		СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейЗЛ);
		...
		Если ВыводитьВФайл Тогда 
			СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейДанныеЗЛ);
		КонецЕсли;
		...
		Для Каждого КодМесяца Из СписокМесяцев Цикл 
			...
			КонецЕсли;
			Если ВыводитьВФайл Тогда 
				СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейСуммы);
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого КодМесяца Из СписокМесяцев Цикл 
			...
			КонецЕсли;
			Если ВыводитьВФайл Тогда 
				СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейВыплатыДТ);
			КонецЕсли;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникам.СотрудникУволен) Тогда
			...
			СправкиПоНДФЛ.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейДатаУвольнения);
		КонецЕсли;
...
Функция ЗапросПоШапкеДляПечати(СписокДокументов)
	...
	|	ДанныеОКорректировкеСведений.ДолжностьРуководителя КАК ДолжностьРуководителя,
