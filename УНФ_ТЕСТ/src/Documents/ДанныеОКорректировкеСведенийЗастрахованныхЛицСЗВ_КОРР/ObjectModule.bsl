...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ДополнительныеСвойства.Свойство("НеПроверятьДанныеОрганизации") Тогда
		...
		Если Не ЗначениеЗаполнено(ИННвКорректируемыйПериод) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен ИНН в корректируемый период'"), , "Объект.ИННвКорректируемыйПериод", ,Отказ);
		ИначеЕсли Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИННвКорректируемыйПериод, РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Организация), "") Или СтрЗаменить(ИННвКорректируемыйПериод, "0", "") = "" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Неверно указан ИНН в корректируемый период'"), , "Объект.ИННвКорректируемыйПериод", ,Отказ);
		КонецЕсли;
		...
		Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Организация) Тогда
			Если Не ЗначениеЗаполнено(КППвКорректируемыйПериод) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен код по КПП в корректируемый период'"), , "Объект.КППвКорректируемыйПериод", ,Отказ);
			ИначеЕсли Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(КППвКорректируемыйПериод, "") Или СтрЗаменить(КППвКорректируемыйПериод, "0", "") = "" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Неверно указан код по КПП в корректируемый период'"), , "Объект.КППвКорректируемыйПериод", ,Отказ);
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьЗапросПоСотрудникамДляПроверкиЗаполнения(МенеджерВременныхТаблиц)
	...
	УчетСтраховыхВзносов.СформироватьТаблицуВТФизическиеЛицаРаботавшиеВОрганизации(Запрос.МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ПерсонифицированныйУчетКлиентСервер.ОкончаниеОтчетногоПериодаПерсУчета(ОтчетныйПериод));
...
Функция ДокументыСЗВПравилаПроверкиДанныхЗЛ(ПроверятьАдрес) 
