...
Процедура ОтчетныйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ОтчетныйПериодРегулированиеОбработка(Объект.ОтчетныйПериод, ПериодСтрока, Направление, '20170101');
	Объект.ОкончаниеОтчетногоПериода = ПерсонифицированныйУчетКлиентСервер.ОкончаниеОтчетногоПериодаСтажаПерсУчета(Объект.ОтчетныйПериод);
...
Процедура КорректируемыйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	КлючевыеРеквизитыЗаполненияФормыОчиститьТаблицы();
	МинимальныйПериод = ?(ИзменениеБезОграничений, '00010101', '20170101');
	ОтчетныйПериодРегулированиеОбработка(Объект.КорректируемыйПериод, КорректируемыйПериодСтрока, Направление, МинимальныйПериод);
...
Процедура СотрудникиСотрудникПриИзмененииНаСервере()
	...
	Документы.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР.СоздатьВТПерсональныеДанныеСотрудников(
		МенеджерВременныхТаблиц, Объект.Организация, Объект.КорректируемыйПериод, СписокСотрудников);
...
Процедура ЗаполнитьДанныеОСтажеСотрудников(Сотрудники)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	ВТДанныеСтажа КАК ДанныеСтажаСотрудников
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Сотрудник,
	               |	ДоговорГПХ УБЫВ,
	...
	ПроверяемыеРеквизиты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		|ТерриториальныеУсловия,ПараметрТерриториальныхУсловий", , Истина, Истина);
...
Процедура ЗаполнитьДанныеОЗаработкеСотрудников(СписокСотрудников)
	...
	УчетСтраховыхВзносов.СоздатьВТСведенияОЗаработке2014(МенеджерВременныхТаблиц, Объект.Организация, Объект.КорректируемыйПериод, ПараметрыПолученияЗаработка, ПараметрыОтбора);
...
&НаКлиенте
Процедура КлючевыеРеквизитыЗаполненияФормыОчиститьТаблицы()
