...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетОПереработкеСерийныеНомера.КлючСвязи КАК КлючСвязи,
	|	ОтчетОПереработкеСерийныеНомера.СерийныйНомер КАК СерийныйНомер
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор КОГДА ОтчетОПереработке.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	...
	|		ИНАЧЕ ОтчетОПереработке.Организация КОНЕЦ КАК Организация
	|	,Выбор КОГДА ОтчетОПереработке.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		ТОГДА ОтчетОПереработке.Организация.ГоловнаяОрганизация
	|		ИНАЧЕ ОтчетОПереработке.Организация КОНЕЦ КАК Поставщик
	...
	|		ИНАЧЕ ОтчетОПереработке.Организация КОНЕЦ КАК Грузоотправитель
	|	,Выбор КОГДА ОтчетОПереработке.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|			ТОГДА ОтчетОПереработке.Организация
	|		КОГДА ОтчетОПереработке.Подразделение.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|				И ОтчетОПереработке.Подразделение.ГоловнаяОрганизация = ОтчетОПереработке.Организация
	|			ТОГДА ОтчетОПереработке.Подразделение
	|		ИНАЧЕ ОтчетОПереработке.Организация КОНЕЦ КАК ОбособленноеПодразделениеПоставщика
	...
	|	,Выбор КОГДА ОтчетОПереработке.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ КОНЕЦ КАК ЭтоФизическоеЛицо
	|	,Выбор КОГДА ОтчетОПереработке.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		ТОГДА 2
	...
	|	,Выбор КОГДА ОтчетОПереработке.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		ИНАЧЕ ОтчетОПереработке.Контрагент КОНЕЦ КАК Контрагент
	|	,Выбор КОГДА ОтчетОПереработке.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И ОтчетОПереработке.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		ТОГДА ОтчетОПереработке.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ ОтчетОПереработке.Контрагент КОНЕЦ КАК Покупатель
	|	,Выбор КОГДА ОтчетОПереработке.Грузополучатель.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И ОтчетОПереработке.Грузополучатель.ГоловнойКонтрагент = ЗаказПокупателя.Контрагент
	...
	|		ИНАЧЕ ОтчетОПереработке.Контрагент КОНЕЦ КАК ОбособленноеПодразделениеПокупателя
	|	,Выбор КОГДА ОтчетОПереработке.Грузополучатель <> Значение(Справочник.Контрагенты.ПустаяСсылка) 
	|		ТОГДА ОтчетОПереработке.Грузополучатель
	|		ИНАЧЕ ОтчетОПереработке.Контрагент КОНЕЦ КАК Грузополучатель
	...
	|		ИНАЧЕ ОтчетОПереработке.ВалютаДокумента КОНЕЦ КАК Валюта
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение
	|		ИНАЧЕ ОтчетОПереработке.ВалютаДокумента КОНЕЦ КАК ВалютаДокумента
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.НаименованиеПолное
	|		ИНАЧЕ ОтчетОПереработке.ВалютаДокумента.НаименованиеПолное КОНЕЦ КАК ВалютаНаименование
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.Код
	|		ИНАЧЕ ОтчетОПереработке.ВалютаДокумента.Код КОНЕЦ КАК ВалютаКод
	...
	|		,Выбор КОГДА ЕдиницаИзмерения Ссылка Справочник.КлассификаторЕдиницИзмерения
	|			ТОГДА ЕдиницаИзмерения.Наименование
	|			ИНАЧЕ Неопределено КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Наименование
	|		,Выбор КОГДА ЕдиницаИзмерения Ссылка Справочник.КлассификаторЕдиницИзмерения
	|			ТОГДА ЕдиницаИзмерения.Код
	|			ИНАЧЕ Неопределено КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Код
	...
	|			ТОГДА
	|				ВЫБОР КОГДА ОтчетОПереработке.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетОПереработке.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ((СуммаАвтоматическойСкидки + СуммаСкидкиНаценки) * ОтчетОПереработке.Курс / ОтчетОПереработке.Кратность КАК Число(15,2))
	|					ИНАЧЕ (СуммаАвтоматическойСкидки + СуммаСкидкиНаценки) КОНЕЦ
	|			ИНАЧЕ (СуммаАвтоматическойСкидки + СуммаСкидкиНаценки) КОНЕЦ КАК СуммаСкидкиПоСтроке
	|		,Выбор КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА
	|				ВЫБОР КОГДА ОтчетОПереработке.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетОПереработке.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(Цена * ОтчетОПереработке.Курс / ОтчетОПереработке.Кратность КАК Число(15,2))
	|					ИНАЧЕ Цена КОНЕЦ
	|			ИНАЧЕ Цена КОНЕЦ КАК Цена
	|		,Выбор КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА
	|				ВЫБОР КОГДА (ОтчетОПереработке.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетОПереработке.Ссылка.ВалютаДокумента <> &НациональнаяВалюта)
	|					ТОГДА ВЫРАЗИТЬ(Сумма * ОтчетОПереработке.Курс / ОтчетОПереработке.Кратность КАК Число(15,2))
	|					ИНАЧЕ Сумма КОНЕЦ
	|			ИНАЧЕ Сумма КОНЕЦ КАК Сумма
	|		,Выбор КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА
	|				ВЫБОР КОГДА ОтчетОПереработке.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетОПереработке.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(СуммаНДС * ОтчетОПереработке.Курс / ОтчетОПереработке.Кратность КАК Число(15,2))
	|					ИНАЧЕ СуммаНДС КОНЕЦ
	|			ИНАЧЕ СуммаНДС КОНЕЦ КАК СуммаНДС
	|		,Выбор КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА
	|				ВЫБОР КОГДА ОтчетОПереработке.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетОПереработке.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(Всего * ОтчетОПереработке.Курс / ОтчетОПереработке.Кратность КАК Число(15,2))
	|					ИНАЧЕ Всего КОНЕЦ
	|			ИНАЧЕ Всего КОНЕЦ КАК Всего
	|		,ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки
	|		,Выбор КОГДА ПроцентСкидкиНаценки <> 0
	|			ТОГДА 1
