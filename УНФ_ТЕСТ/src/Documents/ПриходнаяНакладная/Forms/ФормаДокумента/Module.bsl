...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодборНоменклатурыВДокументах.НазначитьФормуПодбора(ПараметрыОткрытияПодбора, Объект.Ссылка.Метаданные().Имя, ИмяТЧЗапасы());
	ПодборНоменклатурыВДокументах.НазначитьФормуПодбора(ПараметрыОткрытияПодбора, Объект.Ссылка.Метаданные().Имя, ИмяТЧРасходы());
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновлениеФормыПриходнаяНакладная" И ЭтаФорма.УникальныйИдентификатор = Источник Тогда
...
Процедура КонтрагентПриИзмененииФрагментЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		УправлениеНебольшойФирмойКлиент.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦенКонтрагента(ЭтотОбъект, ИмяТЧЗапасы());
		УправлениеНебольшойФирмойКлиент.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦенКонтрагента(ЭтотОбъект, ИмяТЧРасходы());
...
Процедура ЗаполнитьПоОстаткамКомиссия(Команда)
	...
	Если Объект.Запасы.Количество() Тогда
		Оповещение = Новый ОписаниеОповещения("ВопросОчистититьЗапасы", ЭтотОбъект);
...
&НаКлиенте
Процедура ВопросОчистититьЗапасы(Результат, Параметры) Экспорт
...
// @skip-warning
&НаСервере
Процедура ЗаполнитьПризнакМаркируемойПродукции()
	
	Для Каждого СтрокаЗапасы Из Объект.Запасы Цикл
		СтрокаЗапасы.МаркируемаяПродукцияГОСИС = СтрокаЗапасы.Номенклатура.ТабачнаяПродукция;
	КонецЦикла;
...
// @skip-warning
&НаКлиенте
Процедура ЗаполнитьПризнакМаркируемойПродукцииВСтроке(СтрокаЗапасы)
	
	СтрокаЗапасы.МаркируемаяПродукцияГОСИС = УправлениеНебольшойФирмойВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаЗапасы.Номенклатура, "ТабачнаяПродукция");
...
Процедура ПересчитатьКешМаркируемойПродукции()
	...
	ИнтеграцияМОТПУНФ.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект, Истина);
	
КонецПроцедуры

// Если есть команда возобновления проверки
// @skip-warning
&НаКлиенте
Процедура Подключаемый_ВозобновитьПроверкуПродукцииМОТП(Команда)
	
	ПроверкаИПодборПродукцииМОТПКлиент.ВозобновитьПроверку(ЭтотОбъект);
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Дата, ВалютаДокумента, Контрагент, Организация)
	...
	Если ИспользоватьПодключаемоеОборудование Тогда
		ДанныеКИКонтрагента = УправлениеНебольшойФирмойСервер.ПолучитьДанныеКИКонтрагентаДляВыбораНаФорме(Контрагент,
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТабличнойЧасти,
	...
	// Серийные номера
	Если ИспользоватьСерийныеНомераОстатки <> Неопределено
...
Процедура ПересчитатьЦеныПоВалюте(Знач ПараметрыЗаполнения, Знач ДополнительныеПараметры)
	...
	Если НЕ ПараметрыЗаполнения.ПерезаполнитьЦены
		...
		УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект,
...
Процедура ПересчитатьСуммуЕслиИзменилсяПризнакСуммаВключаетНДС(Знач ПараметрыЗаполнения)
	...
	Если НЕ ПараметрыЗаполнения.ПерезаполнитьЦены
		...
		УправлениеНебольшойФирмойКлиент.ПересчитаемСуммуТабличнойЧастиПоФлагуСуммаВключаетНДС(ЭтотОбъект, ИмяТЧЗапасы());
	КонецЕсли;
	
	Если ПараметрыЗаполнения.СуммаВключаетНДС <> ПараметрыЗаполнения.ПредСуммаВключаетНДС Тогда
		УправлениеНебольшойФирмойКлиент.ПересчитаемСуммуТабличнойЧастиПоФлагуСуммаВключаетНДС(ЭтотОбъект, ИмяТЧРасходы());
	КонецЕсли;
...
Процедура ОбновитьВидимостьПоляЗаказыПокупателей(Форма)
	...
	Если НЕ ЕстьЗаказыПокупателей Тогда
		Для каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
			...
			Если ТипЗнч(ЗаказыПокупателей) = Тип("СписокЗначений") И ЗаказыПокупателей.Количество() > 0 Тогда
				// Заказ поставщику содержит заказы покупателей
				ЕстьЗаказыПокупателей = Истина;
				Прервать;
			КонецЕсли; 
		КонецЦикла; 
...
Процедура СодержаниеНачалоВыбора(Элемент, ИмяТабличнойЧасти)
	...
	СвойстваРеквизита.Вставить("НомерСтроки", ТекСтрока.НомерСтроки - 1);
	...
	ЭтотОбъект,
	СвойстваРеквизита,
	СтрШаблон(НСтр("ru = '%1: содержание'"), ТекСтрока.Номенклатура));
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь, ЭтоНовыйОбъект = Ложь)
	...
	ЭлементыГТД.Добавить("ЗапасыНомерГТД");
	...
	СведенияОбИмпортеВидны = (МассивВидовОпераций.Найти(Объект.ВидОперации) <> Неопределено);
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				Если СтрокаТаблицы.Свойство("СтранаПроисхождения")
				...
				// заполнить серийные номера
				Если СтрокаТаблицы.Свойство("СерийныйНомер_ВходящиеДанные") 
					И ЗначениеЗаполнено(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные) 
					И НоваяСтрока.Номенклатура.ИспользоватьСерийныеНомера Тогда
					
						Если СтрНайти(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные, Символы.ПС) > 0 Тогда
							Разделитель = Символы.ПС;
						ИначеЕсли СтрНайти(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные, ",") > 0 Тогда
							Разделитель = ",";
						ИначеЕсли СтрНайти(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные, ";") > 0 Тогда
							Разделитель = ";";
						ИначеЕсли СтрНайти(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные, " ") > 0 Тогда
							Разделитель = " ";
						КонецЕсли;
					    МассивСерийных = СтрРазделить(СтрокаТаблицы.СерийныйНомер_ВходящиеДанные, Разделитель);
						
						Для каждого серийный Из МассивСерийных Цикл
							серийныйСсылка = Справочники.СерийныеНомера.НайтиПоНаименованию(СокрЛП(серийный), Истина, , НоваяСтрока.Номенклатура);
							Если НЕ ЗначениеЗаполнено(серийныйСсылка) Тогда
								
								серийныйОбъект = Справочники.СерийныеНомера.СоздатьЭлемент();
							...
							РаботаССерийнымиНомерамиКлиентСервер.ДобавитьСерийныйНомерВСтроку(НоваяСтрока, серийныйСсылка, Объект); 
