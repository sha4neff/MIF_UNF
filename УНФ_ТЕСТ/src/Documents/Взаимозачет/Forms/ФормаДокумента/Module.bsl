...
Процедура ПодборРасчетыСПокупателемФрагмент()
	...
	ПараметрыПодбора = Новый Структура(
		...
		Объект.Дебитор.Итог("СуммаУчета"),
		Объект.ВидОперации,
		ИмяТЧ
	);
	...
	ОткрытьФорму("ОбщаяФорма.ФормаПодбораАвансовИДолговПокупателей",
		ПараметрыПодбора,
		,,,,
		Новый ОписаниеОповещения("ПодборРасчетыСПокупателемЗавершение", ЭтотОбъект, Новый Структура("АдресДебиторВХранилище", АдресДебиторВХранилище))
	);
...
Процедура ПодборРасчетыСПокупателемЗавершение(Результат1, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.OK Тогда
		ПолучитьДебиторИзХранилища(АдресДебиторВХранилище, ?(ДополнительныеПараметры.Свойство("ИмяТЧ"), ДополнительныеПараметры.ИмяТЧ, ""));
	КонецЕсли;
...
Процедура ПодборРасчетыСПокупателемКредитор()
	...
	ОткрытьФорму("ОбщаяФорма.ФормаПодбораАвансовИДолговПокупателей",
		ПараметрыПодбора,,,,,
		Новый ОписаниеОповещения("ПодборРасчетыСПоставщикомЗавершение",
		ЭтотОбъект,
...
Процедура ПодборРасчетыСПоставщикомДебитор()
	...
	ПараметрыПодбора = Новый Структура(
		...
		Объект.Ссылка,
		Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ПрочийВзаимозачет"),
		Объект.Кредитор.Итог("СуммаУчета"),
		Объект.ВидОперации,
		"Дебитор"
	);
	...
	ОткрытьФорму("ОбщаяФорма.ФормаПодбораАвансовИДолговПоставщикам",
		ПараметрыПодбора,,,,,
		Новый ОписаниеОповещения("ПодборРасчетыСПокупателемЗавершение",
		ЭтотОбъект,
...
Процедура УстановитьВидимостьИДоступность()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя")
		...
		Элементы.РасчетыСДебитором.Видимость = Истина;
		Элементы.РасчетыСДебитором.Заголовок = "Авансы";
		
		Элементы.РасчетыСКредитором.Видимость = Истина;
		Элементы.РасчетыСКредитором.Заголовок = "Долги";
...
Процедура ОпределитьВидимостьНастроекУчетаВНалогообложении()
	...
	Если Не ПлательщикУСН Тогда
		Элементы.ГруппаУчетВНУ.Видимость = Ложь;
	Иначе
		Если Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику Тогда
