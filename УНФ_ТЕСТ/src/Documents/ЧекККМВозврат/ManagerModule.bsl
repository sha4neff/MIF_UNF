	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияПодарочныеСертификатыИзменение Тогда
		...
		Если Не МассивРезультатов[2].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПодарочныеСертификаты(ДокументОбъектЧекККМВозврат, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ДисконтныеКартыСервер.ТекстЗапросаПродажиПоДисконтнойКарте("ЧекККМВозврат");
...
Процедура СформироватьТаблицаОплатаПлатежнымиКартами(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДокумента.ЭквайринговыйТерминал.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
...
Функция ЧекЕГАИСXML(ДокументСсылка, ДополнительныеПараметры)
	...
	ТекстыЗапроса.Добавить(
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМВозврат.Запасы КАК ТабличнаяЧастьТовары
