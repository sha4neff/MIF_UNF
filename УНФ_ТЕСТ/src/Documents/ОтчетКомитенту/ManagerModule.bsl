...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетКомитентуСерийныеНомера.КлючСвязи КАК КлючСвязи,
	|	ОтчетКомитентуСерийныеНомера.СерийныйНомер КАК СерийныйНомер
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОтчетКомитенту, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыПринятыеИзменение
		...
		Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить()
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятыеВРазрезеГТД(ДокументОбъектОтчетКомитенту, ВыборкаИзРезультатаЗапроса, Отказ);
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор КОГДА ОтчетКомитенту.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		ТОГДА ОтчетКомитенту.Организация.ГоловнаяОрганизация
	|		ИНАЧЕ ОтчетКомитенту.Организация КОНЕЦ КАК Организация
	...
	|		ИНАЧЕ ОтчетКомитенту.Организация КОНЕЦ КАК ОбособленноеПодразделениеПоставщика
	...
	|	,Выбор КОГДА ОтчетКомитенту.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ КОНЕЦ КАК ЭтоФизическоеЛицо
	|	,Выбор КОГДА ОтчетКомитенту.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		ТОГДА 2
	...
	|	,Выбор КОГДА ОтчетКомитенту.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		ИНАЧЕ ОтчетКомитенту.Контрагент КОНЕЦ КАК Контрагент
	...
	|		ИНАЧЕ ОтчетКомитенту.ВалютаДокумента КОНЕЦ КАК ВалютаДокумента
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.НаименованиеПолное
	|		ИНАЧЕ ОтчетКомитенту.ВалютаДокумента.НаименованиеПолное КОНЕЦ КАК ВалютаНаименование
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.Код
	|		ИНАЧЕ ОтчетКомитенту.ВалютаДокумента.Код КОНЕЦ КАК ВалютаКод
	...
	|	,Выбор КОГДА ОтчетКомитенту.ОснованиеПечатиСсылка ССЫЛКА Справочник.ДоговорыКонтрагентов
	|		ТОГДА ОтчетКомитенту.ОснованиеПечатиСсылка.НомерДоговора
	|		ИНАЧЕ ОтчетКомитенту.ОснованиеПечатиСсылка.Номер КОНЕЦ КАК ОснованиеНомер
	|	,Выбор КОГДА ОтчетКомитенту.ОснованиеПечатиСсылка ССЫЛКА Справочник.ДоговорыКонтрагентов
	|		ТОГДА ОтчетКомитенту.ОснованиеПечатиСсылка.ДатаДоговора
	|		ИНАЧЕ ОтчетКомитенту.ОснованиеПечатиСсылка.Дата КОНЕЦ КАК ОснованиеДата
	...
	|	,СУММА(ВЫБОР КОГДА ОтчетКомитентуЗапасы.Ссылка.СуммаВключаетНДС
	|			ТОГДА ОтчетКомитентуЗапасы.СуммаВознаграждения
	|			ИНАЧЕ ОтчетКомитентуЗапасы.СуммаВознаграждения + ОтчетКомитентуЗапасы.СуммаНДСВознаграждения КОНЕЦ) КАК Всего
	...
	|ИЗ Документ.ОтчетКомитенту.Запасы КАК ОтчетКомитентуЗапасы
	|ГДЕ ОтчетКомитентуЗапасы.Ссылка В(&МассивОбъектов)
	|СГРУППИРОВАТЬ ПО 
	|	ОтчетКомитентуЗапасы.Ссылка
	|	,ОтчетКомитентуЗапасы.СтавкаНДС
	|
	// :::Пересчет в сумму национальной валюты
	|;Выбрать
	...
	|	,ЗапасыВВалютеДокумента.КодТНВЭД
	...
	|		ТОГДА
	|			ВЫБОР КОГДА ЗапасыВВалютеДокумента.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				ИНАЧЕ
	|					ВЫБОР КОГДА ЗапасыВВалютеДокумента.Количество = 0
	|						ТОГДА (СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС)
	|						ИНАЧЕ ВЫРАЗИТЬ((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / ЗапасыВВалютеДокумента.Количество КАК ЧИСЛО(15, 2)) КОНЕЦ
	|			КОНЕЦ
	|		ИНАЧЕ ЗапасыВВалютеДокумента.Цена КОНЕЦ КАК Цена
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях 
	|		ТОГДА
	|			ВЫБОР КОГДА ЗапасыВВалютеДокумента.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				ИНАЧЕ
	|					ВЫБОР КОГДА ЗапасыВВалютеДокумента.Ссылка.СуммаВключаетНДС
	...
	|			КОНЕЦ
	|		ИНАЧЕ ЗапасыВВалютеДокумента.Сумма КОНЕЦ КАК Сумма
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях 
	|		ТОГДА
	|			ВЫБОР КОГДА ЗапасыВВалютеДокумента.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|			КОНЕЦ
	|		ИНАЧЕ ЗапасыВВалютеДокумента.СуммаНДС КОНЕЦ КАК СуммаНДС
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях 
	|		ТОГДА
	|			ВЫБОР КОГДА ЗапасыВВалютеДокумента.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|			КОНЕЦ
	|		ИНАЧЕ ЗапасыВВалютеДокумента.Всего КОНЕЦ КАК Всего
	|	,ЗапасыВВалютеДокумента.СтранаСсылка
	|	,ЗапасыВВалютеДокумента.СтранаПредставление
	|	,ЗапасыВВалютеДокумента.СтранаКод
	|	,ЗапасыВВалютеДокумента.ПредставлениеГТД
	|ИЗ ЗапасыВВалютеДокумента КАК ЗапасыВВалютеДокумента
	...
	|ГДЕ ЗапасыВВалютеДокумента.Ссылка В(&МассивОбъектов)
