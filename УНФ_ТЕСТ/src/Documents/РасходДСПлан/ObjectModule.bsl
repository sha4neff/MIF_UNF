...
Процедура ЗаполнитьПоНачислениеЗарплаты(ДокументСсылкаНачислениеЗарплаты) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(НачислениеЗарплатыНалогиНаДоходы.Сумма), 0) КАК СуммаНалогов
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(НачислениеЗарплатыВзносы.Сумма), 0) КАК СуммаВзносов
	|ИЗ
	...
	ВыборкаТаблицаДокумента = Результаты[0].Выбрать();
	ВыборкаТаблицаДокумента.Следующий();
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаТаблицаДокумента);
	
	Если Не Результаты[1].Пустой() Тогда
		ВыборкаНалогиНаДоходы = Результаты[1].Выбрать();
		ВыборкаНалогиНаДоходы.Следующий();
		СуммаДокумента = СуммаДокумента + ВыборкаНалогиНаДоходы.СуммаНалогов;
	КонецЕсли;
	
	Если Не Результаты[2].Пустой() Тогда
		ВыборкаВзносы = Результаты[2].Выбрать();
		ВыборкаВзносы.Следующий();
		СуммаДокумента = СуммаДокумента + ВыборкаВзносы.СуммаВзносов;
	КонецЕсли;
