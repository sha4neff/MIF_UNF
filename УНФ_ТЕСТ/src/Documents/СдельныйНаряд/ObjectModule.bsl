...
Функция ОстаткиЗаказа(Основание, Запрос)
	...
	|	
	|	ВЫБРАТЬ
	|		ВыпускПродукцииОборот.ЗаказПокупателя,
	|		ВыпускПродукцииОборот.Номенклатура,
	|		ВыпускПродукцииОборот.Характеристика,
	|		ТаблицаОпераций.Операция,
	|		ТаблицаОпераций.Этап,
	|		ВыпускПродукцииОборот.КоличествоПланОборот * ТаблицаОпераций.Количество
	|	ИЗ
	...
	|						СГРУППИРОВАТЬ ПО
	|							ЗаказПокупателяЗапасы.Ссылка)
	|					И НЕ (ЗаказПокупателя, Номенклатура, Характеристика, Спецификация) В
	|							(ВЫБРАТЬ
	|								ЗаказПокупателяЗапасы.Ссылка,
	...
	|							ИЗ
	|								ВТЗаказПокупателяЗапасы КАК ЗаказПокупателяЗапасы)) КАК ВыпускПродукцииОборот
	|			ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОпераций КАК ТаблицаОпераций
	|			ПО ВыпускПродукцииОборот.Спецификация = ТаблицаОпераций.Спецификация
	|	ГДЕ
	|		ВыпускПродукцииОборот.КоличествоПланОборот > 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	Если ТаблицаОстатков.Количество() > 0 И ВыборкаДанных.Количество() > 0 Тогда
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			Если ТаблицаОстатковЗаказа<>Неопределено 
				...
				Если МассивОстатковПоЗаказу[0].Остаток <= 0 Тогда
					ТаблицаОстатковЗаказа.Удалить(МассивОстатковПоЗаказу[0]);
				КонецЕсли;
			КонецЕсли; 
...
Процедура ЗаполнитьПоСборкеЗапасовСборка(ДокументСсылкаСборкаЗапасов)
	...
	|		Операции.Операция,
	|		Операции.Этап,
	|		Продукция.Количество * Операции.Количество
	|	ИЗ
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	Для каждого СтрокаПродукция Из ТаблицаПродукции Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПродукция.Этап) Тогда
			...
			Отказ);
		КонецЕсли; 
		Если НЕ ЗначениеЗаполнено(СтрокаПродукция.ЗаказНаПроизводство) И НЕ ЗначениеЗаполнено(СтрокаПродукция.ЗаказПокупателя) Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			ЭтотОбъект,
...
Процедура ПроверитьЗачислениеСебестоимости(Отказ)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Изготовитель %3 документа %1 не соответствует структурной единице указанной в сдельном наряде (%2)'"), 
		Выборка.Документ, 
		Выборка.СтруктурнаяЕдиница, 
		Выборка.СтруктурнаяЕдиницаФакт);
		Если ПоложениеСтруктурнойЕдиницы=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "Операции", Выборка.НомерСтроки, "СтруктурнаяЕдиница", Отказ);
		Иначе
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, , , "СтруктурнаяЕдиница", Отказ);
		КонецЕсли; 
	КонецЦикла; 
...
Функция СтруктурныеЕдиницыПроверки(СтрокаОперации)
	...
	ИначеЕсли ТипЗнч(СтрокаОперации.Исполнитель)=Тип("СправочникСсылка.Бригады") Тогда
		СтрокиБригады = УправлениеНебольшойФирмойКлиентСервер.СтрокиПоКлючуСвязи(СоставБригады, СтрокаОперации.КлючСвязи);
...
Процедура ПроверитьСчетаУчетаЗатратНоменклатуры()
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Номенклатура %1 использует счет учета затрат с типом ""%2"". Для таких позиций расчет себестоимости не будет выполнен.
                                         |Следует использовать счета с типом ""Незавершенное производство"" и ""Косвенные затраты""'"), Выборка.Номенклатура, Выборка.ТипСчета);
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "Операции", СтрокаТабличнойЧасти.НомерСтроки, "Номенклатура");
	КонецЦикла; 
