...
Процедура ПродукцияНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("СтруктураНастроек") Тогда
			НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтаФорма, "ЗаказНаПроизводсто", НастройкиФормыВыбораНоменклатуры, Истина, "Продукция", ВыбранноеЗначение.СтруктураНастроек);
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруткураПараметров = Новый Структура("ОрганизацияОбъекта, Номенклатура, Характеристика, Партия, Склад,
	...
	СтруткураПараметров.ЗаказПокупателя =?(ЗначениеЗаполнено(ТекущиеДанные.ЗаказПокупателя), ТекущиеДанные.ЗаказПокупателя, Объект.ЗаказПокупателя);
	СтруткураПараметров.НомерВариантаКП = 0;
	СтруткураПараметров.ОрганизацияОбъекта = Объект.Организация;
	СтруткураПараметров.ИмяКолонкиСклада = "СтруктурнаяЕдиница";
...
Процедура ШапкаТабличнаяЧастьЗавершениеНаСервере(Знач Результат)
	...
	Если Результат.Свойство("ПоложениеСкладаВДокументахПроизводства") 
		...
		Если Не РежимОстаткиИРезервы Тогда
			...
			КонецЕсли; 
		КонецЕсли;
		Если Объект.ПоложениеЗаказаПокупателя = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке И НЕ ЗначениеЗаполнено(Объект.ЗаказПокупателя) Тогда
			СтрокаСообщения = НСтр("ru = 'Для отображения склада в документе требуется указать заказ покупателя'");
	...
	Для Каждого СтрокаРесурсов Из Объект.РесурсыПредприятия Цикл
		...
		Если ЗначениеЗаполнено(СтрокаРесурсов.ВидПовтора) И Не СтрокаРесурсов.ВидПовтора = ПредопределенноеЗначение(
			...
			РасписаниеПредставление = СтрШаблон("%1, %2 %3 %4%5%6", СтрокаРесурсов.ВидПовтора, Междометие,
				СтрокаРесурсов.ИнтервалПовторения, Окончание, ВыбранныеДниНедели, ДополнениеПоМесяцуГоду(СтрокаРесурсов));
		Иначе
	...
	Если СтрокаРесурсов.ВидПовтора = ПредопределенноеЗначение("Перечисление.ВидыПовторовРасписания.Ежемесячно")
		...
		ИначеЕсли ЗначениеЗаполнено(ДеньНеделиМесяца) Тогда
			...
			Иначе
				...
				Результат = СтрШаблон(НСтр("ru = '%1%2 кажд. %3-й недели'"), " ",
...
Процедура Подбор(Команда)
	...
	КонецЕсли;
	ПараметрыПодбора.Вставить("ИспользуетсяРезервирование", ИспользуетсяРезервирование(Объект.ВидОперации,
		Объект.ЗаказПокупателя, Объект.ПоложениеЗаказаПокупателя, ИмяТабличнойЧасти));
...
Процедура УстановитьВидимостьОтПользовательскихНастроек(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИзменитьРезервЗаполнитьПоОстаткамБезРазбиения", "Видимость",Форма.РезервированиеЗапасов);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаЗаполнитьПоВсемСкладам", "Видимость", Форма.РежимОстаткиИРезервы);
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов()
	...
	Если РежимОстаткиИРезервы Тогда
		...
		Элементы.ЗапасыСтруктурнаяЕдиницаОстатки.Видимость = РазрешитьСкладыВТабличныхЧастях;
	...
	Иначе
		...
		Элементы.ЗапасыСтруктурнаяЕдиницаТабличнаяЧасть.Видимость = РезервированиеЗапасов И НЕ СкладВШапке;
...
Функция СписокДокументовПоЗаказу(ТипОперации, СтруткураПараметров)
	Возврат НоменклатураВДокументахСервер.СписокДокументовПоЗаказу(Объект, ТипОперации,СтруткураПараметров,,,"ЗаказНаПроизводство");
КонецФункции
