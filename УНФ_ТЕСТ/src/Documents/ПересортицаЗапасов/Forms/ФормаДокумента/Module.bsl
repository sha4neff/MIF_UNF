...
Процедура ЗапасыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ТекущаяСтрокаИдентификатор Из Элементы.Запасы.ВыделенныеСтроки Цикл
		...
		РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомера, ТекущиеДанные,
			ИспользоватьСерийныеНомераОстатки);
		РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераОприходование, ТекущиеДанные,
			ИспользоватьСерийныеНомераОстатки, , "СерийныеНомераОприходование");
	КонецЦикла;
...
Процедура ОчиститьИнформациюОСписанииПриходовании(ЭтоОприходование = Ложь)
	...
	Для Каждого ТекущаяСтрокаИдентификатор Из Элементы.Запасы.ВыделенныеСтроки Цикл
		...
		// Серийные номера
		Если ЭтоОприходование <> Неопределено И ЭтоОприходование Тогда
			РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераОприходование, ТекущаяСтрока,
				ИспользоватьСерийныеНомераОстатки, , "СерийныеНомераОприходование");
		Иначе
			РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомера, ТекущаяСтрока,
				ИспользоватьСерийныеНомераОстатки);
		КонецЕсли;
