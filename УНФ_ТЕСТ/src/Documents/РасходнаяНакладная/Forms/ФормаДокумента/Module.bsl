...
Процедура ВидОперацииПриИзмененииСервер()
	...
	ОпределитьВидимостьНастроекУчетаВНалогообложении(Истина);
	ЗаполнитьСписокДокументОснование();
	...
	Если Элемент.ТекущийЭлемент = Элементы.ЗапасыОстатокОбщий
		...
		Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
			...
			Если СписокДокументов.Количество() Тогда
				СписокДокументов.Добавить("ОтчетОстаткиНаСкладах", "Остатки на складах (отчет)", ,
					БиблиотекаКартинок.Отчет);
				СписокВыбораСклад.Добавить("ОтчетОстаткиНаСкладах", "Остатки на складах (отчет)", ,
					БиблиотекаКартинок.Отчет);
			КонецЕсли;
...
Функция СформироватьНадписьЦеныИВалюта(ПоляНадписи)
	...
	Если ЗначениеЗаполнено(ПоляНадписи.ДисконтнаяКарта)
		И ДисконтныеКартыВызовСервера.СтарыйМеханизмСкидок(ПоляНадписи.ДисконтнаяКарта) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
...
Процедура СохранитьДанныеХарактеристикиПартииВКэш()
	...
	КэшХарактеристиИПартии = Новый ТаблицаЗначений;
	
	КэшХарактеристиИПартии.Колонки.Добавить("Номенклатура");
	КэшХарактеристиИПартии.Колонки.Добавить("ИспользоватьХарактеристики");
	КэшХарактеристиИПартии.Колонки.Добавить("ПроверятьЗаполнениеХарактеристики");
	КэшХарактеристиИПартии.Колонки.Добавить("ЗаполнениеХарактеристикиПроверено");
	КэшХарактеристиИПартии.Колонки.Добавить("ИспользоватьПартии");
	КэшХарактеристиИПартии.Колонки.Добавить("ПроверятьЗаполнениеПартий");
	...
	Для Каждого ИмяТабличнойЧасти Из МассивИменТабличныхЧастей Цикл
		...
		Для Каждого СтрокаТЧ Из Объект[ИмяТабличнойЧасти] Цикл
			...
			НоваяСтрока = КэшХарактеристиИПартии.Добавить();
...
Процедура ВосстановитьДанныеХарактеристикиПартииИзКэш()
	...
	Если КэшХарактеристиИПартии = Неопределено Тогда Возврат КонецЕсли;
	...
	Для Каждого ИмяТабличнойЧасти Из МассивИменТабличныхЧастей Цикл
		...
		Для Каждого СтрокаТЧ Из Объект[ИмяТабличнойЧасти] Цикл
			...
			НайденныеСтроки = КэшХарактеристиИПартии.НайтиСтроки(ПараметрыОтбора);
...
Процедура УстановитьВидЭД()
	...
	Элементы.ВидЭД.СписокВыбора.Добавить(Перечисления.ВидыЭД.ТОРГ12Продавец, "Товарная накладная");
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(ГруппаПереработка, "Объект.ВидОперации", Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку);
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(ГруппаПереработка, "Объект.Запасы.ЗаказПокупателя", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(ГруппаПереработка, "Объект.Запасы.Резерв", 0);
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	Иначе
		...
		Если Не СкладВШапке Тогда 
			Для Каждого СтрокаТаблицы Из Объект.Запасы Цикл
				Если СтрокаТаблицы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы <> ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Склад") Тогда
	...
	УправлениеНебольшойФирмойСервер.НастроитьГруппуЦеныИВалюты(ЭтотОбъект);
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				ЗаполнитьГТД = СтрокаТаблицы.Свойство("СтранаПроисхождения")
