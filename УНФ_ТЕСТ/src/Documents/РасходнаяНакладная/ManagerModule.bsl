...
Процедура СформироватьТаблицаЗапасыВРазрезеГТД(СтруктураДополнительныеСвойства)
	...
	|	И ТаблицаЗапасыНаСкладах.ВидОперации В (Значение(Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю), Значение(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	// :::::::::: [1] Запасы собственные (передача на комиссию)
	|;Выбрать
	|	Минимум(ТаблицаЗапасыНаСкладах.НомерСтроки) КАК НомерСтроки
	...
	|
	// :::::::::: [2] Запасы принятые на комиссию (продажа, возврат комитенту)
	|;Выбрать
	|	Минимум(ТаблицаЗапасыНаСкладах.НомерСтроки) КАК НомерСтроки
	|	,Значение(ВидДвиженияНакопления.Расход) КАК ВидДвижения
	|	,ТаблицаЗапасыНаСкладах.Период КАК Период
	...
	|	И ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> Значение(Справочник.СтраныМира.ПустаяССылка)
	|	И ТаблицаЗапасыНаСкладах.НомерГТД <> Значение(Справочник.НомераГТД.ПустаяССылка)
	|	И (ТаблицаЗапасыНаСкладах.Партия <> Значение(Справочник.ПартииНоменклатуры.ПустаяСсылка) И ТаблицаЗапасыНаСкладах.Партия.Статус = Значение(Перечисление.СтатусыПартий.ТоварыНаКомиссии))
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Если Передача Тогда
				...
				Если Не СтрокаТаблицаЗапасы.НеУчитыватьЗаказыПриПередачеНаКомиссию Тогда
...
Процедура СформироватьТаблицаПотребностьВЗапасах(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|			ТОГДА ТаблицаПотребностьВЗапасах.Заказ.ЗаказПокупателя
	|		КОГДА &ИспользоватьРезервирование
	|				И ТаблицаПотребностьВЗапасах.Заказ ССЫЛКА Документ.ЗаказПоставщику
...
Процедура СформироватьТаблицаЗакупкиДляКУДиР(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ТаблицаРезультат = РезультатЗапроса[РезультатЗапроса.Количество()-3].Выгрузить();
	
	ТаблицаОплаты = РезультатЗапроса[РезультатЗапроса.Количество()-2].Выгрузить();
	ВыборкаДокументовСТоварами = РезультатЗапроса[РезультатЗапроса.Количество()-1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура СформироватьТаблицаРезервыТоваровОрганизаций(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МассивОрганизаций", 
		РегистрыСведений.НастройкаПередачиТоваровМеждуОрганизациями.ПолучитьСписокОрганизацийДляОстатков(СтруктураДополнительныеСвойства.ДляПроведения.Организация));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МИНИМУМ(ТаблицаЗапасыНаСкладах.НомерСтроки) КАК НомерСтроки,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	...
	|	ТаблицаЗапасыНаСкладах.Период,
	|	ТаблицаЗапасыНаСкладах.Организация,
	|	ТаблицаЗапасыНаСкладах.ХозяйственнаяОперация,
	|	ТаблицаЗапасыНаСкладах.Номенклатура,
	|	ТаблицаЗапасыНаСкладах.Характеристика,
	|	ТаблицаЗапасыНаСкладах.Партия,
	|	ТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница,
	...
	|ПОМЕСТИТЬ ОстаткиИРезервыЗапасов
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|				&МоментКонтроля,
	|				(СтруктурнаяЕдиница, Номенклатура, Характеристика, Партия) В
	|					(ВЫБРАТЬ
	|						ВТЗапасыНаСкладах.СтруктурнаяЕдиница,
	|						ВТЗапасыНаСкладах.Номенклатура,
	|						ВТЗапасыНаСкладах.Характеристика,
	|						ВТЗапасыНаСкладах.Партия
	|					ИЗ
	|						ВТЗапасыНаСкладах)) КАК ЗапасыНаСкладахОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И (ЗапасыНаСкладах.СтруктурнаяЕдиница, ЗапасыНаСкладах.Номенклатура, ЗапасыНаСкладах.Характеристика, ЗапасыНаСкладах.Партия) В
	|				(ВЫБРАТЬ
	|					ВТЗапасыНаСкладах.СтруктурнаяЕдиница,
	|					ВТЗапасыНаСкладах.Номенклатура,
	|					ВТЗапасыНаСкладах.Характеристика,
	...
	|						ВТЗапасыНаСкладах.Партия
	|					ИЗ
	|						ВТЗапасыНаСкладах)) КАК РезервыТоваровОрганизацийОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|					ВТЗапасыНаСкладах.Партия
	|				ИЗ
	|					ВТЗапасыНаСкладах)) КАК ОстаткиИРезервы
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТЗапасыНаСкладах.НомерСтроки КАК НомерСтроки,
	|	ВТЗапасыНаСкладах.Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ВТЗапасыНаСкладах.Организация КАК Организация,
	|	ВТЗапасыНаСкладах.Организация КАК КорОрганизация,
	|	ВТЗапасыНаСкладах.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТЗапасыНаСкладах.Номенклатура КАК Номенклатура,
	|	ВТЗапасыНаСкладах.Характеристика КАК Характеристика,
	|	ВТЗапасыНаСкладах.Партия КАК Партия,
	|	ВТЗапасыНаСкладах.Ячейка КАК Ячейка,
	|	ВТЗапасыНаСкладах.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ОстаткиИРезервыЗапасов.КоличествоОстаток, 0) <= 0
	|			ТОГДА ВТЗапасыНаСкладах.Количество
	|		КОГДА ВТЗапасыНаСкладах.Количество > ЕСТЬNULL(ОстаткиИРезервыЗапасов.КоличествоОстаток, 0)
	|			ТОГДА ВТЗапасыНаСкладах.Количество - ЕСТЬNULL(ОстаткиИРезервыЗапасов.КоличествоОстаток, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Количество
	|ПОМЕСТИТЬ ВТТоварыСОтрицательнымиОстатками
	|ИЗ
	|	ВТЗапасыНаСкладах КАК ВТЗапасыНаСкладах
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиИРезервыЗапасов КАК ОстаткиИРезервыЗапасов
	|		ПО ВТЗапасыНаСкладах.Организация = ОстаткиИРезервыЗапасов.Организация
	|			И ВТЗапасыНаСкладах.СтруктурнаяЕдиница = ОстаткиИРезервыЗапасов.СтруктурнаяЕдиница
	|			И ВТЗапасыНаСкладах.Номенклатура = ОстаткиИРезервыЗапасов.Номенклатура
	|			И ВТЗапасыНаСкладах.Характеристика = ОстаткиИРезервыЗапасов.Характеристика
	|			И ВТЗапасыНаСкладах.Партия = ОстаткиИРезервыЗапасов.Партия
	|			И (ВТЗапасыНаСкладах.Ячейка = ОстаткиИРезервыЗапасов.Ячейка
	|				ИЛИ ВТЗапасыНаСкладах.Ячейка = НЕОПРЕДЕЛЕНО)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ЕСТЬNULL(ОстаткиИРезервыЗапасов.КоличествоОстаток, 0) <= 0
	|				ТОГДА ВТЗапасыНаСкладах.Количество
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиИРезервыЗапасов.Организация КАК Организация,
	|	ВТЗапасыНаСкладах.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТЗапасыНаСкладах.Период КАК Период,
	|	ВТЗапасыНаСкладах.Номенклатура КАК Номенклатура,
	|	ВТЗапасыНаСкладах.Характеристика КАК Характеристика,
	|	ВТЗапасыНаСкладах.Партия КАК Партия,
	|	ВТЗапасыНаСкладах.Ячейка КАК Ячейка,
	|	ЕСТЬNULL(ОстаткиИРезервыЗапасов.КоличествоОстаток, 0) КАК КоличествоОстаток
	|ИЗ
	|	ВТТоварыСОтрицательнымиОстатками КАК ВТЗапасыНаСкладах
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОстаткиИРезервыЗапасов КАК ОстаткиИРезервыЗапасов
	|		ПО (ОстаткиИРезервыЗапасов.Организация <> &Организация)
	|			И ВТЗапасыНаСкладах.СтруктурнаяЕдиница = ОстаткиИРезервыЗапасов.СтруктурнаяЕдиница
	|			И ВТЗапасыНаСкладах.Номенклатура = ОстаткиИРезервыЗапасов.Номенклатура
	|			И ВТЗапасыНаСкладах.Характеристика = ОстаткиИРезервыЗапасов.Характеристика
	|			И ВТЗапасыНаСкладах.Партия = ОстаткиИРезервыЗапасов.Партия
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТЗапасыНаСкладах.НомерСтроки,
	|	Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТТоварыСОтрицательнымиОстатками.НомерСтроки КАК НомерСтроки,
	|	ВТТоварыСОтрицательнымиОстатками.Период КАК Период,
	|	ВТТоварыСОтрицательнымиОстатками.ВидДвижения КАК ВидДвижения,
	|	ВТТоварыСОтрицательнымиОстатками.Организация КАК Организация,
	|	ВТТоварыСОтрицательнымиОстатками.КорОрганизация КАК КорОрганизация,
	|	ВТТоварыСОтрицательнымиОстатками.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТТоварыСОтрицательнымиОстатками.Номенклатура КАК Номенклатура,
	|	ВТТоварыСОтрицательнымиОстатками.Характеристика КАК Характеристика,
	|	ВТТоварыСОтрицательнымиОстатками.Партия КАК Партия,
	|	ВТТоварыСОтрицательнымиОстатками.Ячейка КАК Ячейка,
	|	ВТТоварыСОтрицательнымиОстатками.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВТТоварыСОтрицательнымиОстатками.Количество КАК Количество
	|ИЗ
	|	ВТТоварыСОтрицательнымиОстатками КАК ВТТоварыСОтрицательнымиОстатками
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТТоварыСОтрицательнымиОстатками.НомерСтроки";
	...
	ТаблицаДокументовПередачи = ТаблицаРезервыТоваровОрганизаций.Скопировать(,"Организация,КорОрганизация");
	...
	Для Каждого СтрокаТовара Из ТаблицыТоваров Цикл
		...
		СтруктураОтбора.Вставить("Партия", СтрокаТовара.Партия);
		СтруктураОтбора.Вставить("СтруктурнаяЕдиница", СтрокаТовара.СтруктурнаяЕдиница);
		СтруктураОтбора.Вставить("Ячейка", СтрокаТовара.Ячейка);
		СтрокиСТоварами = ТаблицаОстатков.НайтиСтроки(СтруктураОтбора);
		КоличествоТовараНеХватило = СтрокаТовара.Количество;
		Для Каждого НайденнаяСтрока Из СтрокиСТоварами Цикл
			...
			СтрокаПередачи = ТаблицаДокументовПередачи.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПередачи, ПерваяСтрока);
			КПередаче = Мин(КоличествоТовараНеХватило, НайденнаяСтрока.КоличествоОстаток);
			ПерваяСтрока.Количество = КПередаче;
			ВтораяСтрока.Количество = ПерваяСтрока.Количество;
			НайденнаяСтрока.КоличествоОстаток = НайденнаяСтрока.КоличествоОстаток - КПередаче;
	...
	// Создаем документы передачи, если их нет
	ТаблицаДокументовПередачи.Свернуть("Организация,КорОрганизация");
	Для Каждого СтрокаТаблицы Из ТаблицаДокументовПередачи Цикл
		Документы.ПередачаТоваровМеждуОрганизациями.ОпределитьДокументПередачиТоваровМеждуОрганизациями(СтрокаТаблицы.КорОрганизация, СтрокаТаблицы.Организация, СтруктураДополнительныеСвойства.ДляПроведения.Дата);
	КонецЦикла;
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ДисконтныеКартыСервер.ТекстЗапросаПродажиПоДисконтнойКарте("РасходнаяНакладная");
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Содержание", "Содержание",
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Резерв", "Резерв",
	...
	ВидимостьЗаказаПокупателя = (НастройкиЗагрузкиДанных.ПоложениеЗаказаВДокументе
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаСерийныеНомераКРасходу(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);
	// Серийные номера
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.ПоложениеСклада КАК ПоложениеСклада,
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаШапка,
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК СтруктурнаяЕдиницаШапкаОрдерныйСклад,
	...
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.Организация КАК ОрганизацияВНУ,
	|	&Организация КАК Организация,
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВЫБОР
	...
	|	ВТРасходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗатрат КАК СчетУчетаЗатрат,
	|	ВЫБОР
	...
	|					ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.СуммаНДС * ВТРасходнаяНакладнаяЗапасы.Ссылка.Курс / ВТРасходнаяНакладнаяЗапасы.Ссылка.Кратность
	|				КОНЕЦ
	...
	|			ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.Всего * ВТРасходнаяНакладнаяЗапасы.Ссылка.Курс / ВТРасходнаяНакладнаяЗапасы.Ссылка.Кратность
	...
	|						КОГДА ВТРасходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерийныеНомера
	|							ТОГДА ВТРасходнаяНакладнаяЗапасы.Количество
	|						ИНАЧЕ 1
	|					КОНЕЦ
	...
	|	ВТРасходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера,
	...
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.Курс КАК Курс,
	|	ВТРасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК Кратность,
	|	ВЫБОР
	|		КОГДА ВТРасходнаяНакладнаяЗапасы.Ссылка.ПоложениеПроекта <> ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|			ТОГДА ВТРасходнаяНакладнаяЗапасы.Ссылка.Проект
	...
	|		ПО (ВременнаяТаблицаЗапасы.КлючСвязи = РасходнаяНакладнаяСерийныеНомера.КлючСвязи)
	|ГДЕ
...
Процедура ВыполнитьКонтроль(ДокументСсылкаРасходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		|			И (ЕСТЬNULL(РезервыТоваровОрганизацийОстатки.КоличествоОстаток, 0) < 0)
		|ГДЕ
		...
		Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить() Тогда
			...
			Если НЕ МассивРезультатов[10].Пустой() Тогда
				...
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятыеВРазрезеГТД(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса, Отказ);
		...
		Если Не МассивРезультатов[11].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[10].Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПодарочныеСертификаты(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура АдаптироватьДанныеУниверсальногоЗапросаДляМХ1(ДанныеОбъектовПечати)
	...
	// 2. Контрагент (и его адрес) выступает в роли поклажехранителя
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Если КонтактнаяИнформацияТоварногоЧека <> Перечисления.КонтактнаяИнформацияТоварногоЧека.Организация Тогда
			...
			Если ОбластьМакета <> Неопределено Тогда
				...
				ФактАдресСтруктурнойЕдиницы = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Шапка.СтруктурнаяЕдиницаСсылка, Справочники.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы);
				...
				ТелефонСтруктурнойЕдиницы = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Шапка.СтруктурнаяЕдиницаСсылка, Справочники.ВидыКонтактнойИнформации.ТелефонСтруктурнойЕдиницы);
		...
		СтруктураЗаполненияСекции.Вставить("Сумма", УправлениеНебольшойФирмойСервер.ФорматСумм(Итоги.Сумма));
		СтруктураЗаполненияСекции.Вставить("Всего", УправлениеНебольшойФирмойСервер.ФорматСумм(Итоги.Всего));
		...
		Иначе
			...
			СтруктураЗаполненияСекции.Вставить("СуммаНДС", УправлениеНебольшойФирмойСервер.ФорматСумм(Итоги.СуммаНДС));
...
Процедура СформироватьМ15(ТабличныйДокумент, ТекущийДокумент)
	...
	СведенияОГрузополучателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент,
	...
	ОбластьМакетаШапка.Параметры.ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
		СведенияОбОрганизации);
	ОбластьМакетаШапка.Параметры.КонтрагентНаименование = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(
...
Процедура СформироватьБланкТоварногоНаполнения(ТабличныйДокумент, ТекущийДокумент)
	...
	Пока ВыборкаСтрокЗапасы.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСерийныеНомера = РаботаССерийнымиНомерами.СтрокаСерийныеНомераИзВыборки(ВыборкаСтрокСерийныеНомера,
