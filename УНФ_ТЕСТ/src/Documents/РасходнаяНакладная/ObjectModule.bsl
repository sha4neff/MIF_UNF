	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПоложениеПроекта = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
				НоваяСтрока.Проект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Заказ, "Проект");
			КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПоставщику(ЗаказПоставщику) Экспорт
	...
	Если УправлениеНебольшойФирмойПовтИсп.РеквизитВШапке("ПоложениеПроекта") И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЗаказПоставщику, "Проект") Тогда
		Заказ = ЗаказПоставщику.Проект;
	КонецЕсли;
...
Процедура ЗаполнитьПоПеремещениюЗапасов(ДанныеЗаполнения) Экспорт
	...
	Иначе
		...
		Заказ = ДокументСсылкаПеремещениеЗапасов.ЗаказПокупателя;
