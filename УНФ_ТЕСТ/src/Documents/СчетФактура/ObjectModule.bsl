...
Функция НеобходимоОтразитьСуммыДокументаВНациональнойВалюте()
...
Процедура ЗаполнитьСведенияШапкиПоПередачеТоваровМеждуОрганизациями(ДанныеЗаполнения, СведенияДокументовОснований)
	...
	СведенияДокументовОснований.Договор = Справочники.ДоговорыКонтрагентов.ПолучитьДоговорПоУмолчаниюПоОрганизацииВидуДоговора(ДанныеЗаполнения.КонтрагентПолучатель, ДанныеЗаполнения.Организация);
	ЗаполнитьЗначениеСПроверкой(СведенияДокументовОснований, ДанныеЗаполнения, "ВалютаДокумента", Истина);
	ЗаполнитьЗначениеСПроверкой(СведенияДокументовОснований, ДанныеЗаполнения, "Кратность");
	ЗаполнитьЗначениеСПроверкой(СведенияДокументовОснований, ДанныеЗаполнения, "Курс");
	ЗаполнитьЗначениеСПроверкой(СведенияДокументовОснований, ДанныеЗаполнения, "ПодписьРуководителя");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДокументыОснования.Количество() < 1 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Необходимо заполнить документы, которые являются основаниями счета-фактуры.'");
	...
	Если НеобходимоОтразитьСуммыДокументаВНациональнойВалюте() Тогда
	...
	Для каждого СтрокаОснование Из ДокументыОснования Цикл
		...
		Если ЗначениеЗаполнено(ДокументОснование) 
			...
			Если ЗначениеЗаполнено(НайденныйСчетФактура) 
				...
				ТекстСообщения = НСтр("ru = 'Для документа ""%1"" 
										|уже введен счет-фактура ""%2"". 
										|Запись еще одного документа ""счет-фактура"" не допускается!'");
				
				ТекстСообщения = СтрШаблон(ТекстСообщения, ДокументОснование, НайденныйСчетФактура.Ссылка);
				
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.ДокументОснование", , Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НеобходимоОтразитьСчетФактуройЗапасыВРазрезеГТД()
		ИЛИ НеобходимоОтразитьСуммыДокументаВНациональнойВалюте() Тогда
