...
Функция ЗаполнитьПоказатели(СтруктураДанных)
	...
	УправлениеНебольшойФирмойСервер.ДобавитьПараметрыВСтруктуру(СтруктураДанных.ВидНачисленияУдержания.Формула, СтруктураПараметров);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ДругоеПодрУвольнение Тогда
				...
				Для каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
					ЭлементМассива.ДатаОкончания = Выборка.ДатаДействияНачало - ?(Выборка.ДругоеПодрУвольнение = 1, 60*60*24, 0);
				КонецЦикла;
				Продолжить;
			КонецЕсли; 
...
Процедура РассчитатьПоФормулам()
	...
	Для каждого СтрокаНачислений Из Объект.НачисленияУдержания Цикл
		...
		КонецЦикла;
		Попытка
			// РассчитаннаяСумма = Вычислить(Формула); // Стандарт "Ограничения на использование Выполнить и Вычислить на сервере".
