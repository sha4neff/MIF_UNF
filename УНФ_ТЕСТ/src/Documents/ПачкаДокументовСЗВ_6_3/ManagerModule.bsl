...
// Сформировать пачки СЗВ6-3
// 
// Параметры:
// 	Организация - СправочникСсылка.Организации - .
// 	ОтчетныйПериод - Дата - .
// 	КорректирующиеСведения - Булево - .
// 	УчитыватьИсправления - Булево - .
// 	МассивПачекСИсправлениями - Массив - .
// 	МассивПачекСЗВ6_3 - Массив - .
// Возвращаемое значение:
// 	Массив - .
Функция СформироватьПачкиСЗВ_6_3(Организация, ОтчетныйПериод, КорректирующиеСведения = Ложь, УчитыватьИсправления = Ложь,
	МассивПачекСИсправлениями = Неопределено, МассивПачекСЗВ6_3 = Неопределено) Экспорт
	...
	УчетСтраховыхВзносов.СформироватьТаблицуВТКадровыеДанныеФизическихЛиц(Запрос.МенеджерВременныхТаблиц,
		СписокФизическихЛиц, Ложь, КонецГода(ДатаНачалаОтчетногоПериода));
		...
		Пока Выборка.СледующийПоЗначениюПоля("КатегорияЗастрахованныхЛиц") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("ТипДоговора") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
					...
					ЗаполнитьРеквизитыДокумента(ПачкаОбъект, Организация, ОтчетныйПериод,
						Выборка.КатегорияЗастрахованныхЛиц, Выборка.ТипДоговора, КорректирующиеСведения);
					УчетСтраховыхВзносов.ПроставитьНомерПачки(ПачкаОбъект);
					НомерПачки = НомерПачки + 1;
					//
					ЗаполнитьЗначенияСвойств(ПачкаОбъект, СправкиПоНДФЛ.ДанныеПодписывающихЛиц(Организация,
						ДатаНачалаОтчетногоПериода, , Истина));
...
Процедура СформироватьВТСведенияОДоходах(Организация, ОтчетныйПериод, МенеджерВременныхТаблиц,
	КатегорияЗастрахованныхЛиц = Неопределено, ТипДоговора = Неопределено, МассивФизлиц = Неопределено)
...
Процедура ЗаполнитьРеквизитыДокумента(ДокументОбъект, Организация, ОтчетныйПериод, КатегорияЗастрахованныхЛиц,
	ТипДоговора, КорректирующиеСведения)
	ДокументОбъект.Организация = Организация;
	ДокументОбъект.ОтчетныйПериод = ОтчетныйПериод;
	ДокументОбъект.КатегорияЗастрахованныхЛиц = КатегорияЗастрахованныхЛиц;
	ДокументОбъект.ТипСведенийСЗВ = ?(КорректирующиеСведения, Перечисления.ТипыСведенийСЗВ.КОРРЕКТИРУЮЩАЯ,
		Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ);
