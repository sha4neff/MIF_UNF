...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	УправлениеНебольшойФирмойСервер.ПолучитьЦеныТабличнойЧастиПоВидуЦен(СтруктураДанных, Запасы);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСерийныеНомера") Тогда
		СерийныеНомера.Загрузить(ДокументСсылкаРасходнаяНакладная.СерийныеНомера.Выгрузить());
...
Процедура ЗаполнитьПоОтчетуКомиссионера(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах Тогда
		ТекстИсключения = НСтр("ru = 'Документ вводится только на основании отчета комиссионера с видом операции - отчет комиссионера о продажах.'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Строка Из Предоплата Цикл
		...
		Если НайденнаяСтрокаПокупатели = Неопределено Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Отчет комиссионера в строке %1 таблицы ""Предоплата"" не найден в табличной части ""Покупатели""!'"), Строка.НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				,
				ТекстСообщения,
				"Объект.Предоплата",
	...
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах Тогда
		...
		Для Каждого Строка Из Покупатели Цикл
			...
			Если ЗначениеЗаполнено(Строка.ОтчетКомиссионера) Тогда Продолжить КонецЕсли;
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не выбран Отчет комиссионера в строке %1 таблицы ""Покупатели""'"), Строка.НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			,
			ТекстСообщения,
			"Объект.Покупатели",
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если Не ДанныеЗаполнения = Неопределено 
		И Не ДанныеЗаполнения.Метаданные().Реквизиты.Найти("Договор") = Неопределено Тогда
