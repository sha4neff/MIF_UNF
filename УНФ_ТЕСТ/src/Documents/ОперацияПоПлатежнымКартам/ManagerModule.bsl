...
Функция ПолучитьДанныеДляФискализации(Ссылка) Экспорт
	...
	АдресМагазина = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(
		Объект.Организация,
...
Процедура СформироватьРасчетыПоЭквайрингу(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	|	КОНЕЦ КАК СуммаВал,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаРасшифровкаПлатежа.ЭквайринговыйТерминал.Договор.РасчетКомиссиВОтчетеЭквайера
	|			ТОГДА 0
	...
	|				И ВременнаяТаблицаРасшифровкаПлатежа.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
	|			ТОГДА -ВременнаяТаблицаРасшифровкаПлатежа.СуммаУчетаКомиссииПоДоговору
	...
	|		КОГДА ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю)
	|				И ВременнаяТаблицаРасшифровкаПлатежа.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
	|			ТОГДА -ВременнаяТаблицаРасшифровкаПлатежа.СуммаКомиссииПоДоговору
...
Процедура СформироватьТаблицаОплатаПлатежнымиКартами(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРасшифровкаПлатежа.ЭквайринговыйТерминал.Договор.РасчетКомиссиВОтчетеЭквайера
	|			ТОГДА 0
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаРасшифровкаПлатежа.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
...
Функция ПолучитьТекстЗапросаИнициализироватьДанные()
	...
	|	ТаблицаДокумента.Ссылка.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату КАК КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату,
	...
	|	ТаблицаДокумента.Ссылка.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Договор.ЭтоДоговорОбслуживания
	|				И ТаблицаДокумента.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|			ТОГДА ИСТИНА
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОперацияПоПлатежнымКартам, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваВКассахККМИзменение Тогда
		...
		|	ОперацияПоПлатежнымКартам.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату КАК КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
		|ПОМЕСТИТЬ Шапка
		|ИЗ
		...
		|						И Шапка.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
