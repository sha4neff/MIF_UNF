...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РасчетКомиссиВОтчетеЭквайера = Объект.ЭквайринговыйТерминал.Договор.РасчетКомиссиВОтчетеЭквайера;
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостьРеквизитовРасшифровкиПлатежа();
...
Функция ПолучитьСписокДляВыбораДокументаОснования(ВидОперации)
...
Процедура ДокументОснованиеНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ТолькоПросмотр И НавигационнаяСсылкаФорматированнойСтроки = "выбрать" Тогда
		...
		ПоказатьВыборИзМеню(ОписаниеОповещения,
			ПолучитьСписокДляВыбораДокументаОснования(Объект.ВидОперации),
			Элементы.ДокументОснованиеНадпись
		);
...
Процедура ВыборТипаОснованияЗавершение(ВыбИмяФормы, Параметры) Экспорт
	...
	Если ВыбИмяФормы<>Неопределено Тогда
		...
		ОткрытьФорму(ВыбИмяФормы.Значение, ПараметрыОткрытияФормы, ЭтотОбъект, ,,,ОповещениеОЗакрытии);
	КонецЕсли;	
...
Процедура ЭквайринговыйТерминалПриИзмененииНаСервере()
	...
	Объект.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату = Объект.ЭквайринговыйТерминал.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату;
	...
	Если РасчетКомиссиВОтчетеЭквайера Тогда
...
&НаКлиенте
Процедура ВыбратьВидОплатыЗавершение(ВыбЗначение, Параметры) Экспорт

	Если ВыбЗначение<>Неопределено Тогда
		Объект.ЭквайринговыйТерминал = ВыбЗначение;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьВидимостьРеквизитовРасшифровкиПлатежа()
	
	Если Объект.СпособОплаты = ПредопределенноеЗначение("Перечисление.СпособыОплатыЭквайринг.ОплатаОнлайн") Тогда
		Элементы.ГруппаОплатаССайта.Видимость = Истина;
		Элементы.ГруппаОплатаКартой.Видимость = Ложь;
	Иначе
		Элементы.ГруппаОплатаССайта.Видимость = Ложь;
...
Процедура РасшифровкаПлатежаДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		СтруктураПараметры = Новый Структура("Отбор, ЭтоРасчетыСПокупателями, ТипДокумента, СтрокаИслючений",
			СтруктураОтбор,
...
Процедура НапечататьЧек(Команда)
	...
	Иначе
		...
		Если НЕ ИспользоватьПодключаемоеОборудование 
			...
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Печать не выполнена. В карточке кассы ККМ установлен флаг ""Без подключения"" или выключена опция ""Подключаемое оборудование"".'"));
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ИспользоватьПодключаемоеОборудование 
			...
			Если НЕ ЗначениеЗаполнено(Объект.НомерЧекаККМ) Тогда
				Объект.НомерЧекаККМ = 1;
			КонецЕсли;
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Просмотр невозможен. В карточке кассы ККМ установлен флаг ""Без подключения"" или выключена опция ""Подключаемое оборудование"".'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя Тогда
		...
		Элементы.ГруппаКомиссия.Заголовок = "Комиссия";
		Элементы.СуммаКомиссииПоДоговору.Заголовок = "Комиссия";
		Элементы.ЭквайрерБеретКомиссиюЗаВозвратДС.Видимость = Ложь;
		Элементы.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату.Видимость = Ложь;
		
	Иначе
		
		Если Объект.ЭквайрерБеретКомиссиюЗаВозвратДС Тогда
			Если Объект.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату Тогда
				Элементы.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату.Видимость = Истина;
				Элементы.ЭквайрерБеретКомиссиюЗаВозвратДС.Видимость = Ложь;
				Элементы.СуммаКомиссииПоДоговору.Заголовок = "К возврату";
				Элементы.ГруппаКомиссия.Заголовок = "Возврат комисси";
			Иначе
			    Элементы.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату.Видимость = Ложь;
				Элементы.ЭквайрерБеретКомиссиюЗаВозвратДС.Видимость = Истина;
				Элементы.СуммаКомиссииПоДоговору.Заголовок = "Комиссия";
