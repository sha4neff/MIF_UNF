...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ТаблицаЗапасыПринятые.Количество) <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
КонецПроцедуры // СформироватьТаблицаРезервыТоваровОрганизаций()
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства) Экспорт
	...
	|						КОГДА ОтчетОРозничныхПродажахЗапасы.Номенклатура.ИспользоватьСерийныеНомера
	|							ТОГДА ОтчетОРозничныхПродажахЗапасы.Количество
	|						ИНАЧЕ 1
	|					КОНЕЦ
	...
	|	ОтчетОРозничныхПродажахЗапасы.Номенклатура.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера,
	...
	|	РезервированиеЗапасовЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	...
	|	РезервированиеЗапасовЗапасы.СтруктурнаяЕдиница,
	...
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(РезервированиеЗапасовЗапасы.СтруктурнаяЕдиница) = ТИП(Справочник.СтруктурныеЕдиницы)
	|					ТОГДА ВЫБОР
	|							КОГДА РезервированиеЗапасовЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|								ТОГДА РезервированиеЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
	...
	Запрос.УстановитьПараметр("СерийныеНомера", СтруктураДополнительныеСвойства.ДанныеСерийныхНомеровДляПроведения.СерийныеНомера);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОтчетОРозничныхПродажах, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Тогда
		...
		Если Не МассивРезультатов[5].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПодарочныеСертификаты(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ, МассивСообщений);
		...
		Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить()
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыВРазрезеГТД(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ДисконтныеКартыСервер.ТекстЗапросаПродажиПоДисконтнойКарте("ОтчетОРозничныхПродажах");
	Запрос.УстановитьПараметр("ПроцентСкидки", 0);
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	Для Каждого СтрокаРезультата Из РезультатЗапроса Цикл
		СтрокаРезультата.Скидка = УправлениеНебольшойФирмойСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(СтруктураДополнительныеСвойства.ДляПроведения.Дата,
																										  СтрокаРезультата.ДисконтнаяКарта);
	КонецЦикла;
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПродажиПоДисконтнойКарте", РезультатЗапроса);
...
Процедура ЗаполнитьОтчетОРозничныхПродажахПоДаннымВЧеках(ОтчетОРозничныхПродажахОбъект) Экспорт
	...
	|	ЧекККМПродажиВозвраты.СерийныйНомер КАК СерийныйНомер,
	...
	Для каждого СтрокаТЧ Из ОплатаПлатежнымиКартами Цикл
		...
		Если СтрокаТЧ.Сумма <> 0
			ИЛИ СтрокаТЧ.СуммаБонусов <> 0 Тогда
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСерийныеНомера") Тогда
		...
		СерийныеНомера = Результат[Результат.Количество()-4].Выгрузить();
		Для каждого СтрокаТЧ Из ОтчетОРозничныхПродажахОбъект.Запасы Цикл
			...
			КоличествоСерийныхНужноЗаполнить = СтрокаТЧ.Количество;
			ЭтоСтрокаВозврата = (КоличествоСерийныхНужноЗаполнить < 0);
			ВыбранныеСерийныеНомера = Новый Массив;
			...
			Если СерийныеНомераПоОтбору.Количество()>0 Тогда
				...
				Для каждого стр Из СерийныеНомераПоОтбору Цикл
					// Если все серийные номера добавили, то выходим из цикла.
					Если КоличествоСерийныхНужноЗаполнить=0 Тогда
						Прервать;
					КонецЕсли;
					// Для продажи обрабатываем серийные номера продажи.
					// Для возврата обрабатываем серийные номера возврата.
					ЭтоСерийныйНомерВозврата = (стр.ПризнакПродаж < 0);
					Если ЭтоСтрокаВозврата
						И Не ЭтоСерийныйНомерВозврата Тогда
						Продолжить;
					КонецЕсли;
					Если Не ЭтоСтрокаВозврата
						И ЭтоСерийныйНомерВозврата Тогда
						Продолжить;
					КонецЕсли;
					
					// Добавим информацию о серийном номере в табличную часть серийных номеров.
					// Одной единице номенклатуры соответствует один номер.
					...
					Если ЭтоСтрокаВозврата Тогда
						КоличествоСерийныхНужноЗаполнить = КоличествоСерийныхНужноЗаполнить + 1;
					Иначе
						КоличествоСерийныхНужноЗаполнить = КоличествоСерийныхНужноЗаполнить - 1;
					КонецЕсли;
					ВыбранныеСерийныеНомера.Добавить(стр);
				КонецЦикла;
				
				// Удалить из таблицы серийные номера, которые уже были заполнены в строке документа
				Для каждого строкаСН Из ВыбранныеСерийныеНомера Цикл
					СерийныеНомера.Удалить(строкаСН);
				КонецЦикла; 
...
Процедура ДобавитьЗапасыВРазрезеГТД(ДокументОбъектОтчетОРозничныхПродажах)
	...
	// Промежужточная обработка - распределяем проданные ГТД, которые вычислили при первом вызове, на возвраты (в другой процедуре).
...
Функция ПодобратьГТДДляСтрокВозвратаТекущейСмены(ДокументОбъектОтчетОРозничныхПродажах)
	...
	// Для текущей смены мы в качестве отстатков будем брать ГТД из текущих строк.
	...
	Для каждого СтрокаТЧ Из ТаблицаФормы Цикл
		...
		// Вычислим коэффициент идиницы измерения.
	...
	// Серийные номера должны распределяться после вызова этой процедуры.
...
Функция СформироватьПечатнуюФормуОтчетаОРозничныхПродажах(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Иначе
			...
			СтруктураДанных.Вставить("СуммаНДС", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДС));
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	|	КОНЕЦ КАК СуммаВал,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаОплатаПлатежнымиКартами.ЭквайринговыйТерминал.Договор.РасчетКомиссиВОтчетеЭквайера
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВременнаяТаблицаОплатаПлатежнымиКартами.Сумма > 0
	|						ИЛИ ВременнаяТаблицаОплатаПлатежнымиКартами.ЭквайринговыйТерминал.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
...
Процедура СформироватьТаблицаСерийныеНомера(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Запасы", СтруктураДополнительныеСвойства.ДанныеСерийныхНомеровДляПроведения.Запасы);
	Запрос.УстановитьПараметр("СерийныеНомера", СтруктураДополнительныеСвойства.ДанныеСерийныхНомеровДляПроведения.СерийныеНомера);
	...
	|ИЗ
	|	&СерийныеНомера КАК ОтчетОРозничныхПродажахСерийныеНомера
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	1 КАК Количество
	|ИЗ
	|	ВТ_Запасы КАК ВременнаяТаблицаЗапасы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СерийныеНомера КАК СерийныеНомера
