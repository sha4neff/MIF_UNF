	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НЕ ЗначениеЗаполнено(НоваяСтрокаПродукция.Спецификация) Тогда
				НоваяСтрокаПродукция.Спецификация = УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(
...
Процедура ЗаполнитьПоПриходныйОрдер(ДокументСсылкаПриходныйОрдер) Экспорт
	...
	СерийныеНомера.Загрузить(ДокументСсылкаПриходныйОрдер.СерийныеНомера.Выгрузить());
...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	ИменаРеквизитов.Добавить("НДСВключатьВСтоимость, Курс, Кратность, ПоложениеЗаказаПокупателя, Заказ");
