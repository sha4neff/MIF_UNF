...
Процедура ОбновитьСписокПодчиненныеЗаказНаряды()
	...
	Если Не СписокЗаказНарядров.Количество() Тогда Возврат КонецЕсли;
	
	ЗначениеЗаказНаряда = СписокЗаказНарядров[0].Значение;
	ПометкаУдаления = СписокЗаказНарядров[0].Пометка;
	...
	Если СписокЗаказНарядров.Количество() > 1 Тогда
...
Процедура ЗаполнитьПодчиненныеЗаказНаряды()
	...
	Пока Результат.Следующий() Цикл
		СписокЗаказНарядров.Добавить(Результат.Ссылка,,Результат.ПометкаУдаления);
	КонецЦикла;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПроведениеЗаказНаряда" Тогда
		Если ЗначениеЗаполнено(Параметр.СсылкаНаСобытие) И Параметр.СсылкаНаСобытие = Объект.Ссылка Тогда
			НайденноеЗначение = СписокЗаказНарядров.НайтиПоЗначению(Параметр.СсылкаНаЗаказНаряд);
			Если НайденноеЗначение = Неопределено Тогда
				СписокЗаказНарядров.Добавить(Параметр.СсылкаНаЗаказНаряд)
			КонецЕсли;
			ОбновитьСписокПодчиненныеЗаказНаряды();
		КонецЕсли;
	КонецЕсли;
...
Процедура СуществующиеЗаказНарядыНажатие(Элемент)
	...
	Если СписокЗаказНарядров.Количество() = 1 Тогда 
		ОткрытьФорму("Документ.ЗаказПокупателя.Форма.ФормаЗаказНаряда", Новый Структура("Ключ", СписокЗаказНарядров[0].Значение),ЭтаФорма);
		Возврат;
	КонецЕсли;
		
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораЭлемента", ЭтаФорма);
	
	СписокЗаказНарядров.ПоказатьВыборЭлемента(Оповещение, "Подчиненные заказ-наряды");
...
Процедура КонтрагентОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Тип") Тогда
		ОпределитьТипПоляКонтргент(ВыбранноеЗначение);
	КонецЕсли;
...
Процедура КонтрагентОчистка(Элемент, СтандартнаяОбработка)
	...
	ОпределитьТипПоляКонтргент(Неопределено);
...
Процедура СоздатьКонтрагент(Команда)
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Контрагент уже выбран (можно создать только при несопоставленном контакте).'"),, "Контрагент");
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Лиды") И ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Лид уже выбран (можно создать только при несопоставленном контакте).'"),, "Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ПривязатьКСуществующемуКонтрагентуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПривявязатьСобытиеКонтактаККонтрагенту(Результат, ДополнительныеПараметры.Контакт);
...
Процедура НавигационнаяСсылкаЗаказНарядНажатие(Элемент)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеСозданияЗказа", ЭтаФорма);
...
&НаКлиенте
Процедура ПослеСозданияЗказа(ЭлементСписка, Параметры) Экспорт
...
&НаСервере
Процедура ОпределитьТипПоляКонтргент(Тип)
...
Процедура СопоставитьКонтактыСобытияСКонтактнымиЛицами(Знач КонтактныеЛица, КакСвязаться)
	...
	КонтактнаяИнформацияКонтактныхЛиц = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(КонтактныеЛица, Перечисления.ТипыКонтактнойИнформации.Телефон);
...
&НаСервере
Процедура ПривявязатьСобытиеКонтактаККонтрагенту(НовыйКонтрагент, Контакт)
