...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.СписокГруппаСоздать.Видимость Тогда
		Элементы.СписокГруппаСоздать.Видимость = УправлениеНебольшойФирмойУправлениеДоступомПовтИсп.ЕстьПравоДоступа(
			"Добавление", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Документы.Событие.ПустаяСсылка().Метаданные()));
	КонецЕсли;
	
	Если Параметры.Свойство("Контакт") И ЗначениеЗаполнено(Параметры.Контакт) Тогда
		УстановитьОтборПоКонтактам(Параметры.Контакт);
	КонецЕсли;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ТипЗнч(Элемент.ТекущаяСтрока) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		СобытиеАктивнойСтроки = ПредопределенноеЗначение("Документ.Событие.ПустаяСсылка");
	Иначе
		СобытиеАктивнойСтроки = ?(Элемент.ТекущиеДанные = Неопределено, Неопределено, Элемент.ТекущиеДанные.Ссылка);
	КонецЕсли;
	
	Если СобытиеАктивнойСтроки <> ТекущееСобытие Тогда
		ТекущееСобытие = СобытиеАктивнойСтроки;
...
Процедура ОтборПропущенныеПриИзменении(Элемент)
	...
	УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "Неотвеченный", ОтборПропущенные, ОтборПропущенные);
...
&НаСервере
Процедура УстановитьОтборПоКонтактам(Контакт)
	
	КонтекстКонтакт = Контакт;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ТЧУчастники.Контакт", ОтборКонтакты, ВидСравненияКомпоновкиДанных.ВСписке);
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере(АдресРезультата)
	...
	ЗагруженоПрописью = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(";%1 письмо;;%1 письма;%1 писем;%1 письма", ЗагруженоПисем,ВидЧисловогоЗначения.Количественное,"ЧН=0");
