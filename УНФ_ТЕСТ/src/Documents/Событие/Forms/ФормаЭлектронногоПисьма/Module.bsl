...
Функция ВводитсяНаОснованииПисьма(Знач Основание)
	...
	Если ТипЗнч(ОснованиеЗаполнения) = Тип("ДокументСсылка.Событие") Тогда
		...
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОснованиеЗаполнения, "ТипСобытия") = Перечисления.ТипыСобытий.ЭлектронноеПисьмо;
...
Процедура ОбработатьПереданныеПараметры(Параметры)
	...
	Если ТипЗнч(Параметры.ДокументыОснования) = Тип("Массив") ИЛИ ТипЗнч(Параметры.ДокументыОснования) = Тип("ФиксированныйМассив") Тогда
...
Процедура ЗаполнитьКонтактыПоАдреснойКниге(АдресВХранилище)
	...
	Для Каждого ПодобраннаяСтрока Из ТаблицаАдресатов Цикл
		...
		Если СтрокаУчастники <> Неопределено И ЗначениеЗаполнено(СтрокаУчастники.Контакт) И ЗначениеЗаполнено(СтрокаУчастники.КакСвязаться) Тогда
			СтрокаУчастники = Неопределено;
		КонецЕсли;
...
Процедура УстановитьЗаголовокГиперссылкиДокументовОснований()
	...
	Элементы.ОткрытьДокументыОснования.Заголовок = СтрШаблон(НСтр("ru = '%1 и ещё %2.'"), ПервыйДокумент, ОснованияПрописью);
...
Процедура ЗаполнитьТекстОтвета(ДанныеИсходногоСообщения)
	...
	Если ЗначениеЗаполнено(ДанныеИсходногоСообщения.СодержаниеHTML) Тогда
		...
		ЗаполнитьОтветаКакГипертекст(ДанныеИсходногоСообщения);
		
	ИначеЕсли ЗначениеЗаполнено(ДанныеИсходногоСообщения.Содержание) Тогда
		
		ЗаполнитьОтветКакПростойТекст(ДанныеИсходногоСообщения);
...
&НаСервере
Процедура ЗаполнитьОтветаКакГипертекст(Знач ДанныеИсходногоСообщения)
	
	Если ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиЭлектроннойПочты", "ВключатьПодписьПриОтветеИлиПересылке", Ложь) Тогда
		ПодписьПриОтветеHTML = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиЭлектроннойПочты", "ПодписьПриОтветеHTML", "");
		ДополнитьКартинкиHTML(ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиЭлектроннойПочты", "КартинкиПодписиПриОтвете", Новый Структура));
	Иначе
...
&НаСервере
Процедура ЗаполнитьОтветКакПростойТекст(Знач ДанныеИсходногоСообщения)
	
	КомпонентыПодписи = Новый Массив;
	
	Если ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиЭлектроннойПочты", "ВключатьПодписьПриОтветеИлиПересылке", Ложь) Тогда
		КомпонентыПодписи.Добавить(СтроковыеФункцииКлиентСервер.ИзвлечьТекстИзHTML(
		ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиЭлектроннойПочты", "ПодписьПриОтветеHTML", "")));
	КонецЕсли;
...
Функция ПолучитьСписокВыбораКонтактов(знач СтрокаПоиска)
	...
	ДанныеВыбораКонтактныхЛиц = Справочники.Лиды.ПолучитьДанныеВыбора(ПараметрыВыбора);
	
	Для Каждого ЭлементСписка Из ДанныеВыбораКонтактныхЛиц Цикл
		ДанныеВыбораКонтактов.Добавить(ЭлементСписка.Значение, Новый ФорматированнаяСтрока(ЭлементСписка.Представление, " (лид)"));
	КонецЦикла;
...
Процедура НастроитьФормуМобильныйКлиент()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаВложения", "Заголовок", "Вложения (" + Строка(Вложения.Количество()) + ")");
