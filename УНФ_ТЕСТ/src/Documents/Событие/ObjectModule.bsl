...
Процедура ОтправитьЭлектронноеПисьмо(Записывать = Истина, Знач ВложенияПисьма = Неопределено) Экспорт
	...
	ТекстПисьмаHTMLсКартинками = СодержаниеHTML;
...
Процедура УстановитьСсылкиУчастниковПоАдресуЭП(Контакт, АдресЭП) Экспорт
	
	Если КонтактУжеЯвляетсяУчастником(Контакт) Тогда
		Возврат
	КонецЕсли;
...
Процедура ЗапланироватьРесурсыПредприятия(Отказ)
	...
	ПланированиеРесурсовУНФ.СформироватьЗаписиКолендаряСотрудникаПоРесурсам(Ссылка, ТаблицаДвиженийРесурсы, Отказ);
