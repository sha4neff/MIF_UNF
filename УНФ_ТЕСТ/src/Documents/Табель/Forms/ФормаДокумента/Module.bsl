	...
	Для каждого СтрокаТаблицы Из ДеталиСотрудников Цикл
		...
		Если ОбновитьСтроки Тогда
			...
			Для Каждого НомерСтроки Из НайденныеСтроки Цикл
				...
				СписокДатСОтклоненниями = Новый СписокЗначений;
				ЗаполнитьОтклоненияПоСтроке(НайденнаяСтрока, ТаблицаОтклонений, СтрокаТаблицы.ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачало, ПериодаОкончание);
				...
				Пока ВыборкаРабочихДней.Следующий() Цикл
					...
					Если ВыборкаРабочихДней.ДатаГрафика >= ПериодНачало
						...
						Если Не СписокДатСОтклоненниями.НайтиПоЗначению(ВыборкаРабочихДней.ДатаГрафика) = Неопределено Тогда
							Продолжить;
						КонецЕсли;
		...
		СписокДатСОтклоненниями = Новый СписокЗначений;
		ЗаполнитьОтклоненияПоСтроке(НоваяСтрока, ТаблицаОтклонений, СтрокаТаблицы.ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачало, ПериодаОкончание);
	...
	Для каждого СтрокаТаблицы Из ДеталиСотрудников Цикл
		...
		Если ОбновитьСтроки Тогда
			...
			Для Каждого НомерСтроки Из НайденныеСтроки Цикл
				...
				СписокДатСОтклоненниями = Новый СписокЗначений;
				ЗаполнитьОтклоненияПоСтрокеПериод(НайденнаяСтрока, ТаблицаОтклонений, СтрокаТаблицы.ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачало, ПериодаОкончание);
		...
		СписокДатСОтклоненниями = Новый СписокЗначений;
		ЗаполнитьОтклоненияПоСтрокеПериод(НоваяСтрока, ТаблицаОтклонений, СтрокаТаблицы.ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачало, ПериодаОкончание);
...
&НаСервере
Процедура ЗаполнитьОтклоненияПоСтроке(СтрокаТаблицы, ТаблицаОтклонений, ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачала, ПериодОкончания)
	...
	Для ПериодНачало = ПериодНачалаДень По ПериодОкончаниеДень Цикл
		...
		СписокДатСОтклоненниями.Добавить(ДатаДня);
...
&НаСервере
Процедура ЗаполнитьОтклоненияПоСтрокеПериод(СтрокаТаблицы, ТаблицаОтклонений, ЗанимаемыхСтавок, СписокДатСОтклоненниями, ПериодНачала, ПериодОкончания)
...
Процедура ОтработанноеВремяЗаПериодПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ФормаРасшифровкиЗаПериодМобильногоКлиента(Элементы.ОтработанноеВремяЗаПериод.ТекущаяСтрокав);
