...
Процедура ВыполнитьКонтроль(ДокументСсылка, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыПереданныеИзменение Тогда
		...
		Если Выборка.Количество()
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданные(ДокументОбъектОтчетКомиссионера, Выборка, Отказ);
		КонецЕсли;
...
Функция ПечатнаяФормаПереоценка(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = "Переоценка товаров на комиссии № "
			+ НомерДокумента
			+ " от "
		...
		ОбластьМакета.Параметры.ВсегоСтарая = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаСтарая);
		ОбластьМакета.Параметры.ВсегоОтклонение = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.Отклонение);
		ТабличныйДокумент.Вывести(ОбластьМакета);
