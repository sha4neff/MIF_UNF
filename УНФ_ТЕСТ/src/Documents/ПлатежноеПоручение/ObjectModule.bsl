...
Процедура ЗаполнитьНазначениеПлатежа(НазначениеВФорме = "", ИзменениеСуммы = Ложь) Экспорт
	...
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%ЗначениеСтавкиНДС%", ?(БезНалогаНДС ИЛИ (НЕ ЗначениеЗаполнено(СуммаНДС) И НЕ ЗначениеЗаполнено(СтавкаНДС)), НСтр("ru = 'Без налога (НДС)'"), НСтр("ru = 'В т.ч.'")));
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументОснование) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.ВидНалога = Справочники.ВидыНалогов.АвансовыйПлатежПриУСН
			...
			КодБК = РегламентированнаяОтчетностьУСН.ПолучитьКБКУплаты(Организация, ДокументОснование.Дата, Выборка.ВидНалога);
		Иначе
			КодБК = РегламентированнаяОтчетностьУСН.ПолучитьКБКУплаты(Организация, Дата, Выборка.ВидНалога);
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Оплата Тогда
		...
		Если СуммаПлатежа <> СуммаДокумента Тогда
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВалютаДенежныхСредств%", СокрЛП(Строка(ВалютаДокумента)));
...
Процедура ПриЗаписи(Отказ)
	...
	Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
		УстановитьСтаусПлатежногоПорученияВДокументахПланирования();
	КонецЕсли; 
...
Процедура УстановитьСтаусПлатежногоПорученияВДокументахПланирования()
