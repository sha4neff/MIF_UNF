...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидКонтрагента = УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(Объект.Контрагент, "ВидКонтрагента");
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка)
	   И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) И НЕ ЭтоВидОперацииБезНДС(Объект.ВидОперации) Тогда
			Объект.СтавкаНДС = РасчетыРаботаСФормамиВызовСервера.СтавкаНДСПоОрганизациияНалогообложениеНДСИСпособуЗаполнения(Объект.Организация, Объект.Дата, СпособЗаполненияСтавкиНДС);
			ОбновитьСуммуНДС();
		КонецЕсли;
	...
	// Служебные реквизиты для новых видов операций
	РасчетыРаботаСФормамиВызовСервера.ЗаполнитьРеквизитыКурсИКратность(ЭтотОбъект);

	Если Объект.ВидОперации <> Перечисления.ВидыОперацийПлатежноеПоручение.Зарплата И Объект.ВидОперации <> Перечисления.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога Тогда
	...
	ДвиженияДенежныхСредствВызовСервера.ЗаполнитьСписокВыбораВидовОпераций("ПлатежноеПоручение", Элементы.ВидОперации.СписокВыбора);
	
	ФункциональнаяОпцияКассовыйМетодУчетаДоходовИРасходов = ПолучитьФункциональнуюОпцию("КассовыйМетодУчетаДоходовИРасходов");
	...
	ПараметрыОтображения = ОбменСБанками.ПараметрыСозданияФормыДокумента();
...
Процедура УстановитьВидимостьДоговора()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) И ЗначениеЗаполнено(Объект.Контрагент)
		И ВидОперации <> Перечисления.ВидыОперацийПлатежноеПоручение.ПереводНаДругойСчет И ВидОперации <> Перечисления.ВидыОперацийПлатежноеПоручение.ЛичныеСредстваПредпринимателя Тогда
...
&НаКлиенте
Процедура СчетКонтрагентаРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Сотрудники.Форма.ФормаЭлемента",СтруктураПараметров, ЭтаФорма,,,,ОписаниеОповещенияОЗакрытии);
...
Функция ВозможноПеречислениеПособияНаКартуМир(ВидОперации, ВидКонтарегнта, Контрагент)
	
	ВыплатаФизлицу = (ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Прочее")
		И Не Контрагент.Пустая() И ВидКонтарегнта = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ФизическоеЛицо"));
...
&НаСервереБезКонтекста
Функция ВозможноВзысканиеСПеречисленияФизлицу(Знач Дата, ВидОперации, ВидКонтарегнта, Контрагент)
	...
	КонтрагентФизическоеЛицо =
		...
		И ВидКонтарегнта = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ФизическоеЛицо");
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Знач ПолучательПлатежа, ВалютаДокумента, ВидОперации)
	...
	СтруктураДанные.Вставить("КПППолучателя", 		?(НеобходимостУказанияКПП И НЕ ЭтоРасчетыССотрудником, ПолучательПлатежа.КПП, ""));
	...
	СтруктураДанные.Вставить("ВидКонтрагента", УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(Объект.Контрагент, "ВидКонтрагента"));
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	ПараметрыВыбораСчетКонтрагента.Добавить(Новый СвязьПараметраВыбора("Отбор.Владелец", "Объект.Контрагент"));
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Оплата") Тогда
		...
		Элементы.СуммаНДСИтог.Видимость = Истина;
		Элементы.СтавкаНДС.Видимость = Истина;
		
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Покупателю") Тогда
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Прочее") Тогда
		
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.ИтогиРасчетыПрочие.Видимость = Ложь;
		Элементы.РеквизитыДляПеречисленияНалога.Видимость = Ложь;
		Элементы.Налог.Видимость = Ложь;
		Элементы.РасчетыПрочие.Видимость = Истина;
		
		Элементы.СуммаПлатежаПланирование.Видимость = ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ПереводНаДругойСчет") 
		ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ЛичныеСредстваПредпринимателя") Тогда
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.НаРасходы") Тогда
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ПрочиеРасчеты") Тогда
		...
		Элементы.СуммаПлатежаПрочиеРасчеты.Видимость = УчетВалютныхОпераций;
		Элементы.СуммаРасчетовПрочиеРасчеты.Видимость = НЕ УчетВалютныхОпераций;
		
		Элементы.СуммаНДС.Видимость = Ложь;
		Элементы.СуммаНДСИтогПрочиеРасчеты.Видимость = Ложь;
		Элементы.СтавкаНДС.Видимость = Ложь;
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ЗарплатаСотруднику") Тогда
...
Процедура УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденции(ОчиститьАналитику = Ложь)
	...
	Иначе
		...
		Если НЕ ФункциональнаяОпцияКассовыйМетодУчетаДоходовИРасходов
			И Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога Тогда
			Объект.НаправлениеДеятельности = Неопределено;
		КонецЕсли;
		Объект.Подразделение = Неопределено;
	...
	// Прочие расчеты
	Если (Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Прочее 
...
Процедура УстановитьВидимостьДокументыПланирования()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Покупателю
		...
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Оплата
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Зарплата
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
		Элементы.ДокументыПланирования.Видимость = Ложь;
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.ПрочиеРасчеты Тогда
		Элементы.ДокументыПланирования.Видимость = Ложь;
	Иначе
...
Процедура УстановитьСтатьюДДС()
	...
	Если НЕ Объект.Контрагент.Пустая() И НЕ Объект.Контрагент.СтатьяДДСПоУмолчанию.Пустая() Тогда
		Объект.Статья = Объект.Контрагент.СтатьяДДСПоУмолчанию;
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.РасчетыСКонтрагентом.Видимость = Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога");
	Элементы.РасшифровкаПлатежаСуммаПлатежа.Видимость = УчетВалютныхОпераций;
...
Процедура ВыборТипаОснованияЗавершение(ВыбИмяФормы, Параметры) Экспорт
	...
	Если ВыбИмяФормы<>Неопределено Тогда
		...
		КонецЦикла;
		Если СтрНайти(ВыбИмяФормы.Значение, "Документ.НачислениеНалогов") > 0 Тогда
		...
		КонецЕсли;
		Если СтрНайти(ВыбИмяФормы.Значение, "Документ.РасходДСПлан") > 0 Тогда
		...
		ОткрытьФорму(ВыбИмяФормы.Значение, ПараметрыДляОткрытияФормы, ЭтотОбъект, ,,,_ОповещениеОЗакрытии);
	КонецЕсли;	
	...
	ИначеЕсли СтрокаТабличнойЧасти.СуммаПлатежа = 0 ИЛИ
		...
		Иначе
			СтрокаТабличнойЧасти.СуммаПлатежа = УправлениеНебольшойФирмойКлиент.ПересчитатьИзВалютыВВалюту(
				...
				СтрокаТабличнойЧасти.Курс, Курс,
				СтрокаТабличнойЧасти.Кратность, Кратность
			);
		КонецЕсли;
	Иначе
		СтрокаТабличнойЧасти.Курс = ?(
...
// Завершение подбора долгообразующих документов.
//
&НаКлиенте
...
// Заполняет табличную часть "Расшифровка платежа" данными долгообразующих документов.
//
&НаКлиенте
...
// Завершение заполнения табличной части "Расшифровка платежа" данными долгообразующих документов.
//
&НаКлиенте
...
// Начинает открыте формы распределения платежа.
//
&НаКлиенте
...
// Возвращает структуру адресов хранения данных табличных частей формы для передачи в обработку подбора долгообразующих документов
//
&НаСервере
...
Процедура ОбработатьИзменениеДоговораКонтрагента(СтрокаТабличнойЧасти)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) Тогда
		...
		СтрокаТабличнойЧасти.СтавкаНДС = РасчетыРаботаСФормамиВызовСервера.СтавкаНДСПоОрганизациияНалогообложениеНДСИСпособуЗаполнения(
