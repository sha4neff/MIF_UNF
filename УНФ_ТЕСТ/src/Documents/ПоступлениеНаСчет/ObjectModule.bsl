...
Процедура ЗаполнитьПоОтчетуКомиссионера(ДокументОснование) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		// Попробуем найти счет на оплату, если он выписан на основании Отчета комиссионера (
		ТекущийСчетНаОплату = Документы.СчетНаОплату.ПустаяСсылка();
		РезультатЗапроса = МассивРезультатов[1];
		Если НЕ РезультатЗапроса.Пустой() Тогда
			ВыборкаСчетНаОплату = МассивРезультатов[1].Выбрать();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам
		...
		Если ПоложениеНастроекНалоговогоУчета = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "НалогообложениеНДС");
		Иначе
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ЭквайринговыеОперации.НалогообложениеНДС");
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ПоложениеЭквайринговогоТерминала) ИЛИ ПоложениеЭквайринговогоТерминала = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке Тогда
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ЭквайринговыеОперации.ЭквайринговыйТерминал");
		Иначе
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ЭквайринговыйТерминал");
		КонецЕсли;
		...
		Если ПараметрыЭквайринга.КонтрольВзаиморасчетовЭквайринг Тогда
			...
			Если НЕ ПараметрыЭквайринга.РасчетКомиссиВОтчетеЭквайера Тогда
