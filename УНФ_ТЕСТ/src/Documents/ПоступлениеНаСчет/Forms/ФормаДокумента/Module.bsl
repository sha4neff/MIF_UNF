...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидОперацииКредит = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКредитам;
	...
	ВидОперацииВозвратЗаймаСотрудником = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником;
	ВидОперацииОтНашейОрганизации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации;
	...
	ДвиженияДенежныхСредствВызовСервера.ЗаполнитьСписокВыбораВидовОпераций("ПоступлениеНаСчет", Элементы.ВидОперации.СписокВыбора);
	...
	И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если Не ЗначениеЗаполнено(Параметры.ДокументОснование) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ЛичныеСредстваПредпринимателя Тогда
...
Процедура СуммаКомиссииДокументаПриИзменении(Элемент)
	...
	Если ДанныеДоговораЭквайринга.РасчетКомиссиВОтчетеЭквайера И ДанныеДоговораЭквайринга.КонтрольВзаиморасчетовЭквайринг Тогда
...
Функция ПолучитьДанныеЭквайринговогоДоговора(ДоговорЭквайринга)
	...
	СтруктураДоговора.Вставить("РасчетКомиссиВОтчетеЭквайера", 	  ДоговорЭквайринга.РасчетКомиссиВОтчетеЭквайера);
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС()
	...
	ОпределитьВидимостьНадписиСчетФактура();
...
Процедура УстановитьВидимостьНДС()
	...
	Элементы.СуммаНДС.Видимость = (Объект.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС"));
	Элементы.РасшифровкаПлатежаСтавкаНДС.Видимость = Элементы.СуммаНДС.Видимость;
	Элементы.РасшифровкаПлатежаСуммаНДС.Видимость = Элементы.СуммаНДС.Видимость;
...
Процедура УстановитьВидимостьДокументыПланирования()
	...
	// Эквайринг
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКредитам Тогда
		Если ДоговорЭквайринга.КонтрольВзаиморасчетовЭквайринг ИЛИ НЕ ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			Элементы.ДокументыПланирования.Видимость = Ложь;
		Иначе
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты") Тогда
		...
		Если УправлениеНебольшойФирмойПовтИсп.ТребуетсяКонтрольДоговоровКонтрагентов() Тогда
			...
			Элементы.ПокупкаВалютыДоговор.Видимость = Истина;
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты") Тогда
		...
		Элементы.ДоговорСАгентом.Заголовок = ?(УчетВалютныхОпераций, НСтр("ru = 'Договор (руб.)'"), НСтр("ru = 'Договор'"));
...
Процедура УстановитьВидимостьОтПользовательскихНастроек()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКредитам Тогда
...
Процедура ОбработатьИзменениеДоговораКонтрагентаПрочиеРасчеты()
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) Тогда
		...
		СтрокаТабличнойЧасти.СтатьяДДС = СтруктураДанные.СтатьяДДСПоУмолчанию;
	Иначе
		СтрокаТабличнойЧасти.СтатьяДДС = ДвиженияДенежныхСредствВызовСервера.ПолучитьСтатьюДДСПоУмолчаниюДляСтрокиРасшифровки(СтрокаТабличнойЧасти.Договор,
			СтрокаТабличнойЧасти.ДокументПланирования,
			СтрокаТабличнойЧасти.СтатьяДДС);
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя
				...
				ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты
				ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты
				ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации Тогда
...
&НаСервере
Процедура ОпределитьВидимостьНадписиСчетФактура()
...
Процедура ОпределитьВидимостьНастроекУчетаВНалогообложенииДляОтчетаЭквайера(ЭтоЮрЛицо)
	...
	Если Объект.ВидОперации <> Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам
	...
	Иначе
		ВидимостьУчитыватьВНУ = Элементы.УчитыватьВНУ.Видимость;
		ВидимостьПатент		  = Элементы.Патент.Видимость;
	КонецЕсли; 
...
Процедура ЗаполнитьДокументРасчетов(СтрокаРасчетов)
	...
	Если Объект.ВалютаДенежныхСредств <> ВалютаУчета Тогда
		СтруктураКурсы = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ВалютаУчета, Объект.ВалютаДенежныхСредств, Объект.Дата);
		СуммаРасчетов = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(СуммаРасчетов, СтруктураКурсы.КурсНач, СтруктураКурсы.Курс, СтруктураКурсы.КратностьНач, СтруктураКурсы.Кратность);
	КонецЕсли; 
...
Процедура РасшифровкаПлатежаОтАгентаЗаказПриИзмененииНаСервере(ТекущиеДанныеИдентификатор)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		// Если выбран заказ-наряд, то его можно сразу подсатвить в реквизит "Документ" ТЧ РасшифровкаПлатежаОтАгента.
...
Процедура НастроитьЭлементыРаспределенияДолговНаСервере() Экспорт
	...
	РасчетыРаботаСФормамиВызовСервера.НастроитьЭлементыРаспределенияСуммыНаСервере(ЭтотОбъект, 
		?(ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации,
		Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации,
