...
&НаКлиенте
Процедура ЗаполнитьПоЗаказуНаПроизвоство(Команда)
		...
		Для Каждого СтрокаУслуг Из Объект.РаботыИУслуги Цикл
			Если СтрокаУслуг.СтавкаНДС = СтавкаНДС18 И ЗначениеЗаполнено(СтавкаНДС20) Тогда
				...
				Если Результат = КодВозвратаДиалога.Нет  Тогда
					...
					СтрокаУслуг.Сумма = УправлениеНебольшойФирмойКлиент.НоваяСтоимостьСУчетомИзмененийПоНДС2018(СтрокаУслуг.Сумма, СтавкаНДСТекущая, СтавкаНДСНовая)
				КонецЕсли;
				СтрокаУслуг.СтавкаНДС = СтавкаНДС20;
				УправлениеНебольшойФирмойКлиент.РассчитатьСуммуНДСИВсего(СтрокаУслуг, Новый Структура("СуммаВключаетНДС",Объект.СуммаВключаетНДС));
	...
	Если ТипЗнч(СтруктураЦеныИВалюта) = Тип("Структура")
	   И СтруктураЦеныИВалюта.БылиВнесеныИзменения Тогда
		...
		Если НЕ СтруктураЦеныИВалюта.ПерезаполнитьЦены
			...
			УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, ВалютаРасчетовПередИзменением, "РаботыИУслуги", СтруктураЦеныИВалюта.КурсПересчетаЦен);
		КонецЕсли;
...
Функция ТекстНадписиЦеныИВалюта(ПоляНадписи)
	...
	// Дисконтная карта.
	Если ЗначениеЗаполнено(ПоляНадписи.ДисконтнаяКарта) И ДисконтныеКартыВызовСервера.СтарыйМеханизмСкидок(
...
Процедура ДополнитьТипыНоменклатуры(ТипыНоменклатуры, МассивНоменклатуры)
	...
	Соответсвие = Новый Соответствие(ТипыНоменклатуры);
	...
	Пока Выборка.Следующий() Цикл
		Соответсвие.Вставить(Выборка.Ссылка, Выборка.ТипНоменклатуры);	
	КонецЦикла;
	
	ТипыНоменклатуры = Новый ФиксированноеСоответствие(Соответсвие);
...
Процедура ПолучитьРаботыИУслугиИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти, ЕстьХарактеристики, ЕстьПартии)
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаЗагрузки.Номенклатура) Тогда
			...
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаЗагрузки.Номенклатура, "ЭтоНабор, ТипНоменклатуры");
...
Процедура ОбновитьСоставНабора(НоменклатураНабора, ХарактеристикаНабора, КоличествоНаборов, СоставНабора)
	...
	НаборыСервер.ОбновитьСоставНабораВТабличнойЧасти(НоменклатураНабора, ХарактеристикаНабора, , КоличествоНаборов, СоставНабора, Объект, "РаботыИУслуги");
	Модифицированность = Истина;
