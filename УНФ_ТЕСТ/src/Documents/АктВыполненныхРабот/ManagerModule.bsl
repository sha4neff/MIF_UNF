...
Процедура СформироватьТаблицаЗаказыНаПроизводство(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	ПроизводствоСервер.СформироватьДвиженияЗаказыНаПроизводствоСУчетомОстатков(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	НомерЗапроса = 0;
	Запрос.Текст = УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, Истина);
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияПодарочныеСертификатыИзменение Тогда
		...
		Если Не МассивРезультатов[3].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПодарочныеСертификаты(ДокументОбъектАктВыполненныхРабот, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ДисконтныеКартыСервер.ТекстЗапросаПродажиПоДисконтнойКарте("АктВыполненныхРабот");
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ИспользоватьФаксимиле)
	...
	|	,АктВыполненныхРабот.Организация.ФайлФаксимильнаяПечать КАК ФаксимилеПечати
	|	,АктВыполненныхРабот.Контрагент КАК Контрагент
	|	,ЕстьNULL(АктВыполненныхРабот.Договор.РасчетыВУсловныхЕдиницах, ЛОЖЬ) КАК РасчентыВУсловныхЕдиницах
