...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЗаказНаПроизводство.ПоложениеЗаказаПокупателя = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВШапке)
	...
	Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийЗаказНаПроизводство.Разборка Тогда
		...
		|	ЗаказыОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ЗаказыОстатки.Номенклатура КАК Номенклатура,
		|	ЗаказыОстатки.Характеристика КАК Характеристика,
		...
		|		ВЫБОР
		|			КОГДА &РучноеРаспределение
		|				ТОГДА ЗаказНаПроизводствоПродукция.КлючСвязи
		|			ИНАЧЕ 0
		|		КОНЕЦ КАК КлючСвязи
		|	ИЗ
		...
		|		ВЫБОР
		|			КОГДА &РучноеРаспределение
		|				ТОГДА СборкаЗапасовПродукция.КлючСвязи
		|			ИНАЧЕ 0
		|		КОНЕЦ
		|	ИЗ
		...
		|	ЗаказыОстатки.СтруктурнаяЕдиница,
		|	ЗаказыОстатки.Номенклатура,
		|	ЗаказыОстатки.Характеристика,
	...
	Если СборкаСЭтапами() Тогда
		ОпеределитьВыполненныеЭтапы(ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьПоСборкеЗапасов(ДанныеЗаполнения) Экспорт
	...
	|		КлючСвязи КАК КлючСвязи,
	|		ПОДСТРОКА(ВЫБОР
	|				КОГДА ИспользоватьСерийныеНомера.Значение
	|					ТОГДА СборкаЗапасов.Запасы.СерийныеНомера
	|				ИНАЧЕ """"
	|			КОНЕЦ, 1, 1024) КАК СерийныеНомера
	|	) КАК Запасы,
	...
	|	Константа.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера
	|ГДЕ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТаблицаЗапасы.Свернуть("Номенклатура, Характеристика, ЕдиницаИзмерения, Спецификация, Партия, ПодразделениеЗавершающегоЭтапа, ТипНоменклатуры, СпособПополнения, ЗаказПокупателя, КлючСвязи, СерийныеНомера", "Количество");
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСерийныеНомера") Тогда
			ТаблицаСерийныеНомера = ВыборкаИзРезультатаЗапроса.ТаблицаСерийныеНомера.Выгрузить();
			Для каждого СтрокаСерийныйНомер Из ТаблицаСерийныеНомера Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаСерийныйНомер.КлючСвязи) Тогда
					Продолжить;
				КонецЕсли; 
				СтрокиПродукция = УправлениеНебольшойФирмойКлиентСервер.СтрокиПоКлючуСвязи(Продукция, СтрокаСерийныйНомер.КлючСвязи);
				Если СтрокиПродукция.Количество()=0 Тогда
					Продолжить;
				КонецЕсли; 
				НоваяСтрока = СерийныеНомераПродукция.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСерийныйНомер);
			КонецЦикла; 
		КонецЕсли; 
...
Процедура ЗаполнитьПоОтчетуОРозничныхПродажах(ДанныеЗаполнения) Экспорт
	...
	|	ТаблицаЗапасы.СерийныеНомера КАК СерийныеНомера,
...
Процедура ЗаполнитьСкладыИРезервы()
	...
	Для каждого СтрокаТабличнойЧасти Из ЭтотОбъект[ИмяТЧ] Цикл
		...
		Для каждого СтрокаДанных Из Строки Цикл
			...
			КонецЕсли; 
			Коэффициент = ?(ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения)=Тип("СправочникСсылка.ЕдиницыИзмерения"), СтрокаТабличнойЧасти.ЕдиницаИзмерения.Коэффициент, 1);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		Для Каждого ТекСтрокаТЧ Из ТабличнаяЧасть Цикл
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			ЭтотОбъект,
			СтрШаблон(НСтр("ru = 'Не заполнена колонка ""Склад""  в строке %1 списка ""%2""'"), ТекСтрокаТЧ.НомерСтроки, ИмяТЧ),
			ИмяТЧ,
...
Процедура ОпеределитьВыполненныеЭтапы(ДанныеЗаполнения)
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	Если ПроверяемаяПродукция.Количество() > 0 
		...
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
		ЭтотОбъект,
		НСтр("ru = 'При поэтапном производстве доступен только вид операции ""Сборка"".'"),
		,
		,
		"ВидОперации",
		Отказ);
	...
	Если ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка
		...
		Для каждого СтрокаПродукция Из ПроверяемаяПродукция Цикл
			...
			Если ВыполненныеЭтапы.НайтиСтроки(СтруктураОтбора).Количество()=0 Тогда
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'Не выбраны выполняемые этапы в строке %1.'"), СтрокаПродукция.НомерСтроки),
				"Продукция",
				СтрокаПродукция.НомерСтроки,
				"Этапы",
				Отказ);
			...
			Если НЕ ЗначениеЗаполнено(ЗаказНаПроизводство) И НЕ ЗначениеЗаполнено(СтрокаПродукция.ЗаказПокупателя) Тогда
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'Для использования поэтапной сборки требуется указать заказ покупателя или заказ на производство (строка %1).'"), СтрокаПродукция.НомерСтроки),
				"Продукция",
				СтрокаПродукция.НомерСтроки,
				"Спецификация",
				Отказ);
...
Процедура СвернутьПродукциюСУчетомПодчиненныхТЧ()
	...
	Для каждого СтрокаТабличнойЧасти Из Продукция Цикл
		...
		Если Строки.Количество()=1 Тогда
			ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, Строки[0], "КлючСвязи, СерийныеНомера");
		Иначе
			ПерваяСтрока = Неопределено;
			ПредставлениеСерийныеНомера = "";
			Для каждого НайденнаяСтрока Из Строки Цикл
				Если ПерваяСтрока=Неопределено Тогда
					...
					ПредставлениеСерийныеНомера = Строки[0].СерийныеНомера;
					Продолжить;
				КонецЕсли; 
				СтруктураОтбора = Новый Структура;
				СтруктураОтбора.Вставить("КлючСвязи", НайденнаяСтрока.КлючСвязи);
				СтрокиСерийныеНомера = СерийныеНомераПродукция.НайтиСтроки(СтруктураОтбора);
				Для каждого СтрокаСерийныйНомер Из СтрокиСерийныеНомера Цикл
					СтрокаСерийныйНомер.КлючСвязи = ПерваяСтрока.КлючСвязи;
					ПредставлениеСерийныеНомера = ПредставлениеСерийныеНомера + ?(ПустаяСтрока(ПредставлениеСерийныеНомера), "", "; ") + Строка(СтрокаСерийныйНомер.СерийныйНомер);
				КонецЦикла; 
			КонецЦикла;
			СтрокаТабличнойЧасти.СерийныеНомера = ПредставлениеСерийныеНомера;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ПроверитьТЧНаСовпадениеНоменклатуры(Отказ)
	...
	Если Выборка.Следующий() Тогда
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
		ЭтотОбъект,
		СтрШаблон(НСтр("ru = 'В табличных частях продукции и материалов присутствуют одинаковые запасы. Такой режим
