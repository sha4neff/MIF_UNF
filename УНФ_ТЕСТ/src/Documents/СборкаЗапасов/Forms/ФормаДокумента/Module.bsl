...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИменаТЧ.Добавить(Новый Структура("ИмяПоляПроверки, ИмяПоляОформления", "Объект.Продукция.СтранаПроисхождения",
		"ПродукцияНомерГТД"));
...
Процедура ПродукцияПередУдалением(Элемент, Отказ)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.Продукция.ВыделенныеСтроки Цикл
		...
		РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераПродукция, ТекущиеДанныеСтроки,
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент)
	...
	// Серийные номера
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераПродукция, СтрокаТабличнойЧасти,
		ИспользоватьСерийныеНомераОстатки);
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Если ДанныеШтрихкода.Количество() > 1 Тогда
			...
			, ЭтотОбъект, Новый Структура("СтруктураДанные, ИмяТабличнойЧастиСерийныеНомера", СтруктураДанные, ИмяТабличнойЧастиСерийныеНомера));
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, Параметры) Экспорт
	...
	ИмяТабличнойЧастиСерийныеНомера = Параметры.ИмяТабличнойЧастиСерийныеНомера;
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов()
	...
	Иначе
		...
		Элементы.ЗапасыЯчейка.Видимость = НЕ СкладВШапке И ФОИспользоватьЯчейки;
