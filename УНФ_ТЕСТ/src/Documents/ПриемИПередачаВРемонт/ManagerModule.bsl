...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	...
	|		ИНАЧЕ ПриемИПередачаВРемонт.Организация КОНЕЦ КАК Организация
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		ТОГДА ПриемИПередачаВРемонт.Организация.ГоловнаяОрганизация
	|		ИНАЧЕ ПриемИПередачаВРемонт.Организация КОНЕЦ КАК Поставщик
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|			ТОГДА ПриемИПередачаВРемонт.Организация
	|		ИНАЧЕ ПриемИПередачаВРемонт.Организация КОНЕЦ КАК ОбособленноеПодразделениеПоставщика
	...
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ КОНЕЦ КАК ЭтоФизическоеЛицо
	|	,Выбор КОГДА ПриемИПередачаВРемонт.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		ТОГДА 2
	...
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		ИНАЧЕ ПриемИПередачаВРемонт.Контрагент КОНЕЦ КАК Контрагент
	|	,Выбор КОГДА ПриемИПередачаВРемонт.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И ПриемИПередачаВРемонт.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		ТОГДА ПриемИПередачаВРемонт.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ ПриемИПередачаВРемонт.Контрагент КОНЕЦ КАК Покупатель
	...
	|		ИНАЧЕ ПриемИПередачаВРемонт.ВалютаДокумента КОНЕЦ КАК Валюта
	|	,ПриемИПередачаВРемонт.ВалютаДокумента КАК ВалютаДокументаОригинал
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение
	|		ИНАЧЕ ПриемИПередачаВРемонт.ВалютаДокумента КОНЕЦ КАК ВалютаДокумента
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.НаименованиеПолное
	|		ИНАЧЕ ПриемИПередачаВРемонт.ВалютаДокумента.НаименованиеПолное КОНЕЦ КАК ВалютаНаименование
	|	,Выбор КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА 
	|		ТОГДА НациональнаяВалюта.Значение.Код
	|		ИНАЧЕ ПриемИПередачаВРемонт.ВалютаДокумента.Код КОНЕЦ КАК ВалютаКод
	|	,ПриемИПередачаВРемонт.СтавкаНДС КАК СтавкаНДС
	...
	|	,ПриемИПередачаВРемонт.НДСВключатьВСтоимость
	|	,ПриемИПередачаВРемонт.Курс
	|	,ПриемИПередачаВРемонт.Кратность
	...
	|ИЗ Документ.ПриемИПередачаВРемонт КАК ПриемИПередачаВРемонт
	|	,Константа.НациональнаяВалюта КАК НациональнаяВалюта
	|ГДЕ ПриемИПередачаВРемонт.Ссылка В (&МассивОбъектов)";
...
Процедура СформироватьЧек(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если Шапка.СуммаДокумента > 0 Тогда
			...
			ОбластьМакета.Параметры.СуммаНДС = ?(Шапка.СуммаНДС = 0 И НЕ ЕстьСтавкаНольПроцентов, "-", УправлениеНебольшойФирмойСервер.ФорматСумм(Шапка.СуммаНДС, ,"0,00"));
			ОбластьМакета.Параметры.СуммаДокумента = УправлениеНебольшойФирмойСервер.ФорматСумм(Шапка.СуммаДокумента);
			ТабличныйДокумент.Вывести(ОбластьМакета);
