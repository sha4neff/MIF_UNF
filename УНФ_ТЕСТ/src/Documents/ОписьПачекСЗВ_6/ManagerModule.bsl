...
Процедура ЗаполнитьДанныеОВзносах(Дата, ОтчетныйПериод, Организация, СписокДокументов) Экспорт
	
	Возврат; // Не реализована
...
// Сформировать печатную форму АДВ6_2
// 
// Параметры:
...
Функция СформироватьПечатнуюФормуАДВ6_2(Документ, ОбъектыПечати) Экспорт
	...
	ОбластьШапка.Параметры.ОрганизацияНаименованиеСокращенное = ВРег(
		ОбластьШапка.Параметры.ОрганизацияНаименованиеСокращенное);
	...
	Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("Таблица") Цикл
		...
		Пока ВыборкаПоПачкам.Следующий() Цикл
			...
			ВыводимаяОбласть.Параметры.КодКатегории = УчетСтраховыхВзносов.ПолучитьИмяЭлементаПеречисленияПоЗначению(
...
// Заполнить списки выводимых на печать документов
// 
// Параметры:
// 	ОписьСсылка - ДокументСсылка - .
// 	МассивСЗВ6_2 - Массив - пачка документов.
Процедура ЗаполнитьСпискиВыводимыхНаПечатьДокументов(ОписьСсылка, МассивСЗВ6_2) Экспорт
...
// 
// Параметры:
// 	Опись - ДокументСсылка - .
// 	ТипСведенийСЗВ - ПеречислениеСсылка.ТипыСведенийСЗВ - .
// 	СписокКорректируемыхПериодов - Массив из Дата - .
// Возвращаемое значение:
// 	Массив - Описание
Функция ДокументыОписи(Опись, ТипСведенийСЗВ = Неопределено, СписокКорректируемыхПериодов = Неопределено) Экспорт
	...
	Если СписокКорректируемыхПериодов <> Неопределено Тогда 
		СтрокаУсловие = СтрокаУсловие
			+ " И КомплектыОтчетностиПерсучетаСписокПачекСЗВ.ПачкаДокументов.КорректируемыйПериод В (&СписокКорректируемыхПериодов)";
	КонецЕсли;
...
Функция СформироватьВыходнойФайл(ВыборкаПоШапкеДокумента, ВыборкаПоПачкам)
	...
	Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("Таблица") Цикл
		...
		Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("КатегорияЗастрахованныхЛиц") Цикл
			...
			НаборЗаписей.КодКатегории.Значение = УчетСтраховыхВзносов.ПолучитьИмяЭлементаПеречисленияПоЗначению(
				ВыборкаПоПачкам.КатегорияЗастрахованныхЛиц);
			НаборЗаписей.КоличествоЗЛ.Значение = КоличествоЗЛКатегории;
			
			Если ВыборкаПоПачкам.Таблица = 1 Тогда
				УчетСтраховыхВзносов.ВписатьВзносыВНаборДанных(НаборЗаписей, НачисленоСтраховаяКатегории,
