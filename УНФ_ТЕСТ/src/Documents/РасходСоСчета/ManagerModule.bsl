...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	НомерЗапроса = 0;
	Запрос.Текст = УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками(Запрос.МенеджерВременныхТаблиц, Ложь, НомерЗапроса, ПолучатьРеквизитыРасчетов);
	МассивРезультатов = Запрос.ВыполнитьПакет();
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеЛичныеСредсва", НСтр("ru = 'Перевод собственных денежных средств'"));
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
	Запрос.УстановитьПараметр("ИспользуютсяПереводыВПути",РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата,СтруктураДополнительныеСвойства.ДляПроведения.Организация));
	...
	|			ТОГДА ВЫРАЗИТЬ(&СодержаниеЛичныеСредсва КАК СТРОКА(100))
...
Процедура ВыполнитьКонтроль(ДокументСсылкаРасходСоСчета, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваВРезервеИзменение Тогда
		...
		|								И РасчетыПоЭквайрингуОстатки.ЭквайринговыйТерминал.КомиссияЗаВозвратУменьшаетКомиссиюЗаОплату
		...
		Если МассивРезультатов[0].Пустой() И НЕ МассивРезультатов[5].Пустой() Тогда //Если остатка денежных средств не хватает, то выводить ошибку по резервам нет смысла
			Если ДвижениеДенежныхСредствСервер.КонтролироватьРезервыДенежныхСредств() Тогда
				...
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструДенежныеСредстваСУчетомРезервов(ДокументОбъектРасходСоСчета, ВыборкаИзРезультатаЗапроса, Отказ);
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("РасчетКомиссиВОтчетеЭквайера", ПараметрыЭквайринга.РасчетКомиссиВОтчетеЭквайера);
	...
	Иначе
		...
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.СчетУчетаЭквайринг,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.СчетЗатратЭквайринг,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.НаправлениеДеятельностиЗатраты,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.СчетУчетаЭквайринг.ТипСчета,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.СчетЗатратЭквайринг.ТипСчета,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.ПодразделениеЗатраты
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютБанковскогоСчета
		|		ПО (КурсыВалютБанковскогоСчета.Валюта = &ВалютаДС)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций КАК ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход
		|		ПО ПоступлениеНаСчет.ЭквайринговыйТерминал = ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиПриход.ЭквайринговыйТерминал
		|ГДЕ
		|	ПоступлениеНаСчет.Ссылка = &Ссылка
		|	И &РасчетКомиссиВОтчетеЭквайера
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	&ОплатаПлатежнойКартой,
		|	ПоступлениеНаСчет.Дата,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.СчетУчетаЭквайринг,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.СчетЗатратЭквайринг,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.НаправлениеДеятельностиЗатраты,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.СчетУчетаЭквайринг.ТипСчета,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.СчетЗатратЭквайринг.ТипСчета,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.ПодразделениеЗатраты
		|ИЗ
		|	Документ.ПоступлениеНаСчет КАК ПоступлениеНаСчет
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютУчета
		|		ПО (КурсыВалютУчета.Валюта = &ВалютаУчета)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютБанковскогоСчета
		|		ПО (КурсыВалютБанковскогоСчета.Валюта = &ВалютаДС)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций КАК ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход
		|		ПО ПоступлениеНаСчет.ЭквайринговыйТерминал = ВременнаяТаблицаПараметрыУчетаЭквайринговыхОперацийДляКомиссиРасход.ЭквайринговыйТерминал
		|ГДЕ
		|	ПоступлениеНаСчет.Ссылка = &Ссылка
		|	И &РасчетКомиссиВОтчетеЭквайера";
	КонецЕсли;
